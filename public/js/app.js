  ...
  const LEARN = {
    // Reading the market
    regime: { g: 'Reading the market', t: 'Market Regime (Risk-On / Risk-Off)', plain: 'Whether the whole market is in a calm uptrend (Risk-On) or a fearful downtrend (Risk-Off) — read from the S&P 500 vs its 200-day average plus the VIX "fear gauge" and credit stress.', why: 'It is the single biggest factor in whether to trade at all: almost every long strategy works in Risk-On and fails in Risk-Off.', pro: 'Experts gate every new long on it — in Risk-Off they stand down and protect capital instead of fighting the tape.' },
    tape: { g: 'Reading the market', t: 'The Tape (Trending vs Choppy)', plain: 'The texture of the market right now — Trending (clean, directional moves) or Choppy (whippy, sideways, range-bound).', why: 'It decides WHICH style works: breakouts and momentum need trending tapes; fades and mean-reversion need choppy ones.', pro: 'Match the strategy to the tape — the app ★-marks which strategies are "in their element" today.' },
    trendEff: { g: 'Reading the market', t: 'Trend Efficiency', plain: 'How much of the market\'s movement is one-directional: 1.0 = a clean straight line, 0 = pure noise. Above ~0.35 = trending, below ~0.22 = choppy.', why: 'It is the number behind the Trending/Choppy label.', pro: 'A falling efficiency warns that a trend is tiring and chop is coming — time to tighten up.' },
    // The screeners
    breakout: { g: 'The screeners', t: 'Breakout', plain: 'A stock pushing above a prior high or resistance level, usually on rising volume — a bet the move keeps going.', why: 'Breakouts work when buyers are in control (trending tapes); in chop they "fail" and trap you (false breakouts).', pro: 'Only take breakouts with the regime and tape behind them; in a choppy tape the app downgrades them automatically.' },
    momentum: { g: 'The screeners', t: 'Momentum', plain: 'Buying what is already moving up hard — big % gains and strong relative strength vs the market.', why: 'Winners tend to keep winning… briefly. It is a real but weak, regime-dependent effect.', pro: 'Treat momentum as a tilt, not a sure thing — always pair it with a stop and a regime gate.' },
    meanrev: { g: 'The screeners', t: 'Mean-Reversion / Fading', plain: 'Betting an overextended move snaps back. The Overheated screen fades blow-off tops (a short).', why: 'It works in ranging/choppy tapes where price reverts — and is dangerous in strong trends.', pro: 'The golden rule: never fade a strong trend. Only fade in choppy or neutral tapes.' },
    confluence: { g: 'The screeners', t: 'Confluence', plain: 'When several independent strategies (EMA, Supertrend, RSI, MACD, price-action) all agree on the same stock.', why: 'Agreement filters out noise — but the app\'s own tests show even agreement does not beat the market here.', pro: 'Use confluence to CONFIRM a thesis you already have, not as a standalone buy button.' },
    accumulation: { g: 'The screeners', t: 'Accumulation (Ghost)', plain: 'Quiet buying by big players BEFORE a breakout — flat price, rising volume, insider purchases.', why: 'It tries to get you in early, before the obvious move shows up on the chart.', pro: 'It is a watchlist of names "coiling" — wait for the breakout to confirm before committing.' },
    rvol: { g: 'The screeners', t: 'Relative Volume (RVOL)', plain: 'How today\'s trading volume compares to the stock\'s normal — 2× means twice the usual activity.', why: 'A volume spike is the clearest sign that something (a catalyst, big buyers) is happening right now.', pro: 'Relative volume is the heartbeat of a mover — pros want >1.5–2× before trusting a breakout or momentum signal.' },
    relStrength: { g: 'The screeners', t: 'Relative Strength (vs the market)', plain: 'Whether a stock is outperforming the S&P 500 — rising faster, or falling less, than the index.', why: 'Leaders lead: the strongest names tend to keep leading and hold up best when the market dips.', pro: 'Buy strength, not weakness — pros favor names already beating the market over "cheap" laggards.' },
    score: { g: 'The screeners', t: 'Composite Score', plain: 'A single 0–100 number that blends a screen\'s many factors (trend, volume, momentum, structure) into one rank.', why: 'It lets you compare and rank candidates at a glance instead of juggling a dozen metrics.', pro: 'Use the score to triage, then confirm the top few by hand — never trade a number blindly.' },
    conviction: { g: 'Trusting a signal', t: 'Conviction', plain: 'How strongly the engine rates an idea — higher means more factors agree and the name has a better track record.', why: 'Position size should scale with conviction: bet more on the strongest setups, less on the marginal ones.', pro: 'Top-conviction baskets beat the market more often in the app\'s tests — but even the best only narrowly clears the bar.' },
    // Entering a trade
    entry: { g: 'Entering a trade', t: 'Entry', plain: 'The price you plan to buy at. Each card shows two: a Pullback entry and a Breakout entry.', why: 'A disciplined entry caps your risk and improves your reward-to-risk before you even click buy.', pro: 'Never chase. A patient entry beats a FOMO fill almost every time.' },
    pullback: { g: 'Entering a trade', t: 'Pullback vs Breakout entry', plain: 'Pullback = wait to buy lower on a dip toward support. Breakout = buy now as it moves up.', why: 'Pullback gives a better price (but may not fill); breakout is a sure fill (but you are chasing).', pro: 'Pros favor the pullback — a lower entry means a tighter stop and a bigger reward:risk.' },
    stop: { g: 'Entering a trade', t: 'Stop-Loss', plain: 'The price where you admit the trade is wrong and sell to cap the loss.', why: 'The single most important habit in trading — it turns a bad trade into a small, survivable loss.', pro: 'Place it where the setup is invalidated (below the structure), and never move it down to avoid being stopped.' },
    target: { g: 'Entering a trade', t: 'Target (Take-Profit)', plain: 'Where you plan to take profit.', why: 'Knowing it up front lets you measure reward:risk before entering.', pro: 'Scale out near the target and let a runner ride with a trailing stop.' },
    // Managing risk
    rr: { g: 'Managing risk', t: 'Reward : Risk (R:R)', plain: 'How much you stand to make versus lose — e.g. 1:2 means risking $1 to make $2.', why: 'With 1:2, you can be right less than half the time and still make money.', pro: 'Skip trades under ~1:2; the price-action setup in this app demands 1:2.5 or better.' },
    sizing: { g: 'Managing risk', t: 'Position Sizing (1% risk)', plain: 'How many shares to buy. The rule: risk only ~1% of your account per trade. Shares ≈ (1% of account) ÷ (Entry − Stop).', why: 'It is what keeps a losing streak from blowing up your account — the #1 thing separating survivors from blowups.', pro: 'Fixed-fractional risk is non-negotiable. A wider stop automatically means a smaller position.' },
    beta: { g: 'Managing risk', t: 'Beta', plain: 'How much a stock moves relative to the market — β 2 swings about twice as much as the S&P 500.', why: 'High-beta names need smaller positions, and "beating the market" can secretly just be extra beta.', pro: 'Experts check beta-adjusted returns to tell whether an edge is real skill (alpha) or just leverage.' },
    // Trusting a signal
    beatRate: { g: 'Trusting a signal', t: 'Beat Rate', plain: 'The % of a strategy\'s past picks that beat the market. 50% is a coin flip.', why: 'It tells you whether a signal actually has skill, not just luck.', pro: 'Always read it next to the sample size — a great rate on 5 trades means nothing.' },
    wilsonLB: { g: 'Trusting a signal', t: 'Confidence Floor (Wilson LB)', plain: 'A conservative "floor" on the beat rate that accounts for how few trades there are. A floor below 50% means we cannot yet trust it beats the market.', why: 'It stops you fooling yourself with a small, lucky sample.', pro: 'Pros require the lower bound — not just the average — to clear 50% before sizing up.' },
    selflearning: { g: 'Trusting a signal', t: 'Self-Learning', plain: 'The engine logs every pick, scores it weeks later, and re-weights what is working — per strategy and per stock.', why: 'The app adapts to what actually pays instead of trusting theory forever.', pro: 'Watch the live track record and the strategy-weight panel — that is the real evidence, not the backtest.' },
    backtest: { g: 'Trusting a signal', t: 'Backtest & Out-of-Sample', plain: 'Replaying a strategy across years of history to measure its edge — including "out-of-sample" data it never saw during tuning.', why: 'In-sample results flatter you; out-of-sample is the honest test of whether an edge is real.', pro: 'The app validates every screen this way — and most "edges" vanish out-of-sample, which is why it is upfront about what does not work.' },
    calibration: { g: 'Trusting a signal', t: 'Calibration', plain: 'Whether a forecaster\'s confidence is honest: of all the calls it made with "70% confidence," did about 70% actually come true? A calibrated forecaster\'s stated odds match reality across the board.', why: 'Accuracy alone hides overconfidence. A model can be "right 55% of the time" while wildly overstating its certainty — calibration catches that.', pro: 'Pros trust calibrated sources and discount overconfident ones. The classic failure is overconfidence: claiming 80% and hitting 60%.' },
    brier: { g: 'Trusting a signal', t: 'Brier Score', plain: 'A single number scoring probability forecasts: the average squared gap between the stated probability and what happened (1 or 0). Lower is better — 0 is perfect, 0.25 is what you get always guessing 50%.', why: 'It rewards being both accurate AND honestly confident in one number, punishing confident-but-wrong calls hardest.', pro: 'Below 0.25 means real skill over a coin flip; pros track it to compare forecasters.' },
    forecast: { g: 'Trusting a signal', t: 'Falsifiable Forecast', plain: 'A market call written so it can be proven right or wrong — a specific subject (SPY, ^VIX, a sector), direction, size, and deadline (e.g. "QQQ +3% in 10 days"). The app grades every one against real prices automatically.', why: 'Vague calls ("stocks look strong") can never be wrong, so they teach you nothing. A falsifiable forecast keeps an honest scoreboard.', pro: 'Pros write falsifiable theses and track their hit rate. Short-term prediction is genuinely hard — expect the live accuracy to hover near a coin flip, which is the real lesson.' },
    predmarket: { g: 'Reading the market', t: 'Prediction Markets (the Crowd)', plain: 'Real-money betting markets (Kalshi, Polymarket) where people trade contracts on outcomes — "S&P above 7850 Friday", "Fed cuts in July", "CPI above 3%". The price is the crowd\'s live probability (8¢ = 8% odds).', why: 'Because real money is on the line, the prices aggregate what informed bettors actually expect — a fast read on how the crowd is pricing macro events that move stocks.', pro: 'Pros watch them as a sentiment radar: a sudden volume burst or a sharp odds swing flags that the crowd is repricing a catalyst — often confirming news, not predicting it.' },
    sharpmoney: { g: 'Trusting a signal', t: 'Sharp Money (Informed-Activity Hallmarks)', plain: 'The prediction-market version of "unusual options activity" — patterns a well-informed bettor tends to leave: putting real size on a cheap longshot that\'s rising, volume exceeding the open positions, fresh open-interest building, or a big late bet right before resolution.', why: 'When size and conviction line up on one outcome, it can mean someone is betting on knowledge the crowd doesn\'t have yet — an early tell on a catalyst that could move stocks.', pro: 'Treat it as a lead to investigate, never proof. It flags statistical FINGERPRINTS, not actual insider trading — most hits are coincidence, hedging, or rumor. The screen demands real money behind the bet before flagging it.' },
  };
  const LEARN_GROUPS = ['Reading the market', 'The screeners', 'Entering a trade', 'Managing risk', 'Trusting a signal'];
  ...
  const SB_SECTIONS   = { screener: '🔎 Screener', momentum: '🔥 Momentum', Ghost: '👻 Ghost Accumulation' };
  const SB_TIER_LABEL = { Breakout: 'Breakout', Setup: 'Setup', Early: 'Early', StrongBuy: 'Strong Buy', StrongSell: 'Strong Sell', GHOST: '👻 Ghost', STALKING: '🥷 Stalking' };
