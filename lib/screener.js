...
function lastBarOf(d) {
  return d && Array.isArray(d.candles) && d.candles.length ? d.candles[d.candles.length - 1].date : null;
}
...
async function fetchDailyHistory(ticker, range = '1y', { now = new Date() } = {}) {
  ...
  const primaryLast = lastBarOf(primary);
  // Nothing to do when the primary already reaches the session that is due (the
  // overwhelmingly common case), or when the calendar can't tell us what is due.
  if (!due || (primaryLast && primaryLast >= due)) return primary;

  const alt = await fetchStooqDaily(ticker);
  const altLast = lastBarOf(alt);
  if (!altLast) return primary;
  // Only prefer the fallback when it genuinely resolves the gap. A fallback that is
  // equally behind buys nothing and costs the adjClose leg.
  if (primaryLast && altLast <= primaryLast) return primary;

  logWarn('screener.fetchDailyHistory', 'primary behind calendar — using fallback provider', {
    ticker: String(ticker).toUpperCase(), due, primaryLast, altLast, provider: 'stooq',
  });
  return alt;
}
