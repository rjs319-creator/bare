...
async function fetchDailyHistory(ticker, range = '1y') {
  ...
  const path = `/v8/finance/chart/${sym}?range=${range}&interval=1d`;
  for (const host of ['query1.finance.yahoo.com', 'query2.finance.yahoo.com']) {
    try {
      ...
      for (let i = 0; i < ts.length; i++) {
        ...
        candles.push({ date: new Date(ts[i] * 1000).toISOString().slice(0, 10), open: o, high: h, low: l, close: c, volume: v || 0 });
      }
      if (candles.length < 60) continue;
      return { candles, meta };
    } catch { /* try next host */ }
  }
  logWarn('screener.fetchDailyHistory', 'no daily data (both hosts failed/empty)', { ticker: sym, range });
  return null;
}
...
function screenTicker(candles, meta = {}, opts = {}) {
  ...
  // ── Graded technical score (out of 80; LLM narrative adds up to 20) ──
  let tech = 0;
...
module.exports = { fetchDailyHistory, screenTicker, smaAt, evalSetupAt, rsHighArray };
