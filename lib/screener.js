...
const { fetchStooqDaily } = require('./daily-fallback');
...
// Is this symbol something the provider's US-EQUITY chart endpoint can serve at all?
//
// The nightly logs showed op=scoreboard and op=alertsgrade asking it for crypto (BTC.X),
// foreign listings (BB.TSX, TATAPOWER.NSE) and futures (RTY_F) — symbols that arrive from
// tracked alert/pick ledgers rather than the static universe, so pruning lib/universe.js
// never touched them. Each cost two host attempts a night and came back as a `no daily
// data` warning indistinguishable from a real equity failure.
//
// STRUCTURAL only, deliberately:
//   • a dotted suffix marks an exchange or asset class (.X crypto, .TSX, .NSE, .P). US
//     equities use a HYPHEN for class shares (BRK-B) and never a dot.
//   • `_F` marks a futures contract.
// Forex pairs (USDJPY) and bare index names (VIX, SPX) are NOT filtered — they are not
// structurally distinguishable from a real ticker, and a name blacklist could drop a live
// equity. They stay visible in the logs instead of being guessed at.
function isSupportedEquitySymbol(sym) {
  if (typeof sym !== 'string') return false;
  const s = sym.trim().toUpperCase();
  if (!s) return false;
  if (s.includes('.')) return false;
  if (s.endsWith('_F')) return false;
  return true;
}

async function fetchYahooDaily(ticker, range = '1y') {
  const sym = String(ticker || '').toUpperCase();
  // Skip the network entirely for symbols this endpoint cannot serve, and say so —
  // 'unsupported_symbol' is not the same fact as a ticker that failed.
  if (!isSupportedEquitySymbol(sym)) {
    logWarn('screener.fetchDailyHistory', 'no daily data', { ticker: sym, range, category: 'unsupported_symbol' });
    return null;
  }
  ...
  let lastBars = null;          // how many usable bars came back, when that was the problem
  ...
  for (const host of ['query1.finance.yahoo.com', 'query2.finance.yahoo.com']) {
    try {
      ...
      if (!result) { lastCategory = 'empty'; continue; }
      ...
      // A SHORT series is not a missing one. EA/GREE/GRTX/OLPX return real bars through
      // the current session but fewer than the indicators need; labelling that 'empty'
      // made a live young name look identical to a delisted one in the logs.
      if (candles.length < 60) { lastCategory = 'insufficient_history'; lastBars = candles.length; continue; }
      return {
        candles, meta,
  ...
  logWarn('screener.fetchDailyHistory', 'no daily data', {
    ticker: sym, range, category: lastCategory,
    ...(lastBars != null ? { bars: lastBars } : {}),
  });
  return null;
}
...
//
// On 2026-08-18 the primary feed simply had no 2026-08-17 bar — for SPY and for ~512 of
// the 514 large caps. Nothing errored, so every caller happily computed on Friday's data
// and the whole app published a four-day-old cross-section (see lib/cohort-freshness.js
// for how one lagging benchmark redefines "today" for every scan).
//
// So: when the primary series comes back BEHIND the session the exchange calendar says
// should already be published, ask a second provider. The fallback is used only if it
// actually reaches a newer session — otherwise the primary is kept, because it carries
// the richer adjClose leg. The two series are never spliced (see lib/daily-fallback.js).
//
// The lag check is pure calendar arithmetic on data already in hand, so the normal path
// costs nothing and makes exactly the requests it always did. Set DAILY_FALLBACK=off to
// disable the second provider entirely.
const DAILY_FALLBACK_OFF = String(process.env.DAILY_FALLBACK || '').toLowerCase() === 'off';

// The newest bar AT OR BEFORE the session that is due — i.e. the last SETTLED session
// the series actually carries. Never the newest bar outright: an in-progress bar for
// today sits after `due` and hides whatever is missing behind it.
function lastSettledBar(d, due) {
  if (!d || !Array.isArray(d.candles)) return null;
  for (let i = d.candles.length - 1; i >= 0; i--) {
    if (d.candles[i].date <= due) return d.candles[i].date;
  }
  return null;
}

async function fetchDailyHistory(ticker, range = '1y', { now = new Date() } = {}) {
  const primary = await fetchYahooDaily(ticker, range);
  if (DAILY_FALLBACK_OFF) return primary;

  const due = sessionDue(now);
  if (!due) return primary;   // calendar can't say what is owed — nothing to compare against

  // Ask whether the series REACHES the due session, not whether its tail is behind.
  // Once the market opened on 2026-08-18 the SPY axis read ...08-13, 08-14, 08-18 —
  // Monday absent, today's partial bar present — so a newest-bar test saw 08-18 >= 08-17
  // and concluded the feed was current. It was not: every ranking still ran on Friday.
  // A hole in the middle is exactly as stale as a short tail, and only the settled bar
  // can tell them apart.
  //
  // RE-LANDED after a revert. The first attempt emptied the screener: recovering the
  // benchmark's missing bar advanced the decision session ahead of the constituents,
  // which still sat on the nightly candle cache, and the cohort gate excluded all 514 of
  // them as `prior-session`. That was never this function's bug — the gate anchored the
  // cross-section's date to one series, so ANY divergence blanked the scan, and it
  // recurred on its own hours later when the provider backfilled. lib/cohort-freshness
  // now resolves the session the COHORT can support, so a benchmark that runs ahead of
  // the cache no longer costs the scan. See test/daily-fallback.test.js for the
  // interaction test that pins it.
  const primarySettled = lastSettledBar(primary, due);
  if (primarySettled === due) return primary;   // reaches the due session — the common case

  const alt = await fetchStooqDaily(ticker);
  const altSettled = lastSettledBar(alt, due);
  if (!altSettled) return primary;
  // Only prefer the fallback when it genuinely resolves the gap. A fallback equally
  // behind buys nothing and costs the primary's adjClose leg.
  if (primarySettled && altSettled <= primarySettled) return primary;

  logWarn('screener.fetchDailyHistory', 'primary missing the due session — using fallback provider', {
    ticker: String(ticker).toUpperCase(), due, primarySettled, altSettled, provider: 'stooq',
  });
  return alt;
}
...
function screenTicker(candles, meta = {}, opts = {}) {
  ...
  // Admission requires a real accumulation footprint. Volume surge (RVOL) alone no
  // longer admits a Breakout: this project's own research measured volSurge rank-IC
  // ≈ −0.004 (dead), so a name with unusual volume but NO accumulation evidence is
  // not a supported candidate. RVOL stays published as a liquidity/event DESCRIPTOR
  // (volSurge on metrics), never an admission or scoring credit. (scoring-version
  // bump: see strategy-registry `screener`.)
  const breakout = c_resistance && c_early && c_accumulation;
  ...
  // Volume-surge (rank-IC ≈ −0.004) and the rising-50-DMA bonus no longer add score:
  // both were unvalidated positive contributions, and techScore orders api/picks. The
  // remaining terms are reweighted to keep the same 0–80 scale.
  let tech = 0;
  if (c_accumulation) tech += 26; else if (tightPct <= baseMax * 1.3 && accumRatio >= 0.9) tech += 13;
  if (c_resistance) tech += 26; else if (broke) tech += 12;
  if (c_early) tech += 18; else if (broke && pctAbovePivot <= 0.18) tech += 9;
  if (isVCP) tech += 10;
...
module.exports = {
  isSupportedEquitySymbol, fetchDailyHistory, fetchYahooDaily, screenTicker, smaAt, evalSetupAt, rsHighArray, emergingLeaderSignal };
