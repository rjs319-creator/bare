...
// Wall + concurrency budget for GRADING (op=swingsearchgrade).
//
// THE BUG THIS FIXES (Vercel logs, 2026-08-18 22:02:03 → 504 after 300s):
// grading refetched the daily series once per SNAPSHOT, sequentially. With 19 day
// shards x ~149 snapshots that is ~2,800 network fetches for ~149 DISTINCT names,
// against api/tracker's 300s wall (vercel.json). The logs show EA — delisted, so
// every attempt costs the full 2-host timeout — refetched 7+ times in 75 seconds.
// The invocation died before reaching its writeJSON, so swingsearch/resolved/latest.json
// was never written: the whole forward evaluation was silently dead, every night.
//
// The fix is the pattern runSwingSearchLog already uses (bounded pool + wall budget +
// honest skip count), plus a per-run memo so a ticker — dead or alive — is fetched ONCE.
// Budget sits under the chain deadline (CHAIN_DEADLINE_MS 240s) rather than the raw
// function wall, because grade is the last step of a chain that has already spent time
// on op=swingsearchlog.
const GRADE_BUDGET_MS = 120000;
const GRADE_CONCURRENCY = 6;
const GRADE_MAX_SHARDS = 120;

const shardPath = (b) => (b && (b.pathname || b.url)) || '';

// Pure: the shards to grade, NEWEST FIRST, under a cap.
//
// Blob `list` returns pathname-ASCENDING (oldest first), so the previous
// `blobs.slice(0, 40)` would silently starve the newest days the moment the ledger
// passed 40 shards — a cap that reads as "graded everything" in the response. Newest
// first means growth costs the OLDEST day (already resolved) instead of today's.
function pickGradeShards(blobs, max = GRADE_MAX_SHARDS) {
  return [...(blobs || [])]
    .sort((a, b) => {
      const x = shardPath(a), y = shardPath(b);
      return x < y ? 1 : x > y ? -1 : 0;
    })
    .slice(0, max);
}

// Memoize an async single-argument fetch for the lifetime of one run, caching the
// NEGATIVE result too — a delisted ticker is the expensive case (two host attempts,
// ~20s) and re-paying it per snapshot is what blew the wall. A THROWN fetch is
// treated as transient: it resolves to null for the caller but is evicted, so a
// socket blip does not become a permanent hole in the run.
...
async function runSwingSearchGrade(req, res) {
  ...
  const t0 = Date.now();
  const overBudget = () => Date.now() - t0 > GRADE_BUDGET_MS;
  ...
  const shards = pickGradeShards(blobs, GRADE_MAX_SHARDS);
  const shardsDropped = Math.max(0, blobs.length - shards.length);

  // ONE fetch per distinct ticker for the whole run (see GRADE_BUDGET_MS above).
  const history = memoFetch((t) => fetchDailyHistory(t, '2y'));
  const spy = await history('SPY');
  const spyByDate = new Map((spy && spy.candles || []).map(c => [c.date, c]));

  // ── Pass 1: read the day docs (bounded pool, budget-aware) ──────────────
  const days = [];
  let di = 0, shardsSkipped = 0;
  const reader = async () => {
    while (di < shards.length) {
      const b = shards[di++];
      if (overBudget()) { shardsSkipped++; continue; }
      const day = await readJSON(shardPath(b), null).catch(() => null);
      if (day && Array.isArray(day.snapshots)) days.push(day);
    }
  };
  await Promise.all(Array.from({ length: GRADE_CONCURRENCY }, reader));

  // ── Pass 2: warm each DISTINCT ticker once, in parallel, under the budget ─
  const tickers = [...new Set(days.flatMap(d => d.snapshots.map(s => s && s.ticker)).filter(Boolean))];
  const series = new Map();
  let ti = 0, tickersSkipped = 0;
  const warmer = async () => {
    while (ti < tickers.length) {
      const t = tickers[ti++];
      if (overBudget()) { tickersSkipped++; continue; }
      series.set(t, await history(t));
    }
  };
  await Promise.all(Array.from({ length: GRADE_CONCURRENCY }, warmer));

  // ── Pass 3: grade. Pure + CPU-only — every series it needs is already in
  // `series`, so a budget-skipped ticker is SKIPPED, never silently refetched here.
  const resolved = [];
  for (const day of days) {
    for (const snap of day.snapshots) {
      if (!snap || !series.has(snap.ticker)) continue;
      const d = series.get(snap.ticker);
  ...
  // Report what was NOT covered — a cap or a budget skip must never read as
  // "graded everything" (the blind spot lib/warm-chains.js exists to remove).
  const coverage = {
    shardsTotal: blobs.length, shardsRead: days.length, shardsDropped, shardsSkipped,
    tickersTotal: tickers.length, tickersWarmed: series.size, tickersSkipped,
    elapsedMs: Date.now() - t0, complete: shardsDropped === 0 && shardsSkipped === 0 && tickersSkipped === 0,
  };
  const summary = summarize(resolved);
  await writeJSON(`${RESOLVED_PREFIX}latest.json`, { summary, n: resolved.length, coverage, at: new Date().toISOString() }, 0).catch(() => {});
  return res.status(200).json({ ok: true, graded: resolved.length, coverage, summary });
}
...
module.exports = {
  ...
  pickGradeShards, memoFetch, GRADE_BUDGET_MS, GRADE_CONCURRENCY, GRADE_MAX_SHARDS,
