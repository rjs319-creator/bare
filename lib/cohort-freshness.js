...
function resolveDecisionSession({ benchmarkDates = [], now = new Date() } = {}) {
  ...
  // Is the benchmark behind the sessions the exchange has actually completed?
  const benchmarkStale = !!(calendarSession && latest < calendarSession);
  if (benchmarkStale) source = 'benchmark-stale';
  const sessionsBehind = benchmarkStale ? countSessionsBetween(latest, calendarSession, now) : 0;
