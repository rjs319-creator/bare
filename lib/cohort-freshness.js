...
// Classify every row against one context. Pure; no mutation of `rows`.
function classifyAll(rows, ctx) {
...
// Below this share of the usable universe, a cohort is not a cross-section any more —
// it is a handful of survivors, and the decision session is worth re-examining.
const MIN_COHORT_SHARE = 0.5;

// The session the COHORT can actually support: the most common last-bar date among the
// rows, required to be a date the BENCHMARK also has a bar for (RS and the regime read
// are computed against it, so a session the benchmark cannot price is useless) and no
// newer than what the calendar can justify.
function cohortSupportedSession(rows, axis, ctx) {
  const inAxis = new Set(axis);
  const tally = new Map();
  for (const r of rows) {
    const d = r && r.lastBarDate;
    if (!d || !inAxis.has(d)) continue;
    if (ctx.ceiling && d > ctx.ceiling) continue;
    tally.set(d, (tally.get(d) || 0) + 1);
  }
  let best = null, bestN = 0;
  for (const [d, n] of tally) {
    // Ties break to the NEWER session — never silently prefer older data.
...
function gateCohort(rows, { benchmarkDates, now = new Date() } = {}) {
  const axis = (benchmarkDates || []).filter(Boolean);
  const ctx0 = resolveDecisionSession({ benchmarkDates, now });
  if (!ctx0.session) {
    return {
      decisionSession: null, sessionSource: ctx0.source, admitted: rows.slice(),
      excluded: [], counts: classifyAll([], ctx0).counts, adjudicated: false,
      calendarSession: ctx0.calendarSession, benchmarkStale: false, sessionsBehind: 0,
    };
  }

  let ctx = ctx0;
  let source = ctx0.source;
  let { admitted, excluded, counts } = classifyAll(rows, ctx);

  // THE DECISION SESSION MUST BE ONE THE COHORT CAN SUPPORT.
  //
  // Anchoring it to the benchmark alone breaks in BOTH directions, and each one blanks
  // the app. On 2026-08-18 the fallback provider recovered SPY's missing 08-17 bar while
  // the constituents still sat on the nightly cache at 08-14 — so the session advanced to
  // 08-17, 514 of 516 names were excluded as `prior-session`, and the screener returned a
  // single result. The mirror case (a lagging benchmark against fresh constituents) is
  // handled by the fail-open below. Both are the same mistake: treating one series as the
  // authority on a date the cross-section cannot actually price.
  //
  // So when the cohort collapses, re-resolve to the session the ROWS support — provided
  // the benchmark also has a bar there, since RS and the regime read need one. The result
  // is still a single same-vintage cross-section; it is simply dated honestly, and
  // `benchmarkStale`/`sessionsBehind` are recomputed against it so a session behind is
  // still reported as a session behind.
  const usable = rows.filter(r => r && r.lastBarDate).length;
  let cohortSupported = false;
  if (usable && admitted.length < usable * MIN_COHORT_SHARE) {
    const alt = cohortSupportedSession(rows, axis, ctx);
    if (alt && alt !== ctx.session) {
      const si = axis.lastIndexOf(alt);
      const altCtx = { ...ctx, session: alt, prior: si > 0 ? axis[si - 1] : null };
      // Recompute the calendar verdict for the session we would actually publish.
      altCtx.benchmarkStale = !!(ctx.calendarSession && alt < ctx.calendarSession);
      altCtx.sessionsBehind = altCtx.benchmarkStale ? countSessionsBetween(alt, ctx.calendarSession, now) : 0;
      const altRes = classifyAll(rows, altCtx);
      if (altRes.admitted.length > admitted.length) {
        ctx = altCtx; source = 'cohort-supported'; cohortSupported = true;
        ({ admitted, excluded, counts } = altRes);
      }
    }
  }
  ...
  return {
    decisionSession: ctx.session,
    sessionSource: failedOpen ? 'cohort-ahead-of-benchmark' : source,
    benchmarkLatest: ctx.latest,
    cohortAheadOfBenchmark: failedOpen, cohortSupported,
...
module.exports = { FRESHNESS_CLASS, resolveDecisionSession, classifyBarDate, gateCohort, countSessionsBetween, MIN_COHORT_SHARE };
