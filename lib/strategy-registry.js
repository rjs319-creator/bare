...
const STRATEGY_REGISTRY = [
  ...
  { id: 'insidercluster', label: '🧾 Insider Cluster Buys', kind: 'signal', section: 'InsiderCluster', horizon: 'position', core: false, maturity: 'shadow', scoringVersion: 'insidercluster-v1', policyTiers: ['CLUSTER'], note: 'SHADOW PROSPECTIVE LEDGER (2026-09-09), weight 0. Earned by hypothesis insider-cluster-residual (research/98, SEC bulk Form 345, 2,369 events / 832 dates): frozen gates passed (RES_21 +1.36% [0.25, 2.58] vs matched peers, t 2.41, q 0.016) but the date-level median was +0.19%, the top 1% of events carried the mean (ex-top-1% +0.01%) and the names were FLAT vs SPY (+0.54% n.s. @21s, 0.00% @63s). CLUSTER_EXCLUDED (liquid / illiquid / sub-$1 / 10b5-1) is the control lane. MUST NOT originate, boost, or add verdict weight to anything user-facing.', criteria: 'Prospective CLUSTER cohort on next-open fills at the 1m contract: ≥50 resolved episodes over ≥20 independent dates, cost-net excess vs SPY with the date-level CI clear of zero AND date-level median > 0 (or 10%-trimmed mean > 0) AND the top 1% of episodes contributing < 50% of the mean, ≥3/4 chronological blocks positive, beats the CLUSTER_EXCLUDED control — then an explicit reviewable registry maturity flip. The 2022-2026 retrospective window is spent.' },
  ...
  // AI news pick list (api/picks.js). Registered for the first time (alpha-research
  // pass 3): it publishes a user-facing ranked BUY list with a 1-10 rating and a
  // 'Strong Buy'/'Buy' label, from an LLM read of the news feed — and it had NO registry
  // entry at all. The Picks TAB was therefore dressed in the MOMENTUM strategy's evidence
  // banner (evidence-badge TAB_STRATEGY picks->momentum), showing one engine's resolved
  // record as if it were another's; the two share no code, universe or contract.
  // Shadow/weight-0: an LLM narrative read has no resolved cost-net record of its own.
  { id: 'picks', label: '📰 AI News Picks', kind: 'signal', section: null, horizon: 'position', core: false, maturity: 'shadow', scoringVersion: 'picks-v1', note: 'LLM-generated ranked pick list from the news feed (api/picks.js). Registered 2026-08-13 — it was previously unregistered AND borrowed the Momentum strategy\'s evidence banner. No resolved record of its own: the ratings are a language-model read of headlines, not a measured edge, and the quota that forced exactly 10 picks was removed in the same pass. Weight-0: MUST NOT originate or boost a live trade.', criteria: 'Its own prospective ledger: cost-net excess vs SPY at the position contract, >=50 resolved over >=20 independent dates with the date-level CI clear of zero, and incremental value over the same-day price/setup cohort (a news read must beat the tape it is reading) — then an explicit reviewable registry flip.' },
...
// reason is a DATA PIPELINE THAT HAS ONLY JUST STARTED ACCRUING, not a series of
// individually disappointing records. Recorded here so the ceiling is a stated,
// reviewable fact rather than something a reader has to infer from every strategy
// sitting at `promising` forever.
//
// THE MECHANISM. lib/maturity.gradeTrack requires `fillVerified`, which
...
// over a strategy's own canonical episodes. Until 2026-08-14 no non-Day-Trade module
// emitted a canonical episode carrying a `fillBasis`, so the derivation had nothing to
// read and correctly failed closed.
//
// PIPELINE STATUS (2026-08-14). The sanctioned exit path now EXISTS for ONE strategy:
// op=swingverify (lib/screener-verify.js, screener-verify-v1) grades the screener's
// promoted Early:large cohort on real 5-minute bars — published-trigger stop-entry,
// gap-through at the worse open, chase-ceiling gap-skip, honest NO_FILL, adverse
// exec-v1 slippage, 5-session time exit — and the Scoreboard reduces the accrued
// episode stream into `scoreboard/summary.json` as `fillVerification.screener`.
// Verification accrues PROSPECTIVELY ONLY: the pipeline being live changes no grade
// today. deriveFillVerified fails closed below its resolved minimum (≥20 resolved
// episodes, all on a verified basis), so the ceiling LIFTS ITSELF per strategy the
// moment — and only the moment — the data actually supports it. Every other
// non-Day-Trade strategy still has NO episode pipeline and remains blocked as before.
//
// WHY THE DAILY-BAR READING STAYS REJECTED. `VERIFIED_FILL_BASES` admits 'next-open',
// and the entry-v2.2 migration moved ~23 Scoreboard sections to next-open grading, so a
// literal reading would grant fill verification to most of the registry. That reading
// is rejected: a next-open fill reconstructed from DAILY bars is an approximation of an
// executable entry, not a verification of one — as the Scoreboard's own entryModel
// string says ("the fill time within a bar is a daily-bar approximation, NOT an
// intraday-verified fill"). Promotion unlocks governance clearance and therefore
// deployed capital, so the weaker basis is not accepted for it. Lead-only sleeves
// remain PROXY-labelled.
//
// HOW THE CEILING LIFTS — DATA ONLY, NEVER THIS DECLARATION. This object is carried on
// fail-closed verdicts as CONTEXT; nothing gates on it. Editing it can neither cap nor
// uncap a grade: uncapping happens exclusively when fillVerification[id] flows through
// maturity's existing consumption path with fillVerified:true derived from real
// resolved episodes. When that first happens, revisit this declaration (the tripwire
// tests in test/promotion-ceiling.test.js pin the current facts and will say so).
//
...
const PROMOTION_CEILING = Object.freeze({
  version: 'promotion-ceiling-v2',
  scope: 'non-daytrade',
  blocked: true,
  maxGrade: 'promising',
  blocker: 'intraday-fill-verification-accruing',
  summary: 'No non-Day-Trade strategy can reach `validated` YET: fill verification is derived from canonical '
    + 'episodes, and the one live pipeline (op=swingverify, screener Early:large, live 2026-08-14) accrues '
    + 'prospectively — no strategy has the resolved intraday-verified episodes deriveFillVerified requires, '
    + 'so every grade stays capped by the fail-closed derivation, never by this declaration. All other '
    + 'non-Day-Trade strategies still emit no canonical episodes. A daily-bar next-open reconstruction is '
    + 'deliberately NOT accepted as executable-fill verification for promotion.',
  pipelines: Object.freeze({
    screener: 'LIVE 2026-08-14 — op=swingverify (lib/screener-verify.js, screener-verify-v1): Early:large picks, '
      + 'published-trigger stop-entry verified on 5-min bars, reduced into scoreboard/summary.json as '
      + 'fillVerification.screener; accruing prospectively (~a session\'s picks per day, ≥20 resolved required).',
  }),
  unblockRequires: [
    'a canonical episode stream per strategy (lib/episode-ledger.makeEpisode) — live for screener only',
    'resolved episodes on an intraday-verified fill basis (cf. op=gapgoverify / op=swingverify)',
    'reduced into scoreboard/summary.json as fillVerification[<strategyId>]',
    'deriveFillVerified returning true on its own fail-closed rules (≥20 resolved, all bases verified) — no flag, wording edit, or ceiling edit can substitute',
  ],
