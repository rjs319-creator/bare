...
const STRATEGY_REGISTRY = [
  ...
  { id: 'ghost',      label: '👻 Ghost Accumulation',  kind: 'signal', section: 'Ghost',    horizon: 'swing',    core: true,  maturity: 'shadow', scoringVersion: 'ghost-v1', policyTiers: ['GHOST', 'STALKING'], note: 'DEMOTED from production 2026-08-11. Confirmatory hypothesis `quiet-accumulation-standalone` is no-edge (fails to beat a 12-1 momentum / price-core baseline as a standalone selector), and ghost×screener return correlation ≈0.96 means it was duplicating the Breakout sleeve rather than diversifying it. Keeps logging as a frozen benchmark; the tab and its ledger are untouched. Weight-0: MUST NOT originate or boost a live trade.', criteria: 'A NEW prospective record on a leakage-clean harness (the current ghost-backtest verdict selects refinements in-sample and purges 1 day against a 63-session horizon, so it cannot support promotion): cost-net excess vs SPY AND vs its sector at the swing contract, ≥50 resolved over ≥20 independent dates with the date-level CI clear of zero, PLUS demonstrated incremental value after redundancy adjustment against the screener (a 0.96-correlated sleeve must prove it adds something the Breakout book does not) — then an explicit reviewable registry maturity flip.' },
