...
// Bound on how many day-docs one tick will do actual resolution WORK on (rows attempted
// or resolved). Dates that make NO progress — read-only discoveries (already resolved,
// unreadable) and rows all in a no-attempt-possible waiting state — cost ~nothing and
// never consume the budget: a permanently-stuck old date (e.g. beyond SPY's 2y window)
// must not eat the whole budget every tick while newer resolvable dates starve behind
// it. Matches the old window size, but oldest-first over ALL unresolved days — nothing
// silently ages out any more.
...
async function runOmegaAb(req, res) {
  ...
  res.status(200).json({
    ...
    // Decision days that RESOLVED but yield no series point (one arm ended with zero
    // RESOLVED rows — all UNRESOLVABLE, a state the attempts path actively produces).
    // They enter no statistic, but they are accounted for here rather than vanishing.
    degenerateDates: summary.degenerateDates || [],
  });
}
...
async function runOmegaAbTick(req, res) {
  ...
  // Days already recorded RESOLVED-but-degenerate never re-enter the candidate list —
  // without this memo a resolved doc yielding no series point was re-read every tick
  // forever while appearing in no accounting at all.
  const degenerateDates = [...(summary.degenerateDates || [])];
  const knownDegenerate = new Set(degenerateDates);
  const candidates = matured.filter((d) => !inSeries.has(d) && !knownDegenerate.has(d));   // index.dates is sorted → oldest first
  let summaryDirty = false;
  let resolveBudget = MAX_RESOLVE_PER_TICK;
  const stuckDates = [];
  // Registers a RESOLVED doc in the summary accounting. A resolved doc whose
  // seriesPoint() is null (one arm all-UNRESOLVABLE — a state the attempts path
  // actively produces) lands in degenerateDates instead of the series: it enters no
  // statistic, but the "nothing silently ages out" guarantee holds — it is named in
  // the summary doc, the tick response, and the public read, and never re-read.
  const registerResolved = (doc) => {
    const point = AB.seriesPoint(doc);
    if (!point) {
      if (!knownDegenerate.has(doc.decisionDate)) {
        knownDegenerate.add(doc.decisionDate);
        degenerateDates.push(doc.decisionDate);
        degenerateDates.sort();
        summaryDirty = true;
      }
      return;
    }
    if (!(summary.points || []).some((p) => p.date === point.date)) {
  ...
  for (const d of candidates) {
    ...
    try {
      ...
      if (!doc) {
        // The index names this day but its doc is missing/unreadable — that is a stuck
        // day, not a no-op: surfacing it is the whole point of stuckDates.
        stuckDates.push(d);
        report.errors.push(`read ${d}: day doc missing or unreadable`);
        continue;
      }
      if (doc.resolved) { registerResolved(doc); continue; }    // repair: resolved doc whose summary append was lost
      ...
      // Budget counts only dates where resolution WORK happened (some row resolved,
      // burned an attempt, or went terminal). A date whose rows are all waiting with
      // no attempt possible costs nothing and never blocks iteration to newer dates.
      const progressed = JSON.stringify(next.rows) !== JSON.stringify(doc.rows);
      if (progressed) resolveBudget--;
      if (next.resolved) {
        await STORE.writeJSON(dayKey(d), next, 0);
        registerResolved(next);
        ...
        if (progressed) await STORE.writeJSON(dayKey(d), next, 0);
        stuckDates.push(d);
  ...
  if (summaryDirty || stuckChanged) {
    await STORE.writeJSON(SUMMARY_KEY, { experimentId: AB.EXPERIMENT_ID, points: summary.points || [], stuckDates, degenerateDates, updatedAt: new Date().toISOString() }, 0);
  }

  res.status(200).json({ ok: true, today, universe: UNIVERSE.length, ...report, stuckDates, degenerateDates, resolvedTotal: (summary.points || []).length });
}
