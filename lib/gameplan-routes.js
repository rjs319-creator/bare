...
async function runGamePlan(req, res) {
  ...
  const [macroRaw, headlines, optionsFlow] = await Promise.all([
  ...
  // fail-open — an unscoreable entry stays pending), then hand the model its own measured
  // record + open predictions. Pending entries never inject as record.
  let reflectionDoc = await readJSON(reflection.KEY, { entries: [] }).catch(() => ({ entries: [] }));
  try {
    const { fetchDailyHistory } = require('./screener');
    const spy = await fetchDailyHistory('SPY', '3mo');
    if (spy && Array.isArray(spy.candles)) {
      reflectionDoc = reflection.resolveEntries(reflectionDoc, spy.candles, new Date().toISOString());
    }
  } catch { /* resolution deferred — entries stay pending */ }
  const reflectionText = reflection.reflectionBlock(reflectionDoc);

  const Anthropic = require('@anthropic-ai/sdk');
  const client = new Anthropic({ apiKey: anthropicKey });
  let plan;
  try {
    plan = await gp.synthesize(client, { date, macro, headlines, signals, optionsFlow, priorNarrative, reflection: reflectionText });
  } catch (e) {
  ...
  const out = {
    ...
    ...plan,
    inputs: { regime: macro && macro.regime, vix: macro && macro.vix, headlineCount: headlines.length, hadPriorNarrative: !!priorNarrative, optionsGrade: optionsFlow ? `${optionsFlow.marketGrade} (${optionsFlow.marketScore > 0 ? '+' : ''}${optionsFlow.marketScore})` : null },
  };
  ...
  if (hasStore()) {
    ...
    // Reflection ledger: today's plan enters PENDING (scored on a later build); also
    // persists any resolutions computed above so they aren't recomputed forever.
    const recorded = reflection.recordPlan(reflectionDoc, {
