...
async function runGamePlan(req, res) {
  ...
  const [macroRaw, headlines, optionsFlow, narrativeDoc, reflectionDocRead] = await Promise.all([
  ...
  // hand the model its own measured record + open predictions.
  let reflectionDoc = reflectionDocRead;
  let spyCandles = null;
  if (reflection.hasMaturablePending(reflectionDoc, date)) {
    try {
      const { fetchDailyHistory } = require('./screener');
      const spy = await fetchDailyHistory('SPY', '6mo');   // '3mo' sits on the 60-candle floor in holiday-dense windows
      spyCandles = spy && Array.isArray(spy.candles) ? spy.candles : null;
    } catch { spyCandles = null; }
    if (spyCandles) {
