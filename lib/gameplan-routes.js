...
// sentinel line rather than silently omitting the symbol.
const SNAPSHOT_SYMBOLS = ['SPY', 'QQQ', 'IWM', '^VIX'];
...
// Market-implied event probabilities (Kalshi + Polymarket) with week drift, rendered
// to one compact prompt line. Degrades to '' — never blocks the plan.
async function gatherPredMarkets() {
  try {
    const pm = require('./predmarkets');
    const { readAllPredmktDays } = require('./store');
    ...
    const scored = pm.scoreMarkets(all, pm.buildBaseline(days, today));
    return snap.predMarketsLine(scored);
  } catch { return ''; }
}
...
async function runGamePlan(req, res) {
  ...
  const snapshotText = snap.snapshotBlock(snapCandles, date);
  ...
  const spyCandles = Array.isArray(snapCandles.SPY) && snapCandles.SPY.length ? snapCandles.SPY : null;
