...
const { fetchRecentIndexChanges, fetchRecentSmidIndexChanges } = require('./constituents');
...
async function runCernTick(cern, { nowMs = Date.now(), universe = LARGE, concurrency = 18, deadlineMs = 50000, lockupMinDollarVol = MIN_LOCKUP_DOLLAR_VOL } = {}) {
  ...
  try {
    ...
    // S&P 400/600 changes — SMID event types (own κ-posteriors/tiers). Same
    // passive-rebalance mechanics; several times the 500's event count, which is
    // the live paper book's binding constraint (7 resolved picks in 7 weeks).
    const smid = await fetchRecentSmidIndexChanges(70);
    const smidSyms = [...new Set([...smid.removes, ...smid.adds].map(x => x.ticker))].filter(t => !barsCache.has(t));
    await mapLimit(smidSyms, concurrency, async (t) => {
      try { const d = await fetchDailyHistory(t, '1y'); if (d && d.candles.length >= 30) barsCache.set(t, withMs(d.candles)); } catch {}
    });
    for (const r of smid.removes)
      cern.addEvent({ type: 'INDEX_DELETE_SMID', symbol: r.ticker, dateMs: Date.parse(r.date + 'T00:00:00Z'), estFlowShares: estFlow(r.ticker, 4), direction: -1, meta: { source: 'smid-change', index: r.index, date: r.date, sector: SECTOR_OF[r.ticker] || '?' } });
    for (const a of smid.adds)
      cern.addEvent({ type: 'INDEX_ADD_FADE_SMID', symbol: a.ticker, dateMs: Date.parse(a.date + 'T00:00:00Z'), estFlowShares: estFlow(a.ticker, 4), direction: 1, meta: { source: 'smid-change', index: a.index, date: a.date, sector: SECTOR_OF[a.ticker] || '?' } });
  } catch {}
