...
// attentionZ proxy: today's volume z-score vs the trailing 20 sessions. A volume
// spike is the cleanest free proxy for an attention spike (CERN lowers U when
// attention is high — we don't want to fade flow that the crowd is watching).
function volumeZ(bars) {
  const v = bars.slice(-21, -1).map(b => b.volume);
  if (v.length < 10) return 0;
  const m = mean(v), sd = Math.sqrt(mean(v.map(x => (x - m) ** 2))) || 1;
  return +((last(bars).volume - m) / sd).toFixed(2);
}
...
async function runCernTick(cern, { nowMs = Date.now(), universe = LARGE, concurrency = 18, deadlineMs = 50000 } = {}) {
  const t0 = Date.now();
  ...
  // These are mostly small/mid-caps outside LARGE, so fetch their bars too.
  try {
    ...
    for (const l of locks)
      cern.addEvent({ type: 'LOCKUP_EXPIRY', symbol: l.ticker, dateMs: l.lockupMs, estFlowShares: estFlow(l.ticker, 5), direction: -1, meta: { source: 'lockup', ipoDate: l.ipoDate, lockupDate: l.lockupDate, sector: SECTOR_OF[l.ticker] || '?' } });
  } catch {}
  ...
  return {
    ...
    posteriors: result.posteriors, firesale, downgrades, elapsedMs: Date.now() - t0,
  };
}
