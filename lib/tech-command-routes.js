...
async function runTechCommandHealth(req, res) {
  ...
  try {
    ...
    return res.json({
      ...
      tick: tick ? { ...tick, ...tickFreshness(tick, now) } : { note: 'The privileged tick has not run yet.' },
...
async function runTechCommandTick(req, res) {
  ...
  // ?universerebuild=1 forces a fresh tradable-universe build instead of reusing the
  // cached artifact. PRIVILEGED-ONLY by virtue of living on the tick: it costs a
  // directory fetch plus one screener call per exchange, so it must never be
  // reachable from a public read.
  const forceUniverseRebuild = req.query.universerebuild === '1' || req.query.universerebuild === 'true';
  let snapshot = null, failure = null;
  try {
    snapshot = await BUILD.buildSnapshot({ mode: 'full', now, t0, forceUniverseRebuild });
  } catch (e) {
    failure = err(e);
  }
  ...
  const health = {
    ...
    mode: 'full',
    universeRebuildForced: forceUniverseRebuild,
    universeAsOf: snapshot && snapshot.universe ? snapshot.universe.universeAsOf : null,
  ...
  // The health record is the operator's ONLY window into whether the tick is running.
  // Swallowing a failed write silently was observed to leave it 30 minutes behind
  // while ticks were succeeding — the endpoint reported an old universe count and an
  // old timestamp with no indication anything was wrong. A failed health write no
  // longer disappears: it is reported on the response, so it lands in the workflow log.
  let healthWriteError = null;
  try {
    ...
    healthWriteError = err(e);
  }

  return res.status(failure ? 500 : 200).json({
    ok: !failure,
    tick: health,
    // Not fatal — the board and the ledger are already persisted — but it means the
    // health endpoint is now describing an OLDER run than this one.
    healthWriteError: healthWriteError || undefined,
...
// ── Helpers ─────────────────────────────────────────────────────────────────
// A health RECORD can go stale independently of the tick: the board and ledger
// persist first, and a failed health write leaves this document describing an older
// run. The reader must be told that rather than being left to compare timestamps —
// a stale record and a dead tick look identical otherwise.
function tickFreshness(tick, now) {
  const at = tick && tick.lastAttemptAt ? Date.parse(tick.lastAttemptAt) : NaN;
  if (!Number.isFinite(at)) return { recordAgeMinutes: null, recordStale: null, recordNote: 'The health record carries no timestamp — its age cannot be assessed.' };
  const ageMin = Math.round((new Date(now).getTime() - at) / 60000);
  const due = tick.expectedNextAt ? Date.parse(tick.expectedNextAt) : NaN;
  const overdue = Number.isFinite(due) && new Date(now).getTime() > due;
  return {
    recordAgeMinutes: ageMin,
    recordStale: overdue,
    recordNote: overdue
      ? `This health record is ${ageMin} min old and past its expected next run (${tick.expectedNextAt}). Either the tick is not running, or its health write failed while the board itself kept updating — compare against the snapshot timestamp above before concluding the tick is dead.`
      : null,
  };
}
...
function lifecycleRowsOf(snapshot) {
  ...
  const lt = ((snapshot.longterm && snapshot.longterm.rows) || []).map(r => ({
    ...
    // Key change-detection on the DATED event only. nextProofPoints[0] flips between
    // "Next earnings report" and the generic "Next revenue-growth print…" depending on
    // whether the earnings-calendar call answered — so a rate-limited provider silently
    // rewrote the catalyst for every name at once and the lifecycle read it as "a new
    // catalyst was added". Boilerplate that appears for every name is not a catalyst.
    catalyst: r.earningsDate ? `earnings ${r.earningsDate}` : null,
...
module.exports = {
  ...
  isFresh, lifecycleRowsOf, readAllLifecycle, tickFreshness,
};
