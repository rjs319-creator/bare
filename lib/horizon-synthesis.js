...
// Coarse overall bucket — a LABEL, not an averaged score. "Aligned" is a claim
// about AGREEMENT, so it requires at least TWO available horizons — a single
// bullish read with the others unavailable is "leaning", never "aligned".
function overallFor(sides) {
  ...
  const allBull = vals.length >= 2 && vals.every(v => v > 0);
  const allBear = vals.length >= 2 && vals.every(v => v < 0);
