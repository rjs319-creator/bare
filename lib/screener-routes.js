  ...
  try {
    ...
    if (live) {
      ...
      const picks = [...live.strong, ...live.moderate]
        ...
        .map(p => ({ ticker: p.ticker, tier: p.tier, gapPct: p.gapPct, date: p.date, entry: p.plan.trigger, sector: p.sector, cause: p.cause || 'NONE', metaProb: p.metaProb, metaTier: p.metaTier, advUsd: Math.round(p.avgDollarVol || 0) || null, resolved: false }));
