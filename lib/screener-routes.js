...
async function scanFadeSetups(tickers, deadlineMs, t0, spyClose = {}, caches = []) {
  ...
  const worker = async () => {
    while (i < tickers.length) {
      ...
      if (v) out.push({ ticker: t, date: candles[candles.length - 1].date, signal: v, beta: betaVsSpy(candles, spyClose) });
    }
  };
  await Promise.all(Array.from({ length: 16 }, worker));
  return out;
...
async function attachCarry(pools, regime, newsCap = 40) {
...
function buildBestOpportunities(pool, limit = 8) {
  const sgn = v => (v >= 0 ? '+' : '') + v;
  const why = p => {
    const bits = [];
  ...
  return pool.filter(p => p.pctChange > 0 && p.carry != null)   // green today + scored
    .sort((a, b) => (b.carry - a.carry) || (b.relScore ?? 0) - (a.relScore ?? 0))
    .slice(0, limit)
    .map((p, i) => ({
...
async function runDaytrade(req, res) {
  ...
  const ml = live.scan1.slice(0, 60);
  const es = live.scan2.slice(0, 45);
  const runs = (live.scan3 || []).slice(0, 45);
...
async function runDaytradeTick(req, res) {
  ...
  try {
    ...
    if (live) {
      ...
      // Log only strict A-tier momentum_liquid (keeps the tracked ledger consistent with
      // history; the relaxed B-tier is a display-only "more picks" aid, not tracked).
      const picks = [...live.scan1.filter(p => p.tier === 'A').slice(0, 15), ...live.scan2.slice(0, 15)]
...
async function computeGapLive(t0, deadline, opts = {}) {
  ...
  const { fetchCompanyNews } = require('../lib/fundamentals');
  const { continuationScore, gapTake, suggestedRiskPct, classifyGapCause } = require('../lib/gapgo');
  let ci = 0;
  ...
  try {
    ...
    if (live) {
      ...
      const picks = [...live.strong, ...live.moderate]
        ...
        .map(p => ({ ticker: p.ticker, tier: p.tier, gapPct: p.gapPct, date: p.date, entry: p.plan.trigger, sector: p.sector, cause: p.cause || 'NONE', resolved: false }));
