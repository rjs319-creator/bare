...
// op=daytrade — live screener (lifecycle-authoritative display).
async function runDaytrade(req, res) {
  ...
  // validated and lifecycle-tracked THE SAME WAY as scan candidates. Absent/stale doc → no
  // discovery lane this cycle (honest, no fabrication).
  let discovery = null;
  try { discovery = await require('../lib/intraday-discovery').loadRecentDiscovery({ now: new Date() }); } catch { discovery = null; }
  const known = new Set([...ml, ...es, ...runs, ...exp].map(p => p.ticker));
  const disc = (discovery && discovery.anomalies ? discovery.anomalies : []).filter(a => !known.has(a.ticker));
  ...
  const persist = await saveLifecycleDay('daytrade', sessionDay, act.records);

  // Transition-only alerts (deduped per setup+state in the durable log; compact items into
  // the shared notification feed). Repeated refreshes cannot re-emit: unchanged state
  // produces no transition, and the persisted id dedup catches CDN/cold-start races.
  let alertsOut = { emitted: 0 };
  try { alertsOut = await require('../lib/daytrade-alerts').emitDaytradeAlerts(act.transitions, { date: sessionDay, now: nowIso }); }
  catch { /* alerting must never take down the board */ }
