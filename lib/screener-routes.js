...
async function scanDaytradeUniverse(tickers, params, ctx) {
  ...
  const worker = async () => {
    while (i < tickers.length) {
      ...
      const m = dt.dayMetrics(candles, ctx.spyByDate, undefined, ctx.pace || 1);
      if (!m) continue;
...
async function attachCarry(pools, regime, newsCap = 60) {
  ...
  for (const p of all) {
    ...
    if (s) { p.carry = s.carry; p.overextended = s.overextended; p.carryDrivers = s.drivers; p.catalyst = p._cat || null; }
...
async function computeDaytradeLive(t0, deadline, { pace = false, expanded = false } = {}) {
  ...
  // stale weekend/holiday bar or the pre-open warm cron is never paced). The tick/ledger
  ...
  const ctx = { spyByDate, t0, deadline, fe, state, pace: paceFrac };
...
function buildBestOpportunities(pool, limit = 12) {
  ...
  const why = p => {
    ...
    if (p.relVol >= 3) bits.push(`${p.relVol}× rel-vol`); else if (p.relVol) bits.push(`${p.relVol}× rel-vol`);
    if (p.pctChange >= 3) bits.push(`${sgn(p.pctChange)}% today`);
    if (p.overextended) bits.push('⚠ overextended');
    if (p.catalyst && ['FADE_OFFERING', 'MA'].includes(p.catalyst)) bits.push('⚠ dilution/M&A');
    return bits.slice(0, 3).join(' · ');
  };
  const SRC = { momentum_liquid: 'Momentum & Liquid', momentum_run: 'Multi-day run', explosive_small: 'Explosive small-cap' };
  return pool.filter(p => p.pctChange > 0 && p.carry != null)   // green today + scored
    // Rank by honest carry odds; break ties (carry is a coarse clamped %) by the per-stock
    // LEARNER tilt (fade-engine expAlpha — names whose momentum has actually continued),
    // then by descriptive relative strength. learnedExcess is absent on run-scan picks → 0.
...
async function runDaytrade(req, res) {
  ...
  const exp = (live.scan4 || []).slice(0, 40);   // 🌐 expanded lane (display-only, not tracked)
  ...
  assignRelScores([exp]);
  if (exp.length) await attachCarry([exp], live.regime);
  // 🔮 pcarry — honest, calibrated continuation ODDS on every pick (price overextension +
  // news catalyst + regime + scan base-rate). NOT a winner-picker: tradeable continuation
  // is ~coin-flip (the predictable part is un-tradeable overnight), so this mainly flags
  // FADE risk. See lib/pcarry.js + research/33.
  await attachCarry([ml, es, runs], live.regime);
  ...
  // Day's best opportunities — ranked by honest carry odds across the whole green pool
  // (overextended/explosive names flow through but sort down via low carry).
  const bestOpportunities = riskOff ? [] : buildBestOpportunities([...ml, ...es, ...runs]);
  res.setHeader('Cache-Control', 's-maxage=900, stale-while-revalidate=86400');
  return res.json({
    ok: true, regime: live.regime, condition: live.condition, riskOff, horizon: DAYTRADE_H,
    // ⭐ The day's best opportunities, ranked, drawn from the positive-edge scans.
    bestOpportunities,
    // Momentum/breakout fails in risk-off (the project's core finding) → stand down.
    momentumLiquid: riskOff ? [] : ml,
    explosiveSmall: riskOff ? [] : es,
    // Multi-day momentum runs (FCEL-style). Shown in ALL regimes — this is an
    // identification/watchlist of names ALREADY moving, not a trade signal, so it's
    // not stood down in risk-off (speculative runs/squeezes often happen there).
    momentumRun: runs,
    // 🌐 Expanded-universe momentum lane — same gate, full free universe, NEW names only.
    // Display-only / broader watchlist; NOT in the validated ledger or the counts below.
    momentumExpanded: riskOff ? [] : exp,
    counts: { momentumLiquid: live.scan1.length, momentumLiquidA: live.scan1.filter(p => p.tier === 'A').length, explosiveSmall: live.scan2.length, momentumRun: (live.scan3 || []).length, momentumExpanded: (live.scan4 || []).length },
    ...
    ranking: {
      ...
      bestBasis: 'Best Opportunities are ranked by carry odds across the whole green pool — overextended/explosive names are no longer hard-excluded, the model just discounts them so they sort to the bottom.',
    },
...
async function runDaytradeTick(req, res) {
  ...
  try {
    ...
    if (live) {
      ...
      ].map(c => ({ ticker: c.ticker, scan: c.scan, tier: c.tier || 'A', date: logDate, entry: c.last, score: c.score, sector: c.sector, resolved: false }));
...
async function scanConfluenceUniverse(tickers, ctx) {
  ...
  out.sort((a, b) => (b.indepScore + 8 * b.learnedExcess) - (a.indepScore + 8 * a.learnedExcess));
  return out;
}
...
async function runConfluence(req, res) {
  ...
  const ranked = [...live.large, ...live.small]
    .sort((a, b) => (b.indepScore + 8 * b.learnedExcess) - (a.indepScore + 8 * a.learnedExcess));
  const strong = ranked.filter(p => p.bullishCount >= CONFLUENCE_DISPLAY_BULL);
  ...
  const picks = (relaxed ? ranked.filter(p => p.bullishCount >= CONFLUENCE_MIN_BULL) : strong).slice(0, 25);
  ...
  return res.json({
    ...
    riskOff, horizon: CONFLUENCE_H, minBull: effectiveBull, relaxed,
...
async function scanCoilCohort(scopeQ, deadline = 40000) {
  ...
  await Promise.all(Array.from({ length: 16 }, worker));
  return { calScope, cohortCount: cohort.length, ranked: coil.rankCoil(cohort, calScope) };
}
...
async function scanCoilFull() {
  ...
  for (const r of [...lg.ranked, ...sm.ranked, ...mi.ranked, ...ex.ranked].sort((a, b) => b.score - a.score)) {
    if (seen.has(r.ticker)) continue; seen.add(r.ticker); ranked.push(r);
  }
  return { cohortCount: lg.cohortCount + sm.cohortCount + mi.cohortCount + ex.cohortCount, ranked };
}

// Map a ranked row → the API pick object (incl. breakout trade plan + rank score).
function buildCoilPick(r) {
  const coil = require('./coil');
  ...
  return {
    ...
    explodeProbPct: probPct,                           // calibrated P(abnormal +break in ~10d)
...
async function runCoil(req, res) {
  ...
  const { calScope, cohortCount, ranked } = isFull
    ? { calScope: 'mixed', ...(await scanCoilFull()) }
    : await scanCoilCohort(scopeQ);
  ...
  const picks = ranked.slice(0, limit).map(buildCoilPick).map((p, i) => ({ rank: i + 1, ...p }));
...
// op=coiltick — daily cron: log today's top coil picks (small+large) with the entry
// price + entry-time volatility, so each can later be auto-resolved against the abnormal
// -break outcome. This is what makes the calibrated probability self-validating.
const COIL_TICK_SCOPES = ['small', 'large'];
const COIL_TICK_TOPN = 15;
async function runCoilTick(req, res) {
  ...
  for (const scope of COIL_TICK_SCOPES) {
    let ranked = [];
    try { ({ ranked } = await scanCoilCohort(scope, 20000)); } catch { continue; }
    for (const r of ranked.slice(0, COIL_TICK_TOPN)) {
      const dailyVol = coil.trailingDailyVol(r.candles);
      if (!(dailyVol > 0) || !r.prob) continue;
      logged.push({
        ticker: r.ticker, scope,
        entryPrice: r.feats.price,
        dailyVol: +dailyVol.toFixed(6),           // entry-time vol → reproducible break threshold
  ...
  if (logged.length) await writeCoilDay(date, { picks: logged });
  res.setHeader('Cache-Control', 'no-store');
  return res.json({ ok: true, date, logged: logged.length });
}
...
async function scanGapUniverse(tickers, spyByDate, caches, t0, deadline) {
  ...
  const worker = async () => {
    while (i < tickers.length) {
      ...
      if (!candles) { try { const d = await fetchDailyHistory(t); candles = d && d.candles; } catch { continue; } }
      if (!candles) continue;
      const s = scoreGapGo(candles, spyByDate); if (!s) continue;
...
async function computeGapLive(t0, deadline, opts = {}) {
  ...
  const raw = await scanGapUniverse(universe, spyByDate, [cL, cS, cM, cE], t0, deadline);
  ...
  return res.json({
    ...
    config: {
      ...
      rules: [
        ...
        'Entry: opening-range breakout on the gap session (wait ~30 min, enter only on a break of the opening-range high — do not chase the gap open)',
      ...
      evidence: 'Backtested survivorship-free intraday (2022–2025): clean monotone dose-response (1–2% gap NULL negative → 5% gap +1.89%/trade, PF 1.47, positive all 4 years, OOS = in-sample) and PASSES deflation (Deflated Sharpe 0.99). Earnings gap-ups underperformed non-earnings in every bucket — hence the skip-earnings rule.',
      caveat: 'Edge tilts to high-beta gappers (liquid-only half went OOS-flat), P&L is lumpy/right-skewed (a few runners carry it), and it spans one 3.5y regime cycle. A strong lead to forward-track — confirm on the live ledger before sizing up.',
    },
    generatedAt: new Date().toISOString(),
  });
}
  ...
  try {
    ...
    if (live) {
      ...
      const picks = [...live.strong, ...live.moderate]
        ...
        .map(p => ({ ticker: p.ticker, tier: p.tier, gapPct: p.gapPct, date: p.date, entry: p.plan.trigger, sector: p.sector, cause: p.cause || 'NONE', metaProb: p.metaProb, metaTier: p.metaTier, resolved: false }));
...
async function computeDownDayLive(t0, deadline) {
  ...
  // LEARNED SHORT-RANKING: rank the fade bucket by the fade-engine's per-stock posterior
  // (the same learner behind the 🔥 Overheated tab) instead of the raw pattern score. Pure
  // in-memory (no fetches). Drifted names (edge stopped reverting) sink to the bottom.
  try {
    const fe = require('../lib/fade-engine');
    ...
    if (stateJson) {
      ...
      fades.sort((a, b) => (a.drifted ? 1 : 0) - (b.drifted ? 1 : 0) || (b.learnedExcess || 0) - (a.learnedExcess || 0) || b.score - a.score);
...
async function runDownDay(req, res) {
  ...
  return res.json({
    ...
    bounces: bounces.slice(0, 15), fades: fades.slice(0, 10), reversion: (reversion || []).slice(0, 10),
    ...
    config: {
      ...
      rules: [
        ...
        'Overheated (short): the mirror — a blow-off top rolling over. Ranked by the fade-engine\'s per-stock learned edge (the 🔥 Overheated tab\'s learner), drifted names sink.',
...
async function runGapDown(req, res) {
  ...
  if (!live) return res.status(502).json({ ok: false, error: 'No market data' });
  const { regime, spyChangePct, strong, moderate, earningsExcluded, scanned } = live;
  res.setHeader('Cache-Control', 's-maxage=900, stale-while-revalidate=86400');
  return res.json({
    ok: true, regime, horizon: GAPDOWN_H, spyChangePct, strong, moderate,
    counts: { strong: strong.length, moderate: moderate.length, earningsExcluded, scanned },
    config: {
...
async function runTiming(req, res) {
  ...
  const worker = async () => {
    while (i < picks.length) {
      ...
      results[p.ticker.toUpperCase()] = scoreTiming(snap, { stop: p.stop, target: p.target, trigger: p.trigger }, weights);
    }
  };
...
async function resolveTimingRows() {
  ...
  for (const d of days) for (const p of (d.picks || [])) {
    ...
    const exc = (r.c1 / r.c0 - 1) - (s.c1 / s.c0 - 1);
    rows.push({ date: d.date, ticker: p.ticker, grade: p.grade, f: p.factors, fwd: exc });
  }
...
async function runTimingBook(req, res) {
  ...
  return res.json({
    ...
    horizon: DAYTRADE_H,
    note: `Forward ${DAYTRADE_H}-session excess-vs-SPY of every logged pick, split by the timing light it showed. Green should beat amber should beat red for the grade to be earning its keep; the IC (grade→forward) is the one-number scorecard. Accrues via the daily cron.`,
    generatedAt: new Date().toISOString(),
  });
}
...
async function runTimingTune(req, res) {
  ...
  res.setHeader('Cache-Control', 'no-store');
  return res.json({ ok: true, promoted: cc.promoted, version, activeWeights: cc.promoted ? cc.weights : active, ...cc, defaultWeights: DEFAULT_WEIGHTS });
}
...
module.exports = { runFadeOpt, runFadeSeed, runFadeSignals, runFadeTick, runFadeBook,
  ...
  assignRelScores, buildBestOpportunities };
