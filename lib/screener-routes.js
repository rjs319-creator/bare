...
async function computeDaytradeLive(t0, deadline) {
  ...
  const scan2 = await scanDaytradeUniverse(rotateByDay([...new Set([...UNI_SMALL, ...UNI_MICRO])]), dt.SCANS.explosive_small, { ...ctx, caches: [cacheS, cacheM] });
  ...
  return { regime, condition, scan1, scan2, scan3, state, spyLastDate: spyD.candles[spyD.candles.length - 1].date };
}
