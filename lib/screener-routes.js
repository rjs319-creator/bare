...
async function computeDownDayLive(t0, deadline) {
  ...
  fades.sort((a, b) => b.score - a.score);
  return { tape, regime, bounces, fades, reality: DOWNDAY_REALITY, scanned: universe.length, spyLastDate };
}
...
async function runDownDay(req, res) {
  ...
  if (!live) return res.status(502).json({ ok: false, error: 'No market data' });
  const { tape, regime, bounces, fades, reality, scanned, spyLastDate } = live;
  res.setHeader('Cache-Control', 's-maxage=900, stale-while-revalidate=86400');
  return res.json({
    ok: true, tape, regime, horizon: DOWNDAY_H, spyLastDate,
    bounces: bounces.slice(0, 15), fades: fades.slice(0, 10),
    counts: { bounces: bounces.length, fades: fades.length, scanned },
    reality,
    config: {
      ...
      rules: [
        ...
        'Overheated (short): the mirror — a blow-off top rolling over. Mechanical short levels; see the 🔥 Overheated tab for the learned ranking.',
