...
async function scanCoilFull() {
  ...
  const scopes = [['large', lg], ['small', sm], ['micro', mi], ['expanded', ex]];
  // CROSS-SCOPE VINTAGE GATE. Each scope adjudicates its own session; merging them would
  // re-introduce the mixed-vintage ranking one level up (the cap-band caches are rebuilt
  // by the daily warm cron, the expanded cache is not). The merged board therefore keeps
  // only the scopes that agree with the NEWEST adjudicated session; the rest are reported
  // as excluded with their own vintage so the lag is visible rather than blended away.
  const sessions = scopes.map(([, r]) => r.decisionSession).filter(Boolean).sort();
  const decisionSession = sessions.length ? sessions[sessions.length - 1] : null;
  const admitted = scopes.filter(([, r]) => r.decisionSession && r.decisionSession === decisionSession);
  const excludedScopes = scopes
    .filter(([, r]) => !r.decisionSession || r.decisionSession !== decisionSession)
    .map(([name, r]) => ({ scope: name, decisionSession: r.decisionSession, names: r.rankedCount || 0, reason: r.decisionSession ? 'behind the newest adjudicated session' : 'no dated bars' }));

  // Dedupe (expanded overlaps the cap-band lists) — keep the first (cap-band) ranking.
  const seen = new Set(); const ranked = [];
  for (const r of admitted.flatMap(([, x]) => x.ranked).sort((a, b) => b.score - a.score)) {
    if (seen.has(r.ticker)) continue; seen.add(r.ticker); ranked.push(r);
  }
  return {
    cohortCount: scopes.reduce((a, [, r]) => a + r.cohortCount, 0),
    rankedCount: ranked.length,
    decisionSession,
    excludedStale: scopes.reduce((a, [, r]) => a + (r.excludedStale || 0), 0),
    excludedScopes,
    byScope: Object.fromEntries(scopes.map(([name, r]) => [name, { scanned: r.cohortCount, atSession: r.rankedCount || 0, session: r.decisionSession }])),
  };
}
...
module.exports = { runFadeOpt, runFadeSeed, runFadeSignals, runFadeTick, runFadeBook,
  ...
  runCoil, runCoilTick, runCoilBook, cohortFreshness, avgDollarVol,
