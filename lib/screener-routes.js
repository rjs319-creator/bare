...
async function computeDaytradeLive(t0, deadline) {
  ...
  const scan1 = await scanDaytradeUniverse(rotateByDay([...new Set(UNI_LARGE)]), dt.SCANS.momentum_liquid, { ...ctx, caches: [cacheL] });
...
async function runDaytrade(req, res) {
  ...
  return res.json({
    ...
    momentumRun: (live.scan3 || []).slice(0, 20),
    counts: { momentumLiquid: live.scan1.length, explosiveSmall: live.scan2.length, momentumRun: (live.scan3 || []).length },
...
function buildCoilPick(r) {
  ...
  return {
    ...
    rankScore: plan ? plan.expR : null,                // simplified expected R (rank driver)
    metrics: {
...
async function runCoil(req, res) {
  ...
  // Take the most-coiled `limit` names, then RE-RANK them by the trade-quality score
  // (expected R combines the calibrated break prob with each name's risk geometry).
  const picks = ranked.slice(0, limit).map(buildCoilPick)
