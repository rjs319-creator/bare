...
async function runCoil(req, res) {
  ...
  // DECLARED INFORMATION CUTOFF (lib/data-gates). The scan reads cached daily candles, so
  // the session it could actually observe is the newest bar the cohort SHARES — not the
  // clock. Reported as the modal last-bar date plus the count of names left behind it, so
  // a half-refreshed cache is visible rather than averaged away. Without this the coil
  // route published no cutoff at all and the pre-ranking gate correctly refused it.
  const coilFreshness = cohortFreshness(ranked);
  res.setHeader('Cache-Control', 's-maxage=1800, stale-while-revalidate=86400');
  return res.json({
    ok: true, scope: scopeQ, namesScanned: cohort.length, horizonDays: coil.COIL_HORIZON,
    freshness: {
      decisionSession: coilFreshness.decisionSession,
      staleCandidates: coilFreshness.behind,
      counts: { scanned: cohort.length, atDecisionSession: coilFreshness.atSession },
    },
...
module.exports = { runFadeOpt, runFadeSeed, runFadeSignals, runFadeTick, runFadeBook,
  ...
  runCoil, runCoilTick, runCoilBook, cohortFreshness,
