...
async function runDaytrade(req, res) {
  ...
  // Transition-only alerts, TWO CLASSES: lifecycle transitions (confirmed trigger /
  // maintenance) + upward early-state progressions (Early Watch — explicitly non-actionable).
  // Deduped per (setupId, state) in the durable log; retirement alerts for never-alerted
  // episodes are suppressed; per-ticker/day and per-cycle budgets bound the flood.
  ...
  try {
    ...
    // SHADOW MODEL SCORING — if (and only if) a learned champion has been promoted to shadow
    // via lib/model-registry, its outputs are recorded per transition snapshot beside the
    // deterministic scores. They never touch ranking or display; the artifact's basis string
    // (uncalibrated, research-only) travels with every output. No champion → null, no cost.
    const shadowByTicker = {};
    try {
      const { scoreShadow } = require('../lib/model-registry');
      for (const tr of act.transitions.slice(0, 12)) {
        const x = act.byTicker[tr.ticker];
        if (!x || !x.ev || !x.ev.metrics) continue;
        const s = await scoreShadow(x.ev.metrics).catch(() => null);
        if (!s) break;   // no champion promoted — skip the rest of the loop entirely
        shadowByTicker[tr.ticker] = s;
      }
