...
// Build the day's BEST OPPORTUNITIES — drawn ONLY from the scans with a validated
// positive continuation edge (Momentum & Liquid A-tier + multi-day runs). Explosive
// small-caps are deliberately excluded: they backtested INVERTED (most-explosive fade).
function buildBestOpportunities(mlA, runs, limit = 6) {
  const sgn = v => (v >= 0 ? '+' : '') + v;
  const why = p => {
    const bits = [];
    if (p.relVol >= 3) bits.push(`${p.relVol}× relative volume`); else if (p.relVol) bits.push(`${p.relVol}× rel-vol`);
    if (p.pctChange >= 3) bits.push(`${sgn(p.pctChange)}% today`);
    if (p.excessPct != null && p.excessPct > 0) bits.push(`${sgn(p.excessPct)}% vs SPY`);
    if (p.pct5d != null && p.pct5d > 0) bits.push(`${sgn(p.pct5d)}% 5-day run`);
    return bits.slice(0, 3).join(' · ');
  };
  const pool = [
    ...mlA.map(p => ({ ...p, source: 'Momentum & Liquid' })),
    ...runs.map(p => ({ ...p, source: 'Multi-day run' })),
  ].filter(p => p.pctChange > 0);   // a "best opportunity" should be GREEN today, not a red pullback
  return pool
    .sort((a, b) => (b.relScore ?? 0) - (a.relScore ?? 0) || b.score - a.score)
    .slice(0, limit)
    .map((p, i) => ({
      rank: i + 1, ticker: p.ticker, sector: p.sector, source: p.source, relScore: p.relScore,
...
async function runDaytrade(req, res) {
  ...
  // Raised caps → more picks surfaced. A-tier (strict) sorts ahead of B-tier (building).
  const ml = live.scan1.slice(0, 40);
  const es = live.scan2.slice(0, 30);
  const runs = (live.scan3 || []).slice(0, 30);
  ...
  // Day's best opportunities — from the validated-edge scans only (not explosive small-caps).
  const mlA = ml.filter(p => p.tier === 'A');
  const bestOpportunities = riskOff ? [] : buildBestOpportunities(mlA, runs);
  res.setHeader('Cache-Control', 's-maxage=900, stale-while-revalidate=86400');
  return res.json({
    ...
    ranking: {
      ...
      bestBasis: 'Best Opportunities are ranked from the scans with a validated positive 3-day continuation edge (Momentum & Liquid A-tier + multi-day runs). Explosive small-caps are excluded — they backtested INVERTED (the most-explosive tend to fade / mean-revert next day).',
    },
...
async function computeGapLive(t0, deadline) {
  ...
  const { continuationScore, gapTake, suggestedRiskPct } = require('../lib/gapgo');
  kept.forEach(p => {
    p.continuationScore = continuationScore(p.gapPct, p.relVol, regime);
    p.take = gapTake(p.continuationScore, regime);
...
async function runGapGo(req, res) {
  const live = await computeGapLive(Date.now(), 42000);
  if (!live) return res.status(502).json({ ok: false, error: 'No market data' });
  const { regime, strong, moderate, earningsExcluded, scanned } = live;
  res.setHeader('Cache-Control', 's-maxage=900, stale-while-revalidate=86400');
  return res.json({
    ok: true, regime, horizon: GAP_H,
    strong, moderate,
  ...
  try {
    ...
    if (live) {
      ...
      const picks = [...live.strong, ...live.moderate]
        ...
        .map(p => ({ ticker: p.ticker, tier: p.tier, gapPct: p.gapPct, date: p.date, entry: p.plan.trigger, sector: p.sector, resolved: false }));
...
async function runGapGoBook(req, res) {
  ...
  return res.json({
    ...
    note: `Live forward (${GAP_H}-session) excess-vs-SPY record of logged unscheduled gap-up picks, by tier. Accrues via the warm cron; thin until ~${GAP_H} sessions after the first tick. This is a daily-close PROXY of the intraday ORB strategy — directionally confirms the edge, doesn't reproduce intraday fills.`,
    generatedAt: new Date().toISOString(),
  });
}
...
async function runTiming(req, res) {
  ...
  const worker = async () => {
    while (i < picks.length) {
      ...
      results[p.ticker.toUpperCase()] = scoreTiming(snap, { stop: p.stop, target: p.target, trigger: p.trigger });
    }
  };
