...
async function scanDaytradeRuns(tickers, ctx) {
  ...
  const worker = async () => {
    while (i < tickers.length) {
      ...
      const m = dt.dayMetrics(candles, ctx.spyByDate, undefined, ctx.pace || 1);
      if (!m || !dt.passesRunScan(m)) continue;
      const lv = dt.tradeLevels(candles, { stopAtrMult: 2.5, useLowFloor: false });
      out.push({
        ticker: t, sector: SECTOR_OF[t] || '?', scan: 'momentum_run',
...
async function computeDaytradeLive(t0, deadline, { pace = false } = {}) {
  const { loadCandleCache } = require('../lib/candle-cache');
  const fe = require('../lib/fade-engine');
  const { fetchMacro } = require('../lib/macro');
  const dt = require('../lib/daytrade');
  ...
  return { regime, condition, scan1, scan2, scan3, state, spyLastDate, paceFrac };
}
...
async function runDaytrade(req, res) {
  const t0 = Date.now();
  const live = await computeDaytradeLive(t0, 45000, { pace: true });
  if (!live) return res.status(502).json({ ok: false, error: 'No market data' });
  ...
  return res.json({
    ...
    momentumRun: runs,
    counts: { momentumLiquid: live.scan1.length, momentumLiquidA: live.scan1.filter(p => p.tier === 'A').length, explosiveSmall: live.scan2.length, momentumRun: (live.scan3 || []).length },
...
async function runDaytradeBook(req, res) {
  ...
  const legacy = picks.filter(p => !isB(p));                 // A-tier momentum + explosive (comparable to history)
  ...
  return res.json({
    ...
    overall: agg(legacy),
    byScan: { momentum_liquid: agg(ml.filter(p => !isB(p))), explosive_small: agg(picks.filter(p => p.scan === 'explosive_small')) },
    // The requested A-vs-B comparison: same scan + universe, only the entry gate differs.
    ...
    note: `Live forward (${DAYTRADE_H}-session) excess-vs-SPY record of logged day-trade picks. overall/byScan/byRegime = A-tier momentum + explosive (comparable to history); byTier compares strict A vs relaxed B momentum_liquid on the same universe. Accrues via the warm cron; B-tier is thin until ~${DAYTRADE_H} sessions after the first tick post-deploy.`,
    generatedAt: new Date().toISOString(),
  });
}
...
async function runConfluence(req, res) {
  ...
  if (!live) return res.status(502).json({ ok: false, error: 'No market data' });
  const riskOff = live.regime === 'risk-off';
  // Live screener shows only STRONG (4★/5★) agreement; the ledger still logs ≥3★.
  const picks = [...live.large, ...live.small]
    .filter(p => p.bullishCount >= CONFLUENCE_DISPLAY_BULL)
    .sort((a, b) => (b.score + 8 * b.learnedExcess) - (a.score + 8 * a.learnedExcess)).slice(0, 25);
  res.setHeader('Cache-Control', 's-maxage=900, stale-while-revalidate=86400');
  return res.json({
    ok: true, regime: live.regime, condition: live.condition, favor: require('../lib/confluence').COND_FAVOR,
    riskOff, horizon: CONFLUENCE_H, minBull: CONFLUENCE_DISPLAY_BULL,
...
async function scanCoilCohort(scopeQ, deadline = 40000) {
  ...
  const calScope = scopeQ === 'large' ? 'large' : 'small';   // micro shares the small calibration
  const list = scopeQ === 'micro' ? UNI_MICRO : scopeQ === 'small' ? UNI_SMALL : UNI_LARGE;
...
async function runCoil(req, res) {
  const coil = require('./coil');
  const scopeQ = (req.query.scope || 'small').toLowerCase();
  const limit = Math.max(5, Math.min(50, parseInt(req.query.limit, 10) || 25));
  const { calScope, cohortCount, ranked } = await scanCoilCohort(scopeQ);
  ...
  const cal = coil.CALIBRATION[calScope];
...
async function computeGapLive(t0, deadline, opts = {}) {
  ...
  const universe = rotateByDay([...new Set([...UNI_LARGE, ...UNI_SMALL, ...UNI_MICRO])]);
  const raw = await scanGapUniverse(universe, spyByDate, [cL, cS, cM], t0, deadline);
