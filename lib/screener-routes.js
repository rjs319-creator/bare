...
async function computeDaytradeLive(t0, deadline, { pace = false, expanded = false } = {}) {
  ...
  const ctx = { spyByDate, t0, deadline, fe, state, pace: paceFrac, todayET };
...
// LIVE Best-Opportunities gate = fade avoidance AND current-session actionability. This is what
// buildBestOpportunities filters on, so a stale prior-session bar can never enter the actionable
// lane no matter how bullish yesterday's candle looks.
function passesQualityGate(p) {
  return passesFadeGate(p) && isActionableRow(p);
}
...
async function runDaytrade(req, res) {
  ...
  // The daily-cache scan is Stage-1 DISCOVERY. Here Stage-2 revalidates the shortlist with
  // current 5-min bars: only names with fresh current-session evidence that clear the intraday
  // gate become ACTIONABLE_NOW. Stale prior-session movers (yesterday's +3.35% HIMS) route to
  // PRIOR_SESSION_WATCH; intraday collapses route to Retired Today — neither can be a live buy.
  const { runActionability } = require('../lib/daytrade-actionability');
  const nowIso = new Date().toISOString();
  const act = await runActionability([...ml, ...es, ...runs], {
  ...
  // section carries lifecycleState / actionability / freshness timestamps / thesis validity.
  const enrich = arr => arr.map(p => (act.byTicker[p.ticker] && act.byTicker[p.ticker].card) || p);
  const mlC = enrich(ml), esC = enrich(es), runsC = enrich(runs);

  // Day's best opportunities = the ACTIONABLE-NOW lane, quality-gated (fade avoidance) and
  // ranked by carry. Empty in risk-off, and empty when nothing is live-actionable — an honest
  // empty state, never a backfill of stale prior-session names.
  const bestOpportunities = riskOff ? [] : buildBestOpportunities(act.lanes.actionableNow);
  ...
  const _shown = [...mlC, ...esC, ...runsC, ...exp];
  ...
  return res.json({
    ...
    strategyVersion: 'daytrade-actionability-v1',
    ...
    momentumLiquid: riskOff ? [] : mlC,
    explosiveSmall: riskOff ? [] : esC,
