...
async function runFadeSignals(req, res) {
  ...
  const recs = setups.map(s => {
    ...
    // isPolicy is stamped server-side so the UI filters on the flag instead of
    // duplicating the policy predicate (single source of truth: fe.isPolicyAction).
    return { ...r, sector, beta: s.beta, tier: s.signal.tier, score: s.signal.score, geometry: s.signal.geometry, refLevels: sig, geomFavorable: geomOk, isPolicy: fe.isPolicyAction(r.action) };
  });
  const rank = { SHORT: 3, SHORT_LIGHT: 2, WATCH: 1, SKIP: 0 };
  recs.sort((a, b) => (rank[b.action] - rank[a.action]) || (b.expAlpha - a.expAlpha));
  // Actionable = the policy cohort only (fe.isPolicyAction): SHORT_LIGHT's resolved
  // record earned it control-lane status, so it still ranks/logs but is never a call.
  const actionable = recs.filter(r => r.isPolicy && r.geomFavorable);
  ...
  return res.json({
    ...
    // Registry state rides on the payload so no API consumer can mistake a shadow
    // research read for a cleared trade. The old text ("Validated trade: SHORT …
    // Size = conviction-scaled weight") published an actionable, sized short plan
    // from a zero-weight shadow strategy with no borrow feed.
    maturity: (() => { try { return require('../lib/strategy-gate').statusOf('fade'); } catch { return 'shadow'; } })(),
    tradeEligible: (() => { try { return require('../lib/strategy-gate').isTradeEligible('fade'); } catch { return false; } })(),
    tradePlan: `RESEARCH read (registry: shadow, zero weight — not a trade instruction). The fade posterior is governance-approved as an AVOID filter only. Hypothetical short study: ~${FADE_H}-session horizon, market-neutral vs SPY. expAlpha = modeled per-trade short alpha %; netExpAlpha = after ~0.32% assumed cost, GROSS of borrow (no borrow feed — shorts fail closed in governance). refLevels are pattern reference only. No size is suggested.`,
...
async function computeDaytradeBoard({ t0 = Date.now(), mutate = false, source = 'page' } = {}) {
  ...
  const payload = {
    ...
    // What each legacy lane is FOR. These lanes answer different questions and were being read
    // as though they all answered "what should I buy today?". Nothing here redefines a lane or
    // a historical ledger — it labels them. The same-session explosion question lives in the
    // separate Low-Float Ignition Radar / Intraday Breakout Radar (op=lowfloat,
    // op=intradaycontinuation), which carry their own versions and their own promotion gate.
    lanePurposes: require('../lib/daytrade-config').LANE_PURPOSES,
...
async function runConfluence(req, res) {
  ...
  // HONEST ABSTENTION (2026-08): the old ≥3/5 "relaxed" fallback was a threshold
  // weakened specifically to keep the tab populated on calm days. When nothing
  // clears the strong bar, the tab now abstains — with the rejection counts —
  // because a below-bar pick is not made "confluent" by the tape being quiet.
  // (The ledger still logs ≥3★ independently; display admission is unchanged.)
  const picks = strong.slice(0, 25);
  const served = riskOff ? [] : picks;
  const abstained = served.length === 0;
  const rejectionReasonCounts = abstained ? {
    riskOff: riskOff ? picks.length : 0,
    belowStrongBar: ranked.filter(p => p.bullishCount < CONFLUENCE_DISPLAY_BULL).length,
    singleFamily: ranked.filter(p => p.bullishCount >= CONFLUENCE_DISPLAY_BULL && !multiFamily(p)).length,
  } : null;
  res.setHeader('Cache-Control', 's-maxage=900, stale-while-revalidate=86400');
  return res.json({
    ok: true, regime: live.regime, condition: live.condition, favor: require('../lib/confluence').COND_FAVOR,
    riskOff, horizon: CONFLUENCE_H, minBull: CONFLUENCE_DISPLAY_BULL, minFamilies: CONFLUENCE_MIN_FAMILIES,
    relaxed: false, abstained, rejectionReasonCounts,
    ...
    picks: served, count: served.length,
    learnerUpdatedAt: live.state.updatedAt, generatedAt: new Date().toISOString(),
  });
}
...
async function computeGapLive(t0, deadline, opts = {}) {
  ...
  // A position size is the one thing a registry-SHADOW strategy may never publish
  // (gapgo is an UNPROVEN PROSPECTIVE CHALLENGER — strategy-registry). The Kelly
  // arithmetic keeps running for research ledgers, but the user-facing field is
  // emitted only when the registry clears the strategy for live use.
  const gapgoSizeEligible = (() => { try { return require('../lib/strategy-gate').isTradeEligible('gapgo'); } catch { return false; } })();
  kept.forEach(p => {
    p.continuationScore = continuationScore(p.gapPct, p.relVol, regime);
    p.take = gapTake(p.continuationScore, regime, {
      cause: p.cause, skipFadeCauses: !!opts.skipFadeCauses,
      // Extended-gap avoid (research/86): annotated + ledgered always; the skip is
      // opt-in like the fade-cause skip, off until the forward ledger confirms.
      avoidExtendedGap: p.avoidExtendedGap, skipExtendedAvoid: !!opts.skipExtendedAvoid,
    });
    if (gapgoSizeEligible) p.suggestedRiskPct = suggestedRiskPct(p.tier, p.continuationScore, regime);
...
async function runGapGoBook(req, res) {
  ...
  return res.json({
    ...
    causeTarget: 150,
    // Forward extended-gap-avoid ledger (research/86: >=10% gap + prior-session strict
    // trend template = -0.21R clustered, FDR-surviving, negative 2021-2026). The live
    // OOS test: flagged picks should keep underperforming clear ones. The skip stays
    // opt-in until ~40 flagged picks resolve. Picks logged before the flag shipped
    // fall in `unscored` (null = insufficient history is also unscored, never guessed).
    byExtendedAvoid: {
      flagged: agg(picks.filter(p => p.avoidExtendedGap === true)),
      clear: agg(picks.filter(p => p.avoidExtendedGap === false)),
      unscored: agg(picks.filter(p => p.avoidExtendedGap == null)),
    },
    extendedAvoidTarget: 40,
