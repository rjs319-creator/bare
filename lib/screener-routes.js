...
async function scanCoilFull() {
  const [lg, sm, mi] = await Promise.all([
    scanCoilCohort('large'), scanCoilCohort('small'), scanCoilCohort('micro'),
  ]);
  const ranked = [...lg.ranked, ...sm.ranked, ...mi.ranked].sort((a, b) => b.score - a.score);
  return { cohortCount: lg.cohortCount + sm.cohortCount + mi.cohortCount, ranked };
}
