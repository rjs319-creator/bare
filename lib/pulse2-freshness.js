...
/**
 * Day-horizon gate → { group, why }. PLACEMENT is decision-relevance; VERIFICATION
 * stays with freshVerifiedGate — the two are deliberately different bars:
 *   'current'  the stories that matter for TODAY's decisions: event within 24h,
 *              OR a publication within 18h, OR a material update inside the day
 *              window (the only way an old event becomes current again). A recent
 *              event whose publication time could not be verified still surfaces
 *              here — its date confidence is displayed, never hidden — but it can
 *              never enter "Fresh verified".
 *   'context'  completely undated stories and 1-7 day-old events, rendered in a
 *              clearly separated "Earlier / undated context" group;
 *   'excluded' older than the context window.
 * Note firstPublishedAt only ever moves EARLIER (a fresh syndicated repost cannot
 * re-date an old event), so the publication branch cannot resurrect stale stories.
 * Pure.
 */
function dayGate(ev, now, policy = DEFAULT_POLICY) {
  ...
  const { eventAgeHours: ea, publicationAgeHours: pa } = eventAges(ev, now);
  if (ea == null && pa == null) {
    return { group: 'context', reason, why: 'no event or publication time known — undated context only' };
  }
  const effEa = ea ?? pa;
  if (effEa <= policy.DAY_MAX_EVENT_AGE_H || (pa != null && pa <= policy.DAY_MAX_PUB_AGE_H)) {
    return { group: 'current', reason, why: pa == null ? 'event is recent; publication time unverified — see date confidence' : null };
  }
