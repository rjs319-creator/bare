...
// GAP BOUNDS, GROUNDED IN WHAT THE SCHEDULER ACTUALLY DELIVERS.
//
// The workflow asks GitHub for */5, but GitHub's scheduled workflows are best-effort and
// heavily coalesced. Measured on this repo on 2026-08-19: 08:40, 09:16, 09:53, 10:26,
// 10:56, 11:31, 11:55 — real gaps of 24–37 minutes, every run succeeding. Judging that
// against 3×5min declared the app "stale — the scheduler may not be running" for most of
// every trading day, while the scheduler was running and `consecutiveErrors` was 0. A
// health check that cries wolf through the whole session is worse than none: it trains
// the user to ignore the one time it means something.
//
// So the two conditions are separated. Past LAGGING, the cadence is below target and the
// board should say so. Past STALE — well beyond any gap the scheduler has ever produced —
// something is actually wrong.
const LAGGING_AFTER_INTERVALS = 3;   // 15 min at the 5-min target: slower than asked for
const STALE_AFTER_INTERVALS = 18;    // 90 min: ~2.4× the worst gap ever observed ⇒ down
...
function nextHealth(prevHealth, scanResult, { nowMs, durationMs = null, error = null, targetMs = null } = {}) {
  ...
  const coverage = scanResult && scanResult.coverage ? {
    ...
    volumeCoveragePct: scanResult.coverage.volumeCoveragePct ?? null,
    // Per-provider outcome histogram (statuses, auth result, repair/gap-fill counts). Without
    // it, the 2026-08-05 session-long 1%-coverage outage reported WHICH provider answered but
    // never WHY the better one did not — a rate limit, a failed crumb handshake and an upstream
    // outage all looked identical from the health doc.
    diagnostics: scanResult.coverage.diagnostics ?? null,
  } : null;
...
async function recordScanObservation(result, { source = 'page', now = new Date(), durationMs = null, store = null } = {}) {
  ...
  const record = {
    ...
    // Why the snapshot looked the way it did — the per-provider status histogram. A scan that
    // reports 1% coverage has to say whether it was rate-limited, unauthenticated or down.
    quoteDiagnostics: (result && result.coverage && result.coverage.diagnostics) || null,
...
function healthStateOf(health, nowMs = Date.now()) {
  ...
  const targetMin = (targetMs / 60000).toFixed(0);
  const errs = health.consecutiveErrors || 0;
  // Genuinely stopped: a gap beyond anything the scheduler produces, or a shorter gap
  // with scans actively erroring (a lag with failures behind it is not just throttling).
  if (ageMs > STALE_AFTER_INTERVALS * targetMs || (errs >= 1 && ageMs > LAGGING_AFTER_INTERVALS * targetMs)) {
    return {
      status: 'stale',
      reason: `In-session with no successful scan for ${ageMin} min (target cadence ${targetMin} min)${errs ? `, with ${errs} consecutive error(s)` : ''}. Likely causes: scheduler stopped, CRON_SECRET missing or rejected, or upstream outage.`,
    };
  }
  // Running, just throttled. Named separately so the board reports the real situation
  // instead of accusing a scheduler that is working and erroring on nothing.
  if (ageMs > LAGGING_AFTER_INTERVALS * targetMs) {
    return {
      status: 'lagging',
      reason: `Last successful scan ${ageMin} min ago against a ${targetMin} min target, with no scan errors. GitHub's scheduled workflows are best-effort and commonly coalesce to 25–40 min on this repo, so a gap this size is the scheduler being throttled, not stopped. Movers can be up to this old.`,
    };
  }
  const degradedWhy = [];
...
const STATE_EXPLAIN = {
  ...
  lagging: 'Discovery is running but slower than target — scans are succeeding, just less often.',
