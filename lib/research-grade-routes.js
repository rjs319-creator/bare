...
async function runResearchGrade(req, res) {
  ...
  for (const { date, snap } of todo) {
    ...
    const batch = G.gradeSnapshot(snap, lookup, { asOf, benchCandles: bench });
    ...
    let mhBatch = null;
    try {
      mhBatch = MH.gradeSnapshotHorizons(snap, lookup, { asOf, benchCandles: bench });
...
module.exports = { runResearchGrade, findUngradedDates, horizonBatchHasPending, fetchAll, MAX_DAYS_PER_RUN, MAX_TICKERS, RUN_DEADLINE_MS };
