...
async function findUngradedDates(asOf, lookbackDays) {
  ...
  for (const d of dates) {
    ...
    if (stale) todo.push({ date: d, snap, graded });
...
async function runResearchGrade(req, res) {
  ...
  for (const { date, snap } of todo) {
    ...
    const batch = G.gradeSnapshot(snap, lookup, { asOf, benchCandles: bench, sectorLookup });
    ...
    let mhBatch = null;
    try {
      mhBatch = MH.gradeSnapshotHorizons(snap, lookup, { asOf, benchCandles: bench, sectorLookup });
    ...
    results.push({
      ...
      horizon: { written: mhWritten, resolvedByBar: mhBatch ? mhBatch.resolvedByBar : null },
...
module.exports = { runResearchGrade, findUngradedDates, horizonBatchHasPending, sectorEtfFor, candlesOf, fetchAll, MAX_DAYS_PER_RUN, MAX_TICKERS, RUN_DEADLINE_MS };
