...
async function computeCalibration(fetchHistory = fetchDailyHistory, sections = SECTIONS) {
  ...
  for (const s of perSection) {
    ...
    for (const p of s.picks) {
      ...
      // COST-NET outcome (tier from pick metadata, conservative defaults) — the digest this
      // feeds back into live prompts must never grade a class PROVEN on gross returns.
      const { roundTripCostPct, tierForPick } = require('./costs');
      records.push({ pick: p, exc: r.ret - benchRet - roundTripCostPct(tierForPick(p)) });
    }
