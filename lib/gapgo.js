...
const { minerviniTemplate } = require('./research/trend-template-vcp');
...
// ── EXTENDED-GAP AVOID (research/86-extended-gap-avoid, 2026-08) ────────────────
// A >=10% gap on a name whose PRIOR session passed the strict trend template (above
// rising 50/150/200 SMA stack, near 52w high) underperformed other >=10% gaps by
// -0.21R per trade (date-clustered CI [-0.37, -0.04], survives FDR q=0.037; win 37%
// vs 49%; negative in ALL of 2021-2026). Extension alone was NOT the driver, and the
// effect is absent on 5-10% gaps — so only this exact conjunction is flagged.
// Wiring mirrors gap-cause: ALWAYS annotated + persisted to the ledger (forward
// falsification), the skip itself is OPT-IN (default off) until the live ledger
// confirms out-of-sample. null = insufficient history to decide, never a guess.
const GAP_BIG_AVOID = 10.0;
...
function gapTake(score, regime, opts = {}) {
  ...
  // Opt-in extended-gap avoid (research/86): only an explicit true is skipped —
  // unknown (null) history is KEPT, a name is never dropped on missing data.
  if (opts.skipExtendedAvoid && opts.avoidExtendedGap === true) return false;
  return true;
}
...
// `todayEt` (ET 'YYYY-MM-DD') lets the engine mark whether the gap is the CURRENT official
// session's gap or a stale prior-session gap (barIsToday). Gap-and-Go is a daily-bar (EOD)
// realization so it does NOT pace, but surfacing barIsToday lets the route separate a
// confirmed current-session gap from a premarket/prior-session plan.
function scoreGapGo(candles, spyByDate, todayEt = null) {
  const m = dayMetrics(candles, spyByDate, undefined, 1, todayEt);
  ...
  if (!plan) return null;
  // Extended-gap avoid flag (research/86): template as of the PRIOR session — only
  // data available before the gap open may gate the decision. Cheap guard: the
  // template is only consulted where it can matter (>= 10% gaps).
  let avoidExtendedGap = false;
  if (m.gapPct >= GAP_BIG_AVOID) {
    const tmpl = candles.length >= 2 ? minerviniTemplate(candles, candles.length - 2) : null;
    avoidExtendedGap = tmpl == null ? null : tmpl.pass === true;
  }
  return {
    ...
    candidateDate: m.candidateDate, barIsToday: m.barIsToday,   // current-session gap vs stale prior-session gap
...
// The gap ledger's scoring identity — the ONE version the registry, contract, normalizer
// and ledger all share. The ledger rows were unversioned until the decision-time repair;
// rows without it are legacy-unversioned evidence and never promotion-grade. Execution
// channels carry their own versions (3d daily-close proxy; gapgo-orb-verify-v2).
const GAPGO_SCORING_VERSION = 'gapgo-v1';

// Freeze a scored live pick into the durable ledger record (pure — testable without the
// route). The persisted fields ARE the decision: the verifier may read them but never
// recompute them, so post-hoc data changes cannot alter what was decided.
//   decisionTs        wall-clock ISO of the nightly log write (post-close)
//   dataCutoffSession the completed session whose candle decided the pick (= signal session)
// The verified executor (lib/gapgo-verify) is only allowed to trade sessions AFTER
// dataCutoffSession — a decision logged post-close cannot own that session's own trade.
function freezeGapLedgerPick(p, { decisionTs, sessionDate }) {
  return {
    ticker: p.ticker, tier: p.tier, gapPct: p.gapPct, date: p.date, entry: p.plan && p.plan.trigger,
    sector: p.sector, cause: p.cause || 'NONE', metaProb: p.metaProb, metaTier: p.metaTier,
    avoidExtendedGap: p.avoidExtendedGap === true ? true : p.avoidExtendedGap === null ? null : false,
    advUsd: Math.round(p.avgDollarVol || 0) || null,
    take: p.take === true, continuationScore: Number.isFinite(p.continuationScore) ? p.continuationScore : null,
    earningsCheck: p.earningsCheck || null,
    scoringVersion: GAPGO_SCORING_VERSION, decisionTs, dataCutoffSession: sessionDate,
    resolved: false,
  };
}

module.exports = {
  scoreGapGo, GAP_STRONG, GAP_MODERATE, MIN_DOLLAR_VOL, GAP_BIG_AVOID,
  ...
  GAPGO_SCORING_VERSION, freezeGapLedgerPick,
};
