...
async function runLowFloatTick(req, res) {
  ...
  try {
    ...
    // 3b) IGNITION synthesis layer (lib/ignition-live-routes.js): attention ranks, dilution
    //     filings, spec-shaped views, ignition alerts, and its own snapshot. Runs inside this
    //     tick's lease so one pipeline compute serves both boards. A failure here must never
    //     break the low-float tick's own persistence — it is reported, not thrown.
    let ignitionLive = null;
    try {
      ignitionLive = await require('./ignition-live-routes').processIgnitionTick({
        records, allDiscovered: out.allDiscovered || [], sessionDate, now, regime: out.regime,
      });
    } catch (e) { ignitionLive = { ok: false, error: String((e && e.message) || e) }; }
...
// One FMP /stable/* quote endpoint, probed for its ACTUAL status on this subscription. The
// provider's own error text is the most useful thing here — never swallowed.
// Same probe through lib/fmp-client (categorised, retried, secret-redacted by construction).
async function probeFmpViaClient(path, params) {
  const { fmpRequest } = require('./fmp-client');
  const r = await fmpRequest(path, params, { attempts: 1 });
  const body = Array.isArray(r.body) ? r.body : null;
  const first = body && body[0] ? body[0] : null;
  return {
    status: r.status ?? null,
    category: r.category,
    rows: body ? body.length : 0,
    fields: first ? Object.keys(first).slice(0, 14) : null,
    hasVolume: !!body && body.some(q => q && q.volume != null),
    error: r.ok ? null : (r.error || null),
  };
}

async function probeFmpEndpoint(path, key) {
  const { redactSecrets, redactedMessage } = require('./redact');
  try {
    const r = await require('./http').fetchWithTimeout(`https://financialmodelingprep.com/stable/${path}&apikey=${key}`, { timeoutMs: 12000 });
    const text = await r.text();
    let parsed = null;
    try { parsed = JSON.parse(text); } catch { /* not JSON */ }
    return {
      status: r.status,
      shape: Array.isArray(parsed) ? 'array' : (parsed && typeof parsed === 'object' ? Object.keys(parsed).slice(0, 5) : 'unparseable'),
      rows: Array.isArray(parsed) ? parsed.length : 0,
      fields: Array.isArray(parsed) && parsed[0] ? Object.keys(parsed[0]) : null,
      hasVolume: Array.isArray(parsed) && parsed.some(q => q && q.volume != null),
      // Vendor-controlled text on a public response — redacted by mechanism, not convention.
      error: (!r.ok || !Array.isArray(parsed)) ? redactSecrets(text.slice(0, 300)) : null,
    };
  } catch (e) {
    return { status: null, error: redactedMessage(e) };
  }
}
...
async function runQuoteProbe(req, res) {
  ...
  if (key) {
    const probe = await probeFmpEndpoint(`batch-quote?symbols=${tickers.join(',')}`, key);
    out.fmpStatus = probe.status;
    out.fmpShape = probe.shape;
    out.fmpRows = probe.rows;
    out.fmpSample = probe.fields;
    out.fmpHasVolume = probe.hasVolume;
    if (probe.error) out.fmpError = probe.error;
    // ALTERNATES: batch-quote answers 402 on this plan, but the plan is Premium and the
    // subscription's endpoint matrix is not something to guess at. If one of these IS included,
    // it becomes the paid, rate-limit-free primary and the Yahoo fallback chain stops being
    // load-bearing at all. Probed one request each, only when explicitly asked for.
    if (String((req.query && req.query.alternates) || '') === '1') {
      out.fmpAlternates = {
        'batch-quote-short': await probeFmpEndpoint(`batch-quote-short?symbols=${tickers.join(',')}`, key),
        'quote': await probeFmpEndpoint(`quote?symbol=${tickers[0]}`, key),
        'quote-short': await probeFmpEndpoint(`quote-short?symbol=${tickers[0]}`, key),
        // Extended-hours + movers endpoints (probe ONLY — nothing live reads them). If either
        // is on-plan it is the cheapest possible premarket mover list / premarket quote and
        // would replace the Yahoo v7 extended-hours fields as the primary.
        'batch-aftermarket-quote': await probeFmpViaClient('batch-aftermarket-quote', { symbols: tickers.join(',') }),
        'biggest-gainers': await probeFmpViaClient('biggest-gainers', {}),
      };
    }
  }
  ...
  // SCALE PROBE — the 2026-08-05 outage was invisible to a 5-symbol probe: the chain works fine
  // small and collapses at universe size, where the rate limit actually bites. `?scale=N` runs
  // the real chain over N real symbols and returns coverage + per-provider diagnostics only
  // (never rows), so "is discovery healthy at scale right now?" is one request away.
  const scale = Math.min(1000, Math.max(0, parseInt((req.query && req.query.scale) || '0', 10) || 0));
  if (scale > 0) {
    const U = require('./universe');
    const pool = [...new Set([...(U.LARGE || []), ...(U.SMALL_CAPS || []), ...(U.MICRO_CAPS || [])])].slice(0, scale);
    const startedAt = Date.now();
    try {
      const bulk = await require('./quote-provider').fetchBulkQuotes(pool);
      out.scaleProbe = { requested: pool.length, elapsedMs: Date.now() - startedAt, coverage: bulk.coverage };
    } catch (e) {
      out.scaleProbe = { requested: pool.length, elapsedMs: Date.now() - startedAt, error: String((e && e.message) || e) };
    }
  }

  // The verdict keys off whether VOLUME actually arrived, not off which provider answered.
  // Naming a provider is not the question; three of the seven discovery lanes need a share
  // count, and any provider that supplies one is sufficient.
  const fmpNote = out.fmpConfigured && out.fmpStatus && out.fmpStatus !== 200
    ? ` (FMP's batch-quote is HTTP ${out.fmpStatus} on this plan, so it is being skipped.)`
    : '';
  out.verdict = out.resolvedVolumeAvailable
    ? `Volume IS available via ${out.resolvedProvider} — all seven discovery lanes can run.${fmpNote}`
    : `NO provider returned volume, so the relative-volume, volume-acceleration and low-float-turnover lanes cannot run and recall is materially reduced.${fmpNote}`;
