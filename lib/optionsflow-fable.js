...
//
// This module adds ONE bounded, PARAMETRIC claude-fable-5-1 call (via lib/fable-call) that reads each
...
const { callFableTool, FABLE_MODEL: MODEL } = require('./fable-call');   // claude-fable-5-1, strict tool, refusal-safe
...
function tickerBlock(r) {
  ...
  const split = `${r.callPremiumSharePct ?? r.bullishPct}% of ${usd(r.totalPremium)} premium in calls`;
  const flags = [];
  ...
  return `- $${r.ticker}: premium composition=${r.premiumSkew ?? r.net} (${r.grade}), ${split}, ${spot}${flags.length ? `\n    flags: ${flags.join('; ')}` : ''}\n  top contracts:\n${top}`;
}

function buildAnalysisPrompt(rows) {
  const blocks = rows.map(tickerBlock).join('\n');
  return `You are a senior options desk analyst. Below is today's UNUSUAL options flow (delayed Yahoo chain data — real prints, not live tape), one block per stock. A mechanical scanner flagged these and reported only the PREMIUM COMPOSITION (how the dollars split between calls and puts). That split is NOT a direction: it CANNOT distinguish a conviction directional bet from a hedge, a roll, a spread leg, a premium-selling write, or event positioning. Calls are not bullish and puts are not bearish. Your job is to EXPLAIN what the shown evidence means — not to write a trade plan.
...
async function analyzeFlow(rows, timeoutMs = CALL_TIMEOUT_MS) {
  ...
  const r = await callFableTool({
    tool: OPTIONS_FABLE_TOOL,
    maxTokens: MAX_TOKENS,
    timeoutMs,
    messages: [{ role: 'user', content: buildAnalysisPrompt(list) }],
    label: 'optionsflow-fable',
  });
  if (!r || !r.input) return null;   // graceful — caller keeps the mechanical read
  return parseAnalyses(r.input, list.map(row => row.ticker));
}
...
function mergeAnalyses(rollups, assessDoc) {
  ...
  return (rollups || []).map(r => {
    ...
    return {
      ...
      ai: {
        ...a,
        // The A/B control arm is the LEDGER's historical call/put convention (see
        // optionsflow.ledgerSignOf). It is compared against, never presented as, a view.
        agrees: a.bias === (r.premiumSkew === 'call-dominant' ? 'bullish' : r.premiumSkew === 'put-dominant' ? 'bearish' : 'neutral'),
      },
    };
  });
}
