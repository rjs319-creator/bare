...
// `horizonMin` (optional) truncates the evaluation window to decisionAt + horizonMin minutes
// (a bar-close timeout), enabling the 30/60/120-minute horizon labels.
function gradeOutcome({ decisionPrice, decisionAt, atr, forwardBars, kUp = DEFAULTS.kUp, kDown = DEFAULTS.kDown, slippageBps = DEFAULTS.slippageBps, horizonMin = null } = {}) {
  if (horizonMin != null && Number.isFinite(horizonMin)) {
    const cutoff = Date.parse(decisionAt) + horizonMin * 60000;
    forwardBars = (forwardBars || []).filter(b => Date.parse(b.t) <= cutoff);
  }
...
// Grade one decision at several horizons (default 30/60/120 min — lib/runner-dud
// HORIZONS_MIN). Barrier multiples stay at the PRE-REGISTERED defaults; alternative barrier
// sets are a validation-pipeline experiment (walk-forward), never a silent re-tune here.
function gradeHorizons(args, horizons = null) {
  const hs = horizons || require('./runner-dud').HORIZONS_MIN;
  const out = {};
  for (const h of hs) out[`h${h}`] = gradeOutcome({ ...args, horizonMin: h });
  return out;
}

module.exports = { DEFAULTS, forwardBarsAfter, gradeOutcome, gradeHorizons };
