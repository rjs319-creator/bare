...
// Composite top-exhaustion "stretch" z-score (same 4 features as the fadeopt
// harness): hotter RSI at the peak + steeper run-up + sharper rollover + deeper
// drop-so-far. fadeopt confirmed OUT-OF-SAMPLE that the HIGH-stretch tercile has
// ~zero fade edge (the most blown-off tops are already exhausted) while low/mid
// carry the alpha — so the engine penalises high-stretch setups. Maps live
// signal.geometry keys → the harness feature names. Returns null until the state
// has been seeded with normalization stats (graceful no-op).
const STRETCH_FEATURES = [
  ['rsiAtPivot', 'rsiPivot'],
  ['risePct', 'rise'],
  ['vSharpness', 'vSharp'],
  ['dropOffHighPct', 'dropOff'],
];
function stretchZ(geometry, stretch) {
  if (!geometry || !stretch || !stretch.stats) return null;
  let z = 0, used = 0;
  for (const [gKey, fKey] of STRETCH_FEATURES) {
    const v = geometry[gKey], st = stretch.stats[fKey];
    if (v == null || !st || !st.s) continue;
    z += (v - st.m) / st.s; used++;
  }
  return used ? +z.toFixed(3) : null;
}
...
function emptyState() {
  return { global: newAcc(), groups: {}, stocks: {}, updatedAt: null, cfg: { ...GLOBAL_PRIOR }, stretch: null };
}

function load(json) {
  // v1 state had global.{a,b}; v2 uses global.{W,...}. Old shape → fresh (re-seed).
  if (!json || !json.global || json.global.W == null) return emptyState();
  return { global: json.global, groups: json.groups || {}, stocks: json.stocks || {}, updatedAt: json.updatedAt || null, cfg: { ...GLOBAL_PRIOR, ...(json.cfg || {}) }, stretch: json.stretch || null };
}
const serialize = s => ({ global: s.global, groups: s.groups, stocks: s.stocks, updatedAt: s.updatedAt, cfg: s.cfg, stretch: s.stretch || null });
...
// Minimum reward:risk for a FULL-size short. A fresh setup thinner than this is
// still tradeable but only worth a light, sized-down position (downgraded to
// SHORT_LIGHT). Below this the structural stop costs more than the measured move
// pays — poor geometry even when the per-stock posterior is strong.
const MIN_RR_FULL = 1.0;
...
function recommend(state, { ticker, regime, signal, geometry, sector, beta }) {
  ...
  // Geometry gate: a strong per-stock posterior is only TRADEABLE when today's
  // setup is a fresh, valid short. If the measured move already played out
  // (expired levels / non-positive RR), the entry is late and the structural stop
  // sits implausibly far overhead — demote from an actionable short to WATCH no
  // matter how good the history. (Fixes high-conviction names surfacing with a
  // negative RR / target above entry.)
  const lv = signal || {};
  const freshLevels = lv.rr != null && lv.rr > 0 && !lv.expired;
  if ((action === 'SHORT' || action === 'SHORT_LIGHT') && !freshLevels) {
    action = 'WATCH';
    reason = 'Strong fade history, but the setup already played out (no fresh reward / stop too far) — wait for a new rollover.';
  } else if (action === 'SHORT' && lv.rr < MIN_RR_FULL) {
    // Fresh, but the reward:risk is too thin for full size — take it light.
    action = 'SHORT_LIGHT';
    reason = `Fresh setup but thin reward:risk (${lv.rr} < ${MIN_RR_FULL}) — size down.`;
  }

  // Stretch gate: the most over-extended tops (top stretch tercile) have ~zero
  // fade edge out-of-sample, so demote them one conviction tier — size flows to
  // the calmer rollovers that actually carry the alpha. Inert until seeded.
  const sz = stretchZ(geometry, state.stretch);
  const highStretch = sz != null && state.stretch && state.stretch.hiZ != null && sz >= state.stretch.hiZ;
  if (highStretch) {
    if (action === 'SHORT') { action = 'SHORT_LIGHT'; reason = 'Over-extended top (high stretch — ~zero fade edge historically) — size down.'; }
    else if (action === 'SHORT_LIGHT') { action = 'WATCH'; reason = 'Over-extended top (high stretch) with only light history — watch only.'; }
  }

  let sizePct = (action === 'SHORT' || action === 'SHORT_LIGHT') ? clamp(p.expAlpha * 2, 0, 5) : 0;  // +1% exp alpha → 2% weight, capped 5%
  if (action === 'SHORT_LIGHT') sizePct *= 0.5;                // LIGHT = half weight (now actually sized down)
  ...
  return {
    ...
    expired: !!lv.expired, freshLevels, stretch: sz, highStretch: !!highStretch,
    reason,
  };
}

// The ONLY action that is a policy recommendation (2026-08-09, from the resolved
// Scoreboard record): SHORT_LIGHT had no positive cell at any horizon (n≈230–320 per
// horizon, cost-net vs SPY, inverted-graded) and was CI-negative at 1d/20d/1m, while
// the strict SHORT gate was the strategy's only positive cohort at its 3–5d window.
// SHORT_LIGHT still gets emitted, logged and graded — as a control lane, not a call.
function isPolicyAction(action) { return action === 'SHORT'; }
...
module.exports = { GLOBAL_PRIOR, emptyState, load, serialize, posterior, update, recommend, isPolicyAction, betaVsSpy, summary, groupKey, betaBucket, stretchZ };
