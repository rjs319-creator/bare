...
function makeFetchJSON(host, { fetchJson = fetchWithTimeout } = {}) {
  return async (path) => {
...
function sourceDiagnostics(sources) {
  ...
  return { empty, barren, perSource, present: entries.length - empty.length, total: entries.length };
}
...
async function buildChallengerBoard(fetchJSON, opts = {}) {
  ...
  board.sourceDiag = sourceDiagnostics(gathered.sources);
