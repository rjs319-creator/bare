...
const CHALLENGER_FETCH = { timeoutMs: 12000, retries: 2, retryBaseMs: 300 };
...
async function runChallengerLog(req, res) {
  ...
  if (!board.sourceCount) {
    return res.status(503).json({ ok: false, error: 'no ranked signals gathered (sources empty or unreachable) — refusing to log an empty board', date, logged: 0 });
  }
...
module.exports = {
  ...
  makeFetchJSON, CHALLENGER_FETCH,
};
