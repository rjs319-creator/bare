...
function sourceDiagnostics(sources) {
  ...
  return { empty, present: entries.length - empty.length, total: entries.length };
}
  ...
  if (!board.sourceCount) {
    ...
    try {
      require('./log').logWarn('challenger.gather', 'no ranked signals gathered', {
        date, present: diag.present, total: diag.total, empty: diag.empty.join(',') || 'none',
      });
    } catch { /* logging must not break the error path */ }
    return res.status(503).json({
      ok: false,
      error: `no ranked signals gathered (${diag.present}/${diag.total} sources answered) — refusing to log an empty board`,
