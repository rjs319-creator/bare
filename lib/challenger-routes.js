...
// Self-fetch budget for the gather.
//
// This was a bare fetch(): no timeout, no retry. On 2026-08-18 op=challengerlog failed
// in 155ms because every sibling endpoint it asked was being OOM-killed by the 22:0x
// burst — it gathered zero sources and (correctly) refused to log, costing a whole day
// of predictions with the next attempt 24h away.
//
// lib/http.js fetchWithTimeout already implements the bounded retry for exactly this
// case, and its own docs name the opt-in rule: wide fan-out callers stay at 0 retries,
// low-volume callers opt in. The gather is a handful of self-fetches once a night.
// MEASURED, not guessed (production, 2026-08-20): the sources are not uniformly quick —
// scoreboard ~15.8s and today ~12.7s, while the other 16 return in well under 1s. The
// first version of this used 12s, which cut off precisely the two sources the ranking
// and the regime read depend on. Retries drop to 1 to keep the worst case (2 x 25s)
// comfortably inside the 240s chain deadline.
const CHALLENGER_FETCH = { timeoutMs: 25000, retries: 1, retryBaseMs: 300 };

// `fetchJson` is injectable so the retry/timeout contract is unit-testable.
function makeFetchJSON(host, { fetchJson = fetchWithTimeout, reasons = null } = {}) {
  return async (path) => {
    // A dead source is ONE null source, never an exception: a throw here used to
    // propagate out of the gather and lose the whole board even when every other
    // source answered fine.
    //
    // But `catch { return null }` also threw away WHY, which is how the same 503 got
    // misdiagnosed three times. Record the reason per path: the HTTP status when the
    // source answered, or the error name when the fetch never completed. A transport
    // failure, an abort and a bad body are three different faults and must not look alike.
    const note = (why) => { if (reasons) reasons[path] = why; };
    try {
      const r = await fetchJson(`https://${host}${path}`, { headers: internalHeaders(), ...CHALLENGER_FETCH });
      if (!r || !r.ok) { note(r ? `http:${r.status}` : 'no-response'); return null; }
      try {
        return await r.json();
      } catch (e) {
        note(`${(e && e.name) || 'Error'}: ${String((e && e.message) || e).slice(0, 80)}`);
        return null;
      }
    } catch (e) {
      note(`${(e && e.name) || 'Error'}: ${String((e && e.message) || e).slice(0, 80)}`);
...
// Pure: which gathered sources came back empty.
//
// An empty gather returns 503 and refuses to log the day. Until now that 503 recorded
// only THAT it happened, never which of the 18 sources were missing — so the same
// failure had to be re-guessed from scratch each time (once, wrongly, as an OOM
// symptom). Naming the empty sources turns the next occurrence into evidence.
//
// Total-safe: this runs on a path that is ALREADY failing, so it must never throw and
// turn a 503 into a 500.
function sourceDiagnostics(sources, { host = null, reasons = null } = {}) {
  const entries = (sources && typeof sources === 'object') ? Object.entries(sources) : [];
  const empty = entries.filter(([, v]) => !v).map(([k]) => k);

  // How much did each source that ANSWERED actually contribute? "18/18 answered" next to
  // a 503 is not a diagnosis — a source can return 200 and carry nothing (most plausibly
  // at 22:0x, while the warm scans are rebuilding the candle cache those endpoints read).
  // normalizeAll dispatches per key, so feeding it one key at a time yields that source's
  // own contribution without duplicating any normalisation logic here.
  const { normalizeAll, SIGNAL_SOURCE_KEYS } = require('./decision-sources');
  const perSource = {};
  for (const [k, v] of entries) {
    if (!v) continue;
    try { perSource[k] = normalizeAll({ [k]: v }).signals.length; } catch { perSource[k] = null; }
  }
  const barren = SIGNAL_SOURCE_KEYS.filter(k => perSource[k] === 0);

  return { empty, barren, perSource, host, reasons: reasons || {}, present: entries.length - empty.length, total: entries.length };
}
...
async function buildChallengerBoard(fetchJSON, opts = {}) {
  ...
  board.sourceDiag = sourceDiagnostics(gathered.sources, opts.diagContext || {});
...
async function runChallengerLog(req, res) {
  ...
  let board;
  // Collect per-source failure reasons + the host actually used, so an empty gather
  // reports WHAT happened instead of only that it happened.
  const host = hostFrom(req);
  const reasons = {};
  try { board = await buildChallengerBoard(makeFetchJSON(host, { reasons }), { asOf: (et && et.date) || date, diagContext: { host, reasons } }); }
  catch (e) { return res.json({ ok: false, error: String((e && e.message) || e) }); }

  // An empty gather is a FAILURE, not a market read: the sources always carry hundreds of
  // signals, so zero ranked inputs means the self-fetched endpoints were unreachable or
  // degraded (e.g. the 22:0x cron burst OOM-killing sibling invocations). The old path
  // logged the empty board with ok:true/logged:0 — a month of nightly prediction batches
  // accrued with nothing resolvable in them while op=health stayed green. Return 503 so
  // the chain report records a stepFail and the day is retried by the next run instead of
  // being immutably recorded as an empty decision.
  if (!board.sourceCount) {
    // Say WHICH sources were empty, in the chain report AND in the runtime logs, so the
    // next occurrence is diagnosable from the evidence instead of re-derived by guesswork.
    const diag = board.sourceDiag || sourceDiagnostics(null);
    try {
      require('./log').logWarn('challenger.gather', 'no ranked signals gathered', {
        date, present: diag.present, total: diag.total,
        host: diag.host || 'unknown',
        empty: diag.empty.join(',') || 'none',
        barren: diag.barren.join(',') || 'none',
        perSource: JSON.stringify(diag.perSource),
        reasons: JSON.stringify(diag.reasons).slice(0, 900),
      });
    } catch { /* logging must not break the error path */ }
    return res.status(503).json({
      ok: false,
      error: `no ranked signals gathered (${diag.present}/${diag.total} answered, ${diag.barren.length} barren) — refusing to log an empty board`,
      date, logged: 0, sourceDiag: diag,
    });
  }
...
async function runChallengerResolve(req, res) {
  ...
  for (const p of pending) {
    ...
    // Stamp the scope the record already KNOWS (capTier) — with tierForPick now
    // failing closed to 'small' on unstamped picks, an unstamped large-cap would be
    // over-charged 60bps instead of the liquid 16bps its measured cap tier earns.
    // Unknown capTier stays unstamped and correctly takes the conservative fallback.
    const tier = tierForPick({
      section: p.strategyFamily === 'biotech' ? 'Biotech' : undefined,
      scope: p.capTier === 'small' || p.capTier === 'mid' ? 'small'
        : p.capTier === 'large' ? 'large' : undefined,
    });
...
module.exports = {
  ...
  makeFetchJSON, CHALLENGER_FETCH, sourceDiagnostics,
};
