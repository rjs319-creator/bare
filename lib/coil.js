...
// coil top, stop below the base (ATR-bounded), target the exact abnormal-break level
// the probability is calibrated to (so the % and the target are self-consistent).
...
//   • target = entry × (1 + COIL_SIGMA × own 10-day vol)  = the calibrated break
// Rank score = simplified expected R-multiple using the CALIBRATED break probability:
//   expR = p·(reward/risk) − (1−p)·1   — higher = better breakout asymmetry.
function coilTradePlan(candles, i = candles.length - 1, breakProbPct = null) {
  ...
  return Object.freeze({
    ...
    rr: +rr.toFixed(1),
    expR: expR != null ? +expR.toFixed(2) : null,     // rank score (simplified expected R)
  });
}

module.exports = {
  coilFeatures, scoreCohort, zColumn, rankCoil, explodeProbability, CALIBRATION,
  trailingDailyVol, resolveBreak, coilTradePlan, COIL_HORIZON, COIL_SIGMA,
};
