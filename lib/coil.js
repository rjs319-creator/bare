...
// A coil is a pre-breakout setup, so the plan is a BREAKOUT: buy the break above the
// coil top, stop below the base (ATR-bounded), target the abnormal-break level.
//   • entry  = just above the recent 10-day high (the ceiling of the tight coil)
//   • stop   = the tighter of the 10-day low / entry − 1.5×ATR(14)  (coil-tight risk)
//   • target = entry × (1 + COIL_SIGMA × own 10-day vol)  = the abnormal-break level
//
// HONESTY (the semantic fix): `breakProbPct` is the EMPIRICAL abnormal-EXCURSION rate —
// P(the name reaches an abnormal-break level), NOT P(this trade makes money). The two
// differ because you can be stopped out first or never fill the trigger. So `expR` here
// is retained only as a LEGACY excursion-weighted rank; the honest, executable numbers
// (pTrigger / pTargetBeforeStop | fill / pProfitableNet | fill / expectedNetR, net of cost)
// come from the transparent — and UNCALIBRATED — barrier model in `executable`. Never
// read `breakProbPct` as a win rate.
function coilTradePlan(candles, i = candles.length - 1, breakProbPct = null, opts = {}) {
  if (!Array.isArray(candles) || i < 20) return null;
  const px = candles[i].close;
  if (!(px > 0)) return null;
  const win10 = candles.slice(i - 9, i + 1);          // the tight recent coil, not the whole base
  const hi10 = Math.max(...win10.map(x => x.high));
  const lo10 = Math.min(...win10.map(x => x.low));
  const atr14 = atrAt(candles, i, 14);
  const dv = trailingDailyVol(candles, i);
  if (!(atr14 > 0) || !(dv > 0)) return null;
  const sigma10 = dv * Math.sqrt(COIL_HORIZON);
  const entry = Math.max(hi10, px) * 1.001;           // break just above the coil ceiling
  const stop = Math.max(lo10, entry - 1.5 * atr14);   // tighter of coil-low / 1.5×ATR (coil-tight risk)
  const risk = entry - stop;
  if (!(risk > 0)) return null;
  const target = entry * (1 + COIL_SIGMA * sigma10);  // the abnormal-break level the prob is calibrated to
  const reward = target - entry;
  const rr = reward / risk;
  let expR = null;
  if (breakProbPct != null && isFinite(breakProbPct)) { const p = breakProbPct / 100; expR = p * rr - (1 - p); }
  // Executable estimate — the honest, cost-aware, separated probabilities (uncalibrated).
  const executable = coilExecutable({
    current: px, entry, stop, target, dailyVol: dv, horizon: COIL_HORIZON,
    roundTripCostPct: opts.roundTripCostPct,
  });
  return Object.freeze({
    current: +px.toFixed(2),
    entry: +entry.toFixed(2),
    stop: +stop.toFixed(2),
    target: +target.toFixed(2),
    riskPct: +(risk / entry * 100).toFixed(1),
    rewardPct: +(reward / entry * 100).toFixed(1),
    rr: +rr.toFixed(1),
    expR: expR != null ? +expR.toFixed(2) : null,     // LEGACY excursion-weighted rank (NOT a net win rate)
    executable,                                       // ★ separated executable probs (net of cost, uncalibrated)
  });
}

// Assemble the CANONICAL prediction for a coil pick — the piece that guarantees the
// empirical abnormal-break rate and the executable trade probability can never be
// conflated downstream. `prob` is the explodeProbability() output (empirical excursion
// rate); `plan` is coilTradePlan() (carries the executable barrier estimate). Percentile
// is the cohort rank — explicitly a RANK, not a probability.
function coilPrediction({ prob, plan, percentile = null, scope = 'small', asOf = null }) {
  const ex = plan && plan.executable;
  const pAbn = prob && prob.pct != null ? prob.pct / 100 : null;   // EMPIRICAL excursion rate → [0,1]
  return makePrediction({
    rankPercentile: percentile != null && isFinite(percentile) ? Math.max(0, Math.min(1, percentile)) : null,
    pTrigger: ex ? ex.pTrigger : null,
    pTargetBeforeStopGivenFill: ex ? ex.pTargetBeforeStopGivenFill : null,
    pProfitableNetGivenFill: ex ? ex.pProfitableNetGivenFill : null,
    severeLossProbability: ex ? ex.severeLossProbability : null,
    expectedNetR: ex ? ex.expectedNetR : null,
    uncertaintyInterval: ex ? ex.uncertaintyInterval : null,
    // The empirical decile break rate IS calibrated, but it is an EXCURSION rate, not a
    // trade win rate — so it lives in `extra`, clearly named, never in a p*Given* field.
    calibrationStatus: ex ? 'model-estimate' : 'unknown',
    validationStatus: 'research',
    survivorshipSafe: false,       // coil calibration is present-universe (see lib/coil.js header)
    pointInTimeSafe: false,
    modelVersion: 'coil-bbsqueeze-v1',
    featureVersion: 'coil-feats-v1',
    asOf,
    nulls: {
      pTrigger: ex ? undefined : 'no trade plan (insufficient candles)',
      effectiveSampleSize: 'coil calibration table carries no per-decile n; executable estimate is analytic',
    },
    effectiveSampleSize: null,
    extra: {
      pAbnormalExpansion: pAbn,               // EMPIRICAL P(abnormal upside excursion) — shown separately
      pAbnormalExpansionMajor: prob && prob.pctMajor != null ? prob.pctMajor / 100 : null,
      abnormalBreakDecile: prob ? prob.decile : null,
      abnormalBreakLift: prob ? prob.lift : null,
      scope,
    },
  });
}
...
module.exports = {
  ...
  trailingDailyVol, resolveBreak, coilTradePlan, coilPrediction, COIL_HORIZON, COIL_SIGMA,
};
