...
// Entry actions that actually produce a HELD position. Only ENTER_NEXT_OPEN fills; the
// WAIT_* actions are contingent triggers carried as episodes, and DO_NOT_CHASE / AVOID /
// NO_TRADE are refusals. Frozen and named so the rule is reviewable rather than implied
// by a filter somewhere in a route.
const ENTERABLE_ENTRY_ACTIONS = Object.freeze(['ENTER_NEXT_OPEN']);

// A row is enterable when it declares one of those actions. Fail-closed on a missing
// field: a caller that does not say whether a name can be entered has not established
// that it can. `entryAction` is supplied by lib/atlasx-routes.rankRow.
function isEnterable(row) {
  return !!(row && ENTERABLE_ENTRY_ACTIONS.includes(row.entryAction));
}
...
function buildAtlasPortfolio(rankedCandidates, opts = {}) {
  ...
  //
  //    ALSO fail-closed on ENTRY AVAILABILITY (alpha-research pass 3). `actionable` is
  //    the UTILITY gate — expected value, net-utility floor, prosecutor, R:R, staleness,
  //    applicability, liquidity, regime. It says nothing about whether there is a valid
  //    entry. A candidate can clear every utility hurdle while the entry engine finds no
  //    trigger (`entry.action: 'NO_TRADE'`), and lib/atlasx-routes.js filtered only on
  //    `actionable` — so such a name received a portfolio weight while the board
  //    displayed it under "Avoid / Prosecutor Flags". The book then described positions
  //    that could not have been entered.
  //
  //    A shadow book is a set of HELD positions: only ENTER_NEXT_OPEN produces one. The
  //    WAIT_* actions are contingent triggers tracked as episodes, not holdings, so
  //    including them would overstate deployed exposure. Missing/unknown action fails
  //    closed, per the module's own no-forced-fill design — and the exclusion carries a
  //    reason, because this module's contract is that every omission is explained.
  const eligible = [];
  for (const c of rows) {
    if (c.actionable === false) {
      excluded.push(freezeEx(c.ticker, mapReason(c.abstentionReason)));
    } else if (!isEnterable(c)) {
      excluded.push(freezeEx(c.ticker, 'no-entry-trigger'));
    } else {
      eligible.push(c);
    }
  }
...
module.exports = { SHADOW_WEIGHT, REASON_MAP, buildAtlasPortfolio, ENTERABLE_ENTRY_ACTIONS, isEnterable };
