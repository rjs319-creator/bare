...
// Directional lean from the contract side, MODULATED by the last-print aggressor.
// calls=bullish / puts=bearish is only a simplification on free chain data. When
// the last print was SELLER-initiated (hit the bid) on the naturally-directional
// side — a call sold at the bid, a put sold at the bid — we CANNOT read it as
// directional: it may be a covered-call write, a closing sale, or one leg of a
// hedge, not an opening bullish/bearish bet. Per the honesty rule such flow is
// 'unknown' (informational), NOT bullish/bearish. Buyer-initiated prints (lifted
// the ask) confirm the lean; a mid / missing quote leaves the raw side lean. No
// aggressor arg → the legacy raw lean (backward compatible).
function sentimentOf(side, aggressor) {
  const lean = side === 'call' ? 'bullish' : 'bearish';
  return aggressor === 'bid' ? 'unknown' : lean;
}

// How the sentiment above was established, so the UI can show conviction honestly:
//   'confirmed' — buyer lifted the ask (aggressor agrees with the side lean)
//   'ambiguous' — seller hit the bid → direction not readable → sentiment 'unknown'
//   'lean'      — mid / no two-sided quote → the raw call/put side simplification
function sentimentBasis(aggressor) {
  if (aggressor === 'ask') return 'confirmed';
  if (aggressor === 'bid') return 'ambiguous';
  return 'lean';
}
  ...
  for (const chain of chains) {
    ...
    for (const [side, list] of sides) {
      for (const c of list) {
        ...
        // Aggressor of the last print gates the directional read (see sentimentOf).
        const aggressor = aggressorOf(c.bid, c.ask, c.lastPrice);
        const sig = {
          ticker, side, type: side === 'call' ? 'C' : 'P',
          sentiment: sentimentOf(side, aggressor),
          sentimentBasis: sentimentBasis(aggressor),
...
function flowOutcome(entry, future, sentiment) {
  ...
  // 'unknown' / informational flow makes NO directional claim, so it is not a
  // gradeable directional hit — return null so the grader skips it (rather than
  // silently scoring it as a bullish call it never made).
  if (sentiment !== 'bullish' && sentiment !== 'bearish') return null;
...
module.exports = {
  ...
  premiumUsd, volOiRatio, classify, sentimentOf, sentimentBasis, aggressorOf, moneyness, dteOf, scoreSignal,
