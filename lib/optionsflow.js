...
// ⚠ LEDGER-ONLY CONVENTION — NOT an investor-facing directional claim.
//
// The v1 layer is QUARANTINED as raw delayed-chain diagnostics: calls are not bullish and
// puts are not bearish, and no summary, badge, tooltip, or card may say otherwise. This
// function survives for exactly one reason: the prospective `optionsflow/log.json` ledger
// was accumulated under the old call=up / put=down labelling, and re-signing historical
// rows would silently rewrite a prospective record. It is therefore the LEDGER's sign
// convention, used only by `flowOutcome` when grading already-logged entries.
//
// Anything investor-facing must use `oc.aggregateDirection` (the honest, provisional read
// that can return UNKNOWN/MIXED) or the Options v2 engine.
const LEDGER_SIGN_NOTE = 'Legacy ledger sign convention (call=+, put=−) retained only so the existing prospective log stays comparable. It is NOT a directional claim.';
function ledgerSignOf(side) {
  return side === 'call' ? 'bullish' : 'bearish';
}
// Back-compat alias for the grading harness and its tests.
const sentimentOf = ledgerSignOf;
    ...
    for (const [side, list] of sides) {
      for (const c of list) {
        ...
        const sig = {
          ...
          // Ledger sign only (see ledgerSignOf) — never rendered as a directional read.
          sentiment: ledgerSignOf(side),
          sentimentIsDirectional: false,
...
// PREMIUM-COMPOSITION SCORE for a set of contracts: a -100..+100 measure of how the
// unusual premium SPLITS between calls and puts, weighting aggressive OTM prints more.
//
// This is a COMPOSITION statistic, not a direction. A put-weighted tape can be a hedge, an
// overwrite, a roll, or a closing trade; a call-weighted tape can be a covered-call write.
// The labels below therefore describe the split and say nothing about where the stock goes.
const GRADE_BANDS = [
  [60, 'Heavily call-weighted'], [25, 'Call-weighted'], [8, 'Slightly call-weighted'],
  [-7, 'Balanced'], [-24, 'Slightly put-weighted'], [-59, 'Put-weighted'], [-100, 'Heavily put-weighted'],
];
const GRADE_NOT_DIRECTIONAL = 'Premium composition only — how the unusual premium split between calls and puts. It is NOT a directional read: puts can be hedges/overwrites and calls can be written, not bought.';
function flowGrade(contracts) {
  ...
  return { score, label, isDirectional: false, note: GRADE_NOT_DIRECTIONAL };
}
...
function rollupByTicker(signals) {
  ...
  for (const r of m.values()) {
    ...
    // Composition, named for what it measures. `bullishPct`/`net` are gone: a call share
    // is a call share, not a probability the stock rises.
    r.callPremiumSharePct = r.totalPremium > 0 ? Math.round((100 * r.callPremium) / r.totalPremium) : 50;
    ...
    r.score = g.score; r.grade = g.label; r.gradeIsDirectional = false;
...
// Market-wide DIAGNOSTIC across all signals: total premium and how those dollars split
// between calls and puts. A composition gauge of the delayed-chain tape — deliberately
// not a market direction call.
function flowSummary(signals) {
  ...
  return {
    ...
    callPremiumSharePct: total > 0 ? Math.round((100 * call) / total) : 50,
    premiumSkew: total > 0 ? (call >= put ? 'call-dominant' : 'put-dominant') : 'balanced',
    score: g.score, grade: g.label, gradeIsDirectional: false, gradeNote: GRADE_NOT_DIRECTIONAL,
    signalCount: signals.length,
    tickerCount: new Set(signals.map(s => s.ticker)).size,
  };
}

// Forward-return outcome for an ALREADY-LOGGED signal, signed by the LEDGER convention
// (see ledgerSignOf). This grades the historical prospective record on the terms it was
// recorded under; it is not a claim that calls predict up-moves.
...
module.exports = {
  ...
  premiumUsd, volOiRatio, classify, sentimentOf, ledgerSignOf, LEDGER_SIGN_NOTE,
  moneyness, dteOf, scoreSignal,
  breakevenOf, moveToBreakevenPct, aggressorOf,
  scanChain, scanOptionsFlow, flowOutcome,
  INDEX_ETFS, rollupByTicker, flowSummary, flowGrade, GRADE_BANDS, GRADE_NOT_DIRECTIONAL,
};
