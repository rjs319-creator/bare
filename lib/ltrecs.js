...
// Net analyst-consensus score for one month's counts, on a -2..+2 scale
// (strong-buy=+2 … strong-sell=-2, weighted by how many analysts), or null.
function consensusNet(c) {
  if (!c) return null;
  const sb = +c.strongBuy || 0, b = +c.buy || 0, h = +c.hold || 0, s = +c.sell || 0, ss = +c.strongSell || 0;
  const total = sb + b + h + s + ss;
  return total ? (sb * 2 + b - s - ss * 2) / total : null;
}

// ── Expert: Wall-Street analyst recommendation consensus (Finnhub trends) ─────
// `trend` carries this month's counts plus last month's (`trend.prev`), so we
// also report whether the Street is UPGRADING or DOWNGRADING recently.
const REC_MOM_MIN = 0.08;   // month-over-month net-score shift that counts as a real move
function expertRec(trend) {
  ...
  // Recent direction of the consensus: this month's net vs last month's.
  let momentum = null, momentumLabel = null;
  const now = consensusNet(trend), was = consensusNet(trend.prev);
  if (now != null && was != null) {
    const d = now - was;
    momentum = d > REC_MOM_MIN ? 'improving' : d < -REC_MOM_MIN ? 'deteriorating' : 'stable';
    momentumLabel = momentum === 'improving' ? 'Analysts upgrading recently' : momentum === 'deteriorating' ? 'Analysts downgrading recently' : 'Ratings steady this month';
  }
  return { rec, score, detail: `${total} analysts · ${buys} buy / ${h} hold / ${sells} sell`, momentum, momentumLabel };
}
