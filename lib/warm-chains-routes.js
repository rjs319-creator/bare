...
async function call(path) {
  ...
  // Read TEXT first, then parse: a response body can only be consumed once, and when the
  // step failed with a non-JSON platform error page the text is the only reason we get.
  const raw = await r.text().catch(() => '');
  let body = null;
  try { body = raw ? JSON.parse(raw) : null; } catch { /* a killed/truncated child may not return JSON */ }
  // Only carried for failures — a successful step's payload can be megabytes.
  return { ok: r.ok, status: r.status, body, raw: r.ok ? null : String(raw).slice(0, 300) };
}

// Child deadline from a nested dispatch's `&budgetMs=` (the parent's REMAINING budget).
// Floor of 15s: a child dispatched with almost no budget still gets to run (and
// budget-skip) enough to persist an honest report rather than being born dead. Root
// dispatches (param absent/non-numeric) keep the full deadline.
//
// ANY finite budget >= 0 counts as inherited — including exactly 0. A parent dispatching
// at precisely elapsed === deadlineMs sends budgetMs=0 (lib/warm-chains pathFor clamps at
// 0), and the old `> 0` guard read that as "absent" and granted the child a FULL fresh
// 240s — the exact parent-wall blowup the parameter exists to prevent.
const CHILD_DEADLINE_FLOOR_MS = 15000;
function inheritedDeadlineMs(budgetMsRaw) {
  // A missing or EMPTY param is a root dispatch (Number('') is 0 — without this guard a
  // bare `&budgetMs=` would masquerade as an explicit zero budget).
  const raw = budgetMsRaw == null ? '' : String(budgetMsRaw).trim();
  if (raw === '') return CHAIN_DEADLINE_MS;
  const inheritedMs = Number(raw);
  return Number.isFinite(inheritedMs) && inheritedMs >= 0
    ? Math.min(CHAIN_DEADLINE_MS, Math.max(CHILD_DEADLINE_FLOOR_MS, inheritedMs))
    : CHAIN_DEADLINE_MS;
}
...
async function runWarmChain(req, res) {
  ...
  try {
    // A nested dispatch passes the parent's REMAINING budget (&budgetMs=) so this child
    // never runs longer than the parent awaiting it can afford (semantics + the
    // zero-budget edge case: see inheritedDeadlineMs above).
    const deadlineMs = inheritedDeadlineMs(req.query && req.query.budgetMs);
    const result = await runChain(name, { call, deadlineMs });
...
module.exports = { runWarmChain, compactChainReport, chainReportPath, CHAIN_REPORT_PREFIX, inheritedDeadlineMs, CHILD_DEADLINE_FLOOR_MS };
