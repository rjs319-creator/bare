...
async function runWarmChain(req, res) {
  ...
  try {
    ...
    // never runs longer than the parent awaiting it can afford. Floor of 15s: a child
    // dispatched with almost no budget still gets to run (and budget-skip) enough to
    // persist an honest report rather than being born dead. Root dispatches (no param)
    // keep the full deadline.
    const inheritedMs = Number(req.query && req.query.budgetMs);
    const deadlineMs = Number.isFinite(inheritedMs) && inheritedMs > 0
      ? Math.min(CHAIN_DEADLINE_MS, Math.max(15000, inheritedMs))
      : CHAIN_DEADLINE_MS;
