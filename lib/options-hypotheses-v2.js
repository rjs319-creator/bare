...
// Windows in which a corporate action can mechanically explain today's option volume.
// Forward: assignment/positioning builds ahead of an ex-date. Backward: the flow from a
// just-passed ex-date is still sitting in today's prints.
const CORP_ACTION_NEAR_DAYS = 7;
const CORP_ACTION_RECENT_DAYS = 3;
...
function buildHypotheses({
  ...
  // ── corporate action ──
  // The three cases below are deliberately distinct. "No action on the calendar" is only
  // a REFUTATION when the calendar actually ANSWERED; if it failed or is not on the plan,
  // silence is not evidence and the hypothesis must stay open.
  if (!corpAction) {
  ...
  } else if (corpAction.exDivDate || corpAction.splitDate) {
    const d = isNum(corpAction.daysUntil) ? corpAction.daysUntil : null;
    const kind = corpAction.exDivDate ? 'ex-dividend' : 'split';
    out.push(d != null && d >= -CORP_ACTION_RECENT_DAYS && d <= CORP_ACTION_NEAR_DAYS
      ? mk('corpAction', VERDICT.SUPPORTED, [
        `a corporate action (${kind}${corpAction.exDivDate ? ` ${corpAction.exDivDate}` : ` ${corpAction.splitDate}`}) falls ${d >= 0 ? `in ${d}d` : `${-d}d ago`} — dividend-capture assignment and split-adjustment flow mechanically produce volume that looks nothing like a directional bet`,
      ], { daysUntilAction: d, actionKind: kind })
      : mk('corpAction', VERDICT.REFUTED, [`the nearest corporate action (${kind}) is ${d != null ? `${Math.abs(d)}d away` : 'outside the window'} — too distant to mechanically drive this activity`], { daysUntilAction: d }));
  } else {
    // No action found. Only refutable when BOTH calendars answered for this name.
    const cov = corpAction.coverage || {};
    const full = cov.dividend === true && cov.split === true;
    out.push(full
      ? mk('corpAction', VERDICT.REFUTED, ['both the dividend and split calendars returned cleanly for this name and neither lists an action in the window — mechanically-driven flow is ruled out'])
      : mk('corpAction', VERDICT.UNRESOLVED, [
        `only ${[cov.dividend ? 'dividend' : null, cov.split ? 'split' : null].filter(Boolean).join(' and ') || 'no'} calendar resolved — a silent calendar is NOT evidence of absence, so this stays open`,
      ]));
  }
