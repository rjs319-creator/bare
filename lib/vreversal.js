...
function analyzeVReversal(candles, opts = {}) {
  ...
  const riskAmt = entry - stop, rewardAmt = target - entry;
  ...
  return {
    ...
    confirmations,
    signals: {
      side: 'long', entry: +entry.toFixed(2), stop, target, rr,
      exits: [
      ...
      note: tier === 'WATCH' ? 'Aggressive bottom-catch — UNCONFIRMED. Size small and honor the stop; many of these fail.'
...
function analyzeInvertedV(candles, opts = {}) {
  ...
  const riskAmt = stop - entry, rewardAmt = entry - target;
  const rr = riskAmt > 0 ? +(rewardAmt / riskAmt).toFixed(2) : null;
  ...
  return {
    ...
    confirmations,
    signals: {
      side: 'short', entry: +entry.toFixed(2), stop, target, rr,
      exits: [
      ...
      note: tier === 'WATCH' ? 'Aggressive top-fade — UNCONFIRMED. Shorting strength is dangerous; size small and honor the stop.'
...
function analyzeLiquiditySweep(candles, opts = {}) {
  ...
  if (dir === 1) {
    ...
    const rr = entry - stop > 0 ? +((target - entry) / (entry - stop)).toFixed(2) : null;
    return {
      tier, score,
      geometry: {
  ...
  const rr = stop - entry > 0 ? +((entry - target) / (stop - entry)).toFixed(2) : null;
  return {
    tier, score,
    geometry: {
...
module.exports = { analyzeVReversal, analyzeInvertedV, analyzeLiquiditySweep, rsiSeries, emaSeries, macdSeries, atrLast };
