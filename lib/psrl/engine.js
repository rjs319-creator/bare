...
const CP = require('./changepoint');
const V = require('./vetoes');
const B = require('./breadth');
const SV = require('./survival');
const CAT = require('./catalyst');
...
function buildCard({ ticker, candles, sector, sectorCandles, spyCandles, asOf, prevCard, eventRisk = null }) {
  ...
  // Causal change-point evidence (addendum §5): absolute + residual paths.
  const stripTrace = (cp) => { const { trace, ...rest } = cp; return rest; };
  const residLevels = residual.index?.levels || [];
  const residRets = [];
  for (let i = 1; i < residLevels.length; i++) {
    if (residLevels[i - 1] > 0) residRets.push(residLevels[i] / residLevels[i - 1] - 1);
  }
  const changepoint = {
    absolute: stripTrace(CP.detectChangepoints(rets.slice(-130))),
    residual: stripTrace(CP.detectChangepoints(residRets.slice(-130))),
  };
...
/**
 * Predictive layer (addendum): vetoes, opportunity value, trend-life,
 * catalyst confirmation, and the explicit veto→entry/rank rule. Pure —
 * returns a NEW card.
 */
function applyPredictive(card, { market = null, trendLifeFit = null, catalystEvents = null, asOf } = {}) {
  if (!card.available) return card;
  const vetoes = V.vetoesFor(card, {
    changepoint: card.changepoint?.residual?.recentChange ? card.changepoint.residual : card.changepoint?.absolute,
    marketState: market?.state ?? null,
  });
  const opportunity = V.opportunityValue({ card, vetoes });
  const trendLife = SV.trendLifeFor(trendLifeFit, card.trendAge);
  const catalyst = CAT.catalystFeatures({ events: catalystEvents || [], asOf });

  // Explicit stage-3 rule: a veto caps the entry state and haircuts the rank.
  let entry = card.entry;
  if (vetoes.vetoed && entry?.state && (entry.state.startsWith('BUY') || entry.state === 'WAIT_FOR_PULLBACK')) {
    entry = { state: 'WATCH', reasons: [...(entry.reasons || []), 'FAILURE_VETO_ACTIVE', ...Object.entries(vetoes)
      .filter(([, v]) => v && v.triggered).map(([k]) => `VETO_${k.toUpperCase()}`)] };
  }
  const rankScore = Number.isFinite(card.scores?.rankScore)
    ? Math.round(card.scores.rankScore * vetoes.rankHaircut)
    : card.scores?.rankScore ?? null;

  // Forecastability disposition (addendum §13). Analogue-density / OOD /
  // conformal intervals require a trained artifact that does not exist yet —
  // documented unavailable; the deterministic inputs (support, freshness,
  // vetoes, event ambiguity) still produce an honest abstention ladder.
  const disposition = (() => {
    const reasons = [];
    if (card.support?.grade === 'STALE') return { disposition: 'DATA_STALE', reasons: card.support.reasons };
    if (card.support?.abstain) return { disposition: 'INSUFFICIENT_EVIDENCE', reasons: card.support.reasons };
    if (vetoes.data.triggered) return { disposition: 'ABSTAIN', reasons: vetoes.data.reasons };
    if (card.support?.grade === 'LOW') reasons.push('LOW_MODEL_SUPPORT');
    if (card.eventRisk) reasons.push('EVENT_AMBIGUITY');
    if (reasons.length) return { disposition: 'WATCH', reasons };
    return { disposition: entry?.state?.startsWith('BUY') ? 'ACTIONABLE' : 'WATCH', reasons: [] };
  })();
  ...
  return {
    ...
    forecastability: {
      ...disposition,
      unavailable: ['analogue-count', 'feature-density', 'ensemble-disagreement', 'OOD-distance', 'conformal-intervals'],
      unavailableReason: 'no trained artifact yet — deterministic abstention ladder only',
    },
    scores: { ...card.scores, rankScore, rankScorePreVeto: card.scores?.rankScore ?? null },
    vetoes: {
      version: vetoes.version, gate: vetoes.gate, vetoed: vetoes.vetoed,
      maxVeto: vetoes.maxVeto, rankHaircut: vetoes.rankHaircut, rule: vetoes.rule,
      stagnation: vetoes.stagnation, plateau: vetoes.plateau,
      breakdown: vetoes.breakdown, data: vetoes.data,
    },
    opportunity,
    trendLife,
    catalyst,
 ...
 */
function runPsrl({ rows, spyCandles, asOf, prev = null, eventRiskByTicker = {}, trendLifeFit = null, catalystsByTicker = {} }) {
  const prevByTicker = new Map((prev?.cards || []).map((c) => [c.ticker, c]));
  const market = B.marketState(spyCandles);
  const cards = [];
  const skipped = [];
  for (const row of rows || []) {
    try {
      ...
      cards.push(applyPredictive(card, {
        market, trendLifeFit, catalystEvents: catalystsByTicker[row.ticker] || null, asOf,
      }));
    } catch (e) {
      skipped.push({ ticker: row.ticker, reason: String(e && e.message || e) });
    }
  }
  const rankedBare = S.rankCards(cards.filter((c) => c.available));
  const sectorAnnotated = B.annotateSectorStructure(rankedBare, {
    spyBars: spyCandles, prevBreadth: prev?.sectorBreadth || null, prevCards: prev?.cards || null,
  });
  const ranked = sectorAnnotated.cards;
  const excluded = cards.filter((c) => !c.available);
  return {
    market,
    sectorBreadth: sectorAnnotated.breadth,
    trendLifeFit: trendLifeFit ? {
      available: !!trendLifeFit.available, resolved: trendLifeFit.resolved ?? 0,
      censored: trendLifeFit.censored ?? 0, reason: trendLifeFit.reason ?? null,
      survival: trendLifeFit.survival ?? null, calibrationStatus: 'uncalibrated',
    } : { available: false, reason: 'NO_LEDGER_FIT' },
    schema: 'PsrlSnapshot',
...
module.exports = { runPsrl, buildCard, applyPredictive, eligibility, atrPctOf, participationOf, dollarVol20, trendAgeSessions, staleSessions };
