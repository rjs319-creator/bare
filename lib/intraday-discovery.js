...
async function runDiscoveryScan({ now = new Date() } = {}) {
  ...
  // PREMARKET: the regular-session fields on a bulk quote are YESTERDAY's close and volume
  // until 09:30 (they stamped every premarket row sessionDateMismatch, so the premarket scan
  // observed nothing). Read the premarket print instead when the venue has one; a name with
  // no premarket print keeps its stale regular fields and is rejected downstream as before —
  // "no print" must not become a fabricated observation.
  const { sessionQuote } = require('./quote-provider');
  const sessionView = r => {
    if (!premarket || !r) return r;
    const sq = sessionQuote(r, 'premarket');
    return sq.basis === 'premarket' ? { ...r, price: sq.price, asOf: sq.asOf, dayVolume: sq.volume, sessionBasis: 'premarket' } : r;
  };
  for (const t of tickers) {
    const row = sessionView(byRow.get(t));
