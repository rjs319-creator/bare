...
async function runDiscoveryScan({ now = new Date() } = {}) {
  ...
  if (session !== 'regular' && !(premarket && D.PREMARKET.ENABLED)) {
    return { ok: true, session, date, anomalies: [], scanned: 0, note: `Discovery scans run during regular hours${D.PREMARKET.ENABLED ? ' and premarket' : ''} only (session: ${session}).`, generatedAt: nowIso };
  }
