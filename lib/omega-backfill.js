...
async function replayOmega({ scope = 'large', limit = 60, step = 10, months = 24, range = '2y', deadlineMs = 50000 } = {}) {
  ...
  const rows = [];
  const stat = { evaluated: 0, byTier: {}, byRegime: {}, deadlineHit: false };
  ...
  for (const date of dates) {
    ...
    for (const [t, candles] of hist) {
      ...
      rows.push({
        date, ticker: t, regime: regimeLabel, tier: card.tier, stage: card.stage, entry,
...
async function runOmegaWalkforward(opts = {}) {
  ...
  const meanOOS = blockICs.length ? +meanOf(blockICs).toFixed(3) : null;
  const passed = allPositive && meanOOS != null && meanOOS > MARGIN;
  const verdict = passed ? 'edge-holds-oos'
  ...
  return {
    ...
    walkforward: { blocks, blockICs, meanOOS, allPositive, margin: MARGIN, passed, verdict },
    verdict,
    shipCriteria: 'Promote a trained model over the baseline formula ONLY when walk-forward.passed=true (≥3 purged blocks, all score→residual IC positive, meanOOS > 0.02) AND tier payoff is monotone (PRIME > QUALIFIED > WATCH).',
...
async function runOmegaLedgerBackfill({ scope = 'large', limit = 80, months = 12, step = 10, range = '2y', deadlineMs = 50000 } = {}) {
  ...
  for (const r of rows) {
    ...
    (byDate[r.date] = byDate[r.date] || []).push({
      ...
      entry: r.entry != null ? +r.entry.toFixed(2) : null, score: r.score, stage: r.stage, setup: r.setup || null,
    });
  }
...
module.exports = { OMEGA_BACKFILL_VERSION, replayOmega, runOmegaWalkforward, runOmegaLedgerBackfill, SECTOR_ETF };
