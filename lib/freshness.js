...
// beside a stale recommendation. Beyond "current-session evidence exists", it requires a
// RECENT intraday bar AND/OR a RECENT quote (within the configured tolerances). A daily-only
// FRESH_TODAY (a bar dated today with no intraday/quote timestamp) is NOT actionable-fresh:
// a completed daily bar is an end-of-session aggregate, not proof the name is live right now.
// Fails closed — any missing timestamp or over-age evidence returns false.
//   tol = { quoteMaxAgeMs, intradayMaxAgeMs } (defaults from lib/daytrade-config).
function isActionableFresh(freshness, { now = new Date(), tol = null } = {}) {
  ...
  const quoteMs = toMs(freshness.quoteAsOf);
  const intraMs = toMs(freshness.intradayBarAsOf);
  const quoteFresh = quoteMs != null && (nowMs - quoteMs) <= cfg.QUOTE_MAX_AGE_MS;
  const intraFresh = intraMs != null && (nowMs - intraMs) <= cfg.INTRADAY_BAR_MAX_AGE_MS;
  // Either a fresh intraday bar OR a fresh quote attests the name is live NOW. A bare daily
  // bar (both timestamps null) cannot — that is the stale-recommendation trap.
