...
// The net TARGET MOVE (%) still available on a signal from HERE, after costs — pure
// geometry (distance to the published target minus round-trip friction). It is NOT
// probability-weighted expectancy: no hit-rate or stop distance enters it, so it must
// never be labeled "expected edge" (quant-redesign-3 H11 — a lottery setup with a far
// target and a 10% hit rate maxes this number on a negative-expectancy trade). Prefers
// the remaining-edge read (measured from the origin); falls back to the trade's full
// net target move when no origin has accrued yet (day 1); null for a lead with no
// levels — which cannot qualify.
function netTargetMoveOf(sig) {
...
function isQualifying(sig) {
  const edge = netTargetMoveOf(sig);
...
function computeOpportunityDensity(signals, { regime = {}, tape = null, config = {}, routerExposureCap = null } = {}) {
  const opts_routerExposureCap = routerExposureCap;
  ...
  // The opportunity SET is the qualifying names (quant-redesign-3): a lead that "cannot
  // qualify to trade" (no levels / no net move left) must not lift the day's density
  // through breadth/track/quality either. `active` is already rank-ordered upstream.
  const top = qualifying.slice(0, cfg.TOP_K);
  ...
  // "Best-pick move" is the net target move on the top-RANKED qualifying name — the pick
  // you'd actually take — NOT the global max, which a single far-target moonshot would
  // dominate and mislead.
  const bestEdge = qualifying.length ? (netTargetMoveOf(qualifying[0]) || 0) : 0;
  ...
  // Optional binding ROUTER exposure cap (quant-redesign-3 Phase 4D): penalty-only, same
  // idiom as the regime/tape gates — it can only LOWER the cap, never raise anything.
  // Callers must supply it only when the router's inputs are valid (bindingReady).
  const routerCap = Number.isFinite(opts_routerExposureCap) ? Math.max(0, Math.min(100, opts_routerExposureCap)) : null;
  if (routerCap != null) exposureCap = Math.min(exposureCap, routerCap);
  ...
  return {
    ...
    // RENAMED (quant-redesign-3 H11): this is the best qualifying name's net TARGET MOVE
    // after costs — pure geometry, NOT probability-weighted expectancy, so it is no
    // longer called "expected edge" anywhere.
    bestNetTargetMovePct: +bestEdge.toFixed(2),
    qualifyingCount, activeCount: active.length,
    byHorizon, components,
    regimeGate: { riskOff, redTape, choppyTape, spyChangePct: t.spyChangePct ?? null, applied: +gateMult.toFixed(2), routerCap, label: gateLabel },
    reasons,
  };
}
...
function buildReasons({ qualifyingCount, bestEdge, medTilt, riskOff, redTape, choppyTape, spyChangePct, cFresh, consumed, activeN, decision }) {
  const r = [];
  if (qualifyingCount === 0) r.push('No name has any net target move left after costs — nothing qualifies to trade today.');
  else r.push(`${qualifyingCount} name${qualifyingCount === 1 ? '' : 's'} with net target move left after costs (best ~${bestEdge.toFixed(1)}% to target — geometry, not probability-weighted expectancy).`);
...
module.exports = { OPPORTUNITY_VERSION, CONFIG, netTargetMoveOf, isQualifying, computeOpportunityDensity };
