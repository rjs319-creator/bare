...
// ── ghostobs outcome resolution (op=premoveresolve) ─────────────────────────
// Grades the full-candidate observation records — selected AND rejected cohorts
// — against cached forward candles. Outcomes append to a separate grader-owned
// doc; the ghostobs/<date>.json observations are never rewritten. This is what
// makes the rejected-candidate capture a false-negative laboratory instead of a
// write-only archive.
const OBS_RESOLVED_PATH = 'ghostobs/resolved.json';

async function runPremoveObsResolve(req, res) {
  if (!hasStore()) return res.status(200).json({ ok: false, error: 'Blob storage not configured.', resolved: 0 });
  const { resolveObservationDays, OBS_RESOLVE_VERSION, OBS_HORIZON_SESSIONS } = require('./premove-obs-resolve');

  const [days, resolvedDoc] = await Promise.all([
    readAllGhostObsDays().catch(() => []),
    readJSON(OBS_RESOLVED_PATH, null).catch(() => null),
  ]);
  const prior = (resolvedDoc && Array.isArray(resolvedDoc.outcomes)) ? resolvedDoc.outcomes : [];
  const existingKeys = new Set(prior.map(o => o.key));

  // Candles are needed only for rows not yet resolved.
  const tickers = [...new Set(days.flatMap(d =>
    ['selected', 'alsoInBreakout', 'nearThreshold', 'rejected']
      .flatMap(c => (d[c] || []).map(r => r && r.ticker))
      .filter(t => t && !existingKeys.has(`${d.date}|${t}`))))];
  const { map } = await buildCandleLookup(tickers);

  const { outcomes, censored, missingCandles } = resolveObservationDays(
    days, (t) => map.get(t) || null, { existingKeys });

  const resolvedAt = new Date().toISOString();
  const stamped = outcomes.map(o => ({ ...o, resolvedAt }));
  let written = false;
  if (stamped.length) {
    await writeJSON(OBS_RESOLVED_PATH, {
      version: OBS_RESOLVE_VERSION,
      horizonSessions: OBS_HORIZON_SESSIONS,
      updatedAt: resolvedAt,
      returnsBasis: 'gross',
      outcomes: [...prior, ...stamped],
    });
    written = true;
  }

  const byCohort = {};
  for (const o of stamped) byCohort[o.cohort] = (byCohort[o.cohort] || 0) + 1;
  res.setHeader('Cache-Control', 'no-store');
  return res.status(200).json({
    ok: true, version: OBS_RESOLVE_VERSION,
    observationDays: days.length,
    newlyResolved: stamped.length, byCohort,
    censored, missingCandles,
    totalResolved: prior.length + stamped.length,
    written,
  });
}

module.exports = { runPremove, runPremoveLog, runPremoveObsResolve, buildInventory, assembleCandidates, STAGE_A_MODEL_PATH, CALIBRATION_PATH, PROMOTION_PATH, CROSS_SECTION_PREFIX, OBS_RESOLVED_PATH };
