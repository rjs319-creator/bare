...
function prosecuteClaim({ samples = [], trades = [], opts = {} } = {}) {
  ...
  // APPLICABILITY. Concentration asks HOW AN EDGE IS EARNED, so it presumes an
  // edge. Asked of a series with no positive mean it answers a question nobody
  // needs: expectancy has already failed, and "your losses are concentrated" is
  // not a finding. The shape is still MEASURED and reported — silence would lose
  // it — but it cannot charge a record that never claimed a positive return.
  const claimsEdge = nets.length > 0 && (nets.reduce((a, b) => a + b, 0) / nets.length) > 0;
  const applicable = cal.ran && claimsEdge;
  checks.push({
    check: 'null-calibrated-concentration',
    ok: applicable ? !cal.concentrated : null,
    detail: claimsEdge ? { ...cal, applicable } : { ...cal, applicable: false, notApplicable: 'non-positive mean — no edge to concentrate' },
  });
