...
async function buildBoard({ fetchHistory, universe, limit, range, artifact }) {
  ...
  const cands = [];
  const excluded = []; // full-universe honesty: every eligible name that was NOT ranked, and WHY.
  for (const ticker of names) {
    let hist; try { hist = await fetchHistory(ticker, range); } catch { hist = null; }
    if (!hist || !hist.candles) { excluded.push({ ticker, reason: 'no-history' }); continue; }
    if (hist.candles.length < 160) { excluded.push({ ticker, reason: 'insufficient-history' }); continue; }
    ...
    if (!snap || !snap.sufficient) { excluded.push({ ticker, reason: 'insufficient-features' }); continue; }
  ...
  return { asOf: (safeNowET() && safeNowET().date) || null, shadow: true, affectsLiveRank: false, trained, count: n, ranked: cands, excluded, universeSize: names.length };
}
...
async function runOrbitMlTick(req, res) {
  ...
  try {
    ...
    // Full-universe candidate ledger: store the ranked cross-section AND the excluded/ineligible
    // names with reasons, so the training population is never "past picks only" (survivorship trap).
    if (date) await store.writeOrbitMlDay(date, { trained: board.trained, count: board.count, universeSize: board.universeSize, predictions: logged, excluded: board.excluded || [], versions: versions(), code: codeVersion(), shadow: Adapter.shadowStatus() });
...
async function runOrbitMlResolve(req, res) {
  if (!store.hasStore()) return res.status(200).json({ ok: false, reason: 'no-store', shadow: true });
  // One 2y fetch per ticker per run, misses cached — the per-prediction refetch storm
  // this shared with op=orbitresolve is documented on memoHistory in orbit-routes.
  const { fetchOnce: fetchHistory, stats } = require('./orbit-routes').memoHistory(require('./screener').fetchDailyHistory);
  try {
    const days = await store.readAllOrbitMlDays();
    ...
    for (const day of days) {
      for (const p of (day.predictions || [])) {
        ...
        const hist = await fetchHistory(p.ticker, '2y');
    ...
    return res.status(200).json({ ok: true, shadow: true, newlyResolved: newly, total: Object.keys(resolved).length, stats });
  } catch (e) { return res.status(200).json({ ok: false, shadow: true, error: String(e && e.message || e) }); }
}
...
// ── op=orbitcontrols — negative-controls / leakage-detection battery ────────
// Backfills real samples, runs the adversarial control battery (shuffled-label,
// future-feature, random-ranker, doubled-cost, drop-best-year), the dataset
// suitability gate, and a PIT fingerprint. This is the "prove the edge is real,
// not leakage" surface. EXPENSIVE (backfill + several walk-forwards).
async function runOrbitControls(req, res) {
  const fetchHistory = require('./screener').fetchDailyHistory;
  const Controls = require('./orbit-controls');
  const PIT = require('./pit-contract');
  const limit = Math.min(+(req.query.limit || 24), 120);
  const horizon = req.query.horizon || 'days21';
  try {
    const bf = await Backfill.runBackfill({ universe: LARGE, scope: 'large', fetchHistory, range: req.query.range || '5y', limit, step: +(req.query.step || 10), featureFn: MLFeat.orbitMlFeatures });
    const features = [...require('./orbit-model').FEATURE_SET, ...MLFeat.ML_FEATURE_NAMES];
    const controls = Controls.runControls(bf.samples, { horizon, features, outerBlocks: +(req.query.outer || 6) });
    // The backfill scores the FULL universe cross-section (rejected candidates kept),
    // but from current survivor lists with no delisted names and no PIT universe.
    const suitability = PIT.datasetSuitability({ hasRejectedCandidates: true, hasDelisted: false, pointInTimeUniverse: false });
    return res.status(200).json({
      ok: true, shadow: true, horizon,
      controls, suitability,
      dataFingerprint: PIT.fingerprint(bf.samples.map(s => ({ securityId: s.ticker, tradingDate: s.decisionDate, observationTs: s.decisionDate }))),
      backfill: { nSamples: bf.nSamples, built: bf.built },
      researchValidity: bf.researchValidity,
    });
  } catch (e) { return res.status(200).json({ ok: false, shadow: true, error: String(e && e.message || e) }); }
}

// ── op=promotionreadiness — the frozen "may any learned model go live?" gate ─
// Public, cheap READ. Assembles the already-stored artifacts (eval walk-forward,
// research validity, the full-universe shadow ledger + resolved outcomes) and runs the
// frozen promotion gate. It never trains, never activates anything, and never 500s.
// Today it truthfully reports NOT READY / INSUFFICIENT_DATA on the survivorship-biased,
// under-sampled free-data universe. It is the machine-checkable form of the audit's
// "exact frozen promotion criteria + what must happen before live activation".
async function runPromotionReadiness(req, res) {
  res.setHeader('Cache-Control', 's-maxage=300, stale-while-revalidate=86400');
  const Promo = require('./promotion-readiness');
  try {
    const evalDoc = store.hasStore() ? await store.readOrbitMlEval().catch(() => null) : null;
    const resolved = store.hasStore() ? await store.readOrbitMlResolved().catch(() => ({})) : {};
    const days = store.hasStore() ? await store.readAllOrbitMlDays().catch(() => []) : [];

    // The shadow ledger now stores excluded candidates → the training population is not
    // "past picks only". Delisted names / PIT membership remain absent on the free universe.
    const anyExcluded = days.some(d => Array.isArray(d.excluded) && d.excluded.length > 0);
    const meta = { hasRejectedCandidates: anyExcluded, hasDelisted: false, pointInTimeUniverse: false };
    const coverage = Promo.coverageReport(days, resolved, meta);

    // Map the stored walk-forward shape → the gate's expected {purgedIC, icir, nOuter}.
    const walkforward = {};
    const wfDoc = (evalDoc && evalDoc.walkforward) || {};
    for (const h of HORIZONS) {
      const w = wfDoc[h];
      const overall = w && w.ok && w.purged && w.purged.overall;
      if (overall) walkforward[h] = { purgedIC: overall.ic, icir: overall.icir, nOuter: (w.purged && w.purged.nOuter) || null };
    }

    const monitor = (() => { try { return MLMonitor.monitorOrbitMl(resolved, { evalDoc, now: (safeNowET() && safeNowET().date) || null, horizon: 'days21' }); } catch { return null; } })();
    // Extract the SCALAR marginal contribution: monitor.incremental.leaveOneOut is a
    // {marginalDelta,…} object (null until enough resolves), so read the number out of it.
    const loo = monitor && monitor.incremental && monitor.incremental.leaveOneOut;
    const marginalDelta = (loo && typeof loo.marginalDelta === 'number') ? loo.marginalDelta
      : (evalDoc && typeof evalDoc.marginalDelta === 'number' ? evalDoc.marginalDelta : null);

    const readiness = Promo.promotionReadiness({
      researchValidity: (evalDoc && evalDoc.researchValidity) || { survivorshipSafe: false, pointInTimeSafe: false },
      coverage,
      walkforward,
      marginalDelta,
      controls: (evalDoc && evalDoc.controls) || {}, // populated once op=orbitcontrols persists a verdict
      prospectiveHealth: (monitor && monitor.health && monitor.health.byHorizon && monitor.health.byHorizon.days21 && monitor.health.byHorizon.days21.status) || null,
    });

    return res.status(200).json({ ok: true, shadow: true, affectsLiveRank: false, readiness, coverage, note: readiness.note });
  } catch (e) {
    return res.status(200).json({ ok: false, shadow: true, error: String(e && e.message || e) });
  }
}

module.exports = { runOrbitMl, runOrbitMlTick, runOrbitMlResolve, runOrbitMlWalkForward, runOrbitMlHealth, runOrbitControls, runPromotionReadiness, buildBoard };
