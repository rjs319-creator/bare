...
const ALERT_KINDS = Object.freeze({
  ...
  NARRATIVE_APPEARED: { cooldownMin: 240, label: 'New event' },                                      // ONLY genuinely recent publications / material updates
  NARRATIVE_DISCOVERED: { cooldownMin: 240, label: 'Newly discovered by Market Pulse (older event)' }, // rediscovery is NOT news
  MATERIAL_UPDATE: { cooldownMin: 120, label: 'Material update' },
  NEW_CORROBORATION: { cooldownMin: 240, label: 'New corroborating source' },
...
// Freshness reasons that may honestly be announced as a NEW event.
const FRESH_APPEARED_REASONS = new Set(['NEW_PUBLICATION', 'NEW_MATERIAL_UPDATE']);
...
function kindOf(t) {
  ...
  switch (t.kind) {
    case 'appeared':
      // "New" requires the PUBLICATION to be new — an old story the pipeline just
      // found is announced as a discovery, never as a new event.
      return FRESH_APPEARED_REASONS.has(t.freshnessReason) ? 'NARRATIVE_APPEARED' : 'NARRATIVE_DISCOVERED';
    case 'material-update': return 'MATERIAL_UPDATE';
    case 'corroborated': return 'NEW_CORROBORATION';
    case 'syndication': return null;   // diagnostics only — never a prominent alert
    case 'evidence':
...
function deriveAlerts({ transitions = [], context = {}, prevState = null, now = () => new Date().toISOString() } = {}) {
  ...
  for (const t of transitions) {
    ...
    alerts.push({
      ...
      freshnessReason: t.freshnessReason || null,
