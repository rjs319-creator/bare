...
async function runDualReadBackfill(req, res) {
  ...
  // Merge into the backfill doc (dedup by ticker+date), or replace on ?reset=1.
  const prev = req.query.reset ? null : await readJSON('dualread/backfill.json', null);
  const prevRows = (prev && prev.rows) || [];
  const seen = new Set(prevRows.map(r => `${r.ticker}|${r.date}`));
  const merged = prevRows.slice();
  for (const r of fresh) { const k = `${r.ticker}|${r.date}`; if (!seen.has(k)) { seen.add(k); merged.push(r); } }

  const nextStart = start + batch.length < list.length ? start + batch.length : null;
  await writeJSON('dualread/backfill.json', { rows: merged, builtAt: new Date().toISOString(), scope, range, horizon: DUAL_H, step, count: merged.length }, 0);
  res.setHeader('Cache-Control', 'no-store');
  return res.json({ ok: true, scope, range, processed, newRows: fresh.length, totalRows: merged.length, nextStart,
...
async function runDualReadTune(req, res) {
  ...
  if (source !== 'live') {
    const bf = await readJSON('dualread/backfill.json', null);
    const bfRows = (bf && bf.rows) || [];
...
module.exports = { runDualRead, runDualReadLog, runDualReadBook, runDualReadTune, runDualReadBackfill, loadDualReadWeights, DUAL_H };
