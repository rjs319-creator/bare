...
async function runDualRead(req, res) {
  ...
  const sw = body && body.sw;   // optional independent swing read (action/evidence/reasons)
  if (!ticker || !st || !lt || !mech || !mech.quadrant) {
    return res.status(400).json({ ok: false, error: 'expected JSON { ticker, price, st:{action,confidence,reasons}, lt:{trend,score,reasons,factors}, mech:{quadrant,verdict,setupClass}, sw?:{action,evidenceStrength,reasons} }' });
  }

  const { readJSON, writeJSON, hasStore } = require('./store');
  const cachePath = `${CACHE_PREFIX}${ticker}.json`;
  // Cache identity must include the SWING action — otherwise a changed swing read
  // (same short×long quadrant) would keep serving a stale narrative that no longer
  // matches the swing card the user sees.
  const cacheKey = `${mech.quadrant}|sw:${(sw && sw.action) || 'none'}`;

  // Serve a fresh, same-identity cached narrative without re-calling Fable.
  if (hasStore()) {
    const cached = await readJSON(cachePath, null).catch(() => null);
    if (cached && cached.cacheKey === cacheKey && cached.at &&
  ...
  const ai = await analyzeDualRead({ ticker, price: body.price, st, lt, mech, sw });
  if (ai && hasStore()) {
    await writeJSON(cachePath, { ticker, quadrant: mech.quadrant, cacheKey, ai, at: new Date().toISOString() }, 0).catch(() => {});
  }
  res.setHeader('Cache-Control', 'no-store');
...
async function loadDualReadWeights() {
  ...
  try {
    ...
    // Central adaptive policy: 'disable' reverts to shipped weights.
    if ((await require('./adaptive-layers').layerPolicy('dualread-adapt')) === 'disable') return DEFAULT_LT_WEIGHTS;
...
async function resolveDualForward(H) {
  ...
  // Independent episodes only: a read that PERSISTS across consecutive days is one
  // observation, not H fresh ones (its forward windows overlap). First appearance per
  // ticker+quadrant wins; the same pair only re-enters after its window has elapsed.
  const cooldownDays = Math.ceil(H * 1.5);              // H sessions in calendar days
  const lastAt = new Map();
  const raw = [];
  for (const d of [...days].sort((a, b) => (a.date < b.date ? -1 : 1))) for (const p of (d.picks || [])) {
    const key = `${p.ticker}|${p.quadrant}`;
    const prev = lastAt.get(key);
    if (prev && (new Date(d.date) - new Date(prev)) / 86400000 < cooldownDays) continue;
    lastAt.set(key, d.date);
  ...
  for (const e of raw) {
    ...
    if (!st || !m) { pending++; continue; }           // horizon not elapsed → pending, never a label
    const gross = ((st.c1 - st.c0) / st.c0 - (m.c1 - m.c0) / m.c0) * 100;
    // COST-NET is the learning label; gross rides along for display continuity.
    const fwd = +(gross - roundTripCostPct(tierForPick(e))).toFixed(3);
    entries.push({ ...e, fwd, fwdGross: +gross.toFixed(3) });
  }
...
async function runDualReadBook(req, res) {
  ...
  return res.json({
    ...
    note: `Forward ${H}-session COST-NET excess-vs-SPY of each dual-read's first appearance per episode (persistent reads dedup'd; overlapping windows are one observation). The read earns its keep if "aligned up"/"pullback-buy" beat SPY while "downtrend"/"bear-bounce" lag. The self-tuner (op=dualreadtune) re-weights the long-term factors from this same ledger once ≥${MIN_RESOLVED} resolve. Accrues via the daily cron.`,
  });
}

// ── op=ltrecs — long-term "second opinions": momentum / technical / fundamental /
// expert lenses to cross-check the custom model. GET ?ticker=X. Each lens is real
// data or null (never a guess). Cached — these move slowly. The dual banner calls
// this async and renders the consensus panel alongside the custom long-term read.
async function runLtRecs(req, res) {
  const ticker = cleanTicker(req.query.ticker);
  if (!ticker) return res.status(400).json({ ok: false, error: 'expected ?ticker=' });
  const { fetchDailyHistory } = require('./screener');
  const { fetchFundamentals, fetchRecommendation } = require('./fundamentals');
  const { momentumRec, technicalRec, fundamentalRec, expertRec } = require('./ltrecs');

  const [daily, fd, trend] = await Promise.all([
    fetchDailyHistory(ticker, '2y').catch(() => null),   // 2y so 12-month momentum resolves
    fetchFundamentals(ticker).catch(() => null),
    fetchRecommendation(ticker).catch(() => null),
  ]);
  const candles = daily && daily.candles;

  const recs = {
    momentum: momentumRec(candles),
    technical: technicalRec(candles),
    fundamental: fundamentalRec(fd),
    expert: expertRec(trend),
  };
  res.setHeader('Cache-Control', 's-maxage=3600, stale-while-revalidate=86400'); // slow-moving; warm once/hr
  return res.json({ ok: true, ticker, recs });
}

// ── op=dualreadbackfill — replay the read over history to warm-start the tuner ──
// Heavy + manual (like op=backfill/fundbuild) — NOT in the daily cron. Time-boxed +
// resumable: run per scope, following nextStart until complete. Writes the resolved
// rows to dualread/backfill.json, which op=dualreadtune?source=all then consumes.
async function runDualReadBackfill(req, res) {
  const { hasStore, writeDualReadShard } = require('./store');
  const { fetchDailyHistory } = require('./screener');
  const { replayTicker } = require('./dualread-backfill');
  ...
  const list = scope === 'small' ? U.SMALL_CAPS : scope === 'micro' ? U.MICRO_CAPS : U.LARGE;
  const limit = Math.max(1, parseInt(req.query.limit, 10) || 80);
  const start = Math.max(0, parseInt(req.query.start, 10) || 0);
  const range = req.query.range || '2y';
  const step = Math.max(1, parseInt(req.query.step, 10) || DUAL_H);

  const spy = await fetchDailyHistory('SPY', range).catch(() => null);
  const spyC = spy && spy.candles;
  if (!spyC) return res.json({ ok: false, error: 'SPY history unavailable' });

  const batch = list.slice(start, start + limit);
  const t0 = Date.now();
  const fresh = []; let processed = 0, i = 0;
  const worker = async () => {
    while (i < batch.length) {
      const tk = batch[i++];
      if (Date.now() - t0 > 45000) return;   // stay under the function wall
      try {
        const d = await fetchDailyHistory(tk, range);
        if (d && d.candles) { fresh.push(...replayTicker(d.candles, spyC, { horizon: DUAL_H, step, ticker: tk })); processed++; }
      } catch { /* skip name */ }
    }
  };
  await Promise.all(Array.from({ length: 6 }, worker));

  // Write THIS batch as its own shard (idempotent by scope+start — no read-modify-write,
  // so concurrent/rapid batches can't lose each other's rows). The tuner merges shards.
  await writeDualReadShard(`${scope}-${String(start).padStart(5, '0')}`, { rows: fresh, scope, start, range, horizon: DUAL_H, step, count: fresh.length });

  const nextStart = start + batch.length < list.length ? start + batch.length : null;
  res.setHeader('Cache-Control', 'no-store');
  return res.json({ ok: true, scope, range, processed, shardRows: fresh.length, nextStart,
    note: nextStart != null ? `resume: op=dualreadbackfill&scope=${scope}&start=${nextStart}` : `scope '${scope}' complete — run op=dualreadtune to learn from it` });
}
...
async function runDualReadTune(req, res) {
  ...
  // Rows = live resolved ledger and/or the historical backfill (source=live|backfill|all).
  const source = req.query.source || 'all';
  let rows = [];
  if (source !== 'backfill') {
    const { entries } = await resolveDualForward(DUAL_H);
    rows = entries.filter(e => e.signals).map(e => ({ signals: e.signals, fwd: e.fwd, date: e.date, group: e.group || 'other' }));
  }
  if (source !== 'live') {
    const { readDualReadBackfillRows } = require('./store');
    const bfRows = await readDualReadBackfillRows();
    rows = rows.concat(bfRows.filter(r => r.signals && Number.isFinite(r.fwd)).map(r => ({ signals: r.signals, fwd: r.fwd, date: r.date, group: r.group || 'other' })));
  }
  ...
  // Central adaptive policy: 'freeze'/'disable' block adoption of new fits —
  // the champion/challenger diagnostics still run and are reported.
  const dualPolicy = await require('./adaptive-layers').layerPolicy('dualread-adapt');
  const policyBlockedAdoption = dualPolicy !== 'allow';
  if (!policyBlockedAdoption) {
  ...
  res.setHeader('Cache-Control', 'no-store');
  return res.json({ ok: true, globalPromoted: gcc.promoted && !policyBlockedAdoption, policyBlockedAdoption, adaptivePolicy: dualPolicy, global: doc.global, groups: groupDoc, resolved: gcc.resolved, defaultWeights: DEFAULT_LT_WEIGHTS });
}

module.exports = { runDualRead, runDualReadLog, runDualReadBook, runDualReadTune, runDualReadBackfill, runLtRecs, loadDualReadWeights, DUAL_H };
