 ...
 */
/**
 * The concentration math itself, over a strike→volume tally. Pure.
 *
 * Split out so the FULL-CHAIN tally can be built once during the observation walk (where
 * every contract is already being visited) without shipping every chain row through
 * storage, while the statistics stay defined in exactly one place.
 */
function concentrationFromTally(byStrike, { basis = 'FULL_CHAIN' } = {}) {
  const entries = byStrike instanceof Map ? [...byStrike.entries()] : Object.entries(byStrike || {});
  const withVol = entries.filter(([, v]) => isNum(v) && v > 0);
  if (!withVol.length) return unavailable('no strikes with volume in the supplied tally');
  const total = withVol.reduce((s, [, v]) => s + v, 0);
  const sorted = withVol.sort((a, b) => b[1] - a[1]);
  const top1 = sorted[0][1] / total;
  const top3 = sorted.slice(0, 3).reduce((s, [, v]) => s + v, 0) / total;
  // Herfindahl over strike shares: 1 = a single strike, →0 = perfectly diffuse.
  const hhi = sorted.reduce((s, [, v]) => s + (v / total) ** 2, 0);
  return {
    available: true,
    basis,
    distinctStrikes: sorted.length,
    totalVolume: total,
...
function strikeConcentration({ chainRows = null, chainComplete = false, fullChainSummary = null } = {}) {
  // Preferred path: a summary computed over the FULL chain at observation time.
  if (fullChainSummary && fullChainSummary.available) return { ...fullChainSummary };
  if (!chainComplete) {
    return unavailable('strike concentration was NOT computed: only retained display rows were supplied, and no full-chain summary was attached. Retention is a ranking decision, so concentration over it measures the display cut, not the chain.');
  }
  ...
  return concentrationFromTally(byStrike);
}
...
module.exports = {
  EXEC_VERSION, T,
  contractLiquidity, strikeConcentration, concentrationFromTally, volatilityContext,
