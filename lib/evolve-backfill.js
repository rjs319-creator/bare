...
// v2: triple-barrier labels enter at the NEXT session's open (was the signal-day close).
const BACKFILL_VERSION = 'evolve-backfill-v2';
const MAX_HOLD = 63;                 // longest barrier window → forward bars every cohort must have
// Canonical fixed-barrier map derived DIRECTLY from EVOLVE_HORIZONS so it can never drift out
// of sync with the declared horizons. A hard-coded {fast,swing,position} map used to omit
// `micro` (the first horizon), so the default volAdjust=false loop hit `undefined.window` and
// threw, crashing the whole backfill on the first fired cohort row.
const BARRIERS = Object.fromEntries(L.EVOLVE_HORIZONS.map((h) => [h, L.barriersFor(h)]));
...
async function runEvolveBackfill({ scope = 'large', limit = 80, step = 21, months = 18, deadlineMs = 50000, now = null, volAdjust = false, range = '2y', deps = {} } = {}) {
  // Dependency-injection seam: default to the real fetchers, but let tests drive the full
  // orchestration path with synthetic history (no network, no clock) — this is how the
  // micro-horizon backfill crash is exercised end-to-end rather than through a pure helper.
  const fetchHist = deps.fetchDailyHistory || fetchDailyHistory;
  const screen = deps.screenTicker || screenTicker;
  const buildMacro = deps.buildMacroLookup || buildMacroLookup;
  const t0 = Date.now();
  const list = deps.tickers || (scope === 'micro' ? MICRO_CAPS : scope === 'small' ? SMALL_CAPS : LARGE);
  ...
  const spy = await fetchHist('SPY', range).catch(() => null);
  ...
  const sectorCandles = {};
  await mapLimit(SECTOR_ETFS, 6, async (sym) => { const d = await fetchHist(sym, range).catch(() => null); if (d) sectorCandles[sym] = d.candles; });

  const macro = await buildMacro(range).catch(() => null);
  const regimeAt = (date) => { const s = macro && macro.at(date); return (s && s.regime) || 'neutral'; };

  // Ticker candle histories (bounded concurrency).
  const hist = new Map();
  await mapLimit(tickers, 6, async (t) => { const d = await fetchHist(t, range).catch(() => null); if (d) hist.set(t, { candles: d.candles, meta: { ...d.meta, symbol: t } }); });
  ...
  const additions = {};
  // Horizon counters seeded from EVOLVE_HORIZONS so every declared horizon (incl. micro) is
  // represented — no hard-coded subset that can silently drop a horizon from the stats.
  const byHorizon = Object.fromEntries(L.EVOLVE_HORIZONS.map((h) => [h, 0]));
  const stat = { rows: 0, byHorizon, bySpecialist: {}, byRegime: {}, wins: 0, deadlineHit: false };
  ...
  for (const date of dates) {
    ...
    for (const [t, { candles, meta }] of hist) {
      ...
      const r = screen(candles.slice(0, idx + 1), meta, { spyByDate });
      if (!r) continue;
      const fired = specialistsFiring(r);
      if (!fired.length) continue;

      // NEXT-OPEN entry (exec-v2): the specialist fires on candles[idx].close, so the earliest
      // fill is the next session's open — not the same close. Labels stay GROSS (no slippage;
      // friction is charged at EVOLVE's payoff layer, not baked into the outcome label). The
      // benchmark legs below already anchor on next-open (benchmarkReturn's p0 = first.open),
      // so this makes the trade entry consistent with them.
      const fill = planFill(candles, date, { policy: POLICIES.NEXT_OPEN, side: 'long' });
      if (!fill.filled || !(fill.fillPrice > 0)) continue;
  ...
  stat.executionPolicy = { policy: POLICIES.NEXT_OPEN, entry: 'next-open (gross)', policyVersion: EXECUTION_POLICY_VERSION };
