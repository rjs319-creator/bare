...
async function runEvolveBackfill({ scope = 'large', limit = 80, step = 21, months = 18, deadlineMs = 50000, now = null } = {}) {
  const t0 = Date.now();
  ...
  const sectorCandles = {};
  await mapLimit(SECTOR_ETFS, 6, async (sym) => { const d = await fetchDailyHistory(sym, '2y').catch(() => null); if (d) sectorCandles[sym] = d.candles; });
  ...
  const hist = new Map();
  await mapLimit(tickers, 6, async (t) => { const d = await fetchDailyHistory(t, '2y').catch(() => null); if (d) hist.set(t, { candles: d.candles, meta: { ...d.meta, symbol: t } }); });
    ...
    for (const [t, { candles, meta }] of hist) {
      ...
      for (const h of L.EVOLVE_HORIZONS) {
        ...
        additions[`bf|${date}|${t}|${h}`] = {
          ...
          resolvedAt: now || 'backfill', backfill: true,
        };
