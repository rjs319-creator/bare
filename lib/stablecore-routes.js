...
const { buildNavLedger } = require('./nav-ledger');
...
async function runCorePerf(req, res) {
  ...
  // ONE history fetch per unique ticker feeds BOTH lanes below (the MTM latest-close
  // marks and the daily NAV ledger), capped to stay inside the function budget. A
  // ticker over the cap or with a failed fetch stays in the record as an EXPLICIT
  // coverage gap — nav-ledger fails closed on it, aggregatePerformance counts it.
  const uniqueTickers = [...new Set(signals.map(s => s.ticker))];
  const histories = {};
  const overCap = uniqueTickers.length > DRIFT_FETCH_CAP;
  await pool(uniqueTickers.slice(0, DRIFT_FETCH_CAP), 6, async t => {
    try { const d = await fetchDailyHistory(t, '2y'); if (d && d.candles && d.candles.length) histories[t] = d.candles; }
    catch { /* missing history becomes a declared coverage gap, never a silent drop */ }
  });
  // MARK-TO-MARKET lane (EA-1): latest close for OPEN signals so the record includes
  ...
  for (const s of openSigs) {
    ...
    const last = h && h.length ? h[h.length - 1].close : null;
  ...
  const rtCost = roundTripCostPct('small');
  const perf = core.aggregatePerformance(signals, resolved, bench, MAX_HOLD, { costPct: rtCost, marks });
  // PORTFOLIO NAV lane: the daily mark-to-market ledger (cash + open marks + realized
  // exits + per-side costs), fail-closed on any missing mark. This — not the
  // resolved-only compound — is the only number allowed to headline "since inception".
  const nav = overCap
    ? { available: false, reason: `position ledger spans ${uniqueTickers.length} tickers — over the per-request fetch budget (${DRIFT_FETCH_CAP}); NAV withheld rather than computed on partial coverage`, coverage: { positions: signals.length, gaps: [] } }
    : buildNavLedger(signals, resolved, histories, { perSidePct: rtCost / 2, bench });
  // The full daily curve can be large — serve a decimated curve for the chart plus the totals.
  if (nav.curve && nav.curve.length > 130) {
    const step = Math.ceil(nav.curve.length / 130);
    nav.curve = nav.curve.filter((_, i) => i % step === 0 || i === nav.curve.length - 1);
  }
  res.setHeader('Cache-Control', 's-maxage=600');
  return res.status(200).json({
    ok: true, benchmark: 'IWM', holdSessions: MAX_HOLD, nav,
