...
const PILLAR_LABEL = {
  ...
  SF: 'Up/down-volume flow (proxy)', BONUS: 'Catalyst overlay', IN: 'Insider buying',
};
...
function pillarsOf(c) {
  ...
  // Missing accum falls to NEUTRAL 50, never to the volume-surge percentile — vol
  // measured rank-IC ≈ −0.004, so substituting it silently put a dead factor
  // inside a production pillar exactly when the real accumulation data was absent.
  const AF = r0(q.accum != null ? q.accum : 50);
