...
// Formula version stamped onto every logged Apex/Loaded pick so historical records
// stay attributable to the scoring rules that produced them. BUMP this whenever the
// pillar formulas / tier thresholds below change (a recalibrated weight set is tracked
// separately via the model.json activeId, which is stamped alongside this).
const SCORING_VERSION = 'apex-v3';
...
const PILLAR_LABEL = { p1: 'Momentum / RS', p2: 'Technical structure', p3: 'Fundamental acceleration', p4: 'Supply / accumulation (proxy)' };
...
function pillarsOf(c) {
  ...
  // Missing accum/ud fall back to vol-adjusted momentum alone (validated), else a
  // NEUTRAL 45 — never to the dead volume-surge percentile, which silently smuggled
  // a zero-IC factor into a production pillar whenever the newer fields were absent.
...
//
// Regime gate (opt-in via opts.gateRiskOff): in a RISK_OFF regime the breakout
// edge inverts — it goes negative, the project's one durable, backtested finding
// (see the exits/longshort/ghost-backtest studies). So when gating is enabled we
// cap the actionable tiers at 'watch': no new apex/loaded LONGS in risk-off, the
// same lever the conviction sleeve enforces with longOk. Default OFF so the
// ledger (apexlog), backfill baseline, and exits study keep reconstructing EVERY
// regime (they study risk-off on purpose); only the live display opts in.
function tierOf(score, pl, c, opts = {}) {
  ...
  if (opts.gateRiskOff && opts.regime === 'RISK_OFF' && (tier === 'apex' || tier === 'loaded')) tier = 'watch';
  return tier;
}
...
// `opts.gateRiskOff` enables the risk-off long gate in tierOf (see above).
function scoreCandidate(c, regime, weights, opts = {}) {
  const w = weights || PRESETS[regime] || PRESETS.NEUTRAL;
  const pillars = pillarsOf(c);
  const score = composite(pillars, w);
  return { pillars, score, tier: tierOf(score, pillars, c, { ...opts, regime }) };
}

module.exports = { PRESETS, RG_LABEL, PILLAR_LABEL, KEYS, SCORING_VERSION, rawRegime, pillarsOf, composite, tierOf, scoreCandidate };
