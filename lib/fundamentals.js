...
async function fetchFundamentals(ticker) {
  ...
  try {
    ...
    // Shares outstanding (Finnhub reports it in millions) → absolute count, for the
    // short-interest %-of-shares flag. Null when absent.
    const shOutM = num(m.shareOutstanding);
    const sharesOut = shOutM != null ? shOutM * 1e6 : null;
    ...
    try {
      ...
      const er = await fetchWithTimeout(`https://finnhub.io/api/v1/calendar/earnings?from=${f}&to=${t}&symbol=${sym}&token=${KEY}`);
      if (er.ok) {
    ...
    return { epsGrowth, revGrowth, revAccel, epsAccel, netMargin, opMarginTTM, opMarginBase, marginExpanding, pe, sharesOut, earningsDate, earningsInDays };
  } catch { return null; }
}

// Company news headlines over a date window, for the Gap-and-Go cause tagger. Uses FMP
// (matches the research/27 pilot; the FMP key is the paid Starter feed) with a free
// Finnhub fallback. Returns [{title, text, datetime}] (bounded), [] on any failure.
async function fetchCompanyNews(ticker, fromDate, toDate) {
  const sym = ticker.toUpperCase();
  const FMP = process.env.FMP_API_KEY;
  if (FMP) {
    try {
      const r = await fetchWithTimeout(`https://financialmodelingprep.com/stable/news/stock?symbols=${sym}&from=${fromDate}&to=${toDate}&limit=50&apikey=${FMP}`);
      if (r.ok) {
        const rows = await r.json();
        if (Array.isArray(rows) && rows.length) return rows.map(x => ({ title: x.title || '', text: (x.text || '').slice(0, 300), datetime: x.publishedDate }));
      }
    } catch { /* fall through to Finnhub */ }
  }
  if (!KEY) return [];
  try {
    const r = await fetchWithTimeout(`https://finnhub.io/api/v1/company-news?symbol=${sym}&from=${fromDate}&to=${toDate}&token=${KEY}`);
    if (!r.ok) return [];
    const rows = await r.json();
    if (!Array.isArray(rows)) return [];
    return rows.slice(0, 50).map(x => ({ title: x.headline || '', text: (x.summary || '').slice(0, 300), datetime: x.datetime }));
  } catch { return []; }
}

// Like fetchCompanyNews but PRESERVES provenance (url, publisher/site, symbol) — the
// Evidence engine needs source primacy (primary vs secondary), which fetchCompanyNews drops.
// FMP `stable/news/stock` carries `url` + `site` + `publisher`; Finnhub carries `url`+`source`.
// Returns [{ title, text, datetime (ISO/epoch), url, publisher, symbol }].
// CONTRACT: [] means a provider ANSWERED "no articles". When every provider failed
// (429/5xx/timeout) this THROWS with both statuses in the message — swallowing failures
// into [] made the nightly tick's rate-limited runs indistinguishable from quiet names.
// Low-volume caller (≤14/tick) so retries:2 is safe; wide fan-out callers must not copy it.
async function fetchCompanyNewsRich(ticker, fromDate, toDate, limit = 30) {
  const sym = ticker.toUpperCase();
  const FMP = process.env.FMP_API_KEY;
  let fmpStatus = null; // null = no key; 'empty' = answered no articles; number/name = failed
  if (FMP) {
    try {
      const r = await fetchWithTimeout(`https://financialmodelingprep.com/stable/news/stock?symbols=${sym}&from=${fromDate}&to=${toDate}&limit=${limit}&apikey=${FMP}`, { retries: 2 });
      if (r.ok) {
        const rows = await r.json();
        if (Array.isArray(rows) && rows.length) {
          return rows.map(x => ({
            title: x.title || '', text: (x.text || '').slice(0, 400), datetime: x.publishedDate || null,
            url: x.url || null, publisher: x.publisher || x.site || null, symbol: x.symbol || sym,
          }));
        }
        fmpStatus = 'empty';
      } else fmpStatus = r.status;
    } catch (e) { fmpStatus = (e && e.name) || 'error'; }
  }
  let fhStatus = null;
  if (KEY) {
    try {
      const r = await fetchWithTimeout(`https://finnhub.io/api/v1/company-news?symbol=${sym}&from=${fromDate}&to=${toDate}&token=${KEY}`, { retries: 2 });
      if (r.ok) {
        const rows = await r.json();
        if (Array.isArray(rows)) {
          return rows.slice(0, limit).map(x => ({
            title: x.headline || '', text: (x.summary || '').slice(0, 400),
            datetime: x.datetime ? new Date(x.datetime * 1000).toISOString() : null,
            url: x.url || null, publisher: x.source || null, symbol: sym,
          }));
        }
        fhStatus = 'badBody';
      } else fhStatus = r.status;
    } catch (e) { fhStatus = (e && e.name) || 'error'; }
  }
  if (fmpStatus === 'empty' || (fmpStatus == null && fhStatus == null)) return []; // answered quiet / no keys
  throw new Error(`news providers unavailable (fmp:${fmpStatus == null ? 'no-key' : fmpStatus} finnhub:${fhStatus == null ? 'no-key' : fhStatus})`);
}
...
async function fetchInsiders(ticker) {
  ...
  try {
    ...
    const r = await fetchWithTimeout(`https://finnhub.io/api/v1/stock/insider-transactions?symbol=${sym}&from=${from}&to=${to}&token=${KEY}`);
    if (!r.ok) return null;
...
// Wall-Street analyst recommendation trends (one Finnhub call) — the latest
// month's { strongBuy, buy, hold, sell, strongSell } analyst counts. Feeds the
// long-term "expert consensus" lens (lib/ltrecs.expertRec). Null on any failure.
async function fetchRecommendation(ticker) {
  if (!KEY) return null;
  const sym = ticker.toUpperCase();
  try {
    const r = await fetchWithTimeout(`https://finnhub.io/api/v1/stock/recommendation?symbol=${sym}&token=${KEY}`);
    if (!r.ok) return null;
    const rows = await r.json();
    if (!Array.isArray(rows) || !rows.length) return null;
    // Newest period first (Finnhub returns newest-first, but sort to be safe).
    const sorted = rows.slice().sort((a, b) => (a.period < b.period ? 1 : -1));
    const pick = x => x ? { strongBuy: x.strongBuy, buy: x.buy, hold: x.hold, sell: x.sell, strongSell: x.strongSell } : null;
    const latest = sorted[0];
    // Keep last month's counts too, so we can tell if the Street is upgrading/downgrading.
    return { period: latest.period, ...pick(latest), prev: pick(sorted[1]) };
  } catch { return null; }
}
...
module.exports = { fetchFundamentals, fetchInsiders, fetchEarningsInfo, isEarningsAdjacent, fetchCompanyNews, fetchCompanyNewsRich, fetchRecommendation };
