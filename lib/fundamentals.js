...
async function fetchCompanyNews(ticker, fromDate, toDate) {
  ...
  if (FMP) {
    try {
      const r = await fetch(`https://financialmodelingprep.com/stable/news/stock?symbols=${sym}&from=${fromDate}&to=${toDate}&limit=50&apikey=${FMP}`);
      if (r.ok) {
  ...
  if (!KEY) return [];
  try {
    const r = await fetch(`https://finnhub.io/api/v1/company-news?symbol=${sym}&from=${fromDate}&to=${toDate}&token=${KEY}`);
    if (!r.ok) return [];
    const rows = await r.json();
...
async function fetchRecommendation(ticker) {
  ...
  const sym = ticker.toUpperCase();
  try {
    const r = await fetch(`https://finnhub.io/api/v1/stock/recommendation?symbol=${sym}&token=${KEY}`);
    if (!r.ok) return null;
    const rows = await r.json();
