...
async function fetchRecommendation(ticker) {
  ...
  try {
    ...
    // Most recent period first (Finnhub returns newest-first, but sort to be safe).
    const latest = rows.slice().sort((a, b) => (a.period < b.period ? 1 : -1))[0];
    return { period: latest.period, strongBuy: latest.strongBuy, buy: latest.buy, hold: latest.hold, sell: latest.sell, strongSell: latest.strongSell };
  } catch { return null; }
}
