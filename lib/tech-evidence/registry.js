...
function validateMapping(m) {
  ...
  if (host && !OFFICIAL_API_HOSTS.includes(host) && !displayHosts.includes(host) && !isCompanyHost(host, m.domain)) {
    issues.push(`sourceUrl host "${host}" is neither an official API host nor on the company domain`);
  }
  return { ok: issues.length === 0, issues };
}

function isCompanyHost(host, domain) {
  if (!host || !domain) return false;
  const d = String(domain).toLowerCase();
  // Same registrable domain, or the well-known status-host convention (<name>status.com).
...
// Full allowlist for outbound collection fetches.
