...
// One event = one row, keyed (cutoff, ticker, arm). Dedup makes the append idempotent:
// a crash between the rows write and the index write only re-appends no-ops on retry.
const rowKey = (r) => `${r.d}|${r.t}|${r.arm}`;
async function appendForwardRows(newRows) {
  const doc = await readForwardRows();
  const seen = new Set(doc.rows.map(rowKey));
  const fresh = newRows.filter((r) => !seen.has(rowKey(r)));
  const rows = [...doc.rows, ...fresh];
  const overflow = Math.max(0, rows.length - EVENT_ROW_CAP);
  const next = { ...doc, rows: rows.slice(overflow), trimmed: (doc.trimmed || 0) + overflow, savedAt: new Date().toISOString() };
  await STORE.writeJSON(KEYS.forwardRows, next, 0);
  return { total: next.rows.length, trimmed: next.trimmed, added: fresh.length };
}
...
module.exports = {
  ...
  readForwardRows, appendForwardRows,
