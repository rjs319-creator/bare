...
// stored as a revision observation, never an overwrite.
function observationId({ source, entity, metric, effectiveDate, value }) {
  return `obs-${shortHash({ source, entity, metric, effectiveDate, value })}`;
}
...
function makeObservation({
  ...
  const id = observationId({ source, entity, metric, effectiveDate, value });
  return Object.freeze({
    v: SCHEMA_VERSION, id, source, ticker: ticker || null, entity, metric,
    effectiveDate, value, unit,
