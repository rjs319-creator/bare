...
function evaluateArmHorizon(rows, arm, horizon, { hasAnyObservations = false } = {}) {
  ...
  // summary.ci95 is ROUNDED TO 2dp for display — on fraction-scale residuals (~0.005)
  // that would make the gate meaningless (the known avgExact/seExact trap). Rebuild the
  // interval at full precision: widest of exact Student-t and an exact seeded bootstrap.
  const ci95Exact = (() => {
    if (!summary) return null;
    const se = Number.isFinite(summary.seExact) ? summary.seExact : summary.se;
    if (!Number.isFinite(avgExact) || !Number.isFinite(se) || !Number.isFinite(summary.tCritical)) return null;
    const tCI = { lo: avgExact - summary.tCritical * se, hi: avgExact + summary.tCritical * se };
    const boot = ES.movingBlockCI95(deduped.series, {});
    return boot && Number.isFinite(boot.lo)
      ? { lo: Math.min(tCI.lo, boot.lo), hi: Math.max(tCI.hi, boot.hi) }
      : tCI;
  })();
