...
async function collectOneSource(source, { now, fetchImpl, basis = 'live', extra = {}, budget = null } = {}) {
  ...
  try {
    if (source === 'pricing') {
      ...
      result = await ADAPTERS.pricing({ mappings, now, fetchImpl, budget, prevSnapshots: (series.entities && series.entities.snapshots) || {} });
    } else {
      result = await ADAPTERS[source]({ mappings, now, fetchImpl, basis, budget, ...extra });
    }
  } catch (e) {
  ...
  const withSnaps = source === 'pricing' && result.nextSnapshots
    ...
    : nextSeries;
  // Audit ledger FIRST: if the series write below fails, the next run re-folds the same
  // fresh set and the append dedupes. The reverse order would mark facts as "seen" in
  // the series while the audit ledger permanently missed them.
...
// Deadline pressure skips the sources at the END of the order — a fixed order would
// starve the same sources every night. Rotate the start point by day of month so every
// source gets early slots on a regular cadence.
function rotatedSources(sources, now) {
  const offset = now.getUTCDate() % sources.length;
...
async function runCollection({ now = new Date(), deadlineMs = DEFAULT_DEADLINE_MS, t0 = Date.now(), fetchImpl = null, sources = LIVE_SOURCES } = {}) {
  ...
  let health = {};
  for (const source of rotatedSources(sources, now)) {
    ...
    // The same budget threads INTO the adapter so a slow source stops launching new
    // fetches mid-flight instead of overrunning until its whole mapping list drains.
    const r = await collectOneSource(source, { now, fetchImpl, extra, budget: { t0, deadlineMs } });
