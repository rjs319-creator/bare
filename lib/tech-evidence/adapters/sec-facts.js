...
async function collectSec({ companies, now = new Date(), fetchImpl = null, basis = 'live',
  sinceFiled = null, fetchRecentFilingsImpl = null, throttleMs = 150 } = {}) {
  ...
  for (const c of companies) {
    ...
    try {
      const recent = await fetchRecent(c.cik, { forms: ['8-K'], fromDate: sinceFiled, maxFilings: 40 });
      observations.push(...cyber8kObservations(recent, { mapping: c, retrievedAt, basis }));
    } catch (e) {
      errors.push(`${c.ticker} 8-K scan: ${String((e && e.message) || e).slice(0, 120)}`);
    }
