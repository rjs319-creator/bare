...
async function collectJobs({ source, mappings, now = new Date(), fetchImpl = null, budget = null } = {}) {
  ...
  const results = await mapWithBudget(mappings, async (m) => {
    ...
    return { id: m.sourceId, observations: snapshotObservations(summary, { source, mapping: m, day, retrievedAt }), total: summary.total };
  }, { budget });
  for (const res of results) {
    ...
    perEntity[res.id] = { ok: true, total: res.total };
  }
