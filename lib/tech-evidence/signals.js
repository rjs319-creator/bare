...
function robustZ(x, history) {
  ...
  // orbit-math.mad is ALREADY ×1.4826 (σ-consistent for Gaussian data) — dividing by a
  // second 1.4826 here would understate every z by 1.48× and silently raise the
  // eligibility bar from |z|≥1 to |z|≥1.48.
  const sigma = mad(clean, median(clean));
  if (!Number.isFinite(sigma) || sigma <= 0) return { z: null, mad: 0, reason: 'zero dispersion (MAD=0) — z undefined, not infinite' };
  return { z: (x - median(clean)) / sigma, mad: sigma, reason: null };
}
...
function revenueAcceleration(facts, cutoffDate) {
  ...
  // Surprise must share the z-numerator's center (accel − own historical median) so the
  // event's direction can never contradict its own z-score sign.
  const cleanHistory = history.filter(Number.isFinite);
  const ownExpected = cleanHistory.length >= 4 ? median(cleanHistory) : 0;
  const surprise = latest.accel - ownExpected;
  ...
  return {
    available: true, metricLabel: 'YoY revenue growth acceleration (ΔYoY) vs own history, at filing',
    windows: { latestQuarterEnd: latest.end, filed: latest.filed, baselineSamples: history.length, latestYoy: latest.yoy, prevYoy: latest.prevYoy, rawAccel: latest.accel },
    recentGrowth: latest.accel, ownExpected, surprise, z, dispersion, quality, caveats,
