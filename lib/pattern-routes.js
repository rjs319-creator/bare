...
const AUTO_SLICE_LIMIT = 25;                      // names per nightly auto slice (self-bounded 40s)
...
async function collectResearchSlice(start, limit, now) {
  const tickers = scanUniverse();
  ...
  const shard = { start, processed, advanced, records, controls, builtAt: now, modelVersion: MODEL_VERSION };
...
// Merge all shards → per-cell evaluation → the versioned evidence artifact the live
// radar's recommendationEligibility gates on.
async function evaluateResearchShards(now) {
  let shards = [];
  try {
...
async function runPatternResearch(req, res) {
  ...
  if (mode === 'auto') {
    ...
    // Each call advances one slice; when the sweep completes it evaluates once, then
    // idles until the artifact is older than the rebuild window.
    ...
    const start = inFlight ? state.nextStart : 0;
    const startedAt = (inFlight && state.startedAt) || now;
    const r = await collectResearchSlice(start, AUTO_SLICE_LIMIT, now);
    if (r.nextStart != null) {
      await store.writeJSON(RESEARCH_STATE_PATH, { nextStart: r.nextStart, startedAt, updatedAt: now }, 0);
      return res.status(200).json({ ok: true, mode, phase: 'collect', ...r, startedAt });
    }
    const ev = await evaluateResearchShards(now);
    await store.writeJSON(RESEARCH_STATE_PATH, { nextStart: null, startedAt, completedAt: now, evaluatedAt: ev.ok ? now : null, updatedAt: now }, 0);
    return res.status(200).json({ ok: true, mode, phase: 'evaluate', collect: r, evaluate: ev, startedAt });
  }
