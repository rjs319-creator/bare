...
// patternlog must FINISH inside tracker's 60s maxDuration or every write is lost (night-1
...
// The soft deadline is measured from INVOCATION start; the 22s left over covers the worst
// single straggler that can start just before the deadline — fetchDailyHistory is 10s ×
// 2 hosts = 20s — plus the end-of-tick writes.
const PATTERNLOG_SOFT_DEADLINE_MS = 38000;
...
async function runPatternLog(req, res) {
  ...
  const timing = { elapsedMs, initMs, requestedMs, grantedMs: budgetMs, softDeadlineMs: PATTERNLOG_SOFT_DEADLINE_MS, advanceTruncated };
  ...
  console.info('[patternlog]', JSON.stringify({ date, ...timing, advanced, created, cursor: scanState.cursor, universe: tickers.length }));
