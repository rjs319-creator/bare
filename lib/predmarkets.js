...
async function getJSON(url) {
  // AbortSignal timeout: a venue that accepts the TCP connection but never responds
  // must not hang every caller (gameplan/crowd builds sit on function wall-clocks).
...
/**
 * Shared load-and-score pipeline (both venues + the trailing volume baseline) so
 * consumers don't grow private copies of the fetch→baseline→score sequence.
 * `limitDays` bounds the Blob reads (the baseline needs only a trailing window).
 * Returns scoreMarkets output (heat-sorted) or [] — never throws.
 */
async function loadScoredMarkets({ limitDays = 8 } = {}) {
  try {
    const { readAllPredmktDays } = require('./store');   // lazy: keeps fetch/score usable store-free
    const [k, p, days] = await Promise.all([
      fetchKalshi().catch(() => []),
      fetchPolymarket().catch(() => []),
      readAllPredmktDays(limitDays).catch(() => []),
    ]);
    const all = [...k, ...p];
    if (!all.length) return [];
    const today = new Date().toISOString().slice(0, 10);   // UTC — matches the predmkt/ ledger's day keys
    return scoreMarkets(all, buildBaseline(days, today));
  } catch { return []; }
}

module.exports = { fetchKalshi, fetchPolymarket, fetchKalshiResult, scoreMarkets, scoreSharp, buildBaseline, buildPrevOI, daysToClose, summarizeSharpValidation, loadScoredMarkets, SHARP_TELLS, KALSHI_SERIES, UNUSUAL_HEAT };
