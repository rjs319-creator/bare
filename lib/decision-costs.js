...
function costModel(signal) {
  ...
  // A move fully eaten by the entry leg has no gross to share the cost against — the
  // penalty floors outright rather than dividing by zero (or flattering to neutral).
  const costShare = grossMovePct > 0 ? +(roundTripPct / grossMovePct).toFixed(3) : null;
  const penalty = costShare == null
    ? MAX_COST_DRAG_FLOOR
    : +Math.max(MAX_COST_DRAG_FLOOR, Math.min(1, 1 - costShare)).toFixed(3);
