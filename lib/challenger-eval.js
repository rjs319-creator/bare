...
function clusteredExpectancyCI(rows, outcomes) {
  try {
    ...
    if (s && s.ci95 && isNum(s.ci95.lo) && isNum(s.ci95.hi)) {
      return { mean: round(s.avg, 3), lo: round(s.ci95.lo, 3), hi: round(s.ci95.hi, 3), n: outcomes.length, dates: s.n, effectiveN: s.effectiveN ?? null, basis: 'date-clustered HAC + moving-block (widest wins)' };
    }
