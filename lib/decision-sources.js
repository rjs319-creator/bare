...
// The gathered keys that actually CONTRIBUTE signals, as read by normalizeAll below.
// `sectors`, `today` and `scoreboard` are context (sector strength, regime/density,
// ranking history), so a zero contribution from them is normal rather than a fault —
// the distinction matters when reporting which sources came back barren.
const SIGNAL_SOURCE_KEYS = [
  'screener', 'screenerSmall', 'gapgo', 'daytrade', 'coil', 'gapdown', 'biotech',
  'coremo', 'downday', 'optionsflow', 'rt', 'an', 'sw', 'ca', 'ts',
];
...
module.exports = { SOURCE_URLS, SIGNAL_SOURCE_KEYS, normalizeAll, buildRankedSignals, gatherRankedSignals };
