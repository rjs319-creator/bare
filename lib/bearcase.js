...
function parseBearCases(input, validTickers) {
  ...
  for (const c of (input && Array.isArray(input.cases)) ? input.cases : []) {
    ...
    // Strip decoration BEFORE clipping — '$NVDA (long)' must sanitize to NVDA, not
    // truncate into junk that fails the whitelist and silently drops the case.
    const tk = String(c.ticker || '').toUpperCase().replace(/[^A-Z.^-]/g, '').slice(0, 8);
