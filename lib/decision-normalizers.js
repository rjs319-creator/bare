...
function fromDayTrade(json) {
  ...
  return items.filter(d => d && d.entry > 0 && d.stop > 0 && daytradeIsLive(d)).map(d => ({
    ...
    liquidity: { price: d.currentPrice ?? d.last },   // dollarVol not exposed → execution stays neutral
...
module.exports = { fromScreener, fromGhostTop, mapGhostTopRows, fromGapGo, fromDayTrade, fromCoil, fromGapDown, fromBiotech, fromCoreMomentum, fromAiScreeners, fromDownDay, fromOptionsFlow, mapOptionsFlowRows, sectorStrength, classifyEarnings, AI_MAP };
