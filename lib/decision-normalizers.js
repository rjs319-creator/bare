...
function fromScreener(json, opts = {}) {
  ...
  return results.filter(r => r && r.levels && r.status && r.levels.entry > 0).map(r => {
    ...
    // Ghost's accumulation read may only ADD evidence weight while ghost itself is
    // trade-eligible. The registry holds ghost at maturity 'rejected' ("MUST NOT
    // originate, boost, or add verdict weight") — an ungated push here let a rejected
    // strategy raise live ranks through the evidence multiplier.
    const gt = r.ghost && r.ghost.tier;
    if ((gt === 'GHOST' || gt === 'STALKING') && isTradeEligible('ghost', opts.registry)) {
      fams.push('volumeAccum'); origins.volumeAccum = 'ghost';
    }
    const f = r.factors || {};
    // revAccel lives on r.fundamentals, not r.factors — reading it off factors made the
    // acceleration half of this condition dead (the one fundamentals signal the app's
    // own research measured as additive).
    if ((fd.revAccel ?? 0) > 3 || (fd.revGrowth ?? 0) >= 25) {
...
function fromBiotech(json) {
  ...
  return items.filter(b => b && (b.tier === 'Hot' || b.tier === 'Emerging') && b.ticker).map(b => ({
    ...
    // RVOL NO LONGER MANUFACTURES AN EVIDENCE FAMILY (alpha-research pass 3).
    //
    // This appended a second family whenever relVol >= 1.5, which raised `evidenceUnits`
    // → `evidenceMultiplier` (up to 1.25x on the live op=today composite), raised
    // `confidenceScore`, and acted as a sort tiebreaker. But this repo MEASURED raw
    // volume-surge at rank-IC ~= -0.004 and weights it zero elsewhere — the screener-v2
    // pass removed it from admission, techScore, the Apex p4 fallback and the Ghost AF
    // fallback for exactly this reason. This site was missed.
    //
    // Worse than a dead factor: unlike the screener normalizer, it recorded no
    // `evidenceOrigins` entry, so the redundancy model could not discount it and it read
    // as genuinely INDEPENDENT confirmation. A single catalyst lead was being counted as
    // two evidence families on the strength of a factor measured at zero.
    //
    // RVOL remains available as a liquidity/participation descriptor; it is simply no
    // longer evidence.
    evidenceFamilies: ['catalystForcedFlow'],
...
function mapOptionsFlowRows(json) {
  ...
  // DIRECTION COMES FROM THE HONEST READ, NOT FROM CALL/PUT DOMINANCE.
  // This used to key off `r.net` — the call-premium-share label — which is exactly the
  // "calls = bullish" equation the engine is not allowed to make. It now requires the
  // provisional direction state produced by lib/options-classify's aggregateDirection,
  // which can (and usually does) return UNKNOWN/MIXED and is therefore filtered out.
  const DIR = { PROVISIONAL_BULLISH: 'long', PROVISIONAL_BEARISH: 'short' };
  return rows.filter(r => r && r.ticker && !r.isIndex   // SPY/QQQ/GLD are the tape, not picks
    ...
    && DIR[r.directionState]).map(r => ({
    source: 'optionsflow', section: 'OptionsFlow', tier: r.grade || 'flow',
    horizon: 'swing', side: DIR[r.directionState],
    ...
    setup: r.directionLabel || r.directionState,
    note: `${r.directionLabel || r.directionState} options positioning${r.grade ? ` (premium composition: ${r.grade})` : ''}`,
