...
async function runTechEvResolve(req, res) {
  ...
  try {
    ...
    for (const day of matured.slice(0, limit)) {
      ...
      // this day next run. The scorecard rows are NOT appended here — see the rebuild
      // below (RMW appends lose rows to Blob overwrite read-back lag).
    ...
    // Always rebuild the derived rows cache from the immutable day docs — one listing +
    // one write per pass, self-healing against any historical lost-update corruption.
    report.rowsRebuild = await TSTORE.rebuildForwardRows({ toRow: FORWARD.toRow });
