...
async function runTechEvDetail(req, res) {
  ...
  try {
    ...
    for (const source of [...new Set(mappings.map((m) => m.source))]) {
      ...
      const entities = mappings.filter((m) => m.source === source).map((m) => m.sourceId);
...
async function runTechEvTick(req, res) {
  ...
  const t0 = Date.now();
  try {
    const collection = await COLLECT.runCollection({ t0, deadlineMs: 150000 });
...
async function runTechEvResolve(req, res) {
  ...
  try {
    ...
    for (const day of matured.slice(0, limit)) {
      if (Date.now() - t0 > 200000) { report.days.push({ day, status: 'skipped:budget' }); break; }
      ...
      for (const event of doc.events) {
        ...
        if (!bench) { postpone = true; postponeWhy = `benchmark ${event.benchmark} unavailable`; break; }
        if (!candles) {
          fetchFailures += 1;
          if (attempts < RESOLVE_MAX_ATTEMPTS) { postpone = true; break; }
          outcomes.push({ id: event.id, status: 'no-history-after-retries' });
          continue;
        }
        const r = FORWARD.resolveEvent(event, candles, bench);
        if (r.status === 'not-mature') { postpone = true; break; }
        outcomes.push(r);
      }
      if (postpone) {
        await TSTORE.writeForwardDay(day, { ...doc, resolveAttempts: attempts });
        report.days.push({ day, status: `postponed (attempt ${attempts}, fetchFailures ${fetchFailures}${postponeWhy ? ', ' + postponeWhy : ''})` });
        break; // later days are strictly less mature — stop the whole pass
      }
    ...
    if (resolvedList.length !== idx.resolved.length) {
