...
// Per-factor weights (normalized fractions, sum ≈ 1). These are what the
// self-tuner (lib/dualread-adapt.js) learns: each factor is a signed unit signal
// (-1/0/+1) and the composite is their weight-blended mean over PRESENT factors,
// so the score is weight-invariant to missing data. The defaults reproduce the
// original hand-set point values (2/2/1/2/1/1/1 → these fractions), so at default
// weights the read is byte-for-byte the same as before it became tunable.
const DEFAULT_LT_WEIGHTS = { trend200: 0.20, cross: 0.20, trend50: 0.10, rs3m: 0.20, rs6m: 0.10, slope50: 0.10, high52: 0.10 };
const LT_FACTORS = Object.keys(DEFAULT_LT_WEIGHTS);

// Composite thresholds (normalized to [-1, 1]). ±0.3 == the old ±3 sum at the
// default weights (whose present-total is 1.0 and max score 10).
const LT_BULL = 0.30;
const LT_BEAR = -0.30;
...
// Weight-blended mean of the present factor signals → composite in [-1, 1].
function compositeFrom(signals, weights = DEFAULT_LT_WEIGHTS) {
  let num = 0, den = 0;
  for (const k of LT_FACTORS) {
    if (signals && signals[k] != null && weights[k] != null) { num += weights[k] * signals[k]; den += weights[k]; }
  }
  return den > 0 ? num / den : 0;
}

function trendFromComposite(c) {
  return c >= LT_BULL ? 'bullish' : c <= LT_BEAR ? 'bearish' : 'neutral';
}
...
// ({close, high, …}); SPY may be null (RS factors are then skipped). `weights`
// (the self-tuner's active weights) blend the factor signals into the composite.
function longTermRead(dailyCandles, spyCandles, weights = DEFAULT_LT_WEIGHTS) {
  if (!dailyCandles || dailyCandles.length < 60) {
    return { trend: 'neutral', score: 0, composite: 0, factors: {}, signals: {}, reasons: ['Not enough history for a long-term read.'], insufficient: true };
  }
  ...
  const signals = {};   // per-factor signed unit vote (-1/0/+1); absent factors omitted
  ...
  if (sma200 != null) {
    ...
    signals.trend200 = px > sma200 ? 1 : -1;
    reasons.push(px > sma200 ? `Above the 200-day (${pctFrom200 >= 0 ? '+' : ''}${pctFrom200.toFixed(0)}%)` : `Below the 200-day (${pctFrom200.toFixed(0)}%)`);
  }

  // 2. 50-day vs 200-day (golden / death structure).
  if (sma50 != null && sma200 != null) {
    factors.sma50 = +sma50.toFixed(2);
    signals.cross = sma50 > sma200 ? 1 : -1;
    reasons.push(sma50 > sma200 ? '50-day above 200-day (uptrend structure)' : '50-day below 200-day (downtrend structure)');
  }

  // 3. Price vs the 50-day (intermediate trend).
  if (sma50 != null) {
    signals.trend50 = px > sma50 ? 1 : -1;
    reasons.push(px > sma50 ? 'Holding above the 50-day' : 'Trading below the 50-day');
  }
  ...
  if (rs3 != null) {
    ...
    signals.rs3m = rs3 > RS_STRONG_3M ? 1 : rs3 < -RS_STRONG_3M ? -1 : 0;
    if (signals.rs3m > 0) reasons.push(`Outperforming SPY over 3mo (+${rs3.toFixed(0)}pts)`);
    else if (signals.rs3m < 0) reasons.push(`Lagging SPY over 3mo (${rs3.toFixed(0)}pts)`);
  }
  ...
  if (rs6 != null) {
    ...
    signals.rs6m = rs6 > RS_STRONG_6M ? 1 : rs6 < -RS_STRONG_6M ? -1 : 0;
    if (signals.rs6m > 0) reasons.push('Six-month leader vs SPY');
    else if (signals.rs6m < 0) reasons.push('Six-month laggard vs SPY');
  }
  ...
  if (closes.length >= 71) {
    ...
    if (sma50 != null && sma50Prev != null) {
      ...
      signals.slope50 = slope > 1 ? 1 : slope < -1 ? -1 : 0;
      if (signals.slope50 > 0) reasons.push('50-day still rising');
      else if (signals.slope50 < 0) reasons.push('50-day rolling over');
    }
  }
  ...
  if (hi52 > 0) {
    ...
    signals.high52 = fromHigh > -15 ? 1 : fromHigh < -25 ? -1 : 0;
    if (signals.high52 > 0) reasons.push('Near 52-week highs');
    else if (signals.high52 < 0) reasons.push(`${fromHigh.toFixed(0)}% off the 52-week high`);
  }

  const composite = compositeFrom(signals, weights);
  const score = Math.round(composite * 10);   // intuitive ±10 display (== the old point sum at default weights)
  const trend = trendFromComposite(composite);
  factors.score = score;
  return { trend, score, composite: +composite.toFixed(3), factors, signals, reasons: reasons.slice(0, 5) };
}
...
module.exports = {
  longTermRead, combineDualRead, stSide, compositeFrom, trendFromComposite,
  DEFAULT_LT_WEIGHTS, LT_FACTORS, LT_BULL, LT_BEAR,
};
