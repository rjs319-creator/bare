 ...
 */
function eligibilityAt(entry, date, cfg, { sector = null, sessionIndex = null, requireEntryBar = true } = {}) {
  ...
  // Tradability under the next-open convention.
  //
  // TRAINING/EVALUATION require the entry bar: a row whose fill never happened cannot be
  // labelled, and admitting it would put an unfillable name in a backtest.
  //
  // SERVING must NOT require it. At the newest session the entry bar is TOMORROW, which has not
  // happened yet — that is the normal state of a live prediction, not a disqualification.
  // Demanding it unconditionally excluded every name on the latest date, so op=forecastrank
  // could never score the current session. `requireEntryBar` is passed as `requireLabel` by
  // dataset.buildDate, so the training and evaluation paths are byte-identical to before.
  if (requireEntryBar && idx + 1 > candles.length - 1) { out.reason = REASONS.NOT_TRADABLE; return out; }
 ...
 */
function buildUniverseSnapshot(panel, date, cfg, { recordExclusions = true, requireEntryBar = true } = {}) {
  const members = [];
  const exclusions = [];
  ...
  for (const [ticker, entry] of panel.dataset) {
    ...
    const e = eligibilityAt(entry, date, cfg, { sector, sessionIndex: panel.sessionIndex, requireEntryBar });
