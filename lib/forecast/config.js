...
const DEFAULTS = Object.freeze({
  ...
  portfolio: Object.freeze({
    ...
    noTradeBand: 2.0,
    // SWITCH-COST TEST — the principled form of the band: replace an incumbent only when the
    // challenger's expected edge clears 0.5*(RT_out + RT_in). Needs an expected-return scale, so
    // the arm's score is calibrated to one on training rows (backtest.fitExpectedReturnMap);
    // without a usable map the band is used instead.
    switchCostTest: true,
    // Portfolio parameters SELECTED PER FOLD on the training window's own out-of-fold
    // predictions — never on a test block. Empty grid disables the search and pins the defaults.
    parameterGrid: Object.freeze({
      topK: Object.freeze([10, 20, 40]),
      noTradeBand: Object.freeze([1, 2, 4]),
      switchCostTest: Object.freeze([false, true]),
    }),
    selectParameters: true,
    // Minimum training-window rows before a portfolio search — and before the expected-return
    // map is fitted — is justified at all. Below it the arm keeps the configured defaults, so a
    // thin fold cannot hand itself a portfolio tuned on noise.
    minSelectionRows: 5000,
  }),
