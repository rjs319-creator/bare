...
function scoreTestBlock({ frames, meta, weights, calibrators, reliability, cfg }) {
  ...
  const scored = frames.map((f) => {
    ...
    return {
      ...
      estimatedCostFraction: BT.costFor({ adv: f.adv }).cost,
 ...
 */
function buildArmRows({ frames, scored, meta, metaNoFoundation, shuffled, weights, baseNames, fold, horizon, cfg, stride, trainThrough, fitted, ridgeRank = null, crossFitFrames = null, portfolioDiag = null }) {
  ...
  // EVERY ARM, CONTROLS INCLUDED, gets a training-window score so it can select its own
  // portfolio parameters. Comparing a tuned candidate against untuned controls confounds the
  // levers with the handicap: a random ranking forced to churn at full turnover loses to
  // anything, which flatters the candidate for a reason that has nothing to do with signal.
  const cff = crossFitFrames || [];
  const delayedTrain = new Map(ARMS.delayedSignal(cff, metaScoreByKey, 1).map((p) => [p.rowKey, p.score]));

  const armDefs = [
    { name: 'control-random', role: 'control',
      preds: ARMS.armPredictions(frames, (f) => ARMS.hashOrder(`${f.ticker}|${f.decisionDate}`)),
      trainScoreOf: (f) => ARMS.hashOrder(`${f.ticker}|${f.decisionDate}`) },
    ...baseNames.map((n) => ({ name: n, role: roleOf(n), preds: ARMS.baseArm(frames, n), trainScoreOf: (f) => (ARMS.usable(f.base[n]) ? f.base[n].point : null) })),
    { name: 'static-ensemble', role: 'ensemble', preds: ARMS.armPredictions(frames, ARMS.staticEnsemblePoint), trainScoreOf: ARMS.staticEnsemblePoint },
    { name: 'dynamic-ensemble', role: 'ensemble', preds: ARMS.armPredictions(frames, (f) => ENS.weightedPoint(f, weights.weights).point), trainScoreOf: (f) => ENS.weightedPoint(f, weights.weights).point },
  ];
  ...
  if (ridgeRank && ridgeRank.fitted) {
    armDefs.push({ name: 'ridge-rank', role: 'baseline', preds: ARMS.armPredictions(frames, (f) => RIDGE.predictPoint(ridgeRank, f)), trainScoreOf: (f) => RIDGE.predictPoint(ridgeRank, f) });
  }
  if (metaNoFoundation && metaNoFoundation.ok) {
    armDefs.push({ name: 'meta-no-foundation', role: 'control', preds: ARMS.armPredictions(frames, (f) => metaNoFoundation.scores.get(f.rowKey)), trainScoreOf: (f) => metaNoFoundation.scores.get(f.rowKey) });
  }
  if (meta.ok) {
    armDefs.push({
      ...
      trainScoreOf: (f) => meta.scores.get(f.rowKey),
    });
  }
  if (shuffled.ok) {
    armDefs.push({ name: 'control-shuffled-label', role: 'control',
      preds: ARMS.armPredictions(frames, (f) => shuffled.scores.get(f.rowKey)),
      trainScoreOf: (f) => shuffled.scores.get(f.rowKey) });
  }
  armDefs.push({ name: 'control-delayed-signal', role: 'control',
    preds: ARMS.delayedSignal(frames, metaScoreByKey, 1),
    trainScoreOf: (f) => delayedTrain.get(f.rowKey) });

  const universeSize = new Set(frames.map((f) => f.ticker)).size;

  // Rows for the SAME arm over the training window, from the cross-fitted (out-of-fold)
  // predictions. This is what the portfolio parameters are chosen on — never the test block.
  const trainRowsFor = (scoreOf) => (crossFitFrames || [])
    .map((f) => ({ decisionDate: f.decisionDate, ticker: f.ticker, sector: f.sector, score: scoreOf(f), adv: f.adv, price: f.price, label: f.label }))
  ...
  return armDefs.map((arm) => {
    ...
    // Per-fold portfolio parameter selection on the training window, then applied to the test
    // block. Arms whose training-window scores are unavailable (the controls, which are defined
    // only on the test frames) keep the configured defaults.
    let armCfg = cfg, selection = null, expectedMap = null;
    if (cfg.portfolio.selectParameters && arm.trainScoreOf && crossFitFrames && crossFitFrames.length) {
      const trainRows = trainRowsFor(arm.trainScoreOf);
      const minRows = cfg.portfolio.minSelectionRows;
      if (trainRows.length >= minRows) {
        expectedMap = BT.fitExpectedReturnMap(trainRows, { minRows });
        selection = BT.selectPortfolioParameters(trainRows, cfg, { horizon, stride, expectedMap });
        armCfg = { ...cfg, portfolio: { ...cfg.portfolio, ...selection.selected } };
      }
    }
    if (portfolioDiag && arm.trainScoreOf) {
      portfolioDiag[arm.name] = selection
        ? { selected: selection.selected, metric: selection.metric || null, best: selection.best || null, fallback: selection.fallback || null }
        : { selected: { topK: cfg.portfolio.topK, noTradeBand: cfg.portfolio.noTradeBand, switchCostTest: false }, fallback: `fewer than ${cfg.portfolio.minSelectionRows} training rows — defaults kept` };
    }

    const backtest = btRows.length >= 50 ? BT.runBacktest(btRows, armCfg, { horizon, stride, expectedMap }) : null;
...
function runFold({ fold, horizon, allRows, featureKeys, cfg, caps, panel, axis, priorObservations, fixture = null, stride = 1 }) {
  ...
  const shuffled = META.fitAndScore({
    // Scores the CROSS-FITTED frames too, so this control can select its own portfolio
    // parameters on a training window exactly as the real arms do. Prediction is cheap next to
    // fitting, and a control that is denied the levers is not a control — it is a handicap.
    trainFrames: ARMS.shuffleLabelsWithinDate(cf.frames, cfg.seed + horizon), predictFrames: outer.frames.concat(cf.frames),
  ...
  const rows = buildArmRows({
    ...
    crossFitFrames: cf.frames, portfolioDiag,
  });
  diagnostics.portfolio = portfolioDiag;
