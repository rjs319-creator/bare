...
function runInference({
  ...
  for (const h of hs) {
    ...
    if (!cf.frames.length) {
      // Say WHICH constraint bound, with the numbers. A live panel is often much shorter than a
      // research panel, and "cross-fitting produced no frames" sends the operator hunting when
      // the answer is arithmetic: too few labelled training decision dates to carve inner folds.
      const trainDatesWithRows = new Set(trainRows.map((r) => r.decisionDate)).size;
      byHorizon[h] = {
        ok: false,
        reason: cf.diagnostics.reason || 'cross-fitting produced no frames',
        detail: {
          labelledTrainingDates: trainDatesWithRows,
          labelledTrainingRows: trainRows.length,
          innerFoldsRequested: cfg.walkforward.innerFolds,
          minHistorySessions: cfg.universe.minHistorySessions,
          panelSessions: sessions.length,
          hint: `a name needs ${cfg.universe.minHistorySessions} sessions of history before it is eligible, so a panel of ${sessions.length} sessions yields labelled decision dates only after that point; lengthen the panel, lower universe.minHistorySessions, or lower walkforward.innerFolds`,
        },
      };
      continue;
    }
    ...
    byHorizon[h] = {
      ...
      rows: ranked.map((e, i) => {
        ...
        return makeScoredRow({
          ...
          // The full frozen-beta set travels WITH the scored row. A consumer persisting
          // decision-time betas (the shadow ledger) must not have to re-run the serving
          // pass to recover the sector orthogonalization leg — a second pass can drift
          // from the cohort the scores describe.
          sectorMarketBeta: srcRow ? srcRow.betaSectorMarket : null, sectorEtf: srcRow ? srcRow.sectorEtf : null,
          sector: e.sector, estimatedCostFraction: e.estimatedCostFraction,
