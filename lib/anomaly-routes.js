...
async function detect(limit) {
  ...
  for (const [s, list] of scopes) {
    ...
    for (const t of list) {
      ...
      // OBSERVED SESSION, not "session of a hit": asOf must advance for every name the
      // scan actually READ. Setting it only for candidates meant a quiet tape (no movers)
      // published asOf:null — a scan that saw a full session claiming to have seen none,
      // which the pre-ranking gate then had to refuse.
