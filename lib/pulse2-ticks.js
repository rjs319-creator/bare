...
const freshnessLib = require('./pulse2-freshness');
...
const { callFableTool, FABLE_MODEL: REFINE_MODEL } = require('./fable-call');   // claude-fable-5-1, strict tool, refusal-safe
...
// Fetch the cross-asset proxy bars once and build both the matrix and the regime stack.
// Bounded concurrency; every symbol that fails simply does not appear in `dailyByTicker`,
// and the matrix reports that leg UNAVAILABLE with the reason rather than skipping it.
async function buildRegimeLayers({ state, vixState, prevDoc, now }) {
  const crossAssetLib = require('./pulse2-cross-asset');
  const stackLib = require('./pulse2-regime-stack');
  const { fetchDailyHistory } = require('./screener');

  const wanted = crossAssetLib.REQUIRED_TICKERS;
  const dailyByTicker = {};
  let i = 0;
  const worker = async () => {
    while (i < wanted.length) {
      const t = wanted[i++];
      try { const d = await fetchDailyHistory(t); if (d && d.candles) dailyByTicker[t] = d.candles; } catch { /* leg reports UNAVAILABLE */ }
    }
  };
  await Promise.all(Array.from({ length: Math.min(6, wanted.length) }, worker));

  const spy = state.indexes && state.indexes.SPY;
  const equityChangePct = spy && spy.available ? spy.dayReturnPct : crossAssetLib.changePct(dailyByTicker.SPY, 1);

  const crossAsset = crossAssetLib.buildCrossAssetMatrix({
    dailyByTicker, equityChangePct,
    vix: vixState && vixState.available
      ? { level: vixState.level, level3m: crossAssetLib.lastClose(dailyByTicker['^VIX3M']) }
      : { level: crossAssetLib.lastClose(dailyByTicker['^VIX']), level3m: crossAssetLib.lastClose(dailyByTicker['^VIX3M']) },
  });

  // Strategic macro legs from FRED (free, keyed). With no FRED_API_KEY this resolves to
  // `inputs: null` and the strategic layer reports itself UNAVAILABLE with that reason —
  // it is never inferred from price. Diagnostics ride along either way so the gap is
  // visible rather than silent.
  let macro2 = { inputs: null, diagnostics: { available: false, reason: 'macro build not attempted' } };
  try {
    macro2 = await require('./pulse2-macro').buildMacroInputs({
      // Index earnings yield is an equity input, not a FRED series; absent it the
      // valuation leg reports UNAVAILABLE rather than substituting a constant.
      earningsYieldPct: null,
      now: now instanceof Date ? now.getTime() : Date.now(),
    });
  } catch { /* additive layer — a failure must not cost the market-state snapshot */ }

  const regimeStack = stackLib.buildRegimeStack({
    marketState: state,
    crossAsset,
    macroInputs: macro2.inputs,
    sectorLeadership: state.sectorLeadership && state.sectorLeadership.available ? state.sectorLeadership : null,
    prevStack: prevDoc && prevDoc.regimeStack ? prevDoc.regimeStack : null,
    now,
  });

  return { crossAsset, regimeStack, macroDiagnostics: macro2.diagnostics };
}
...
async function runPulse2StateTick(req, res) {
  ...
  // ── Cross-asset confirmation matrix + three-layer regime stack ──
  // Both are ADDITIVE: a failure here leaves them null and the rest of the snapshot is
  // unchanged, so the existing market-state contract cannot regress. Every leg the daily
  // fetch could not supply is reported UNAVAILABLE with a reason, never as neutral.
  const { crossAsset, regimeStack, macroDiagnostics } = await buildRegimeLayers({ state, vixState, prevDoc, now })
    .catch(() => ({ crossAsset: null, regimeStack: null, macroDiagnostics: null }));
  ...
  // A tick that measured NOTHING (weekend/holiday or total provider outage) must not
  // clobber the last snapshot that had real data — keep last-known-good; the read-time
  // clocks age it honestly and health records the empty attempt.
  const replaceVerdict = marketState.shouldReplaceSnapshot(prevDoc, state);
  if (!replaceVerdict.replace) {
    if (store.hasStore()) {
      await store.mergeHealth('marketState', {
        lastAttemptAt: nowIso, lastError: null,
        keptLastGood: true, keptReason: replaceVerdict.reason,
        coverage: doc.coverage, durationMs: doc.elapsedMs,
      }).catch(() => {});
    }
    return res.status(200).json({
      ok: true, persisted: false, keptLastGood: true, reason: replaceVerdict.reason,
      mode: state.mode.mode, coverage: doc.coverage, elapsedMs: doc.elapsedMs,
    });
  }

  let wr = { written: false, verified: false };
  if (store.hasStore()) {
    if (prevDoc) await store.writeJSON(store.KEYS.MARKET_STATE_PREV, prevDoc, 0).catch(() => {});
    wr = await store.writeChecked(store.KEYS.MARKET_STATE, doc, b => b.at === nowIso);
    ...
    await store.mergeHealth('marketState', {
      ...
      lastSuccessAt: wr.written ? nowIso : undefined,
      lastError: wr.written ? null : 'persist-failed',
      verifiedReadback: wr.verified,
      coverage: doc.coverage, mode: state.mode.mode,
      durationMs: doc.elapsedMs,
    }).catch(() => {});
  }
  const failed = store.hasStore() && !wr.written;
  return res.status(failed ? 500 : 200).json({
    ok: !failed, persisted: wr.written, verifiedReadback: wr.verified,
    mode: state.mode.mode, alerts: derived.alerts.length,
...
async function runPulse2Collect(req, res) {
  ...
  // Sources gain the timestamp-provenance block here: discoveredAt is THIS tick's
  // clock; publishedAt only ever comes from the provider/URL evidence chain; and
  // wrong-year / evergreen pages are flagged reference (never fresh evidence).
  const index = provenance.buildSourceIndex(sources, { discoveredAt: nowIso, now: nowIso, asOfDate: date });
  ...
  // 2. Claim-level provenance (anti-fabrication: unmatched URLs dropped) + the
  // item's own timestamp story (earliest real publication vs discovery — never equal).
  const items = rawItems.map(it => {
    ...
    const srcTimes = provenance.summarizeClaimSources(claims, index);
    return {
      ...
      eventOccurredAt: /^\d{4}-\d{2}-\d{2}$/.test(String(it.eventDate || '')) ? it.eventDate : null,
      publishedAt: srcTimes.publishedAt,
      dateConfidence: srcTimes.dateConfidence,
      discoveredAt: nowIso,
      sourceCount: srcTimes.sourceCount,
      referenceSourceCount: srcTimes.referenceCount,
      claims,
  ...
  const observations = items.map(it => {
    ...
    return {
      ...
      publishedAt: it.publishedAt, dateConfidence: it.dateConfidence, sourceCount: it.sourceCount,
  ...
  const freshPolicy = freshnessLib.policyFromEnv();
  ...
  for (const ev of activeRecent) {
    ...
    // Views — each card carries its freshness block (reason, group, the three
    // timestamps, ages, precomputed rank score) for honest display + ranking.
    ...
    day.freshness = freshnessLib.cardFreshness(ev, nowIso, { horizon: 'day', tradeState: day.tradeState, reaction: reactionInfo.reaction, policy: freshPolicy });
    swing.freshness = freshnessLib.cardFreshness(ev, nowIso, { horizon: 'swing', tradeState: swing.tradeState, reaction: reactionInfo.reaction, policy: freshPolicy });
    investor.freshness = freshnessLib.cardFreshness(ev, nowIso, { horizon: 'investor', tradeState: investor.tradeState, reaction: reactionInfo.reaction, policy: freshPolicy });
  ...
  // "Fresh verified" is age-gated on PUBLICATION time, not discovery time: only
  // NEW_PUBLICATION / NEW_MATERIAL_UPDATE events with a known publication ≤6h and
  // event ≤12h qualify — a six-day-old story discovered this cycle stays out.
  const active = folded.events.filter(e => !e.expired);
  const sections = {
    freshVerified: active.filter(e => (e.evidence === 'VERIFIED' || e.evidence === 'SUPPORTED')
      && freshnessLib.freshVerifiedGate(e, nowIso, freshPolicy).pass).map(e => e.id),
  ...
  // 8. Persist (generation-guarded). View lists are RANKED (recency-aware score,
  // deterministic tie-break — never input order); the day view drops events older
  // than its context window entirely, and its context group renders collapsed.
  const rankedDay = [...dayList].filter(d => !d.freshness || d.freshness.group !== 'excluded').sort(freshnessLib.compareRanked);
  const rankedSwing = [...swingCandidates].filter(s => !s.freshness || s.freshness.group !== 'excluded').sort(freshnessLib.compareRanked);
  const rankedInvestor = [...investorList].sort(freshnessLib.compareRanked);
  const narrativesDoc = {
    ...
    items,
    views: { day: rankedDay, swing: rankedSwing, investor: rankedInvestor },
    sections,
  ...
  const wr = await store.writeChecked(store.KEYS.NARRATIVES, narrativesDoc, b => b.generation === generation);
  await store.writeChecked(store.KEYS.EVENTS, { events: folded.events, updatedAt: nowIso }, b => b.updatedAt === nowIso);
  const persisted = wr.written;
...
async function runPulse2Refine(req, res) {
  ...
  const r = await callFableTool({
    client: c,
    tool: REFINE_TOOL,
    maxTokens: 3000,
    timeoutMs: REFINE_TIMEOUT_MS,
    effort: 'low',     // bound Fable's always-on thinking (proven pattern)
  ...
  if (!r || r.error) {
    return res.status(200).json({ ok: false, error: 'refine failed: ' + (r ? r.error : 'no client'), served: 'collected stage remains' });
  }
  if (r.refused) return res.status(200).json({ ok: false, error: 'refine refused: ' + (r.category || '?'), served: 'collected stage remains' });
  const refined = r.input && Array.isArray(r.input.reads) ? r.input.reads : null;
  ...
  const wr = await store.writeChecked(store.KEYS.NARRATIVES, updated, b => b.stage === 'refined' && b.generation === generation);
  await store.mergeHealth('narratives', { lastRefineAt: nowIso, refinePersisted: wr.written, refineVerifiedReadback: wr.verified }).catch(() => {});
  return res.status(wr.written ? 200 : 500).json({ ok: wr.written, verifiedReadback: wr.verified, generation, refined: items.filter(x => x.crowding).length });
}
...
async function runPulse2Grade(req, res) {
  ...
  const wr = await store.writeChecked(store.KEYS.OUTCOMES,
    { outcomes: merged, summary, comparison, updatedAt: nowIso }, b => b.updatedAt === nowIso);
  await store.mergeHealth('grading', {
    lastAttemptAt: nowIso, lastSuccessAt: wr.written ? nowIso : undefined,
    lastError: wr.written ? null : 'persist-failed',
    verifiedReadback: wr.verified,
    horizonsAppended: appendedCount, pairsPending: pairs.length - batchPairs.length,
    durationMs: Date.now() - t0,
  }).catch(() => {});
  return res.status(wr.written ? 200 : 500).json({
    ok: wr.written, verifiedReadback: wr.verified, graded: appendedCount, pairs: batchPairs.length,
