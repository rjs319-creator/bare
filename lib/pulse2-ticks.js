...
async function buildRegimeLayers({ state, vixState, prevDoc, now }) {
  ...
  const regimeStack = stackLib.buildRegimeStack({
    marketState: state,
    crossAsset,
    // Growth / inflation / liquidity / earnings-revision / valuation series are not wired
    // to any provider on this plan. Passing null makes the strategic layer report itself
    // UNAVAILABLE with that reason instead of being inferred from price.
    macroInputs: null,
...
async function runPulse2StateTick(req, res) {
  ...
  const { crossAsset, regimeStack } = await buildRegimeLayers({ state, vixState, prevDoc, now })
    .catch(() => ({ crossAsset: null, regimeStack: null }));
