...
const SYSTEM = [
  ...
  'Return your answer ONLY by calling the submit_game_plan tool.',
].join(' ');
...
function buildUserMessage({ date, macro, headlines, signals, priorNarrative }) {
  ...
  lines.push('', `Write today's game plan. Cap drivers at ${MAX_DRIVERS} and predictions at ${MAX_PREDICTIONS}. Call submit_game_plan.`);
  return lines.join('\n');
}
...
async function synthesize(client, state, { model = MODEL, maxTokens = 2200 } = {}) {
