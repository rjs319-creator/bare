...
const SYSTEM = [
  ...
  'You are also given an OPTIONS FLOW grade per stock (-100 very bearish to +100 very bullish) measuring unusual smart-money options positioning today. Use it to CONFIRM or CONTRADICT the news-driven view: e.g. a bullish headline backed by a high positive options grade is higher-conviction; a bullish story with bearish flow is a yellow flag. Reference specific graded names where relevant.',
...
// Compact the unusual-options-flow grades (smart-money positioning) into a line.
function optionsFlowLine(flow) {
  if (!flow || !flow.topStocks || !flow.topStocks.length) return 'Options flow: unavailable.';
  const sign = n => (n > 0 ? '+' : '') + n;
  const top = flow.topStocks.slice(0, 8).map(s => `${s.ticker} ${s.grade} (${sign(s.score)})`).join(', ');
  return `Options flow (unusual smart-money positioning, graded -100=very bearish to +100=very bullish): market grade ${flow.marketGrade} (${sign(flow.marketScore)}). Notable single-stock grades — ${top}.`;
}
