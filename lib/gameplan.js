...
// The exact unavailability marker the verified snapshot renders — imported so the
// SYSTEM instruction and the rendered sentinel can never silently diverge.
const SNAPSHOT_UNAVAILABLE_MARKER = require('./verified-snapshot').NO_DATA_SENTINEL.split(' — ')[0];
const MAX_DRIVERS = 4;
...
const SYSTEM = [
  ...
  `If a VERIFIED MARKET SNAPSHOT is provided, it is the ONLY source of truth for exact index prices and levels: derive any numeric level you cite from it, flag (rather than reconcile) any input that conflicts with it, and when it marks a symbol ${SNAPSHOT_UNAVAILABLE_MARKER} say the data is unavailable instead of estimating.`,
  ...
  'You are also given an UNUSUAL OPTIONS ACTIVITY read per stock, derived from DELAYED, free option-chain data (volume vs open interest + estimated premium). This is a SHADOW, research-grade confirmation overlay — it is NOT proven "smart money", it cannot see who bought or sold, and it must NEVER by itself make you more or less bullish on a name. Use it ONLY to note agreement or tension with an independently news/price-driven view (e.g. "options activity is consistent with / runs counter to the story"), always as provisional and reversible. If a name\'s options read is DIRECTION UNKNOWN or MIXED, say so plainly rather than inventing a lean. Do not describe it as smart money, sweeps, or confirmed positioning.',
  ...
  'If a MEASURED RECORD of your prior plans is provided, treat it as ground truth about your own recent calibration — it is computed deterministically from realized SPY moves, never self-reported. A record running wrong means correct toward the tape, not repeat the stance. Resolve every listed OPEN PREDICTION in narrativeUpdate with a short clause each ("X confirmed; Y invalidated; Z open"), oldest first — this per-prediction accounting is allowed to extend narrativeUpdate beyond its usual cap by one sentence.',
...
// Compact the unusual-options-activity read into a line. Labeled as a SHADOW,
// delayed-data confirmation overlay — never "smart money".
function optionsFlowLine(flow) {
  if (!flow || !flow.topStocks || !flow.topStocks.length) return 'Options activity: unavailable.';
  const sign = n => (n > 0 ? '+' : '') + n;
  const top = flow.topStocks.slice(0, 8).map(s => `${s.ticker} ${s.grade} (${sign(s.score)})`).join(', ');
  return `Unusual options activity (SHADOW confirmation overlay, delayed free-chain data, graded -100=most bearish-leaning to +100=most bullish-leaning; a provisional lean, NOT proof, NOT smart money): market read ${flow.marketGrade} (${sign(flow.marketScore)}). Names with notable activity — ${top}. Use only to note agreement/tension with the price/news view.`;
}

// Build the full user message from the assembled market state.
function buildUserMessage({ date, macro, headlines, signals, optionsFlow, priorNarrative, reflection, snapshot, predMarkets }) {
  const lines = [
    `DATE: ${date}`,
    macroLine(macro),
    signalsLine(signals),
    optionsFlowLine(optionsFlow),
  ];
  if (predMarkets && predMarkets.trim()) lines.push(predMarkets.trim());
  if (snapshot && snapshot.trim()) lines.push('', snapshot.trim());
  lines.push('', headlinesBlock(headlines));
...
module.exports = {
  ...
  macroLine, headlinesBlock, signalsLine, optionsFlowLine, buildUserMessage, synthesize,
};
