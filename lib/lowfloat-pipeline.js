...
async function runPipeline({
  ...
  // PROVIDER-CAPABILITY GUARD. The volume-capable bulk provider (FMP batch-quote) covers the
  // whole universe in ~24 requests. Without it the only fallback is the keyless Yahoo spark
  // endpoint, which chunks at 40 symbols — a ~4,700-name universe becomes ~118 requests, and
  // it returns NO volume, so three of the seven discovery lanes are dead anyway. Fanning out
  // that far to get a strictly worse answer is the wrong trade, so the scan is bounded and
  // the reduced coverage is REPORTED rather than hidden.
  const volumeCapableProvider = !!process.env.FMP_API_KEY;
  const quoteCap = volumeCapableProvider
    ? CFG.DISCOVERY.UNIVERSE_QUOTE_CAP
    : Math.min(CFG.DISCOVERY.UNIVERSE_QUOTE_CAP, 800);
  const scanUniverse = securities.slice(0, quoteCap);
  ...
  diagnostics.stages.discovery = {
    ...
    capabilityNote: volumeCapableProvider ? null
      : 'No volume-capable bulk-quote provider is configured (FMP_API_KEY missing). The scan is bounded and the relative-volume, volume-acceleration and low-float-turnover lanes cannot run — recall is materially reduced and this is NOT full-market coverage.',
  };
