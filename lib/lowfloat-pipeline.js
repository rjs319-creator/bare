...
async function runPipeline({
  ...
  const discovery = await runMoverDiscovery({
    universe: scanUniverse, state: usableState, floatByTicker: floatSeed,
    now, minutesSinceOpen: session.minutesSinceOpen, t0,
    session: session.session ? String(session.session).toLowerCase() : null,   // 'premarket' | 'regular' | ...
  });
  ...
  for (const c of withFloat) {
    ...
    records.push({
      ...
      // Structure features the IGNITION synthesis layer (lib/ignition-live.js) consumes.
      // Additive projection only — nothing upstream reads these back.
      structureFeatures: structure ? {
        vwapExtensionAtr: structure.vwapExtensionAtr ?? null,
        vwapDistancePct: structure.vwapDistancePct ?? null,
        sessionRunPct: (structure.features && structure.features.sessionRunPct) ?? null,
        upperWickRisk: structure.upperWickRisk ?? null,
        lastBarVolMult: (structure.features && structure.features.lastBarVolMult) ?? null,
        aboveVwap: structure.aboveVwap ?? null,
        consecutiveHigherLows: structure.consecutiveHigherLows ?? null,
        compressionRatio: structure.compressionRatio ?? null,
        emaStacked: (structure.features && structure.features.emaStacked) ?? null,
        atr5m: structure.atr5m ?? null,
      } : null,
      catalystPresent: catalyst ? catalyst.catalystPresent ?? null : null,
      catalystRecycled: catalyst ? catalyst.isRecycled ?? null : null,
      catalystMateriality: catalyst ? catalyst.materiality ?? null : null,
      supplyRiskDetail: supplyRisk ? {
        riskScore: supplyRisk.riskScore ?? null,
        newsAvailable: supplyRisk.newsAvailable === true,
        atmRisk: supplyRisk.atmRisk === true,
        activeOfferingRisk: supplyRisk.activeOfferingRisk === true,
        shelfRisk: supplyRisk.shelfRisk === true,
        convertibleRisk: supplyRisk.convertibleRisk === true,
        warrantOverhang: supplyRisk.warrantOverhang === true,
        reverseSplitRecent: supplyRisk.reverseSplitRecent === true,
      } : null,
      // Stage 3
