...
function buildTickerSamples(input, opts = {}) {
  ...
  for (let i = minBars; i < candles.length - 6; i += step) {
    const snap = orbitFeatures(candles, factorClosesAll, { asOfIdx: i });
...
async function runBackfill(opts = {}) {
  ...
  return {
    ...
    samples: allSamples,
    researchValidity: { productionGrade: false, survivorshipSafe: false, pointInTimeUniverse: false, reason: 'Universe enumerated from current survivor lists; delisted names absent.' },
  };
}
