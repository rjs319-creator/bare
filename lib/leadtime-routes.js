...
async function assemblePicks() {
  ...
  const groups = [
    ...
    // Backfilled (historically reconstructed) ignition rows are NOT detections — a
    // reconstructed 2025 row must never win the first-appearance anchor a live pick's
    // lead-time is measured from.
    ...rawIgn.filter(p => p && p.backfill !== true).map(p => ({ ...p, section: p.section || 'Ignition' })),
