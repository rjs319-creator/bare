...
async function extractEvents(ticker, opts = {}) {
  ...
  const events = raw.events.slice(0, MAX_EVENTS).map(rawEv => {
    ...
    return normalizeEvent(rawEv, {
      ticker, sources, detectedAt: opts.detectedAt || null,
...
module.exports = { extractEvents, newsFingerprint, callExtract, MODEL, PROMPT_VERSION, MAX_HEADLINES, EXTRACT_TOOL };
