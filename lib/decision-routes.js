...
function buildToday(sources = {}, prev = null, redundancy = null) {
  const s = sources;
  ...
  const active = D.rankSignals(merged, { regime, scoreboard, redundancy });
  const all = D.rankSignals(merged, { regime, scoreboard, includeInactive: true, redundancy });
...
async function runToday(req, res) {
  ...
  const payload = buildToday(sources, prev, redundancy);
