...
// Called through the module object (never destructured) so tests can stub the store
// and exercise the daily-log path without a real Blob token.
...
// Cost-aware weight solver (optimizer-v1) — consumed ONLY by the SHADOW lane below.
// Kept on the module object (never destructured) so tests can stub `PO.optimizeBook`
// and prove the live book is byte-identical without it.
const PO = require('./portfolio-optimizer');
...
// MEASURED 2026-09-09 (prod, cache-busted, single calls): op=scoreboard 16.3s (4.7MB),
// op=daytrade 10.6s, op=gapgo 7.4s, op=coil 5.4s, screener?scope=expanded 4.8s. The old
// 12s ceiling cut the scoreboard pull on EVERY call — Today ranked with NO realized-record
// evidence (every tilt neutral, freshness said "scoreboard unavailable") — and cut
// daytrade/expanded intermittently. Pulls run in parallel, so the wall is the slowest one.
const PULL_TIMEOUT_MS = 30000;
async function pull(path, { timeoutMs = PULL_TIMEOUT_MS, fetchImpl = fetch } = {}) {
  const t0 = Date.now();
  try {
    const r = await fetchImpl('https://' + HOST + path, { headers: internalHeaders(), signal: AbortSignal.timeout(timeoutMs) });
...
// The rank's evidence record is the PERSISTED scoreboard summary — written only by a
// COMPLETE op=scoreboard run and the same document op=maturity / governance judge on —
// not the 4.7MB live payload (16s, which the pull used to time out on). ~300KB, one Blob
// read. The live endpoint is only a fallback when the doc is absent (fresh store).
const SCOREBOARD_SUMMARY_PATH = 'scoreboard/summary.json';
async function pullScoreboard({ readJSON = (p) => STORE.readJSON(p, null), pullLive = () => pull('/api/tracker?op=scoreboard') } = {}) {
  const t0 = Date.now();
  try {
    const doc = await readJSON(SCOREBOARD_SUMMARY_PATH);
    if (doc && Array.isArray(doc.groups) && doc.groups.length) {
      return { path: SCOREBOARD_SUMMARY_PATH, ok: true, status: 200, ms: Date.now() - t0, data: doc, basis: 'persisted-summary' };
    }
  } catch { /* unreadable doc → live fallback below */ }
  const live = await pullLive();
  return { ...live, basis: 'live-fallback' };
}

// ── OPTIMIZER SHADOW LANE (optimizer-v1) ────────────────────────────────────
// lib/portfolio-optimizer solves for WEIGHTS against one net economic objective
// (net-EV − impact − risk − turnover; cash is a first-class answer). The live book
// (portfolio-v1 above) deliberately selects and never sizes — so the optimizer runs
// here as a weight-0 SHADOW over the SAME sizing-eligible set, to accrue prospective
// evidence before sizing could ever bind. It must never change the published book,
// ranks, exposure, or any user-facing recommendation.
//
// FIELD MAPPING — every optimizer input comes from a value the decision row already
// carries; nothing is recomputed on a different basis:
//   expectedNetPct        ← cost.netMovePct, ONLY when cost.known. This is the exec-v1
//                           entry-basis net figure (next-open + adverse slippage for
//                           non-daytrade; published level for the pinned daytrade
//                           section — the cost payload's own entryBasis stamp says
//                           which) — the same basis the rank and the graded record use.
//   dollarVol             ← liquidity.dollarVol (measured only; null stays null, and
//                           the impact term charges nothing it cannot measure).
//   sector                ← the live book's own unknown-placeholder rule
//                           (decision-portfolio knownSector), so two '?' names are not
//                           capped against each other as a fake shared sector. NOTE:
//                           inside the optimizer a null sector joins its 'Unknown'
//                           bucket for the sector cap — stricter than the live book's
//                           "unknown is never a violation", disclosed here.
//   idioVol / uncertaintyPenaltyPct ← ABSENT. Decision rows carry no per-name daily
//                           vol and no calibrated per-name uncertainty; the optimizer's
//                           conservative defaults (σ=0.02, penalty 0) apply.
// Rows with no known cost (leads without levels) have no net-EV for the objective to
// size and are skipped WITH A COUNT — a net-EV objective must not invent one.
// Duplicate tickers across horizons collapse to the highest-ranked row, mirroring the
// live book's duplicate-underlying rule (one position per underlying).
function optimizerCandidatesOf(sizingSet) {
  const seen = new Set();
  const candidates = [];
  let skippedNoNetEv = 0, dedupedByTicker = 0;
  for (const row of sizingSet || []) {
    if (!row || !row.ticker) continue;
    const net = row.cost && row.cost.known === true && Number.isFinite(row.cost.netMovePct)
      ? row.cost.netMovePct : null;
    if (net == null) { skippedNoNetEv += 1; continue; }
    if (seen.has(row.ticker)) { dedupedByTicker += 1; continue; }
    seen.add(row.ticker);
    const dv = row.liquidity && Number.isFinite(row.liquidity.dollarVol) ? row.liquidity.dollarVol : null;
    candidates.push({
      ticker: row.ticker,
      sector: PF.knownSector(row.sector),
      expectedNetPct: net,
      dollarVol: dv,
      // Carried for the shadow display only — the optimizer ignores these. Weight is
      // gross capital committed to the trade; direction stays on the row (the book is
      // "long-only" in the w≥0 sense, and netMovePct is the trade's own favorable move).
      side: row.side === 'short' ? 'short' : 'long',
      horizon: row.horizon ?? null,
    });
  }
  return { candidates, skippedNoNetEv, dedupedByTicker };
}

// Name-overlap diagnostic between the live selection and the shadow weights — an
// agreement READ, never a re-ranking claim.
function bookAgreement(liveSelected, shadowWeights) {
  const live = [...new Set((liveSelected || []).map(x => x.ticker))];
  const shadow = Object.entries(shadowWeights || {}).filter(([, w]) => w > 0).map(([t]) => t);
  const liveSet = new Set(live);
  const shadowSet = new Set(shadow);
  const overlap = live.filter(t => shadowSet.has(t));
  const unionSize = new Set([...live, ...shadow]).size;
  return {
    overlapCount: overlap.length,
    // Both books empty ⇒ no overlap statement to make — null, not a fake 0 or 1.
    jaccardVsLive: unionSize ? +(overlap.length / unionSize).toFixed(3) : null,
    onlyLive: live.filter(t => !shadowSet.has(t)),
    onlyShadow: shadow.filter(t => !liveSet.has(t)),
  };
}

// Build the shadow sizing read. NEVER throws — an observer must not be able to break
// the board it watches; any failure returns { shadow:true, error } instead.
function buildPortfolioShadow(sizingSet, livePortfolio) {
  try {
    const { candidates, skippedNoNetEv, dedupedByTicker } = optimizerCandidatesOf(sizingSet);
    const out = PO.optimizeBook(candidates);
    const byTicker = new Map(candidates.map(c => [c.ticker, c]));
    const selected = Object.entries(out.weights)
      .filter(([, w]) => w > 0)
      .sort((a, b) => b[1] - a[1])
      .map(([ticker, weight]) => {
        const c = byTicker.get(ticker) || {};
        return { ticker, weight, sector: c.sector ?? null, side: c.side || 'long',
          horizon: c.horizon ?? null, expectedNetPct: c.expectedNetPct ?? null };
      });
    const cmp = out.comparisons || {};
    const slim = (b) => (b ? { value: b.value, gross: b.gross, cash: b.cash, turnover: b.turnover } : null);
    return {
      method: PO.OPTIMIZER_VERSION,
      shadow: true, liveWeight: 0,
      selected,
      weights: out.weights,
      cash: out.book.cash,
      book: out.book,               // { value, gross, cash, turnover } under the objective
      config: out.config,
      reasons: out.reasons,         // per-ticker machine-readable allocate/skip reason
      riskModel: out.riskModel,     // the diagonal-idio-vol honesty disclosure, verbatim
      comparisons: {
        objectiveBasis: cmp.objectiveBasis || null,
        equalWeight: slim(cmp.equalWeight), rankWeight: slim(cmp.rankWeight),
      },
      candidateBasis: {
        basis: 'the SAME sizing-eligible set the live book was built from',
        sizingEligible: (sizingSet || []).length,
        mapped: candidates.length,
        skippedNoNetEv,
        dedupedByTicker,
        fieldMapping: 'expectedNetPct=cost.netMovePct (cost.known only; exec-v1 entryBasis as stamped on the row) · dollarVol=liquidity.dollarVol · sector=live-book knownSector rule · idioVol/uncertainty absent → conservative optimizer defaults',
      },
      agreement: bookAgreement(livePortfolio && livePortfolio.selected, out.weights),
      note: 'SHADOW / weight-0 research: this sized book is computed alongside the live selection and influences nothing — not the published portfolio, ranks, exposure, or any recommendation. It accrues prospective evidence that cost-aware sizing beats selection-only before it could ever bind.',
    };
  } catch (e) {
    return { method: PO.OPTIMIZER_VERSION, shadow: true, error: String((e && e.message) || e) };
  }
}
...
function buildToday(sources = {}, prev = null, redundancy = null, origins = null, opts = {}) {
  ...
  // SHADOW sizing read (optimizer-v1): the SAME sizing-eligible set, solved for weights
  // under the net-EV/impact/risk/turnover objective. Report-only, wrapped, weight 0 —
  // see buildPortfolioShadow above. The live `portfolio` is already final at this point
  // and nothing below reads the shadow back into it.
  const portfolioShadow = buildPortfolioShadow(sizingSet, portfolio);
  ...
  return {
    ...
    // Explicitly-shadowed sized book (optimizer-v1). Payload-only: no frontend renders
    // it, and the live `portfolio` above is provably byte-identical without it.
    portfolioShadow,
...
// WRITE-ONCE daily parity record, FAIL-CLOSED (review finding #8, 2026-08-15): the first
// logged board of the day is THE recommendation of record — later calls must not rewrite
// it. Existence is gated on STORE.blobExists (same contract as store.writeOmegaFunnelDay):
// only "definitely absent" (probe succeeded, no exact pathname) permits the write. The old
// guard was `readJSON(ppath, null).catch(() => null)` — a transient Blob failure folded
// into "absent" and let a later call silently overwrite the record with a different
// boardHash. Unverifiable existence now SKIPS the write and says so; the content read on
// the already-recorded path is report-only and never gates anything.
async function recordParityOnce(date, servedHash, doc) {
  const ppath = `${PARITY_PREFIX}${date}.json`;
  let exists;
  try { exists = await STORE.blobExists(ppath); }
  catch { return { recorded: false, reason: 'skipped-existence-unverifiable', date, hash: null, matchesServed: null }; }
  if (exists) {
    const existing = await STORE.readJSON(ppath, null).catch(() => null);
    return {
      recorded: false, reason: 'already-recorded (write-once)', date,
      hash: existing ? existing.boardHash : null,
      matchesServed: existing ? existing.boardHash === servedHash : null,
    };
  }
  await STORE.writeJSON(ppath, doc, 0);
  return { recorded: true, date, hash: servedHash, matchesServed: true };
}
...
async function runToday(req, res) {
  ...
  const results = await Promise.all(keys.map(k => (k === 'scoreboard' ? pullScoreboard() : pull(paths[k]))));
  ...
  // Started here (not awaited until after the board hash) so the display-only side-map
  // adds zero wall time to the hot path.
  const BCR = require('./bearcase-routes');
  const bearcasePromise = BCR.servingDate()
    ? STORE.readJSON(BCR.dayKey(BCR.servingDate()), null).catch(() => null)
    : Promise.resolve(null);
  const prev = await STORE.readJSON(SNAP_PATH, null).catch(() => null);
  ...
  const originsDoc = await STORE.readJSON(ORIGINS_PATH, null).catch(() => null);
  ...
  const governance = await STORE.readJSON('governance/latest.json', null).catch(() => null);
  ...
  payload.configured = STORE.hasStore();
  ...
  // Structured bear cases (weight 0, display-only): attached AFTER the board hash is
  // computed, as a SIDE-MAP — no row is mutated and the hash provably ignores it.
  // The read was STARTED alongside the other pre-reads (zero added wall time) and is
  // keyed to the last COMPLETED session, so the cases argued against yesterday's close
  // stay visible through today's premarket/RTH. Absent → null, never blocks.
  try {
    const bc = await bearcasePromise;
    payload.bearCases = bc && bc.cases && Object.keys(bc.cases).length
      ? { arguedAgainst: bc.date, cases: bc.cases, generatedAt: bc.generatedAt, model: bc.model, disclosure: bc.disclosure }
      : null;
  } catch { payload.bearCases = null; }

  // Persist snapshot for tomorrow's lanes (cron-only, gated by ?log=1 + store).
  if (req.query.log === '1' && STORE.hasStore()) {
    const { date } = nowET();
    const active = activeRows;
    const ids = activeIds;
    try { await STORE.writeJSON(SNAP_PATH, { date, generatedAt: payload.generatedAt, ids, boardHash: payload.boardHash.hash }, 0); payload.logged = ids.length; }
    catch (e) { payload.logError = String((e && e.message) || e); }
    // WRITE-ONCE daily parity record (fail-closed — see recordParityOnce): the first
    // logged board of the day is THE recommendation of record — later calls must not
    // rewrite it. Graders and audits can re-derive the hash from any stored snapshot
    // and prove the row set they are grading is the one that was served.
    try {
      payload.parity = await recordParityOnce(date, payload.boardHash.hash,
        { version: SCHEMA_VERSION, date, generatedAt: payload.generatedAt, boardHash: payload.boardHash.hash, rows: ids.length, ids });
    ...
    try {
      ...
      await STORE.writeJSON(ORIGINS_PATH, { version: RO.ORIGINS_VERSION, date, origins: nextOrigins }, 0);
    ...
    try {
      const doc = (await STORE.readJSON(OPP_LOG_PATH, null).catch(() => null)) || { days: [] };
      ...
      const row = { date, generatedAt: payload.generatedAt, decision: o.decision, score: o.score,
        ...
        // Abstention series (2026-08-11): the daily row records whether the governed
        // board abstained and how many rows cleared, so the abstention RATE is a
        // measurable time series instead of something inferred from empty lanes.
        abstained: payload.governanceGate ? payload.governanceGate.abstained === true : null,
        actionableCount: payload.governanceGate ? (payload.governanceGate.actionableCount ?? null) : null,
        // Optimizer SHADOW book (optimizer-v1), compact: the daily record the future
        // sized-vs-selection evaluation needs. Additive field on the SAME rolling doc —
        // deliberately no new Blob write and no new op.
        portfolioShadow: (() => {
          const sh = payload.portfolioShadow;
          if (!sh) return null;
          if (sh.error) return { method: sh.method, shadow: true, error: sh.error };
          return { method: sh.method, shadow: true, weights: sh.weights, cash: sh.cash,
            value: sh.book ? sh.book.value : null,
            agreement: sh.agreement
              ? { overlapCount: sh.agreement.overlapCount, jaccardVsLive: sh.agreement.jaccardVsLive }
              : null };
        })() };
      const days = [...(doc.days || []).filter(d => d.date !== date), row].slice(-OPP_LOG_MAX);
      await STORE.writeJSON(OPP_LOG_PATH, { version: OD.OPPORTUNITY_VERSION, days }, 0);
...
module.exports = { runToday, buildToday, snapRow, boardHashOf, recordParityOnce, PARITY_PREFIX, pull, pullScoreboard, PULL_TIMEOUT_MS, SCOREBOARD_SUMMARY_PATH };
