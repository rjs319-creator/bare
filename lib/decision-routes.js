...
function buildToday(sources = {}, prev = null, redundancy = null, origins = null, opts = {}) {
  ...
  const active = rankedActive.map(sig => ({
    ...
    ...(gate ? { eligibility: EL.assessSignal(sig, gate.perSource[sig.source] || EL.assessSource(sig.source, {}), { borrowFeed: opts.borrowFeed || null }) } : {}),
  }));
...
async function runToday(req, res) {
  ...
  const activeRows = [...payload.top, ...Object.values(payload.horizons).flat()]
