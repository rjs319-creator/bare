...
function buildToday(sources = {}, prev = null, redundancy = null, origins = null) {
  ...
  const active = rankedActive.map(sig => ({ ...sig, failure: FM.assessSignal(sig, { regime }) }));
  ...
  // all? Built from the active board's own quality — regime is only a penalty, never a booster.
  const opportunity = OD.computeOpportunityDensity(active, { regime });
  return {
  ...
  if (req.query.log === '1' && hasStore()) {
    ...
    try {
      ...
      const row = { date, generatedAt: payload.generatedAt, decision: o.decision, score: o.score,
        qualifyingCount: o.qualifyingCount, expectedBestEdgePct: o.expectedBestEdgeAfterCostsPct,
