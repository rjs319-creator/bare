...
function buildToday(sources = {}, prev = null, redundancy = null, origins = null, opts = {}) {
  ...
  const mergedAll = D.mergeSignals(raw).map(m => D.makeSignal(m).signal);
  const merged = boardRaw === raw ? mergedAll : D.mergeSignals(boardRaw).map(m => D.makeSignal(m).signal);
  ...
  const active = rankedActive.map(sig => {
    const eligibility = gate
      ? EL.assessSignal(sig, gate.perSource[sig.source] || EL.assessSource(sig.source, {}), { borrowFeed: opts.borrowFeed || null })
      : null;
    return {
      ...sig,
      failure: FM.assessSignal(sig, { regime }),
      // Per-signal eligibility verdict (display ≠ trade ≠ size). Annotation only — in
      // 'annotate' mode it never moves a rank; in 'enforce' the exclusion already happened.
      ...(eligibility ? { eligibility } : {}),
      // UNMISTAKABLE class separation: a card either carries earned, current clearance
      // (ACTIONABLE) or it is a research/shadow lead — never an implied recommendation.
      evidenceClass: eligibility ? (eligibility.tradeEligible ? 'ACTIONABLE' : 'RESEARCH') : null,
    };
  });
  ...
  const opportunity = OD.computeOpportunityDensity(active, { regime, tape: s.tape || null });
  ...
  try {
    ...
    research = LB.buildDecisionSnapshot(all, {
      decisionTs,
  ...
  if (gate) {
    ...
    const gatedRanked = mode === 'enforce' ? active : (() => {
      const gatedMerged = D.mergeSignals(gate.tradeable).map(m => D.makeSignal(m).signal);
      return D.rankSignals(gatedMerged, { regime, scoreboard, redundancy, origins });
    })();
    // ACTIONABLE LANE (Phase-2 item 8): per-horizon shortlists containing ONLY cleared
    // strategies, from the boost-free merge. When nothing is cleared this is honestly
    // EMPTY — it is never backfilled with research signals. Research remains fully
    // visible in horizons/topByHorizon (annotate mode) with RESEARCH class labels.
    actionableByHorizon = Object.fromEntries(D.HORIZONS.map(h => [
      h,
      gatedRanked.filter(x => x.horizon === h).slice(0, 5)
        .map(x => (x.evidenceClass ? x : { ...x, evidenceClass: 'ACTIONABLE' })),
    ]));
    let shadowComparison = null;
    if (mode === 'annotate') {
      const currentTop = active.slice(0, 10).map(x => x.id);
      const proposedTop = gatedRanked.slice(0, 10).map(x => x.id);
      shadowComparison = {
        currentTop, proposedTop,
        wouldRemoveFromTop: currentTop.filter(id => !proposedTop.includes(id)),
        currentSignals: active.length, proposedSignals: gatedRanked.length,
      };
    }
    // RESEARCH VISIBILITY UNDER ENFORCEMENT: in 'enforce' mode the board itself is
    // cleared-only, so the payload must still carry the full ungated ranked
    // cross-section for display — hiding research from view is not the control that
    // matters, and an operator flipping enforcement must not lose observability.
    // Each row is explicitly classed so no research card can pass as a recommendation.
    if (mode === 'enforce') {
      const researchRanked = D.rankSignals(mergedAll, { regime, scoreboard, redundancy, origins });
      researchByHorizon = Object.fromEntries(D.HORIZONS.map(h => [
        h,
        researchRanked.filter(x => x.horizon === h).slice(0, 5).map(x => {
          const el = EL.assessSignal(x, gate.perSource[x.source] || EL.assessSource(x.source, {}), { borrowFeed: opts.borrowFeed || null });
          return { ...x, eligibility: el, evidenceClass: el.tradeEligible ? 'ACTIONABLE' : 'RESEARCH' };
        }),
      ]));
    }
    governanceGate = {
      ...
      shadowComparison,
      note: mode === 'enforce'
        ? 'Fail-closed eligibility ENFORCED: only explicitly-production strategies with earned, fresh governance clearance can originate or boost a live pick; shorts require observed borrow. Excluded names remain on their own tabs as research/watch.'
        : 'Annotate mode: the board is unchanged; this block reports what fail-closed enforcement (DECISION_ELIGIBILITY_MODE=enforce) would do. Review shadowComparison before enabling.',
    };
  }
  return {
    ...
    actionableByHorizon,
    // Enforce mode only: the full ungated ranked cross-section per horizon (RESEARCH-
    // classed) so enforcement never hides research from view. Null in annotate mode —
    // there topByHorizon already IS the full cross-section.
    researchByHorizon,
    ...
    failureModel: {
      ...
      note: 'Shadow: each name carries a failure probability + expected mode, but it does NOT change the ranking. Run op=failuremodel to test whether the rejected names actually underperform before it could ever bind.',
    },
  };
}
