...
async function runToday(req, res) {
  ...
  // computed, as a SIDE-MAP — no row is mutated and the hash provably ignores it. The
  // doc is generated once daily by op=bearcasetick; absent → null, never blocks.
  try {
    const bc = await STORE.readJSON(require('./bearcase-routes').dayKey(nowET().date), null);
    payload.bearCases = bc && bc.cases && Object.keys(bc.cases).length
      ? { cases: bc.cases, generatedAt: bc.generatedAt, model: bc.model, disclosure: bc.disclosure }
      : null;
