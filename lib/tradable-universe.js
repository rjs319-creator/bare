...
//
// THE TRUNCATION DEFECT THIS CLOSES. The screener hard-caps its response at
// SCREENER_PAGE_CAP rows and IGNORES a higher `limit` — a single `country=US` call
// returned exactly 10,000 rows and silently dropped the rest, leaving ~969 of ~4,711
// eligible securities (20%) with no sector, industry or market cap at all. Nothing
// surfaced that: the unmatched names simply kept null metadata.
//
// The fix needs no new provider and no new entitlement: query PER EXCHANGE over the
// exchanges this universe actually supports. Each response then sits well under the
// cap (NASDAQ ~6.1k, NYSE ~3.0k, AMEX ~0.4k) and coverage goes 79% → 99%. A page that
// still comes back at the cap is reported as `truncated` rather than trusted.
//
// PARTIAL COVERAGE IS KEPT. One exchange failing degrades detail for that exchange
// only — it never discards the exchanges that answered (same contract as the bulk-quote
// path: partial coverage beats collapsing to nothing).
const SCREENER_PAGE_CAP = 10000;

async function fetchScreenerMetadata({ timeoutMs = 12000, exchanges = null } = {}) {
  const key = process.env.FMP_API_KEY;
  if (!key) return { rows: [], available: false, error: 'FMP_API_KEY not configured', perExchange: [] };

  const wanted = exchanges || [...SUPPORTED_EXCHANGES];
  const perExchange = [];
  const bySymbol = new Map();

  await Promise.all(wanted.map(async (exchange) => {
    const url = 'https://financialmodelingprep.com/stable/company-screener'
      + `?exchange=${encodeURIComponent(exchange)}&isEtf=false&isFund=false&limit=${SCREENER_PAGE_CAP}&apikey=${key}`;
    try {
      const r = await fetchWithTimeout(url, { timeoutMs });
      if (!r.ok) { perExchange.push({ exchange, ok: false, rows: 0, error: `FMP ${r.status}` }); return; }
      const j = await r.json();
      if (!Array.isArray(j)) { perExchange.push({ exchange, ok: false, rows: 0, error: 'unexpected shape' }); return; }
      for (const row of j) {
        const sym = String((row && row.symbol) || '').trim().toUpperCase();
        if (sym && !bySymbol.has(sym)) bySymbol.set(sym, row);
      }
      // At the cap the provider is withholding rows — say so instead of trusting it.
      perExchange.push({ exchange, ok: true, rows: j.length, truncated: j.length >= SCREENER_PAGE_CAP, error: null });
    } catch (e) {
      perExchange.push({ exchange, ok: false, rows: 0, error: redactedMessage(e) });
    }
  }));

  const answered = perExchange.filter(p => p.ok);
  const failed = perExchange.filter(p => !p.ok);
  if (!answered.length) {
    return { rows: [], available: false, error: failed.map(f => `${f.exchange}:${f.error}`).join('; ') || 'no exchange answered', perExchange };
  }
  return {
    rows: [...bySymbol.values()],
    available: true,
    // Available with a named partial failure — detail is thinner for that exchange,
    // and the caller can see exactly which one.
    error: failed.length ? `partial: ${failed.map(f => `${f.exchange}:${f.error}`).join('; ')}` : null,
    perExchange,
    truncated: perExchange.some(p => p.truncated === true),
  };
}
...
async function buildTradableUniverse({ now = new Date(), enrich = true } = {}) {
  ...
  try {
    ...
    providerFailures.push({ provider: 'nasdaq-trader', error: redactedMessage(e) });
  }
  ...
  if (enrich && securities.length) {
    ...
    if (!meta.available) {
      ...
      metadata = {
        available: true, enriched: applied.enriched, delisted: applied.delisted,
        error: meta.error, perExchange: meta.perExchange || null, truncated: meta.truncated === true,
      };
      // A partial answer is still a provider failure worth recording — coverage for
      // that exchange is thinner and the report must not read as complete.
      if (meta.error) providerFailures.push({ provider: 'fmp-company-screener', error: meta.error });
    }
  }
...
function buildCoverageReport({ totalReceived, eligible, excludedByReason, typeCounts, metadata, providerFailures }) {
  ...
  return {
    ...
    metadataUnmatched: Math.max(0, eligible.length - (metadata.enriched || 0)),
    metadataPerExchange: metadata.perExchange || null,
    metadataTruncated: metadata.truncated === true,
...
module.exports = {
  SUPPORTED_EXCHANGES, EXCHANGE_CODE_MAP, BAD_FIN_STATUS, QUOTABLE_SYMBOL, TYPE_PATTERNS, SCREENER_PAGE_CAP,
