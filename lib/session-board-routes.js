...
// LITERAL requires only. Vercel's function bundler traces `require('<literal>')` statically;
// a `require(variable)` is invisible to it, so the file never ships and prod reported
// `module-missing` for modules that were on main (2026-09-19). Both modules are real
// dependencies now; the try/catch is only so a load-time throw is reported by message
// instead of taking the whole board down.
function loadPremarket() {
  try { return { mod: require('./premarket-snapshot'), error: null }; }
  catch (e) { return { mod: null, error: String((e && e.message) || e).slice(0, 200) }; }
}
function loadLiveState() {
  try { return { mod: require('./session-live-state'), error: null }; }
  catch (e) { return { mod: null, error: String((e && e.message) || e).slice(0, 200) }; }
}
...
function defaultDeps() {
  ...
  const preLoad = loadPremarket();
  const liveLoad = loadLiveState();
  const pre = preLoad.mod;
  const live = liveLoad.mod;
  return {
    now: () => new Date(),
    ...
    moduleErrors: { premarket: preLoad.error, live: liveLoad.error },
...
async function buildBoard(deps) {
  ...
  if (deps.fetchPremarketSnapshot) {
    ...
    sources.push({ source: 'premarket', ok: false, ms: 0, reason: (deps.moduleErrors && deps.moduleErrors.premarket) || 'module-missing' });
  }
  ...
  if (deps.liveStatus) {
    ...
    sources.push({ source: 'live', ok: false, ms: 0, reason: (deps.moduleErrors && deps.moduleErrors.live) || 'module-missing' });
  }
