...
//
// Default when a registry entry omits `maturity`: 'shadow' — FAIL CLOSED
// (quant-redesign-3). The historic default was 'production', which meant a missing
// field silently granted live-trade eligibility while a typo'd value and an
// unregistered id both failed closed — the one gap an author could fall into by
// doing nothing. Every registry entry now carries an EXPLICIT maturity, so this
// default only ever fires for a future entry someone forgets to mark — and forgetting
// must never grant live influence.

const { STRATEGY_REGISTRY } = require('./strategy-registry');

const STATUSES = ['production', 'shadow', 'experimental', 'rejected'];
const DEFAULT_STATUS = 'shadow';
