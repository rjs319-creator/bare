...
// States that may NEVER carry a buy-like entry grade — the timing light must answer "is this a
// good moment to enter a currently-VALID setup?", and it must refuse to answer before the setup
// is confirmed valid. These return "not eligible" outright.
const TIMING_INELIGIBLE_STATES = new Set(['STALLING', 'FAILED', 'EXPIRED', 'PRIOR_SESSION_WATCH', 'TOO_EXTENDED', 'MANAGING', 'CLOSED']);
// Only these may receive a green light; everything else is capped below green.
const TIMING_GREEN_STATES = new Set(['ACTIONABLE_NOW', 'REVERSAL_RECLAIM']);
const NOT_ELIGIBLE = reason => ({ score: null, light: 'grey', emoji: '⛔', label: 'Not eligible', notEligible: true, reasons: [reason] });

// Binding pre-checks from the lifecycle/thesis context. Returns a not-eligible object when the
// setup is stale, invalidated, failed, or stop-breached — the timing light must not grade a
// broken thesis just because price is momentarily above VWAP or near an OLD stop. Null = proceed.
function timingGateFail(ctx) {
  if (!ctx) return null;
  if (ctx.currentSessionFresh === false) return NOT_ELIGIBLE('Stale — no current-session evidence; not gradeable right now');
  if (ctx.thesisValid === false) return NOT_ELIGIBLE('Thesis invalidated — the original setup has failed');
  if (ctx.planValid === false) return NOT_ELIGIBLE('Plan invalid — recompute levels before timing an entry');
  if (ctx.breakoutFailed === true) return NOT_ELIGIBLE('Breakout failed — no valid entry');
  if (ctx.stopBreached === true) return NOT_ELIGIBLE('Stop breached — invalidated');
  if (ctx.lifecycleState && TIMING_INELIGIBLE_STATES.has(ctx.lifecycleState)) return NOT_ELIGIBLE(`${ctx.lifecycleState} — not an actionable entry`);
  return null;
}
...
// `context` (optional) binds the lifecycle/thesis/freshness state onto the timing grade so it
// can never answer "good moment to enter" for a setup that is stale, failed, or not yet valid.
// When omitted, behavior is exactly the mechanical grade (backward compatible). Fields honored:
// { currentSessionFresh, thesisValid, planValid, lifecycleState, breakoutFailed, stopBreached,
//   lossFromDetectionAtr, dayChangePct, residualMomentum }.
function scoreTiming(snapshot, levels = {}, weights = DEFAULT_WEIGHTS, context = null) {
  // Lifecycle/thesis gate FIRST — a broken or stale setup is "not eligible" regardless of the
  // instantaneous price snapshot. This is what stops a name down 10% (thesis failed) from
  // scoring green merely because it sits just above VWAP or has drifted toward an old stop.
  const gateFail = timingGateFail(context);
  if (gateFail) return { ...gateFail, lifecycleState: context ? context.lifecycleState || null : null };
  ...
  // LIFECYCLE CAP — only a confirmed ACTIONABLE_NOW / REVERSAL_RECLAIM setup may be green.
  // ARMED shows "waiting for the trigger" (a real setup, not a buy-now); everything else that
  // reached here (WATCHING/BUILDING/OR forming) is neutral setup progress, capped below green.
  let stateLabelOverride = null;
  if (context && context.lifecycleState) {
    const st = context.lifecycleState;
    if (!TIMING_GREEN_STATES.has(st)) {
      score = Math.min(score, 6);   // no green light without a confirmed actionable setup
      if (st === 'ARMED') stateLabelOverride = 'Waiting for trigger';
    }
  }
  ...
  if (stateLabelOverride) reasons.unshift(stateLabelOverride);
  return { score, ...LIGHT(score), label: stateLabelOverride || LABEL(score), factors, reasons: reasons.slice(0, 4),
    lifecycleState: context ? context.lifecycleState || null : null,
