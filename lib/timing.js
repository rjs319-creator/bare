...
function scoreTiming(snapshot, levels = {}) {
  ...
  // Weight only the factors we have; renormalize.
  const W = { rr: 0.32, extension: 0.24, trend: 0.16, rvol: 0.16, trigger: 0.12 };
  let num = 0, den = 0;
  for (const k of Object.keys(W)) if (factors[k] != null) { num += W[k] * factors[k]; den += W[k]; }
...
module.exports = { scoreTiming, fetchTimingSnapshot, rrScore, extensionScore, trendScore, rvolScore, triggerScore };
