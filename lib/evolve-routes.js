...
// op=today (the decision engine EVOLVE composes over) does a full multi-screener merge and
// takes ~12s COLD — right at the old 12s pull ceiling, so a cache-cold op=today (e.g. right
// after a deploy flushes the CDN, before the daily warm cron re-primes it) timed out and
// EVOLVE degraded to an empty "decision engine unavailable" payload. The tracker function has
// a 60s budget, so give the op=today self-fetch real headroom. Other pulls keep the short one.
...
function buildEvolvePayload(signals, { regimeVector = null, regime = {}, perf = null, model = null, strengthTiltAllowed = true } = {}) {
  ...
  const ctx = {
    ...
    // #9: promote the tilt only if OOS-earned AND the central adaptive policy
    // allows it (strengthTiltAllowed=false ⇒ identity behavior).
    strengthPromoted: !!(perf && perf.strengthOOS && perf.strengthOOS.promote) && strengthTiltAllowed,
    priorP: 0.4,
  };
  ...
  return {
    ...
    // `calibrated` now means the calibrator BEAT raw P out-of-fold, not merely that one
    // exists — an in-sample map that fails its own OOF check is not calibration.
    modelHealth: { calibrated: E.calibratorUsable(ctx.calibrator), calibratorPresent: !!ctx.calibrator,
      calibrationHelpsOOS: ctx.calibrator ? (ctx.calibrator.calibrationHelpsOOS ?? null) : null,
      calibrationError: ctx.calibrator ? ctx.calibrator.error : null,
...
async function runEvolve(req, res) {
  const [today, sectors] = await Promise.all([pull('/api/tracker?op=today', TODAY_TIMEOUT), pull('/api/sectors')]);
  ...
  // Derived-flag layer: the tilt promotion is re-derived from its OOS report every
  // run, so applying it IS an adoption. Only an explicit 'allow' applies it —
  // 'freeze' (the fail-closed default) and 'disable' both pin identity behavior.
  const strengthTiltAllowed = (await require('./adaptive-layers').layerPolicy('evolve-strength-tilt')) === 'allow';
  const payload = buildEvolvePayload(signals, { regimeVector, regime: today.data.regime || {}, perf, model, strengthTiltAllowed });
...
function recomputePerf(resolved, { weighted = true } = {}) {
  const rows = Object.values(resolved).sort((a, b) => (a.predDate < b.predDate ? -1 : 1));
  // Gap B (live): overlapping labels contribute a fraction of a sample, so the effective
  // counts pooledRate/metaWeights consume are de-duplicated for temporal autocorrelation.
  const wmap = weighted ? U.uniquenessWeights(rows) : null;
  const bySpecialist = {};
  ...
  for (const r of rows) {
    ...
    for (const cb of (r.contribs || (r.specialists || []).map(s => ({ specialist: s, p: r.probability })))) {
      ...
      sp._recent.push({ p: cb.p, ret: r.terminalReturn, won });   // drift uses raw recency (unweighted)
    }
  }
  // Gap C (live): the deflated-Sharpe / multiple-testing gate. Cells whose SPY-relative
  // Sharpe survives the deflation for the number of cells tried are the only ones a live
  // candidate may TRADE on; everything else is capped at WATCH by decideState.
  const grid = DSR.gridDeflatedSharpe(rows, { weights: wmap });
...
async function runEvolveHealth(req, res) {
  ...
  // Gap C: deflated-Sharpe / multiple-testing gate over the specialist×regime×horizon grid,
  // with Gap-B uniqueness weights so overlapping labels don't inflate significance. A cell is
...
async function runEvolveOmegaWalkforward(req, res) {
  ...
  // Overlap weighting is ON by default (alpha-research pass 3). It used to be opt-IN,
  // so the walk-forward that VALIDATES the ensemble ran unweighted while the live perf
  // recompute (recomputePerf, this file) already defaulted weighted:true — the research
  // gate was more permissive than the fit it was validating. `?uniqueness=0` still
  // disables it for a deliberate unweighted comparison.
  const weighted = !(req.query.uniqueness === '0' || req.query.uniqueness === 'false');
  ...
  const { buildEvidenceView } = require('./evolve-evidence-view');
  const out = await WF.runEvolveOmegaWalkForward({ scope, limit, months, step, folds, embargo, volAdjust, regimeAllow, range, weighted, now: date, deadlineMs: 48000 });
  const payload = { ok: true, ...out };
  // Render-ready §20 evidence summary (champion/challenger, DSR grid, promotion decision).
  // Built server-side from ONE tested view-model so the frontend holds no promotion logic.
  payload.evidenceView = buildEvidenceView(payload);
  res.setHeader('Cache-Control', 's-maxage=1800, stale-while-revalidate=86400');
