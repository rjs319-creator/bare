...
async function buildRegimeContext(sectorsData) {
  ...
  // Sector daily EXCESS-vs-SPY returns from /api/sectors (breadth here = "beating SPY").
  const rot = (sectorsData && sectorsData.rotation) || [];
  const sectors = rot.map(r => ({ name: r.name, changePct: r.daily1d }));
  const { date } = nowET();
...
module.exports = {
  runEvolve, runEvolveScore, runEvolveResolve, runEvolveHealth, runEvolveWalkforward, runEvolveBackfillOp,
