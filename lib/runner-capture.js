...
function buildCaptureReport({ date, dailyByTicker = {}, discoveryLog = null, records = {}, baselineTickers = null } = {}) {
  ...
  // "Sparse" is judged against the SAME configured cadence the scan runner health uses
  // (2× the target interval = one fully missed tick), so the two health surfaces agree.
  const sparseGapMin = (2 * require('./daytrade-scan-runner').targetIntervalMs()) / 60000;
  const scannerSparse = maxGapMin != null ? maxGapMin > sparseGapMin : scans.length < 5;
