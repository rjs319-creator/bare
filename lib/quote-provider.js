...
async function mapChunks(chunks, fn, concurrency = CHUNK_CONCURRENCY) {
  ...
  const worker = async () => {
    while (i < chunks.length) {
      ...
      catch (e) { out[idx] = { rows: [], status: null, error: String((e && e.message) || e) }; }
    }
  };
...
async function fetchBulkQuotes(tickers, deps = {}) {
  ...
  if (fmpKey) {
    ...
    } catch (e) { diagnostics.fmp = { error: String((e && e.message) || e) }; }
  }
  ...
  } catch (e) { diagnostics.yahooQuote = { error: String((e && e.message) || e) }; }
  ...
  } catch (e) { diagnostics.spark = { error: String((e && e.message) || e) }; }
