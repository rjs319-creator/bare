...
// ── Extended-hours fields (Yahoo v7 naming) ────────────────────────────────────────────────
const EXTENDED_HOURS_NULL = Object.freeze({
  preMarketPrice: null, preMarketChangePct: null, preMarketVolume: null, preMarketTime: null,
  postMarketPrice: null, postMarketChangePct: null, postMarketTime: null, marketState: null,
});
const numOrNull = v => (v != null && Number.isFinite(+v) ? +v : null);
const isoOrNull = sec => (sec ? new Date(sec * 1000).toISOString() : null);
function extendedHoursOf(q) {
  return {
    preMarketPrice: numOrNull(q.preMarketPrice),
    preMarketChangePct: numOrNull(q.preMarketChangePercent),
    preMarketVolume: numOrNull(q.preMarketVolume),
    preMarketTime: isoOrNull(q.preMarketTime),
    postMarketPrice: numOrNull(q.postMarketPrice),
    postMarketChangePct: numOrNull(q.postMarketChangePercent),
    postMarketTime: isoOrNull(q.postMarketTime),
    marketState: q.marketState != null ? String(q.marketState) : null,
  };
}

// The price/timestamp/volume that describe "right now" for a given exchange session.
// Premarket → the premarket print when the venue has one (basis 'premarket'); after-hours →
// the post-market print; otherwise the regular-session fields. When the session asks for an
// extended-hours print and the row has none, the REGULAR fields are returned with basis
// 'regular-stale' — the caller decides whether yesterday's close is acceptable, and the
// freshness gates downstream (session-date match, age) will reject it as they should.
// Premarket volume is reported as-is: it must NEVER be paced against the regular-hours
// volume curve (thin tape, different participants).
function sessionQuote(row, session) {
  if (!row) return null;
  const regular = { price: row.price ?? null, asOf: row.asOf || null, volume: row.dayVolume ?? null, basis: 'regular' };
  if (session === 'premarket') {
    if (Number.isFinite(row.preMarketPrice) && row.preMarketPrice > 0) {
      return { price: row.preMarketPrice, asOf: row.preMarketTime || null, volume: row.preMarketVolume ?? null, basis: 'premarket' };
    }
    return { ...regular, basis: 'regular-stale' };
  }
  if (session === 'afterhours' || session === 'postmarket') {
    if (Number.isFinite(row.postMarketPrice) && row.postMarketPrice > 0) {
      return { price: row.postMarketPrice, asOf: row.postMarketTime || null, volume: null, basis: 'postmarket' };
    }
    return { ...regular, basis: 'regular-stale' };
  }
  return regular;
  ...
  const outcomes = await mapChunks(chunk(tickers, FMP_CHUNK), async syms => {
    ...
    return {
      ...
      rows: j.map(q => (q && q.symbol && q.price != null) ? {
        ...
        ...EXTENDED_HOURS_NULL,   // batch-quote carries no extended-hours print
      } : null).filter(Boolean),
    };
  });
  ...
  const fetchChunk = async (syms, allowCrumbRefresh = true) => {
    ...
    return {
      ...
      rows: result.map(q => (q && q.symbol && q.regularMarketPrice != null) ? {
        ...
        // ADDITIVE extended-hours fields (2026-09-19). The same v7 response has always carried
        // them; until now the mapper dropped them, so between 04:00 and 09:30 ET every
        // "current" price was yesterday's close and every premarket scan measured nothing new.
        // Null when the venue reports none — a consumer can always tell "no premarket print"
        // from "unchanged". lib/premarket-snapshot.js and the premarket gap lane read these.
        ...extendedHoursOf(q),
      } : null).filter(Boolean),
    };
  };
    ...
    return {
      ...
      rows: results.map(entry => {
        ...
        return {
          ...
          ...EXTENDED_HOURS_NULL,   // spark meta carries no extended-hours print either
        };
      }).filter(Boolean),
    };
  }, SPARK_CONCURRENCY);
...
module.exports = {
  ...
  missingFrom, mergeRows, volumeStats, coverageOf, extendedHoursOf, sessionQuote, EXTENDED_HOURS_NULL,
