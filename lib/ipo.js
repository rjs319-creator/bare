...
async function fmpIpos(from, to) {
  ...
  try {
    const r = await fetch(`https://financialmodelingprep.com/stable/ipos-calendar?from=${from}&to=${to}&apikey=${FMP_KEY}`);
    if (!r.ok) return null;
...
// A ticker is eligible as an insider-lockup candidate only if it is a plain
// common-stock symbol. We exclude SPAC units/warrants/rights — 5+ char tickers
// ending in U/W/R (e.g. WSTNU, BPACU) — which trade near $10 NAV with
// redemption-arbitrage dynamics, NOT the insider lockup dynamics this feed
// models. They were polluting the ledger (and even drew a real TRADE on CCXIU).
function isLockupEligibleTicker(ticker) {
  if (!/^[A-Z][A-Z\-]{0,5}$/.test(ticker)) return false;
  if (ticker.length >= 5 && /[UWR]$/.test(ticker)) return false;
  return true;
}
...
async function fetchLockupExpiries({ nowMs = Date.now(), lockupDays = 180, lookbackDays = 70, aheadDays = 10 } = {}) {
  ...
  for (const x of rows) {
    ...
    if (!isLockupEligibleTicker(ticker)) continue;
...
module.exports = { fetchLockupExpiries, isLockupEligibleTicker };
