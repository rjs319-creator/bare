...
function buildBook(features, held = new Set()) {
  ...
  // strength percentile = where a name's score sits within the whole eligible pool (0-100)
  const sortedPool = [...scores].sort((a, b) => a - b);
  const pctOf = s => { let lo = 0; for (const v of sortedPool) { if (v <= s) lo++; else break; } return sortedPool.length ? lo / sortedPool.length : 0; };
  // status: new entry vs held-healthy vs held-but-deteriorating (only surviving via the buffer)
  const statusOf = f => !held.has(f.symbol) ? 'new' : (f.score >= enterCut ? 'held' : 'watch');

  const book = selected.map((f, i) => ({
    rank: i + 1, ticker: f.symbol, company: f.company || f.symbol, sector: f.sector,
    score: +f.score.toFixed(4), strength: Math.round(pctOf(f.score) * 100), status: statusOf(f),
    mom12_1: +(f.m121 * 100).toFixed(1), vol: +(f.vol63 * 100).toFixed(1),
...
// opts (graduation-league EA-1/EA-2, additive — gross fields unchanged):
//   costPct — round-trip cost in % subtracted once per resolved trade for the net lane
//   marks   — Map/object "ticker|date" → latest close, used to MARK open positions at a
//             common evaluation time instead of leaving them out of the record entirely
function aggregatePerformance(signals, resolved = {}, bench = null, bars = MAX_HOLD, opts = {}) {
  const costPct = Number.isFinite(opts.costPct) ? opts.costPct : null;
  ...
  // MARK-TO-MARKET lane (EA-1): resolved-only totals silently exclude exactly the
  // positions still at risk. Open signals with a supplied mark contribute
  // (mark/entry − 1); open signals with NO mark (no fetch, delisted-pending) are
  // counted, never dropped. Additive: the resolved-only fields above are untouched.
  const openSigs = signals.filter(s => { const o = resolved[`${s.ticker}|${s.date}`]; return !o || !o.outcome || o.outcome === 'OPEN'; });
  const openMarked = [];
  if (marks) {
    for (const s of openSigs) {
      const mk = marks instanceof Map ? marks.get(`${s.ticker}|${s.date}`) : marks[`${s.ticker}|${s.date}`];
      if (Number.isFinite(mk) && Number.isFinite(s.entry) && s.entry > 0) openMarked.push(mk / s.entry - 1);
    }
  }
  const openAvg = openMarked.length ? openMarked.reduce((a, b) => a + b, 0) / openMarked.length : null;
  // NET lane (EA-2): one round-trip haircut per trade — open marks are charged the
  // same round trip (the exit leg is a certainty, not an option).
  const c = costPct != null ? costPct / 100 : null;
  const totMeanNet = (c != null && totMean != null) ? totMean - c : null;
  const openAvgNet = (c != null && openAvg != null) ? openAvg - c : null;
  const allMtm = (openMarked.length || allClosed.length)
    ? [...allClosed.map(o => o.r || 0), ...openMarked]
    : [];
  const combinedAvg = allMtm.length ? allMtm.reduce((a, b) => a + b, 0) / allMtm.length : null;
  return {
    quarters,
    ...
    totals: { signals: signals.length, resolved: allClosed.length, open: signals.length - allClosed.length, winRate: allClosed.length ? totWins / allClosed.length : null, meanReturn: totMean, meanReturnNet: totMeanNet, costPct: costPct ?? null },
    mtm: {
      openN: openSigs.length, openMarked: openMarked.length, openUnmarked: openSigs.length - openMarked.length,
      openAvg, openAvgNet, combinedAvg,
      combinedAvgNet: (c != null && combinedAvg != null) ? combinedAvg - c : null,
      basis: 'open positions marked at the supplied latest close (same evaluation time), one round-trip cost per trade; unmarked opens are counted, not dropped',
    },
  };
}
