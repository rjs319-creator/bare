...
function orbitLabels(candles, signalDate, opts = {}) {
  ...
  // Benchmarks for market/sector-relative & residual forward returns. The factor
  // model residualises FEATURES against four factors (market=SPY, sector=SPDR ETF,
  // size=IWM−SPY, vol=^VIX); the forward residual LABEL must strip the same four so
  // target and features share one basis. `smallCandles` (IWM) and `volCandles`
  // (^VIX) let us add the size and vol legs; when they are absent we degrade
  // honestly to the legs we can compute and record which in `residualFactors`.
  const mMap = closeMap(opts.marketCandles), sMap = closeMap(opts.sectorCandles);
  const iMap = closeMap(opts.smallCandles), vMap = closeMap(opts.volCandles);
  ...
  const iDates = opts.smallCandles ? opts.smallCandles.map(c => c.date) : [];
  const vDates = opts.volCandles ? opts.volCandles.map(c => c.date) : [];
  ...
  for (const [key, hDays] of Object.entries(HORIZONS)) {
    ...
    // Factor-residual forward return = stock fwd − Σ β_k · factor_k fwd across the
    // four factors the features are neutralised on. size = IWM−SPY forward,
    // vol = ^VIX forward. Each leg is only subtracted when BOTH its beta and its
    // forward return are available; `residualFactors` records which legs were
    // stripped so a partial residual is never mistaken for a full 4-factor one.
    // Honest null when the market leg (the minimum) is unavailable.
    let residual = null, residualFactors = null;
    if (exp && mktFwd != null) {
      const legs = ['market'];
      let pred = (exp.market || 0) * mktFwd;
      if (secFwd != null && exp.sector != null) { pred += exp.sector * secFwd; legs.push('sector'); }
      const iwmFwd = benchReturn(iMap, iDates, fillDate, exitDate);
      const sizeFwd = (iwmFwd != null) ? (iwmFwd - mktFwd) : null;   // small-minus-big
      if (sizeFwd != null && exp.size != null) { pred += exp.size * sizeFwd; legs.push('size'); }
      const volFwd = benchReturn(vMap, vDates, fillDate, exitDate);
      if (volFwd != null && exp.vol != null) { pred += exp.vol * volFwd; legs.push('vol'); }
      residual = +(grossReturn - pred).toFixed(4);
      residualFactors = legs;
    }
    ...
    horizons[key] = {
      ...
      marketRelReturn: marketRel, sectorRelReturn: sectorRel, residualReturn: residual, residualFactors,
