...
const CFG = {
  ...
  gradeHoldDays: 3,
  minGradedForEdge: 50,
  // Consecutive failed history fetches before an entry is retired as ungradeable. A single
  // miss is not proof — providers blip — but a symbol that misses this many times in a row
  // is not coming back, and retrying it forever is what exhausted the grader's budget.
  maxFetchAttempts: 3,
};
...
// UNGRADEABLE SYMBOLS
//
// The alert log is built from social posts by extracting $cashtags, so it inherits
// StockTwits symbology: crypto carries a .X suffix, foreign listings .TSX / .NSE, futures
// an _F suffix, and forex pairs are bare ISO code pairs. fetchDailyHistory serves US
// equities only, so none of those can EVER be graded.
//
// The filter below is deliberately conservative — it rejects only what is unambiguously
// not a US equity ticker. Judgment calls (a 5-letter Nasdaq unit like SBAQU, an illiquid
// micro cap with no year of history) are NOT guessed at here: they are real listings, and
// dropping them at ingest would silently discard real alerts. Those are handled empirically
// by the attempt cap instead, which retires a symbol only after it has actually proven
// unfetchable.
// ─────────────────────────────────────────────────────────────────────────────
const MAX_US_TICKER_LEN = 5;
const ISO_CURRENCIES = 'USD|EUR|GBP|JPY|AUD|CAD|CHF|NZD|CNY|CNH|HKD|SEK|NOK|DKK|MXN|SGD|INR|KRW|TRY|ZAR|BRL|PLN|XAU|XAG';
const FOREX_PAIR = new RegExp(`^(?:${ISO_CURRENCIES})(?:${ISO_CURRENCIES})$`);

function isGradeableSymbol(sym) {
  if (typeof sym !== 'string') return false;
  const s = sym.trim().toUpperCase();
  if (!s) return false;
  if (s.includes('.')) return false;            // BTC.X, BB.TSX, TATAPOWER.NSE, OPEAZZX.P
  if (s.includes('_')) return false;            // RTY_F and other futures
  if (FOREX_PAIR.test(s)) return false;         // USDJPY, GBPUSD, XAUUSD
  if (s.length > MAX_US_TICKER_LEN) return false;
  return /^[A-Z][A-Z0-9-]*$/.test(s);           // BRK-B is valid; 401JK is not
}

// Count one failed history fetch against an entry, retiring it once the cap is reached.
// Mutates in place because the caller owns the log array it is rewriting — but it never
// sets `graded`, so a retired entry stays out of analyzeEdge's sample and cannot influence
// the edge verdict. That separation is the whole point: unmeasurable is not the same as
// measured-and-flat.
function markUngradeable(entry) {
  entry.fetchFails = (entry.fetchFails || 0) + 1;
  if (entry.fetchFails >= CFG.maxFetchAttempts) {
    entry.ungradeable = true;
    entry.ungradeableReason = 'no-daily-history';
  }
  return entry;
}
...
module.exports = { CFG, rankPosts, gradeExcess, analyzeEdge, accountWeight, tickerDirections, scoreSegment, clusterPosts, dedup, isAlert, normalize, mineText, isGradeableSymbol, markUngradeable };
