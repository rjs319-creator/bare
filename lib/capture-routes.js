...
const { hasStore, readCern, writeCern, readInsider, writeInsider,
        readFundamentals, writeFundShard, writeArchiveDay } = require('./store');
...
async function runArchive(req, res) {
  ...
  const base = ordered.slice(0, ARCHIVE_MAX).map(t => {
    ...
    return {
      ...
      mentions: tr ? tr.mentions : 0,           // StockTwits watchers — null-safe 0 for candidate-only names
      trendRank: tr ? tr.trendRank : null,
      sources: [...(tr ? ['trending'] : []), ...(tags ? [...tags] : [])],
    };
  });
...
module.exports = { runArchive, runInsiderIngest, runInsider, runFundBuild, runFundamentals,
  runCernTickOp, runCern, runCernFsProbe, runCernLockProbe };
