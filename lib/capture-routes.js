...
// ── op=ivrvsample : Coil × IV/RV joinable-sample sufficiency accounting ─────
// Counts how many Coil picks have BOTH a same-date archived atmIV and a
// realized-vol reading, against the pre-registered ≥150-maturable bar. Pure
// accounting — it never computes the experiment and never emits a verdict.
async function runIvRvSample(req, res) {
  if (!hasStore()) {
    return res.status(200).json({ ok: false, error: 'Blob storage not configured (create a Vercel Blob store).' });
  }
  const { assessIvRvSample } = require('./ivrv-sufficiency');
  const { readAllCoilDays } = require('./store');
  const { nowET } = require('./stats');
  const [archiveDays, coilDays] = await Promise.all([
    readAllArchive().catch(() => []),
    readAllCoilDays().catch(() => []),
  ]);
  const report = assessIvRvSample({ archiveDays, coilDays, asOf: nowET().date });
  res.setHeader('Cache-Control', 's-maxage=1800, stale-while-revalidate=86400');
  return res.status(200).json({
    ok: true, ...report,
    // The full per-day table only on request — the headline is the totals.
    perDay: req.query.full === '1' ? report.perDay : undefined,
    at: new Date().toISOString(),
  });
}
...
// (research/41 was runner-heavy 2024–25 only). Rides the daily warm cron (no new
...
async function runIntraCapture(req, res) {
  ...
  // Most-recent COMPLETED session S = the last SPY daily bar (derived from data, not the
  // clock, so it is correct whichever side of the close the cron runs).
  let S = null;
...
module.exports = { runArchive, runBaseline, runIvRvSample, runInsiderIngest, runInsider, runFundBuild, runFundamentals,
