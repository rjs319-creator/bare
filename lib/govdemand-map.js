...
function entry(ticker, company, patterns, via, evidence, effectiveFrom = null, effectiveTo = null) {
  return Object.freeze({
    ticker, company, patterns: Object.freeze(patterns.map(normalizeRecipient)),
...
// followed exclusively by generic corporate-arm tokens. Multi-token patterns
// are distinctive enough for consecutive-run containment.
function matchesPattern(nameTokens, patternTokens) {
...
function mapRecipient(rawName, asOf = null) {
  ...
  for (const e of MAP) {
    ...
    for (const p of e.patterns) {
      if (matchesPattern(nameTokens, p.split(' '))) {
        return {
          ticker: e.ticker, company: e.company, via: e.via,
          matchedPattern: p, evidence: e.evidence,
...
function scanRoster() {
  ...
  for (const e of MAP) {
    ...
    seen.add(e.ticker);
    roster.push({ ticker: e.ticker, company: e.company, query: e.patterns[0] });
  }
  return roster;
}
