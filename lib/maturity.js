...
// Grade a SINGLE pooled track record ({ excessN, avgExcess, beatMktRate }) against its
// benchmark. excessN = resolved picks that had a benchmark to compare to; avgExcess =
// mean excess return vs benchmark (pts); beatMktRate = % of those picks that beat it.
// Returns { grade, reason, stats:{ excessN, avgExcess, beatMktRate, beatLo, beatHi } }.
function gradeTrack(track) {
  ...
  if (!excessN || avgExcess === null || beatMktRate === null) {
    return { grade: 'experimental', reason: 'No resolved picks with a benchmark yet — accruing.', stats: { excessN, avgExcess, beatMktRate, beatLo: null, beatHi: null } };
  }

  // Conservative bound on "beats the benchmark more than half the time", sample-aware.
  const beatWins = Math.round((beatMktRate / 100) * excessN);
  const { lo, hi } = wilson(beatWins, excessN);
  const stats = { excessN, avgExcess, beatMktRate, beatLo: +(lo * 100).toFixed(0), beatHi: +(hi * 100).toFixed(0) };

  // Disabled — enough evidence AND it's significantly (or materially) losing to the benchmark.
  if (excessN >= MIN_VERDICT && avgExcess < 0 && (hi < 0.5 || avgExcess <= -1)) {
    return { grade: 'disabled', reason: `Underperforms benchmark over ${excessN} resolved (avg ${avgExcess > 0 ? '+' : ''}${avgExcess}% excess, beats ${beatMktRate}%).`, stats };
  }
  // Validated — enough evidence AND it significantly beats the benchmark.
  if (excessN >= MIN_VERDICT && avgExcess > 0 && lo > 0.5) {
    return { grade: 'validated', reason: `Beats benchmark ${beatMktRate}% of ${excessN} resolved (avg +${avgExcess}% excess, Wilson lo ${stats.beatLo}%>50%).`, stats };
  }
  // Promising — positive point estimate, not yet significant / not yet enough samples.
  if (excessN >= MIN_PROMISING && avgExcess > 0) {
    return { grade: 'promising', reason: `Positive so far (avg +${avgExcess}% excess over ${excessN}) but not yet proven — Wilson lo ${stats.beatLo}%≤50%.`, stats };
  }
  // Enough samples but flat/underwater without being significantly bad → still experimental.
  if (excessN >= MIN_PROMISING) {
    return { grade: 'experimental', reason: `Mixed so far (avg ${avgExcess > 0 ? '+' : ''}${avgExcess}% excess over ${excessN}) — no verdict.`, stats };
  }
...
function poolSectionTrack(groups, horizon) {
  ...
  let n = 0, wExcess = 0, beatWins = 0;
  for (const g of groups) {
    const h = g.horizons && (g.horizons[metric] || g.horizons['1m'] || g.horizons['5d']);
    if (!h || !Number.isFinite(h.excessN) || !h.excessN) continue;
    if (!Number.isFinite(h.avgExcess) || !Number.isFinite(h.beatMktRate)) continue;
    n += h.excessN;
    wExcess += h.avgExcess * h.excessN;
    beatWins += Math.round((h.beatMktRate / 100) * h.excessN);
  }
  if (!n) return { excessN: 0, avgExcess: null, beatMktRate: null };
  return { excessN: n, avgExcess: +(wExcess / n).toFixed(2), beatMktRate: +((beatWins / n) * 100).toFixed(0) };
}
