...
const NLanes = require('./negative-lanes');   // shared horizon ladder + corroboration threshold
function adjacentHorizonReads(groups, metric) {
  const horizons = {};
  for (const hk of NLanes.HORIZON_ORDER) {
    if (hk === metric) continue;
    const pooled = poolSectionTrack(groups, { metric: hk });   // same pooling as the grade itself
    const dn = pooled.dateNet;
    if (!pooled.netExcessN && !dn) continue;
    const ci = dn && dn.ci95 && Number.isFinite(dn.ci95.lo) && Number.isFinite(dn.ci95.hi) ? { lo: dn.ci95.lo, hi: dn.ci95.hi } : null;
    const effectiveDates = dn ? (Number.isFinite(dn.effectiveN) ? dn.effectiveN : dn.n) : null;
    const enough = Number.isFinite(effectiveDates) && effectiveDates >= NLanes.MIN_ADJACENT_DATES;
    horizons[hk] = {
      avgNetExcess: pooled.avgNetExcess, netExcessN: pooled.netExcessN,
      dateAvg: dn && Number.isFinite(dn.avg) ? dn.avg : null,
