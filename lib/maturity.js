...
const MATURITY_VERSION = 'maturity-v3';
...
function gradeStrategy(entry, summary) {
  ...
  const groups = sectionGroups.filter(g => !isExcludedTier(g.tier) && (!policyTiers || policyTiers.has(g.tier)));
  const excludedTiers = sectionGroups.filter(g => !groups.includes(g)).map(g => g.tier);
  let graded;
  if (!groups.length) {
...
module.exports = {
  ...
  MIN_VALIDATED_EPISODES, MIN_VALIDATED_DATES, HORIZON_METRIC, EXCLUDED_TIER_PREFIXES,
