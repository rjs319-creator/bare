...
function gradeTrack(track, opts = {}) {
  ...
  const stats = { excessN, avgExcess, beatMktRate, beatLo: +(lo * 100).toFixed(0), beatHi: +(hi * 100).toFixed(0), basis, independenceBasis, dates, verdictN, fillVerified, dateNet: (track && track.dateNet) || null, baselines: { market: { n: excessN, avgExcess, beatRate: beatMktRate, basis }, sector } };
...
function gradeStrategy(entry, summary) {
  ...
  if (!groups.length) {
    ...
    graded = gradeTrack(poolSectionTrack(groups, { metric }), { fillVerified: fv.fillVerified, noHistoryRate });
