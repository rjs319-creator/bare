...
function gradeTrack(track) {
  ...
  const stats = { excessN, avgExcess, beatMktRate, beatLo: +(lo * 100).toFixed(0), beatHi: +(hi * 100).toFixed(0), basis, independenceBasis, dates, verdictN, baselines: { market: { n: excessN, avgExcess, beatRate: beatMktRate }, sector } };
  ...
  // Validated — beats the MARKET significantly (cost-net where measured, on independent
  // dates where known) AND (where measurable) beats its SECTOR. FAIL CLOSED on basis:
  // a gross-only record can never earn the promotion grade — friction decides which
  // sleeves are real, so without the net channel the ceiling is Promising.
  if (verdictN >= MIN_VERDICT && avgExcess > 0 && lo > 0.5 && beatsSector) {
    ...
    return { grade: 'validated', reason: `Beats SPY ${netTag}${beatMktRate}% of ${excessN} resolved / ${verdictN} independent (avg +${avgExcess}%, Wilson lo ${stats.beatLo}%>50%)${sectorKnown ? ` and its sector (+${avgSecExcess}%)` : ''}.`, stats };
  }
...
function gradeStrategy(entry, summary) {
  ...
  if (!groups.length) {
    ...
    graded = gradeTrack(poolSectionTrack(groups, { metric }));
