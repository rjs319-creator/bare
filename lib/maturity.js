...
function gradeTrack(track, opts = {}) {
  ...
  const incremental = opts.incremental && opts.incremental.declared === true
    ? { declared: true, measured: opts.incremental.measured === true, cleared: opts.incremental.cleared === true, source: opts.incremental.source || null }
    : null;
  ...
  const stats = { excessN, avgExcess, beatMktRate, beatLo: +(lo * 100).toFixed(0), beatHi: +(hi * 100).toFixed(0), basis, independenceBasis, dates, verdictN, fillVerified, dateNet: (track && track.dateNet) || null, baselines: { market: { n: excessN, avgExcess, beatRate: beatMktRate, basis }, sector }, incremental };
...
function gradeStrategy(entry, summary) {
  ...
  if (!groups.length) {
    ...
    // No measured artifact channel exists yet, so a declarer fails closed at Promising
    // until one is wired — which is the honest state, not a regression.
...
module.exports = {
  ...
  INCREMENTAL_DECLARED_RE, declaresIncrementalGate,
