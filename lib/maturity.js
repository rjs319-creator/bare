...
// ── Adjacent-horizon reads (2026-09-09 alpha pass) — DESCRIPTIVE, never a grade input ──
// The grade is judged at the contract horizon only (anything else is max-of-7 mining),
// but a record whose neighbouring bars are significantly negative deserves a visible
// warning on the reason line: Ghost:STALKING:large graded `promising` on 5d +0.28 while
// its 1d/3d bars were CI-negative over 35 dates. Per non-contract horizon: pooled
// cost-net excess (netExcessN-weighted) + the largest-dates tier's date-level CI, the
// same conservative pooling poolSectionTrack uses. Flagged only over ≥ MIN dates.
const ADJACENT_MIN_DATES = 8;
const HORIZON_KEYS = Object.freeze(['1d', '3d', '5d', '10d', '20d', '1m', '3m']);
function adjacentHorizonReads(groups, metric) {
  const horizons = {};
  for (const hk of HORIZON_KEYS) {
    if (hk === metric) continue;
    let nn = 0, wNet = 0, dateNet = null;
    for (const g of groups || []) {
      const h = g && g.horizons && g.horizons[hk];
      if (!h) continue;
      if (Number.isFinite(h.netExcessN) && h.netExcessN && Number.isFinite(h.avgNetExcess)) { nn += h.netExcessN; wNet += h.avgNetExcess * h.netExcessN; }
      if (h.dateNet && Number.isFinite(h.dateNet.n) && (!dateNet || h.dateNet.n > dateNet.n)) dateNet = h.dateNet;
    }
    if (!nn && !dateNet) continue;
    const ci = dateNet && dateNet.ci95 && Number.isFinite(dateNet.ci95.lo) && Number.isFinite(dateNet.ci95.hi) ? { lo: dateNet.ci95.lo, hi: dateNet.ci95.hi } : null;
    const effectiveDates = dateNet ? (Number.isFinite(dateNet.effectiveN) ? dateNet.effectiveN : dateNet.n) : null;
    const enough = Number.isFinite(effectiveDates) && effectiveDates >= ADJACENT_MIN_DATES;
    horizons[hk] = {
      avgNetExcess: nn ? +(wNet / nn).toFixed(2) : null, netExcessN: nn,
      dateAvg: dateNet && Number.isFinite(dateNet.avg) ? dateNet.avg : null,
      ci95: ci, effectiveDates,
      negative: !!(ci && enough && ci.hi < 0),
      positive: !!(ci && enough && ci.lo > 0),
    };
  }
  const keys = Object.keys(horizons);
  return {
    metric, horizons,
    negative: keys.filter(k => horizons[k].negative),
    positive: keys.filter(k => horizons[k].positive),
    basis: 'descriptive — never a grade input',
  };
}
function adjacentWarning(adj) {
  if (!adj || !adj.negative.length) return '';
  const parts = adj.negative.map(k => { const h = adj.horizons[k]; return `${k} [${h.ci95.lo}, ${h.ci95.hi}] (${h.effectiveDates} dates)`; });
  return ` ⚠ Adjacent horizons CI-negative: ${parts.join(', ')} — descriptive only; the grade is judged at the ${adj.metric} contract.`;
}
...
function gradeStrategy(entry, summary) {
  ...
  if (!groups.length) {
    ...
    if (graded && graded.stats) {
      const adj = adjacentHorizonReads(groups, metric);
      graded = { ...graded, stats: { ...graded.stats, adjacentHorizons: adj }, reason: `${graded.reason || ''}${adjacentWarning(adj)}` };
    }
  }
...
module.exports = {
  ...
  gradeTrack, poolSectionTrack, gradeStrategy, classifyStrategies, adjacentHorizonReads,
};
