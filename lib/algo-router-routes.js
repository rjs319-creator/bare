...
// Regime-specific evidence is only trusted at a statistically meaningful sample:
// ≥REGIME_MIN_DATES DISTINCT same-regime decision dates (same-day picks share the market
// factor and are not independent observations). Below the bar → null (unmeasured), which
// the strict budget path zeroes rather than guesses.
const REGIME_MIN_DATES = 8;      // matches the app's other ≥8 evidence gates
function regimeCompatFor(series, macroLU, currentRegime) {
  ...
  const bucketDates = { 'risk-on': new Set(), neutral: new Set(), 'risk-off': new Set() };
  for (const r of series) {
    if (typeof r.excess !== 'number') continue;
    const st = macroLU.at(r.date);
    if (st && buckets[st.regime]) { buckets[st.regime].push(r.excess); bucketDates[st.regime].add(r.date); }
  }
  const avg = (a) => (a.length ? a.reduce((x, y) => x + y, 0) / a.length : null);
  const cur = buckets[currentRegime];
  if (!cur || bucketDates[currentRegime].size < REGIME_MIN_DATES) return null; // not enough same-regime history
