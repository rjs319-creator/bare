...
// Suggested mechanical trade levels for a momentum-continuation entry:
//   entry  = current price (a continuation buy)
//   stop   = the TIGHTER of "just under today's low" (the move's invalidation) and a
//            1.5×ATR risk cap, so a wide-range day can't blow the risk budget
//   target = a 1:2 reward:risk objective
// These are RISK-MANAGEMENT references for novices, NOT predictions. riskPct is the
// % you'd lose per share if stopped — the input to position sizing.
function tradeLevels(candles, { stopAtrMult = 1.5, rr = 2 } = {}) {
  const i = candles.length - 1;
  const entry = candles[i].close;
  const a = atr(candles);
  if (!(a > 0) || !(entry > 0)) return null;
  const structureStop = candles[i].low - 0.1 * a;   // a break of today's low = setup failed
  const atrStop = entry - stopAtrMult * a;           // never risk more than ~1.5 ATR
  const stop = Math.max(structureStop, atrStop);     // tighter (higher) of the two
  const risk = entry - stop;
  if (!(risk > 0)) return null;
  return {
    entry: +entry.toFixed(2),
    stop: +stop.toFixed(2),
    target: +(entry + rr * risk).toFixed(2),
    rr,
...
module.exports = { AVG_VOL_WINDOW, SCANS, dayMetrics, passesScan, rankScore, atr, tradeLevels };
