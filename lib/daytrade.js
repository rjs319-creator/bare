...
// ── Intraday volume pacing ──────────────────────────────────────────────────
// Relative volume divides today's volume by a full-day 20-day average. Mid-session
// today's daily bar is only PARTIALLY printed, so raw relVol reads far below its
// eventual full-day value (a +6% small-cap can show 0.1× at 09:50) and nothing clears
// the volume gate. `sessionPaceFraction` returns the fraction of the regular session
// (9:30–16:00 ET = 390 min) elapsed, so callers can prorate: relVol = raw / paceFrac.
// LIVE DISPLAY ONLY — the tracked ledger and backtest run on completed bars (pace = 1).
const RTH_START_MIN = 9 * 60 + 30;   // 09:30 ET
const RTH_END_MIN = 16 * 60;         // 16:00 ET
const RTH_MINUTES = RTH_END_MIN - RTH_START_MIN;
const PACE_FLOOR = 0.05;             // don't inflate more than ~20× in the first ~20 min

// Fraction of the RTH session elapsed at `now`, in America/New_York (EST/EDT-safe).
// Returns 1 outside regular hours (pre-open, post-close, weekends) → no pacing then.
function sessionPaceFraction(now = new Date()) {
  ...
  if (cur <= RTH_START_MIN || cur >= RTH_END_MIN) return 1;   // outside RTH → treat bar as complete
  return Math.max(PACE_FLOOR, (cur - RTH_START_MIN) / RTH_MINUTES);
}
...
const SCANS = {
  ...
  // Widened over time (relVol 1.2→1.15→1.05, pct 3.0→2.5→2.0) to broaden the funnel on
  // calm/quiet tapes where the A-tier scan finds 0 names. Quality is preserved downstream:
  // B-tier is DISPLAY-ONLY (the tracked A-tier ledger is untouched) and Best Opportunities
  // ranks the whole pool by honest carry odds, so weaker building names sort to the bottom.
  momentum_building: {
    key: 'momentum_building', label: '📈 Building Momentum',
    priceMin: 5, priceMax: 50,
    minAvgVol: 1_000_000, minDollarVol: 10_000_000,
    minRelVol: 1.05, minPct: 2.0,
  },
  // Relaxed "B-tier" of Explosive Small-Cap — surfaces BUILDING small/micro movers on
  // quiet tapes where the strict +8%/2× ignition gate finds nothing. DISPLAY-ONLY: not
  // logged to the tracked ledger (the strict explosive record stays comparable), and this
  // scan's own rank is known-inverted in the app's backtest, so it's never treated as a
  // winner in Best Opportunities (carry discounts it). Same price/liquidity band as strict.
  explosive_building: {
    key: 'explosive_building', label: '💥 Explosive Small-Cap (building)',
    priceMin: 1, priceMax: 20,
    minAvgVol: 500_000, minDollarVol: 2_000_000,
    minRelVol: 1.3, minPct: 4.0,
  },
};
...
// Relaxed "B-tier" of the multi-day run — surfaces EARLY or slightly-faded runs on
// quiet tapes where the strict +20%/2-hivol-days/within-8%-of-high gate finds ~nothing.
// Same price/liquidity band; loosens the three run gates one notch. DISPLAY-ONLY: the
// run scan is never written to the tracked ledger (only strict momentum_liquid A/B and
// strict explosive are), so the forward record stays untouched, and Best Opportunities
// ranks the whole pool by honest carry odds so weaker/faded runs sort to the bottom.
const RUN_SCAN_RELAXED = {
  key: 'momentum_run', label: '🌊 Momentum Run (building)',
  priceMin: 2, priceMax: 100,
  minAvgVol: 500_000, minDollarVol: 5_000_000,
  minPct5d: 15,            // ≥15% over 5 sessions (vs 20% strict) — catch runs earlier
  minHighVolDays: 1,       // ≥1 unusual-volume day (vs 2) — one strong day of participation
  nearHighFrac: 0.88,      // within 12% of the 5-day high (vs 8%) — allow a mild pullback
};
...
function dayMetrics(candles, spyByDate, avgWindow = AVG_VOL_WINDOW, pace = 1, todayEt = null) {
  ...
  // POINT-IN-TIME PACING GUARD. Pacing (dividing a partial-session volume by the fraction
  // of the session elapsed) is ONLY valid for the CURRENT session's forming bar. A completed
  // prior-session bar already holds full-day volume — prorating it would turn a real 1.0×
  // reading into a phantom 5-10× spike (the stale-candle defect). So pacing is applied ONLY
  // when THIS ticker's own last bar is dated today (`todayEt`). When we can't prove that
  // (todayEt omitted, or the last bar is a stale prior-session bar), pace is forced to 1
  // regardless of what the caller passed — the guard is intrinsic to the metric, never the
  // caller's responsibility. `barIsToday` is surfaced so eligibility/freshness can key off it.
  const barDate = candles[i].date || null;
  const barIsToday = todayEt != null && barDate != null && barDate === todayEt;
  const effPace = (barIsToday && pace > 0 && pace < 1) ? pace : 1;
  const rawRelVol = todayVol / avgVol;                                   // unpaced (full-day basis)
  const relVol = effPace < 1 ? rawRelVol / effPace : rawRelVol;          // intraday-prorated (fresh bar only)
  const jump = Math.max(todayOpen > 0 ? Math.abs(todayOpen - prev) / prev : 0, Math.abs(last - prev) / prev);
  // Artifact guard uses RAW volume: a split-rescale trades normal volume, and pacing must
  // not inflate it past the guard and let a corrupt bar through.
  if (jump > ARTIFACT_JUMP && rawRelVol < ARTIFACT_MIN_RELVOL) return null;
  ...
  return {
    ...
    rawRelVol: +rawRelVol.toFixed(2),
    paced: effPace < 1,                // true only when a FRESH current-session bar was prorated
    candidateDate: barDate,            // ET date of the bar these metrics describe
    barIsToday,                        // pacing-/eligibility-relevant: is this the current session's bar?
...
module.exports = { AVG_VOL_WINDOW, SCANS, RUN_SCAN, RUN_SCAN_RELAXED, dayMetrics, passesScan, passesRunScan, rankScore, runRankScore, atr, ema, tradeLevels, orbLevels, sessionPaceFraction };
