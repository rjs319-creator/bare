...
function evaluateFailureModel(picks, histMap, { spyCandles = null, config = {} } = {}) {
  ...
  // Promotion is deliberately conservative — three independent bars, ALL required:
  //  1) multi-month span + large sample (regime-diversity proxy);
  //  2) a genuine FAILURE signal, not inverse-momentum: the rejected names must actually
  //     underperform the MARKET (negative excess), not merely trail the winners. If "rejected"
  //     roughly matches the market while "approved" beats it, the split is re-expressing
  //     momentum (which the winner model already prices) rather than predicting failure.
  const spanOk = distinctMonths >= cfg.PROMOTE_MIN_MONTHS && scored.length >= cfg.PROMOTE_MIN_TOTAL;
  const rejEx = buckets.rejected.meanExcess;
  const rejectedLagsMarket = rejEx == null ? null : rejEx < 0; // null when no benchmark (can't judge)
  const promoted = verdict === 'predictive' && spanOk && rejectedLagsMarket !== false;
  let promotionBlockedReason = null;
  if (verdict === 'predictive' && !promoted) {
    if (!spanOk) {
      promotionBlockedReason = `Predictive on the available data, but held in SHADOW: it spans ${distinctMonths} month(s) / ${scored.length} picks — below the ${cfg.PROMOTE_MIN_MONTHS}-month, ${cfg.PROMOTE_MIN_TOTAL}-pick bar for a regime-robust result. This app's research has repeatedly shown single-window edges die out-of-sample.`;
    } else if (rejectedLagsMarket === false) {
      promotionBlockedReason = `Predictive vs the approved bucket, but held in SHADOW: the rejected names roughly MATCHED the market (${rejEx > 0 ? '+' : ''}${rejEx}% excess) rather than underperforming it. That means the split is likely re-expressing momentum — separating future winners from market-performers — not identifying genuine failure, which the winner model already prices. A real failure signal makes the rejected group LOSE to the market.`;
    }
  }
  return {
