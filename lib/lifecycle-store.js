...
const { readJSON, writeJSON, hasStore, withReadStats } = require('./store');
...
// Best-effort like every reader here (unreadable shards are skipped, never thrown), but the
// loss is now COUNTED (review 2026-08-15): the merged array carries lib/store's
// withReadStats metadata ({ requested, unreadable, complete }, non-enumerable — existing
// callers keep seeing a plain array) so op=lifecyclegrade can refuse to overwrite a good
// grades doc with grades computed from a truncated day.
async function loadSnapshots(strategy, date) {
  ...
  const shards = await loadSnapshotShards(strategy, date);
  return withReadStats(legacy.concat(shards), shards.requested, shards.unreadable);
}

async function loadSnapshotShards(strategy, date) {
  if (!hasStore()) return withReadStats([], 0, 0);
  try {
    ...
    for (const b of shards) {
      try {
        // No cache-buster: shards are WRITE-ONCE (unique key per append, never
        // overwritten), so a cached copy can never be stale — busting was pure cost.
        // The buster stays on MUTABLE docs only (legacy day doc via readJSON, grades).
    ...
    return withReadStats(out, shards.length, unreadable);
  } catch {
    // The LIST itself failed — the loss extent is unknown. Fail closed: report an
    // incomplete read (unreadable ≥ 1) so nothing downstream mistakes this for empty.
    return withReadStats([], 0, 1);
  }
}
...
// `meta` (optional) merges extra doc-level fields — e.g. { readIncomplete: true } when a
// first-time grade had to run on a truncated snapshot read (review 2026-08-15) — so a
// grades doc built from partial evidence is distinguishable from a complete one.
async function saveGrades(strategy, date, grades, meta = null) {
  if (!hasStore()) return { persisted: false, reason: 'no-store' };
  try {
    await writeJSON(gradeKey(strategy, date), { strategy, date, grades, ...(meta || {}), updatedAt: new Date().toISOString() }, 0);
