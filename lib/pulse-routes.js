...
// Single web-search call (must fit the 60s function budget): the model sweeps the web
// with the web_search tool and emits the structured 10 via submit_pulse in one request.
// max_uses is bounded so it stays well under the timeout.
async function generatePulse() {
  ...
  const msg = await client.messages.create({
    ...
    messages: [{ role: 'user', content: PROMPT + `\n\nAfter searching, you MUST call the submit_pulse tool with all ${N} items — do not answer in plain text.` }],
  }, { timeout: 48000 });    // stay under the 60s function limit → graceful stale fallback
  const items = parsePulse(msg);
