...
const REFINE_TIMEOUT_MS = 52000;   // Fable pass, under the 60s wall (maxRetries:0)
const REFINE_MAX_TOKENS = 6000;
// Fable 5's thinking is ALWAYS ON — the only way to bound its latency is the effort
// knob. Default 'high' made the editorial pass overthink and blow the timeout; 'low'
// still comfortably handles a dedup/re-rank/rewrite and finishes well under the wall.
const REFINE_EFFORT = 'low';
 ...
 *  `diag` (optional) collects a reason string for debugging why a null came back. */
async function refineDraft(draftItems, timeoutMs = REFINE_TIMEOUT_MS, diag) {
  const note = m => { if (diag) diag.reason = m; };
  if (!process.env.ANTHROPIC_API_KEY) { note('no-api-key'); return null; }
  ...
  try {
    const msg = await client.messages.create({
      ...
      output_config: { effort: REFINE_EFFORT },   // bound Fable's always-on thinking
...
async function runPulseRefine(req, res) {
  ...
  const diag = {};
  const refined = await refineDraft(candidates, REFINE_TIMEOUT_MS, diag);
  if (!refined) {
