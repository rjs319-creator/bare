...
// `category` is cheap (one enum) and needed to route macro vs ticker, so it rides the
// gather. The rich insight PROSE lives ONLY in the refine tool: making Haiku generate
// seven prose fields × 16 candidates tripled its output and blew the gather timeout
// (the feed got stuck on the last-known-good snapshot). Fable writes the insight for the
// final 10 instead — fewer items, its own budget, and better concise prose.
const CATEGORY_PROP = {
...
const INSIGHT_PROPS = {
  ...
  // No per-item sourceList: Haiku emitting it blew the gather timeout, and Fable's refine
  // has no search context to cite from (it would only hallucinate droppable URLs). The REAL
  // web_search citations are captured into the snapshot's sourcePool via extractCitations;
  // per-item evidence honestly grades "Search-summary only" when no direct metadata exists.
};
...
4. WRITE the concise insight for each final item — whatChanged (≤140 chars), whyItMatters, traderRead, noviceTranslation (plain English; warn against chasing when apt), primaryRisk, invalidation (one testable condition), horizon. One sentence each. This is the polish the lean draft lacks.
5. Keep sentiment/prominence/buzz honest. Never invent a source, URL, count, or platform.

Return exactly ${N} de-duplicated items via submit_pulse_refined — no plain text.`;
}
