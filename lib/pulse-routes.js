...
const { SENTIMENTS, VELOCITIES, CROWDINGS, CATEGORIES,
...
// Gather budget. Each web_search round-trips a provider, so 5 searches routinely blew the
// 48s ceiling and dropped the whole gather to the last-known-good snapshot. 3 targeted
// searches finish comfortably under the wall, and a slightly higher timeout gives the
// structured-output write room. This is the difference between a feed that refreshes and
// one stuck on a stale snapshot.
const GATHER_MAX_SEARCHES = 3;
const GATHER_TIMEOUT_MS = 52000;
...
// Honest staleness tiers by snapshot age (minutes). "live" is reserved for a
// snapshot younger than one collection hour — a 3.9-hour-old narrative is NOT live
// (that mislabel was Pulse v1's defect #1; v2 separates the narrative clock from the
// market-data clock entirely, see lib/pulse2-clocks).
function freshnessOf(ageMins) {
  if (ageMins == null) return 'unknown';
  if (ageMins < 60) return 'live';
  if (ageMins < 240) return 'recent';
...
// Both LLM calls stay LEAN so each finishes under the 60s wall: the gather is search-laden
// and the refine's always-on thinking is slow, so neither can also generate seven prose
// insight fields for 10-16 items without timing out (both were observed doing exactly that
// in production, dropping the feed to the last-known-good snapshot). The card's plain-English
// / trader guidance is instead DERIVED downstream (pulse-schema deriveInsight) from the
// computed states + the LLM's own idea/whyMoves/crowding/contrarian — reliable and fast.
// `category` is one cheap enum and rides both tools so macro vs ticker routing is honest.
// No per-item sourceList: Haiku emitting it blew the gather budget and Fable has no search
// context to cite from; the REAL web_search citations are captured into the snapshot's
// sourcePool via extractCitations, and evidence honestly grades "Search-summary only".
const CATEGORY_PROP = {
...
const PULSE_REFINE_TOOL = {
  ...
  description: `Return the FINAL top ${N} after merging duplicate/overlapping themes, re-ranking by genuine market-moving weight, and adding a crowding/contrarian read. Ranked #1 = most important. Be concise and fast — the presentational insight is generated downstream.`,
  input_schema: {
    type: 'object',
    properties: {
      items: {
...
const GATHER_PROMPT = `You are a markets desk analyst. Using web search, sweep what finance social media and media are trending on RIGHT NOW that could move US markets: X/FinTwit, StockTwits, r/wallstreetbets, finance YouTube (CNBC/Bloomberg/Yahoo), breaking news, earnings, FDA/biotech catalysts, unusual options / squeeze chatter, and this week's macro calendar.

EFFICIENCY: run at most ${GATHER_MAX_SEARCHES} FOCUSED searches (e.g. "trending stocks today", "most active tickers", "market-moving news today") — do NOT search each platform separately. You are on a strict time budget: after your searches, call submit_pulse immediately.

Return up to ${GATHER_N} candidates — specific stocks OR macro themes. Cast a WIDE net (overlaps are fine; a second pass merges and sharpens them). Keep each candidate LEAN — the polish happens downstream. For each: tickers (empty for macro), a category (ticker vs macro), what the crowd is saying, why it could move US markets, sentiment, a 1-100 prominence estimate, a buzz-trend bucket, and a short free-text note of where it is trending.

Do NOT write long analysis — brevity keeps you inside the time budget. NEVER invent a source, URL, timestamp, count, or platform; keep the trending note honest. Submit via submit_pulse — no plain text.`;
...
async function gatherPulse() {
  ...
  const msg = await c.messages.create({
    model: GATHER_MODEL,
    max_tokens: 6000,
    tools: [{ type: 'web_search_20250305', name: 'web_search', max_uses: GATHER_MAX_SEARCHES }, PULSE_TOOL],
...
4. Keep sentiment/prominence/buzz honest. Never invent a source, URL, count, or platform. Do NOT write long analysis — the card's plain-English/trader guidance is generated downstream from your ranking + crowding read.

Return exactly ${N} de-duplicated items via submit_pulse_refined — no plain text. Speed matters: prioritise the merge, rank and crowding/contrarian read.`;
}
...
async function refineDraft(draftItems, allowedUrls, timeoutMs = REFINE_TIMEOUT_MS, diag) {
  ...
  if (!r) { note('no-api-key'); return null; }
  if (r.error) { note('threw:' + r.error); return null; }
  if (diag) diag.stop = r.stopReason;
  if (r.refused) { note('refused:' + (r.category || '?')); return null; }
  if (!r.input) { note('no-tool-block:' + (r.stopReason || '?')); return null; }
  const refined = parseRefinedPulse(r.input, N, allowedUrls);
...
async function runPulse(req, res) {
  ...
  // PRIVILEGED-WRITER RULE: only a trusted caller (cron/scheduler bearer) may trigger the
  // expensive LLM gather. An anonymous page load past the refresh boundary gets the
  // last-known-good snapshot, honestly aged — it must never initiate LLM spend or a
  // ledger write (Pulse v2 audit #2/#31).
  if (!require('./auth').isTrusted(req)) {
    if (cached) {
      const { raw, sourcePool, ...userDoc } = cached;
      return serve(res, userDoc, { cached: true, stale: !fresh, lastKnownGood: !fresh, persisted: hasStore, regenRequiresScheduler: !fresh }, 's-maxage=600');
    }
    res.setHeader('Cache-Control', 'no-store');
    return res.json({ ok: false, error: 'no pulse snapshot yet — generation runs on the scheduled tick', items: [], disclaimer: DISCLAIMER });
  }
...
async function runPulseRefine(req, res) {
  ...
  // Same privileged-writer rule as the gather: the Fable refine is an LLM call and an
  // episode-ledger write — anonymous readers get the current draft, never a refine run.
  if (!require('./auth').isTrusted(req)) {
    const { raw, sourcePool, ...userDoc } = cached;
    return serve(res, userDoc, { cached: true, refinePending: true, persisted: true }, 's-maxage=600');
  }
  ...
  // Concise sector-leadership context from the RLT shadow scan — a CHEAP read
  // of its persisted summary (rlt/latest.json), never computed here. Absent or
  // failing ⇒ the key is simply null; Market Pulse must never block on RLT.
  const sectorLeadership = await require('./rlt-store').loadLatest()
    .then(d => (d && d.sectorLeadership) || null)
    .catch(() => null);
