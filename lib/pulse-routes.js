...
const REFINE_TIMEOUT_MS = 40000;   // Fable pass; leaves headroom for enrichment+storage under 60s
const REFINE_MAX_TOKENS = 7000;
const REFINE_EFFORT = 'low';       // bound Fable's always-on thinking
const ENRICH_MAX = 24;             // cap PIT price fetches per refine
...
async function runPulseRefine(req, res) {
  ...
  const diag = {};
  const refined = await refineDraft(candidates, allowedUrls, REFINE_TIMEOUT_MS, diag);
  if (!refined) {
  ...
  // ENRICH tickers with point-in-time price context (bounded), then DERIVE states.
  const date = cached.date || etDate();
  const tickers = refined.flatMap(it => (it.category !== 'macro' ? it.tickers : [])).slice(0, ENRICH_MAX);
  let enrichMap = {};
  try { enrichMap = await enrichTickers(tickers, { max: ENRICH_MAX }); } catch { enrichMap = {}; }
...
module.exports = {
  ...
  stateItems, recentTransitions, buildForward, freshnessOf, etDate,
