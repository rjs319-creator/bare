...
// Earliest valid source timestamp — the event's publication time. Never invented:
// sources without a parseable datetime contribute nothing, and an event whose
// sources carry no timestamp gets publishedAt:null (the underreaction engine then
// reports INSUFFICIENT_DATA for it rather than guessing an age).
function earliestPublishedAt(sources) {
  let best = null;
  for (const s of sources || []) {
    const dt = s && s.datetime ? new Date(s.datetime) : null;
...
// opts.extractor = { model, promptVersion } — the exact LLM identity stamped on the
// event so a model/prompt change can never silently contaminate a prospective study.
function normalizeEvent(raw, { ticker, sources = [], detectedAt = null, extractor = null } = {}) {
  ...
  return {
    ...
    // Prospective-study provenance (additive in evidence-v1): the original publish
    // timestamp, the durable source records, and the exact extractor identity.
    publishedAt: earliestPublishedAt(sources),
    sources: (sources || []).slice(0, 4).map(s => ({
      url: (s && s.url) || null,
      publisher: (s && s.publisher) || null,
      publishedAt: s && s.datetime && !isNaN(new Date(s.datetime).getTime()) ? new Date(s.datetime).toISOString() : null,
      type: classifySource(s || {}).type,
    })),
    extractor: extractor && extractor.model
      ? { model: String(extractor.model), promptVersion: extractor.promptVersion ? String(extractor.promptVersion) : null }
      : null,
  };
}
...
module.exports = {
  ...
  hostOf, classifySource, normalizeEvent, validateEvent, earliestPublishedAt,
