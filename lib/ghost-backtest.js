...
const { EXECUTION_POLICY_VERSION } = require('./execution-policy');
...
async function buildCrossSection({ scope, step, months, limit, deadlineMs, insiderData, macroLookup, fundamentalsData }, t0) {
  ...
  for (const date of dates) {
    ...
    // Forward return, cross-sectionally de-meaned → measures DIFFERENTIATION. Entry anchors on
    // the NEXT session's OPEN (exec-v2 — the pillars are known at c.idx close, so idx+1 open is
    // the earliest fill), exit at the idx+MAX_HOLD close. The ~1-session shorter hold vs a
    // close-to-close window is immaterial at this horizon and keeps the idx+MAX_HOLD guard valid.
    const entryOf = (cd, i) => (cd[i + 1] && cd[i + 1].open > 0) ? cd[i + 1].open : cd[i].close;
    const fwd = cohort.map(c => { const e = entryOf(c.candles, c.idx); return (c.candles[c.idx + MAX_HOLD].close - e) / e; });
    const mean = fwd.reduce((a, b) => a + b, 0) / fwd.length;
    // SPY's forward return over the SAME window (also next-open entry) → "beat the market" label.
    let spyFwd = null;
    if (si != null && si + MAX_HOLD < spyCandles.length) {
      const sEntry = (spyCandles[si + 1] && spyCandles[si + 1].open > 0) ? spyCandles[si + 1].open : spyCloses[si];
      spyFwd = spyCloses[si + MAX_HOLD] / sEntry - 1;
    }
...
// PURGE SIZING (alpha-research pass 3). `purgeDates` counts DECISION DATES, and the
// cross-section is built `step` sessions apart (default 10) while every label runs
// MAX_HOLD = 63 sessions forward. A purge of 1 date therefore removed ~10 sessions of
// overlap against a 63-session window: six-sevenths of each label's forward span still
// straddled the fold boundary, so "out-of-sample" blocks shared outcomes with training.
// The purge must cover the label, so it is now DERIVED from the horizon and the step
// rather than defaulted to a number that looks small and safe.
function purgeDatesFor(step) {
  const s = Number.isFinite(step) && step > 0 ? step : 10;
  return Math.max(1, Math.ceil(MAX_HOLD / s));
}
...
async function runGhostBacktest({ scope = 'large', step = 10, months = 12, limit = 0, deadlineMs = 45000, insiderData = null, fundamentalsData = null } = {}) {
  ...
  const cv = purgedBlocks(records, scoreOf, { folds: 4, purgeDates: purgeDatesFor(step) });
  ...
  const cvc = purgedBlocks(records, convScore, { folds: 4, purgeDates: purgeDatesFor(step) });
  ...
  const cvr = purgedBlocks(gated, refScore, { folds: 4, purgeDates: purgeDatesFor(step) });
  ...
  const convictionRefined = {
    ...
    // SELECTION WAS IN-SAMPLE — no re-run on these records can establish generalization.
    //
    // The four refinements above are described in this file's own comment as
    // "data-justified": DROP IN because it showed negative IC *on these records*, BONUS
    // up-weighted 1.5x because it was "the one additive factor (+IC)" *on these records*.
    // Choosing them by looking at the sample and then re-scoring the same sample measures
    // fit, not out-of-sample behaviour, however well-purged the folds are. The purged
    // walk-forward here bounds one leakage channel (overlapping labels); it cannot undo
    // the selection that happened before it.
    //
    // The numbers are kept — they are useful diagnostics — but the generalization CLAIM is
    // withdrawn rather than restated more softly. `generalizes` is now null with an
    // explicit reason; `inSampleGates` preserves what the old boolean measured so nothing
    // is lost, under a name that says what it is.
    selectionBasis: 'in-sample: refinements were chosen from these same records',
    verdict: {
      generalizes: null,
      generalizesReason: 'refinement selection was in-sample; a purged re-run on the selecting sample cannot establish out-of-sample behaviour',
      inSampleGates: { clearedOosGate: rGeneralizes, topQuintileBeatsMarket: rTopBeats },
      topQuintileBeatsMarket: rTopBeats,
      headline: !rGeneralizes
        ? `IN-SAMPLE: refined conviction IC ${levers.refined_regimeGated} did not clear even the in-sample gate — the refinements do not produce a shippable standalone ranker.`
        : `IN-SAMPLE ONLY — refinements were selected from these records, so these figures measure FIT, not generalization. Refined top-quintile Wilson LB ${refLo != null ? Math.round(refLo * 100) : '?'}% vs base ${baseLo != null ? Math.round(baseLo * 100) : '?'}%. Establishing an edge requires re-running the FROZEN refined weights on a later, untouched window.`,
    },
  };
  ...
  return {
    ...
    execution: { entry: 'next-open', policyVersion: EXECUTION_POLICY_VERSION },
    pillarIC,
