...
const SPECIALISTS = [
  ...
  // ORBIT-ML residual-drift ranker (orbit-ml). SHADOW: registered as an archetype
  // for the legend + health surface, but DELIBERATELY has no SOURCE_SPECIALIST
  // mapping below, so sourceToSpecialists never returns it and it never fires on a
  // live candidate — it cannot alter any ensemble P or decision rank. It accrues
  // its own resolved ledger (orbit-ml/) and is measured via redundancy for
  // incremental value before any promotion.
  'idiosyncraticPersistence',
];
const SPECIALIST_META = {
  ...
  idiosyncraticPersistence: { icon: '🛰️', label: 'Idiosyncratic persistence',  blurb: 'ORBIT-ML residual-drift cross-sectional ranker (shadow).' },
};
...
function scoreCandidate(sig, ctx = {}) {
  ...
  const rawP = p;
  const calUsable = calibratorUsable(calibrator);
  if (p != null && calUsable) p = applyCalibrator(calibrator, p);
  ...
  return {
    ...
    // Honest provenance for the displayed probability: 'calibrated' only when the
    // calibrator beat raw P out-of-fold; otherwise the raw ensemble value is shown.
    calibrationStatus: calUsable ? 'calibrated' : 'uncalibrated',
    calibrationHelpsOOS: calibrator ? (calibrator.calibrationHelpsOOS ?? null) : null,
...
// A CALIBRATOR IS ONLY APPLIED IF IT DEMONSTRABLY HELPS (alpha-research pass 3).
//
// fitCalibrator computes the honest out-of-fold read — `oofBrier` (calibrated) vs
// `oofBrierRaw` (the baseline it must beat) — and sets `calibrationHelpsOOS`. But the
// map was applied to every live candidate whenever a calibrator merely EXISTED, so an
// in-sample-fitted 5-bin table could be reshaping the displayed probability even when
// its own out-of-fold evidence said it made things WORSE. fitCalibrator's own comment
// concedes the in-sample number is "optimistic".
//
// Fails CLOSED on null: `calibrationHelpsOOS === null` means the OOF check could not be
// computed (too few rows to fold), which is not evidence that calibration helps. An
// unproven calibration is not applied — the raw ensemble probability is shown instead,
// stamped uncalibrated, matching how atlasx/premove already refuse to show a percentage
// without a valid calibration artifact.
function calibratorUsable(cal) {
  return !!(cal && cal.edges && cal.edges.length && cal.calibrationHelpsOOS === true);
}

// NOTE: applyCalibrator deliberately does NOT self-gate on calibrationHelpsOOS. It is a
// pure map-application used in two places: the LIVE path (gated by the caller) and
// oofCalibratorBrier's fold loop, which applies a fold-fitted {edges,table} that has no
// OOF verdict yet — by construction, since the verdict is what that loop is computing.
// Gating here made the OOF evaluation return raw P for every fold, collapsing
// oofBrier === oofBrierRaw and silently disabling the very measurement the live gate
// depends on. The gate lives at the live call site (see calUsable in scoreCandidate).
...
module.exports = {
  ...
  evolveScore, contextKey, capBucket, scoreCandidate, fitCalibrator, applyCalibrator, calibratorUsable, binnedMap, oofCalibratorBrier, candidateStrengthTilt, strengthOOSComparison, buildEvolve,
};
