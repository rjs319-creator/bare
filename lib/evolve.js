...
const GUARDRAILS = {
  ...
  maxProbeShare: 0.20,     // cap PROBE selections to ≤20% of surfaced candidates
...
function decideState({ p, payoff, effN, threshold, regimeVeto, dataOk, liquidityOk, exploreSelected }) {
  ...
  if (clearsEdge && enoughSample && liquidityOk) return { state: 'TRADE_CANDIDATE', reason: 'calibrated edge + support' };
  ...
  if (clearsEdge && watchSample) return { state: 'WATCH', reason: liquidityOk ? 'promising but below promotion threshold' : 'promising but illiquid' };
...
function scoreCandidate(sig, ctx = {}) {
  ...
  const decision = decideState({ p, payoff, effN, threshold, regimeVeto, dataOk, liquidityOk, exploreSelected });
  ...
  return {
    ...
    regimeVeto, extensionPenalty, contextKey: ctxKey, cap,
...
function buildEvolve(signals, ctx = {}) {
  ...
  const probeCandidates = scored
    ...
    .sort((a, b) => b.explorationBonus - a.explorationBonus);
  // Cap PROBE to a small share of ALL candidates EVOLVE evaluated (a stable, auditable
  // "small configurable share" — and one that still permits a little exploration on a
  // cold ledger where nothing is validated yet, which is exactly when exploration matters).
  const probeCap = Math.max(1, Math.floor(scored.length * GUARDRAILS.maxProbeShare));
  const probeSet = new Set(probeCandidates.slice(0, probeCap).map(s => s.ticker + '|' + s.horizon));
