...
// Entry-time features the survival model may use — the CANONICAL model-feature list from
// lib/intraday-schema (single source of truth; a rename there is a schema-test failure, not
// silent drift). All captured in the snapshot, all point-in-time, small + interpretable.
const { MODEL_FEATURES, toModelRow } = require('./intraday-schema');
...
// Features pass through toModelRow: canonical names, legacy residualVsSpy(%)→residual15
// aliasing, and null-preserving missingness (the model's indicators handle nulls — nothing
// is silently zeroed here).
function gradesToRows(grades) {
  ...
  return list
    ...
    .map(g => ({
      ...
      ticker: g.ticker,
      features: toModelRow(g.features),
      rawFeatures: g.features,
...
function evaluateSurvival(rows, opts = {}) {
  ...
  // Single-signal comparator baselines — the challenger must beat not just the deterministic
  // score but each trivial one-feature ranking (momentum alone, participation alone, CUSUM
  // alone). Null values rank last (missing evidence is never a favorable rank).
  const single = key => precisionAtK(oof.map(r => ({
    score: (r.features && Number.isFinite(r.features[key])) ? r.features[key]
      : (r.rawFeatures && Number.isFinite(r.rawFeatures[key])) ? r.rawFeatures[key] : -Infinity,
    label: r.label,
  })), k);
  const comparators = {
    mom15Alone: single('mom15'),
    relVolAlone: single('timeOfDayRelVol'),
    cusumAlone: single('cusum'),
  };
