...
// IGNITION synthesis layer (lib/ignition-live.js): per-scan spec-shaped views, the per-day
// attention-rank history, and the ignition alert dedup state.
const IGNITION_LIVE_PREFIX = 'ignition-live/';
const ATTENTION_PREFIX = 'lowfloat/attention/';
const IGNITION_ALERT_STATE_KEY = 'lowfloat/ignition-live-alerts.json';
...
const ignitionLiveKey = (date, at) => `${IGNITION_LIVE_PREFIX}${date}/${stamp(at)}.json`;
const attentionKey = date => `${ATTENTION_PREFIX}${date}.json`;
const writeIgnitionLiveSnapshot = (date, doc, at) => writeJSON(ignitionLiveKey(date, at), doc, 0);
const readAttentionDay = date => readJSON(attentionKey(date), null);
const writeAttentionDay = (date, doc) => writeJSON(attentionKey(date), doc, 0);
const readIgnitionAlertState = () => readJSON(IGNITION_ALERT_STATE_KEY, null);
const writeIgnitionAlertState = doc => writeJSON(IGNITION_ALERT_STATE_KEY, doc, 0);
...
module.exports = {
  ...
  IGNITION_LIVE_PREFIX, ATTENTION_PREFIX, IGNITION_ALERT_STATE_KEY,
  ...
  ignitionLiveKey, attentionKey,
  writeDiscoverySnapshot, writeIgnitionSnapshot, writeContinuationSnapshot,
  writeIgnitionLiveSnapshot, readAttentionDay, writeAttentionDay,
  readIgnitionAlertState, writeIgnitionAlertState,
