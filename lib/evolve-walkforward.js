...
//
// DEFAULT FLIPPED TO TRUE (alpha-research pass 3). It was opt-in, which meant the deflated
// Sharpe received `weights: null` and every cell's effective sample equalled its raw count
// — the correction was computed and REPORTED while never being APPLIED. Two reasons it is
// now the default:
//
//   1. TRAIN/SERVE CONSISTENCY. lib/evolve-routes.recomputePerf — the LIVE perf fit —
//      already defaulted `weighted: true`. The walk-forward that validates that fit ran
//      unweighted, so the research gate was strictly more permissive than the thing it
//      was validating. Same class of divergence as the ATLAS-X ranker.
//   2. IT IS THE CONSERVATIVE DIRECTION, and it binds. Measured on overlapping 21-bar
//      swing labels (50 tickers, 5-day step, n=3000): effN falls 3000 -> 542, and cells
//      at a 51-52% win rate that PASSED the deflated-Sharpe bar unweighted (dsr 0.994 /
//      0.9998) now FAIL it (0.856 / 0.936). Marginal edges were clearing on an inflated
//      sample size.
//
// Callers may still pass `weighted: false` for a deliberate unweighted comparison
// (op=evolvewf accepts `?uniqueness=0`).
function fitPerf(events, { weighted = true } = {}) {
...
function walkForward(events, { folds = DEFAULTS.folds, embargo = DEFAULT_EMBARGO, purge = true,
  minTrain = DEFAULTS.minTrain, minTest = DEFAULTS.minTest, weighted = true } = {}) {
...
function evaluate(events, { folds = DEFAULTS.folds, embargo = DEFAULT_EMBARGO, regimeAllow = null, weighted = true } = {}) {
  const all = events || [];
  ...
  return {
    ...
    deflatedSharpe: DSR.gridDeflatedSharpe(list, { weights: weighted ? U.uniquenessWeights(list) : null }),
...
async function runEvolveOmegaWalkForward({ scope = 'large', limit = 80, months = 18, step = 21,
  folds = DEFAULTS.folds, embargo = DEFAULT_EMBARGO, deadlineMs = 48000, now = null, volAdjust = false, regimeAllow = null, range = '2y', weighted = true } = {}) {
