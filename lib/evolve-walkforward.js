...
function walkForward(events, { folds = DEFAULTS.folds, embargo = DEFAULT_EMBARGO, purge = true,
  minTrain = DEFAULTS.minTrain, minTest = DEFAULTS.minTest } = {}) {
...
function evaluate(events, { folds = DEFAULTS.folds, embargo = DEFAULT_EMBARGO, regimeAllow = null } = {}) {
  const all = events || [];
  ...
  for (const h of L.EVOLVE_HORIZONS) {
    ...
    const purged = walkForward(evs, { folds, embargo, purge: true });
    const leaky = walkForward(evs, { folds, embargo, purge: false });
    byHorizon[h] = { n: evs.length, purged, leaky, verdict: verdictOf(purged) };
  }
  const pooledPurged = walkForward(list, { folds, embargo, purge: true });
  const pooledLeaky = walkForward(list, { folds, embargo, purge: false });
  ...
  return {
    version: WF_VERSION, margin: MARGIN, embargo, events: list.length,
    ...
    note: 'Purged+embargoed OOS over reconstructed triple-barrier labels. `leakageInflation` = how much a naive un-purged split flatters the mean OOS IC. Raw ensemble P (no in-fold calibrator).',
  };
}
...
async function runEvolveOmegaWalkForward({ scope = 'large', limit = 80, months = 18, step = 21,
  folds = DEFAULTS.folds, embargo = DEFAULT_EMBARGO, deadlineMs = 48000, now = null, volAdjust = false, regimeAllow = null, range = '2y' } = {}) {
  ...
  const evalOut = evaluate(events, { folds, embargo, regimeAllow });
  return { scope, generatedAt: now, volAdjust: !!volAdjust, backfillStats: stats, ...evalOut };
}

module.exports = {
  WF_VERSION, MARGIN, DEFAULT_EMBARGO,
  runEvolveOmegaWalkForward, evaluate, walkForward, fitPerf, scoreEvent, horizonWindow, verdictOf, labelClearsTestBlock, calDays, regimeOf, regimeHistogram,
};
