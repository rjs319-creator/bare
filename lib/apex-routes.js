...
async function runScoreboard(req, res) {
  ...
  return res.json({ configured: true, totalPicks: picks.length, loggedRows: rawPicks.length + rawGhost.length + cernPicks.length, groups: out, generatedAt: new Date().toISOString() });
}
...
module.exports = { runTrack, runScoreboard, runApexLog, runGhostLog, runEdgeLog, runEdgeBook, runVReversal, runVReversalTest, runDrift, runRecalibrate, runResearchOp, runExitsOp, runLongShortOp, runPeadOp, runBackfillOp, runModel, runNarrative, forwardReturn, forwardPath, summarizeReturns, cernPicksFrom, fadeRowsFrom };
