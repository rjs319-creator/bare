...
async function runScoreboard(req, res) {
  ...
  const sleeveRecs = {};   // { sleeveName: [{date, ret(fraction)}] } at the 1m horizon
  ...
  for (const p of picks) {
    ...
    for (const [hk, bars] of HORIZONS) {
      ...
      // collect the 1-month realized return per long sleeve (raw, not short-inverted)
      if (hk === '1m' && LONG_SLEEVE[p.section] && !p.short) {
        const nm = LONG_SLEEVE[p.section];
        (sleeveRecs[nm] = sleeveRecs[nm] || []).push({ date: p.date, ret: r / 100 });
      }
    }
  }
