...
async function runScoreboard(req, res) {
  ...
  const ledgerErrors = [];
  const load = (name, fn, fallback) => () => fn().catch((e) => {
  ...
  if (hasStore()) {
    ...
    if (ledgerErrors.length) {
      summaryWriteError = `skipped: ${ledgerErrors.length} ledger(s) unreadable (${ledgerErrors.map(e => e.ledger).join(', ')}) — evidence not rewritten from a degraded run`;
    } else {
