...
const ENTRY_BASIS_VERSION = 'entry-v2.1'; // versions the PRIMARY grading basis policy below
...
// Lead-only contracts keep the legacy basis; the daytrade section is pinned to legacy
// (FROZEN — nothing in this redesign may alter Day Trade's displayed record).
function entryBasisForSection(section) {
  ...
  if (!c || typeof c.fillPolicy !== 'string') return null;
...
function dateLevelNetExcess(arr, { horizonBars = null } = {}) {
  ...
  const dateRets = orderedDates.map(d => { const v = byDate.get(d); return v.reduce((s, b) => s + b, 0) / v.length; });
  ...
  const { neweyWest } = require('./research/stats-v3');
  const nw = neweyWest(dateRets, horizonBars != null ? { horizonBars } : {});
  ...
  return {
    ...
    seBasis: hacUsable ? `newey-west (lags ${nw.lags}, floored at IID)` : 'iid (too few dates for HAC)',
...
function summarizeReturns(arr, opts = {}) {
  ...
  return {
    ...
    dateNet: dateLevelNetExcess(arr, { horizonBars: opts.horizonBars ?? null }),
...
async function runScoreboard(req, res) {
  ...
  for (const p of picks) {
    ...
    for (const [hk, bars] of HORIZONS) {
      ...
      const benchRet = spyForwardReturn(benchCandles, p, bars, benchOpts);
