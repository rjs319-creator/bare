...
function dateLevelNetExcess(arr, { horizonBars = null } = {}) {
  ...
  const ded = ES.dedupeToDateSeries(arr || [], { pickValue: (r) => r.netExc, pickDate: (r) => r.date });
...
async function runScoreboard(req, res) {
  ...
  for (const p0 of [...rawPicks].sort(byDate)) {
    ...
    const tier = p0.section === 'momentum' ? momentumLedgerTier(p0) : p0.tier;
    const p = tier === p0.tier ? p0 : { ...p0, tier };
