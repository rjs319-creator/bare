...
const { writeDay, readAllPicks, hasStore, writeApexDay, readAllApex, writeGhostDay, readAllGhost, readAllTone, writeArchiveDay,
...
async function runScoreboard(req, res) {
  ...
  if (!rawPicks.length && !rawGhost.length && !rawFade.length && !cernPicks.length && !rawTone.length) {
  ...
  for (const p of picks) {
    ...
    for (const [hk, bars] of HORIZONS) {
      ...
      if (r == null) continue;
      // Market-relative: pick's (direction-adjusted) return minus SPY's over the
      // same window. null when SPY data is missing or the horizon hasn't elapsed.
      const spyRet = spyForwardReturn(spyCandles, p, bars);
      r.exc = (spyRet == null || !Number.isFinite(r.ret)) ? null : +(r.ret - spyRet).toFixed(2);
  ...
  return res.json({ configured: true, totalPicks: picks.length, loggedRows: rawPicks.length + rawGhost.length + cernPicks.length + rawTone.length, regimeSplit: !!macroLookup, groups: out, allocation, generatedAt: new Date().toISOString() });
}
