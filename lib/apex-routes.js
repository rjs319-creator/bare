...
async function runScoreboard(req, res) {
  ...
  if (hasStore()) {
    ...
    // reduce the accrued canonical screener episode stream (op=swingverify,
    // episodes/screener/*) into `fillVerification.screener` via episode-ledger's
    // deriveFillVerified — the ONLY channel lib/strategy-contracts.fillVerifiedFor
    // accepts. FAIL CLOSED: no accrued episodes, or an unreadable rollup, leaves the
    // key ABSENT and this summary byte-identical to the pre-pipeline one (maturity/
    // governance/eligibility behave exactly as before). Deliberately outside
    // `evidenceHash` (hashed over `groups` only), so its arrival cannot invalidate
    // version-matched promotion artifacts.
    try {
      const SV = require('./screener-verify');
      const fillVerification = SV.fillVerificationForSummary(await SV.readEpisodeRollup());
