...
async function runScoreboard(req, res) {
  ...
  return res.json({ configured: true, totalPicks: picks.length, loggedRows: rawPicks.length + rawGhost.length + cernPicks.length, regimeSplit: !!macroLookup, groups: out, generatedAt: new Date().toISOString() });
}
