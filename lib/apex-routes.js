...
const { writeDay, readAllPicks, hasStore, writeApexDay, readAllApex, writeGhostDay, readAllGhost, readAllEvidence, readAllIgnition, readAllOmega, readAllTone, readAllAttention, writeArchiveDay,
        ...
        readAllGapDays, readAllReadThroughDays, readAllGridlockDays, readAllAnomalyDays, readAllBiotechDays, readAllSecondWaveDays, readAllCrossAssetDays, readAllToneShiftDays, readAllInsiderClusterDays,
...
// The sections whose `exc` channel is a SECTOR-relative statistic (beat your
// peers, not merely the market).
const SECTOR_BENCH_SECTIONS = new Set(['ReadThrough', 'Gridlock', 'Anomaly', 'Biotech', 'SecondWave', 'CrossAsset', 'ToneShift']);

// Which benchmark return a row's `exc` may be measured against (F-04).
//
// A sector-labelled row whose sector-ETF history is missing used to fall back to
// SPY, quietly contributing a MARKET-relative number to a SECTOR-relative
// average. `benchFallback` counted how many rows did it — but a count does not
// un-blend a mean; the average had already mixed two rulers, the same defect
// #345 fixed one level up. Fail closed: no sector return means no sector-relative
// excess, so the row goes null and drops out rather than silently changing what
// the statistic means. The MARKET channel is unaffected — mktExc is always
// measured against SPY.
function excBenchmarkReturn(sectorRet, spyRet, sectorRequired) {
  if (sectorRequired) return Number.isFinite(sectorRet) ? sectorRet : null;
  return Number.isFinite(spyRet) ? spyRet : null;
}
...
// A YYYY-MM-DD that the session calendar says the market was actually open on.
// Weekends and full holidays are not decision dates.
function isTradingSessionDate(date) {
  const MS = require('./market-session');
  if (typeof date !== 'string' || !/^\d{4}-\d{2}-\d{2}$/.test(date)) return false;
  if (MS.FULL_HOLIDAYS.has(date)) return false;
...
function cernPicksFrom(cernState) {
  ...
  for (const ev of entries) {
    ...
    // DECISION DATE. New events carry `sessionDate` — the last COMPLETED regular
    // session as of detection. Legacy events have only `dateMs`, the detector's
    // wall clock, and op=cerntick runs daily INCLUDING WEEKENDS, so 37% of the
    // historical dates landed on non-trading days (8 Sundays + 2 Saturdays,
    // including the section's largest observation). Those dates are PRESERVED
    // rather than silently restated — this repo does not rewrite evidence
    // records — and flagged instead, so they can be excluded or recomputed as a
    // deliberate act. With entry:null the grader resolves a Sunday stamp to the
    // prior Friday's bar, so a Friday- and a Sunday-stamped event can share one
    // bar while counting as two independent decision dates.
    const legacyDate = new Date(ev.dateMs).toISOString().slice(0, 10);
    const date = ev.sessionDate || legacyDate;
    seen.set(key, {
      section: 'CERN',
      tier: ev.type,
      ticker: ev.symbol,
      date,
      sessionAligned: ev.sessionDate ? true : isTradingSessionDate(date),
...
function dateLevelNetExcess(arr, { horizonBars = null } = {}) {
  ...
  return {
    ...
    se: summary.se,
    // Full-precision copies (display `avg`/`se` above are rounded to 2/4dp). pValueOf
    // PREFERS these; without them the registry-wide Benjamini-Hochberg demote gate in
    // lib/maturity computed t from display-rounded values — a real ~0.004/cohort mean
    // rounds to 0.00 and scores p≈1, silently exempting exactly the small-magnitude
    // edges the gate exists to test. Three display consumers were fixed when
    // avgExact/seExact were introduced; this governance projection was the one missed.
    avgExact: summary.avgExact,
    seExact: summary.seExact,
    ci95: summary.ci95,
    ...
    // The per-date portfolio values themselves (one equal-weight number per
    // decision date, so bounded by the date count, not the pick count). Forwarded
    // for F-11: lib/cfl/prosecutor needs the SERIES, not its summary, to ask
    // whether the edge survives excising its best dates or is carried by one of
    // them. Dates are already this repo's independence unit, so they are the
    // honest granularity for that question.
    values: ded.series,
    // The matching dates, same order. Forwarded so a concentration finding can
    // name the SESSION carrying the tail instead of only its position in the
    // array — a verdict you cannot trace to a date is not actionable.
    dates: ded.dates,
...
async function runScoreboard(req, res) {
  ...
  const [
    ...
    rtDays, anomDays, icDays, bioDays, swDays, caDays, tsDays, downDayDays, gapDownDays,
    ...
    load('readAllInsiderClusterDays', () => readAllInsiderClusterDays(), []), // Form 4 cluster buys (CLUSTER / CLUSTER_EXCLUDED), shadow
  ...
  const rawIC = sectionRows(icDays, 'InsiderCluster');
  ...
  // Insider-cluster ledger → an "InsiderCluster" section (CLUSTER policy cohort vs the
  // CLUSTER_EXCLUDED control). Longs, next-open, graded on the `insidercluster` contract.
  for (const p of [...rawIC].sort(byDate)) {
    const key = `InsiderCluster:${p.tier}:${p.ticker}`;
    if (p.tier) firstSeen.add(key, p);
  }
  ...
  const SECTOR_BENCH = SECTOR_BENCH_SECTIONS;   // single source — see excBenchmarkReturn
  ...
  for (const p of picks) {
    ...
    const g = groups[gkey] || (groups[gkey] = { section: p.section, tier: p.tier, scope: p.scope || null, picks: 0, noHistory: 0, noHistoryTickers: [], benchFallback: 0, selfBenchmarked: 0, selfBenchmarkedTickers: [], sessionUnaligned: 0, regPicks: {}, h: {}, reg: {}, liq: {}, sec: {} });
    g.picks++;
    ...
    // Rows whose decision date is not a trading session (legacy CERN stamps).
    if (p.sessionAligned === false) g.sessionUnaligned = (g.sessionUnaligned || 0) + 1;
    ...
    for (const [hk, bars] of HORIZONS) {
      ...
      const sectorRet = sectorBenchCandles ? spyForwardReturn(sectorBenchCandles, p, bars, benchOpts) : null;
      const benchRet = excBenchmarkReturn(sectorRet, spyRet, SECTOR_BENCH.has(p.section));
  ...
  // `date` rides on EVERY lane (overall, byRegime, bySection): before 2026-09-09 only the
  // pooled lane carried it, so every per-section verdict fell back to the pooled-IID
  // descriptive and could never earn a HAC basis — yet that verdict is what the rank's
  // score-informativeness gate now consumes (lib/decision.js scoreInformativeness).
  const toRqItem = (s) => ({ score: s.cscore, outcome: s.ret, won: s.ret > 0, date: (s.pick && s.pick.date) || null });
  const rqItems = scoreItems.map(toRqItem);
  const scoreByRegime = {};
  if (macroLookup) for (const rk of REGIME_BUCKETS) {
    const seg = scoreItems.filter(s => s.regime === rk).map(toRqItem);
  ...
  for (const [sec, idxs] of Object.entries(secIdx)) {
    ...
    const seg = idxs.map(i => toRqItem(scoreItems[i]));
  ...
  const out = Object.values(groups).map(g => ({
    ...
    // Decision dates that were not trading sessions — see cernPicksFrom.
    sessionUnaligned: g.sessionUnaligned || 0,
  ...
  // and, since 2026-09-09, so the Today rank can scale a section score's spread by it
  // (lib/decision.js scoreInformativeness). Computed OUTSIDE the store branch because it
  // now ships on the API response too, not only on the persisted summary.
  const sectionDecile = compactSectionDecile(scoreBySection, SCORE_HK);
  // EVIDENCE-NEGATIVE LANES (2026-09-09): section:tier:scope lanes whose date-level
  // cost-net record at their OWN contract horizon is CI-negative, well sampled and
  // corroborated by an adjacent horizon (lib/negative-lanes.js). Ships on the response
  // and the persisted summary so Quick Hit / Opportunities can label such candidates
  // "excluded control" instead of printing a plan. Derived, never hardcoded.
  const negativeLanes = require('./negative-lanes').negativeLanes({ groups: out });
  ...
  if (hasStore()) {
    ...
    // SANCTIONED FILL-VERIFICATION REDUCTION (registry PROMOTION_CEILING exit path):
    // the accrued canonical screener episode stream (op=swingverify, episodes/
    // screener/*) reaches `fillVerification.screener` via episode-ledger's
    // deriveFillVerified — the ONLY channel lib/strategy-contracts.fillVerifiedFor
    // accepts. op=swingverify persists that reduction as a COMPACT doc alongside its
    // rollup (audit 2026-08-15: this used to download the whole ever-growing rollup
    // and re-derive on every scoreboard run); the summary reads the few-hundred-byte
    // doc only. FAIL CLOSED: no accrued episodes, or an absent/unreadable doc, leaves
    // the key ABSENT and this summary byte-identical to the pre-pipeline one
    // (maturity/governance/eligibility behave exactly as before). Deliberately outside
    // `evidenceHash` (hashed over `groups` only), so its arrival cannot invalidate
    // version-matched promotion artifacts.
    try {
      const fillVerification = await require('./screener-verify').readFillVerificationForSummary();
      if (fillVerification) summary.fillVerification = fillVerification;
    } catch { /* absent key = unchanged fail-closed behavior */ }
...
function compactSectionDecile(scoreBySection, horizon) {
  ...
  for (const [sec, v] of Object.entries(scoreBySection || {})) {
    ...
    out[sec] = {
      ...
      n: v.n, method: v.method, spread: v.topBottomSpread, horizon,
      verdictBasis: v.verdictBasis || null,
