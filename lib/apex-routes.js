...
async function runScoreboard(req, res) {
  ...
  for (const p of picks) {
    ...
    const g = groups[gkey] || (groups[gkey] = { section: p.section, tier: p.tier, picks: 0, regPicks: {}, h: {}, reg: {}, liq: {} });
    g.picks++;
    ...
    if (!candles) continue;
    let ret20 = null;   // 20-session forward return — the horizon the uniform score is validated on
    for (const [hk, bars] of HORIZONS) {
      const r = forwardPath(candles, p, bars);
      if (r == null) continue;
      if (hk === '20d' && Number.isFinite(r.ret)) ret20 = r.ret;
    ...
    if (raw != null && Number.isFinite(ret20)) scoreItems.push({ raw, ret: +ret20.toFixed(2), regime: bucket, section: p.section });
  ...
  // BOARD-WIDE SCORE-DECILE VALIDATION (#4 "performance by score decile"): percentile-rank
  // every scored pick's uniform conviction, then reuse the ranking-quality battery to ask
  // whether higher deciles actually earned better 20-session returns — one honest check
  // across the WHOLE book, not per-section. Reuses lib/rankquality (rank-IC, monotone
  // ladder, top-vs-bottom spread, verdict). Fills in once ≥20 picks have a resolved 20d.
  const { analyzeRankQuality } = require('./rankquality');
  const pcts = toPercentiles(scoreItems.map(s => s.raw));
  scoreItems.forEach((s, i) => { s.uscore = pcts[i]; });
  const rqItems = scoreItems.map(s => ({ score: s.uscore, outcome: s.ret, won: s.ret > 0 }));
  const scoreByRegime = {};
  if (macroLookup) for (const rk of REGIME_BUCKETS) {
    const seg = scoreItems.filter(s => s.regime === rk).map(s => ({ score: s.uscore, outcome: s.ret, won: s.ret > 0 }));
    if (seg.length >= 15) scoreByRegime[rk] = analyzeRankQuality(seg, { minN: 15 });
  }
  const scoreQuality = {
    model: PICKSCORE_VERSION, horizon: '20d', scoredPicks: scoreItems.length,
    overall: analyzeRankQuality(rqItems, { minN: 20 }), byRegime: scoreByRegime,
  };
