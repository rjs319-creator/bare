...
async function runScoreboard(req, res) {
  ...
  const benchTickers = [...new Set(picks.filter(p => p.section === 'ReadThrough' && p.bench).map(p => p.bench))];
  ...
  for (const p of picks) {
    ...
    for (const [hk, bars] of HORIZONS) {
      ...
      const benchCandles = (p.section === 'ReadThrough' && p.bench && benchHist.get(p.bench)) || spyCandles;
