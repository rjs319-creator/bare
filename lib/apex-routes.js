...
async function runScoreboard(req, res) {
  ...
  for (const p of picks) {
    ...
    const g = groups[gkey] || (groups[gkey] = { section: p.section, tier: p.tier, scope: p.scope || null, picks: 0, noHistory: 0, noHistoryTickers: [], benchFallback: 0, regPicks: {}, h: {}, reg: {}, liq: {}, sec: {} });
    g.picks++;
    ...
    for (const [hk, bars] of HORIZONS) {
      ...
      r.mktExc = (spyRet == null || !Number.isFinite(r.ret)) ? null : +(r.ret - spyRet).toFixed(2);
  ...
  if (hasStore()) {
    ...
    const summary = {
      ...
      groups: out.map(g => ({ section: g.section, tier: g.tier, scope: g.scope, picks: g.picks, noHistory: Number.isFinite(g.noHistory) ? g.noHistory : null, benchFallback: Number.isFinite(g.benchFallback) ? g.benchFallback : null, horizons: g.horizons })),
      sectionDecile,
    };
