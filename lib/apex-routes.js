...
async function runScoreboard(req, res) {
  ...
  if (!rawPicks.length && !rawGhost.length && !rawFade.length && !cernPicks.length && !rawTone.length && !rawAttn.length && !rawRT.length && !rawAnom.length && !rawBio.length && !rawSW.length && !rawCA.length && !rawTS.length && !rawDownDay.length && !rawGapDown.length && !rawDaytrade.length && !rawCoil.length) {
  ...
  for (const p of picks) {
    ...
    if (raw != null && Number.isFinite(ret20)) scoreItems.push({ pick: p, raw, ret: +ret20.toFixed(2), regime: bucket, section: p.section });
  ...
  const scoreQuality = {
    model: sectionscore.SECTIONSCORE_VERSION, proxyModel: PICKSCORE_VERSION, horizon: '20d',
