...
const { COST_MODEL_VERSION, BORROW_MODEL_VERSION, roundTripCostPct, borrowCostPct, tierForPick } = require('./costs');
...
const { writeDay, readAllPicks, hasStore, writeApexDay, readAllApex, writeGhostDay, readAllGhost, readAllIgnition, readAllOmega, readAllTone, readAllAttention, writeArchiveDay,
...
function cernPicksFrom(cernState) {
  ...
  for (const ev of entries) {
    ...
    // Anchor the pick DATE to the same bar the entry price was captured on. A
    // SIGNALED event carries ev.signal.{dateMs, entryPrice} where entryPrice is the
    // signal-day close; the event's own dateMs is the (earlier) detection date.
    // Using the event date while entering at the signal-day close measured the
    // forward window from a DIFFERENT bar than the entry, mismeasuring every
    // signaled CERN row. Anchoring the date to signal.dateMs aligns window-start
    // with entry and reconciles the Scoreboard with the internal κ engine, which
    // resolves from ev.signal.dateMs too. Unsignaled events keep the event date
    // with a null entry (forwardReturn then uses the close at that date, and the
    // next-open entry correction applies downstream).
    const anchorMs = (ev.signal && ev.signal.dateMs) || ev.dateMs;
    seen.set(key, {
      section: 'CERN',
      tier: ev.type,
      ticker: ev.symbol,
      date: new Date(anchorMs).toISOString().slice(0, 10),
      entry: (ev.signal && ev.signal.entryPrice) || null,
...
async function runScoreboard(req, res) {
  ...
  const rawOmega = await readAllOmega().catch(() => []);          // OMEGA-SWING picks (OMEGA_PRIME/QUALIFIED/WATCH), carry section/tier/date/entry
  if (!rawPicks.length && !rawGhost.length && !rawFade.length && !cernPicks.length && !rawTone.length && !rawAttn.length && !rawRT.length && !rawAnom.length && !rawBio.length && !rawSW.length && !rawCA.length && !rawTS.length && !rawDownDay.length && !rawGapDown.length && !rawDaytrade.length && !rawCoil.length && !rawIgnition.length && !rawOmega.length) {
  ...
  for (const p of [...rawOmega].sort(byDate)) {
    ...
    if (p.tier && !firstSeen.has(key)) firstSeen.set(key, { ...p, section: 'OMEGA' });
  }
  ...
  for (const p of picks) {
    ...
    for (const [hk, bars] of HORIZONS) {
      ...
      // Short sleeves (fade / gap-down / inverted-V) additionally pay the borrow-v1
      // stock-loan fee, prorated over this horizon (`bars` sessions). Longs pay zero
      // borrow, so their net is byte-identical to before. This stops the board from
      // over-crediting shorts that only look profitable because borrow was ignored.
      const costTier = tierForPick(p);
      const rtCost = roundTripCostPct(costTier);
      const isShortPick = p.short === true || p.tier === 'StrongSell';
      const borrow = isShortPick ? borrowCostPct(costTier, bars) : 0;
      r.net = Number.isFinite(r.ret) ? +(r.ret - rtCost - borrow).toFixed(2) : null;
      r.netExc = (r.exc == null) ? null : +(r.exc - rtCost - borrow).toFixed(2);
