...
async function runScoreboard(req, res) {
  ...
  if (!rawPicks.length && !rawGhost.length && !rawFade.length && !cernPicks.length) {
  ...
  return res.json({ configured: true, totalPicks: picks.length, loggedRows: rawPicks.length + rawGhost.length + cernPicks.length, regimeSplit: !!macroLookup, groups: out, allocation, generatedAt: new Date().toISOString() });
}
