...
const ENTRY_BASIS_VERSION = 'entry-v2';   // versions the PRIMARY grading basis policy below
...
// benchmark; conditional-trigger and lead-only contracts keep the legacy basis until a
// verified-fill engine grades them; the daytrade section is pinned to legacy (FROZEN —
// nothing in this redesign may alter Day Trade's displayed record).
function entryBasisForSection(section) {
  ...
  return (c && typeof c.fillPolicy === 'string' && c.fillPolicy.startsWith('next-session-open')) ? 'next-open' : null;
}
...
// DATE-LEVEL portfolio statistics over the NET-excess channel (maturity-v2).
...
// t-approx CI on the mean. This is the honest independence unit — same-day picks
// share the market factor and must not be counted as separate observations.
// Returns null when fewer than 2 dates carry the net-excess channel.
function dateLevelNetExcess(arr) {
  const byDate = new Map();
  for (const x of arr || []) {
    if (!x || !x.date || x.netExc == null || !Number.isFinite(x.netExc)) continue;
    if (!byDate.has(x.date)) byDate.set(x.date, []);
    byDate.get(x.date).push(x.netExc);
  }
  const dateRets = [...byDate.values()].map(v => v.reduce((s, b) => s + b, 0) / v.length);
  const n = dateRets.length;
  if (n < 2) return null;
  const mean = dateRets.reduce((s, b) => s + b, 0) / n;
  const sd = Math.sqrt(dateRets.reduce((s, x) => s + (x - mean) ** 2, 0) / (n - 1));
  const se = sd / Math.sqrt(n);
  return {
    n,
    avg: +mean.toFixed(2),
    sd: +sd.toFixed(2),
    ci95: { lo: +(mean - 1.96 * se).toFixed(2), hi: +(mean + 1.96 * se).toFixed(2) },
    basis: 'equal-weight per-date portfolio of net excess vs SPY',
  };
}
...
function summarizeReturns(arr) {
  ...
  return {
    ...
    dateNet: dateLevelNetExcess(arr),
...
async function runScoreboard(req, res) {
  ...
  const [
    ...
    () => readAllEvidence().catch(() => []),               // thesis-change signals (STRONG/MODERATE/WEAK)
    () => readAllTone().catch(() => []),                   // earnings-call tone (Bullish/Neutral/Bearish)
    () => readAllAttention().catch(() => []),              // fast-vs-sticky attention (Sticky / Fast)
    () => readAllFadeDays().catch(() => []),               // inverted-V SHORT setups, by day
    () => readCern().catch(() => null),                    // forced-flow event archive
    () => readAllReadThroughDays().catch(() => []),        // second-order beneficiaries (Fresh/Moved)
    () => readAllAnomalyDays().catch(() => []),            // no-news movers (Accumulation/Explained/Noise)
    () => readAllBiotechDays().catch(() => []),            // early biotech runners /100, benchmarked vs XBI
    () => readAllSecondWaveDays().catch(() => []),         // first-leg movers (Primed/Early/Faded)
    () => readAllCrossAssetDays().catch(() => []),         // cross-asset leads (Lead/Inline/Weak)
    () => readAllToneShiftDays().catch(() => []),          // earnings tone deltas (Brightening/Stable/Darkening)
    () => readAllDownDays().catch(() => []),               // oversold-bounce LONGS on red tapes
    () => readAllGapDownDays().catch(() => []),            // gap-down continuation SHORTS (short:true → inverted)
    () => readAllDaytradeDays().catch(() => []),           // Day-Trade ledger (A / B tiers), all longs
    () => readAllCoilDays().catch(() => []),               // Coil ledger (calibrated squeeze band as tier)
    () => readAllIgnition().catch(() => []),               // Momentum Ignition (IGNITION/WATCH)
    ...
    () => readAllOmega({ track: 'live' }).catch(() => []), // OMEGA-SWING prospective picks
    () => readAllGridlockDays().catch(() => []),           // GRIDLOCK physical-constraint candidates (Actionable/Tracked)
    () => readAllPeerPropDays().catch(() => []),           // Peer Propagation (EARLY/CONFIRMING stages)
    () => readAllUnderreactionDays().catch(() => []),      // News Underreaction (FRESH_* states; negatives short)
    () => readAllExpGapDays().catch(() => []),             // Expectation-Gap RISK_REDUCE days (SPY short-framed)
  ...
  const rawCoil = (coilDays || []).flatMap(dd => (dd.picks || []).map(p => ({ ...p, date: p.date || dd.date, section: 'coil', tier: p.band || (p.decile != null ? `D${p.decile}` : 'coil'), entry: p.entry != null ? p.entry : p.entryPrice })));
  ...
  for (const p of [...rawIgnition].sort(byDate)) {
    ...
    if (p.tier) firstSeen.add(key, { ...p, section: 'Ignition' });
  }
  ...
  for (const p of picks) {
    ...
    const g = groups[gkey] || (groups[gkey] = { section: p.section, tier: p.tier, scope: p.scope || null, picks: 0, regPicks: {}, h: {}, reg: {}, liq: {}, sec: {} });
    g.picks++;
    ...
    // basis-consistent benchmark. Conditional-trigger contracts (gapgo/gapdown/coil)
    // keep the legacy basis until their verified-fill engines grade them, and the
    // daytrade section is pinned to legacy behavior (FROZEN).
    ...
    for (const [hk, bars] of HORIZONS) {
      ...
      const benchRet = spyForwardReturn(benchCandles, p, bars, fpOpts);
      ...
      const secRet = secEtf ? spyForwardReturn(benchHist.get(secEtf), p, bars, fpOpts) : null;
  ...
  try {
    ...
    if (budgets && budgets.bindingReady && Array.isArray(budgets.budgets)) {
      const SECTION_BY_ID = { screener: 'Breakout', momentum: 'Momentum', ghost: 'Ghost', downday: 'DownDay', gapgo: 'Gap & Go' };
      routerCaps = {};
  ...
  if (hasStore()) {
    ...
    const summary = {
      ...
      groups: out.map(g => ({ section: g.section, tier: g.tier, scope: g.scope, picks: g.picks, horizons: g.horizons })),
      sectionDecile,
    };
  ...
  return res.json({ configured: true, totalPicks: picks.length, loggedRows: rawPicks.length + rawGhost.length + cernPicks.length + rawTone.length + rawAttn.length + rawRT.length + rawAnom.length + rawSW.length + rawCA.length + rawTS.length + rawDownDay.length + rawGapDown.length + rawDaytrade.length + rawCoil.length + rawIgnition.length + rawOmega.length, regimeSplit: !!macroLookup, groups: out, allocation, scoreQuality, costModel: { version: COST_MODEL_VERSION, roundTripPct: { liquid: roundTripCostPct('liquid'), small: roundTripCostPct('small'), micro: roundTripCostPct('micro'), biotech: roundTripCostPct('biotech') } }, entryModel: { version: ENTRY_MODEL_VERSION, basisVersion: ENTRY_BASIS_VERSION, basis: 'entry-v2: sections with a next-session-open contract fillPolicy are graded from the NEXT open (basis-consistent benchmark); conditional-trigger contracts (gapgo/gapdown/coil) and the frozen daytrade section keep the legacy logged-level/close basis until verified-fill engines grade them; the r.real lane (entry-v1) additionally reports next-open for every pick' }, generatedAt });
}
