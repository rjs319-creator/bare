...
const { writeDay, readAllPicks, hasStore, writeApexDay, readAllApex, writeGhostDay, readAllGhost, readAllTone, readAllAttention, writeArchiveDay,
        ...
        readAllGapDays, readAllReadThroughDays, readAllAnomalyDays,
...
async function runScoreboard(req, res) {
  ...
  if (!rawPicks.length && !rawGhost.length && !rawFade.length && !cernPicks.length && !rawTone.length && !rawAttn.length && !rawRT.length && !rawAnom.length) {
  ...
  const benchTickers = [...new Set(picks.filter(p => (p.section === 'ReadThrough' || p.section === 'Anomaly') && p.bench).map(p => p.bench))];
  ...
  for (const p of picks) {
    ...
    for (const [hk, bars] of HORIZONS) {
      ...
      const benchCandles = ((p.section === 'ReadThrough' || p.section === 'Anomaly') && p.bench && benchHist.get(p.bench)) || spyCandles;
  ...
  return res.json({ configured: true, totalPicks: picks.length, loggedRows: rawPicks.length + rawGhost.length + cernPicks.length + rawTone.length + rawAttn.length + rawRT.length + rawAnom.length, regimeSplit: !!macroLookup, groups: out, allocation, generatedAt: new Date().toISOString() });
}
