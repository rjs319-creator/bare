...
const { writeDay, readAllPicks, hasStore, writeApexDay, readAllApex, writeGhostDay, readAllGhost, readAllTone, readAllAttention, writeArchiveDay,
        ...
        readAllGapDays, readAllReadThroughDays, readAllAnomalyDays, readAllSecondWaveDays,
        readJSON, writeJSON } = require('./store');
...
async function runTrack(req, res) {
  ...
  return res.status(err ? 502 : 200).json({ ok: !err, date, count: picks.length, sp500, url, error: err, at: new Date().toISOString() });
}
...
async function runScoreboard(req, res) {
  ...
  if (!rawPicks.length && !rawGhost.length && !rawFade.length && !cernPicks.length && !rawTone.length && !rawAttn.length && !rawRT.length && !rawAnom.length && !rawSW.length) {
  ...
  const benchTickers = [...new Set(picks.filter(p => (p.section === 'ReadThrough' || p.section === 'Anomaly' || p.section === 'SecondWave') && p.bench).map(p => p.bench))];
  ...
  const LONG_SLEEVE = { screener: 'Breakout', momentum: 'Momentum', Ghost: 'Ghost' };
  ...
  for (const p of picks) {
    ...
    for (const [hk, bars] of HORIZONS) {
      ...
      const benchCandles = ((p.section === 'ReadThrough' || p.section === 'Anomaly' || p.section === 'SecondWave') && p.bench && benchHist.get(p.bench)) || spyCandles;
  ...
  return res.json({ configured: true, totalPicks: picks.length, loggedRows: rawPicks.length + rawGhost.length + cernPicks.length + rawTone.length + rawAttn.length + rawRT.length + rawAnom.length + rawSW.length, regimeSplit: !!macroLookup, groups: out, allocation, generatedAt: new Date().toISOString() });
}
