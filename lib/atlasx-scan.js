...
function eligibilityOf(stats, feats, newestDate) {
  ...
  if (newestDate && feats.lastDate < newestDate) {
    // staleness measured against the scan's own newest bar (session-count proxy
    // via calendar days — conservative for weekends).
    const gapDays = (Date.parse(newestDate) - Date.parse(feats.lastDate)) / 86400000;
    if (!(gapDays <= ELIGIBILITY.maxStaleSessions + 2)) return { eligible: false, reason: 'stale-bars' };
  }
