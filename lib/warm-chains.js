...
const CHAINS = {
  ...
  ledger: ['op=track', 'op=narrative', 'op=apexlog', 'op=ghostlog', '@decision'],
  ...
  universe: ['op=universescan&cursor=1&limit=150', 'op=universescan&cursor=1&limit=150', 'op=universecompile'],
...
async function runChain(name, { call, now = Date.now, deadlineMs = CHAIN_DEADLINE_MS } = {}) {
  ...
  for (const step of steps) {
    ...
    try {
      ...
      const ms = now() - t0;
      // A nested chain is DISPATCHED: its own invocation owns its budget, and it keeps
      // running even if this parent is killed before the response lands.
      const status = step.startsWith('@')
        ? 'dispatched'
        : (r && r.ok === false ? `http:${(r && r.status) || '?'}` : 'ok');
      done.push({ op: step, status, ms });
    } catch (e) {
  ...
  const failed = done.filter(s => s.status !== 'ok' && s.status !== 'dispatched' && s.status !== 'skipped:budget');
  return {
