...
const CHAINS = {
  ...
  atlasx: ['op=atlasxlog', 'op=atlasxresolve', 'op=premovelog'],
  ...
  // Evidence Consensus & Thesis Change — LLM extraction per active-attention name is heavy
  // (~40s of the 60s wall), so it gets its OWN root budget rather than starving a shared chain
  // (same rationale as router/researchgrade/altprobes). Bounded + per-ticker cached inside.
  evidence: ['op=evidencetick'],
...
const ROOT_CHAINS = ['ledger', 'capture', 'ticks1', 'ticks2', 'ticks3', 'aligned', 'universe', 'pulse', 'router', 'maturity', 'researchgrade', 'evolve', 'swing', 'atlasx', 'rlt', 'altprobes', 'swingsearch', 'evidence', 'pattern'];
