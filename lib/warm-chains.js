...
const CHAINS = {
  ...
  // by then track/apexlog have written today's ledger files, so the manifest
  // pins their real content hashes + the deploy SHA into the immutable `runs` chain.
  // `op=ghostlog` was removed 2026-08-13: ghost is RETIRED (registry `rejected`,
  // duplicate of the screener — measured independence credit 0.248). Its historical
  // ledger remains readable; a retired strategy accrues no new standalone evidence.
  ledger: ['op=track', 'op=narrative', 'op=apexlog', 'op=runmanifest', '@decision'],
  ...
  // op=challengerlog USED to run here, third hop deep. It self-fetches 18 endpoints and at
  // that depth every one of them failed — see the `challenger` root below.
  // op=challengerresolve is candle-heavy so it rides ticks3 instead.
  // No longer hands to @evolve — see the `evolve` root below for why it was detached.
  reprime: ['op=today', 'op=ensemble', 'op=orbitlog', 'op=orbitmltick'],
  ...
  // CHALLENGER LOG: its own root, for exactly the reason `evolve` above is one.
  //
  // It sat at `ledger → @decision → @reprime → op=challengerlog`, three hops deep, and
  // self-fetches EIGHTEEN endpoints to gather its board — including op=today, which fans
  // out to sub-endpoints of its own. Every night it returned 503 "no ranked signals
  // gathered". The diagnostic added on 2026-08-20 made it unambiguous:
  //
  //     no ranked signals gathered (0/18 answered, 0 barren) — in 2002 ms
  //
  // 0 of 18, in ~110 ms each. That is not a data problem and not a timeout — the fetch
  // budget is 25s with a retry, and every one of those sources returns 200 with a real
  // payload when called directly (verified on prod 2026-08-21: gapgo 57KB, daytrade 387KB,
  // coremo 3.7MB, coil, gapdown, biotech, downday, screener — all 200). The sources are
  // healthy; the CALLING CONTEXT was not. Same shallow-works/deep-fails signature that
  // `evolve` showed at the same depth with the same self-fetch fan-out.
  //
  // The 503 itself was always correct: refusing to log an empty board is the right call.
  // It just meant the shadow challenger ledger accrued nothing, every night, silently
  // enough that the failure was once misread as an OOM symptom.
  //
  // TRADE-OFF, identical to evolve's: as a root it races the decision spine and may read
  // an op=today scored on the previous cron's model. For a shadow ledger that is
  // negligible against logging nothing at all. Placed LAST in ROOT_CHAINS so its dispatch
  // wave is the latest, giving the spine the most time to re-prime before it gathers.
  challenger: ['op=challengerlog'],
  ...
  // op=challengereval&force=1 runs strictly AFTER op=challengerresolve: the eval is a
  // cache that only recomputes when FORCED, and no cron forced it before 2026-08-11 — so
  // it sat frozen at its first-ever computation (n=0, 2026-07-19) while 109 resolved
  // outcomes accrued invisibly. Forcing it nightly, after resolution, keeps the shadow
  // challenger's promotion evidence current.
  ticks3: ['op=leaderboardtick&src=confluence', 'op=corebuild', 'op=corelog', 'op=coredrift', 'op=attentiontick', 'op=tonetick&limit=6', 'op=challengerresolve', 'op=challengereval&force=1', 'op=orbitresolve', 'op=orbitmlresolve'],
  ...
  // slot=N (not cursor=1): the Blob cursor overwrite propagates with a 10-30s+ read-back
  // lag, so back-to-back cursor steps read the SAME cursor and silently re-scanned the
  // same slice — halving nightly coverage. Slots derive the slice deterministically from
  // (epoch day, slot): nothing stored, nothing to lag, same-night slots never collide.
  // 8 slots/night must match SLOTS_PER_NIGHT in lib/universe-routes.js (pinned by test).
  universescan1: ['op=universescan&slot=0&limit=200', 'op=universescan&slot=1&limit=200'],
  universescan2: ['op=universescan&slot=2&limit=200', 'op=universescan&slot=3&limit=200'],
  universescan3: ['op=universescan&slot=4&limit=200', 'op=universescan&slot=5&limit=200'],
  universescan4: ['op=universescan&slot=6&limit=200', 'op=universescan&slot=7&limit=200'],
  ...
  // OMEGA R10-vs-SCORE prospective shadow A/B (weight-0): stamp both orderings of the
  // frozen options-liquid cohort at the close, resolve matured days. One idempotent op —
  // a re-dispatch is a no-op (write-once day docs).
  omegaab: ['op=omegaabtick'],
  ...
  // 424B5 DILUTION FLAG (shadow, weight-0). Two sequential steps, no shared cursor:
  // the tick refreshes the flag snapshot from EDGAR (~8 windowed FTS calls — not FMP,
  // so it cannot contribute to the 22:00 FMP burst-429s) and appends the write-once
  // ledger day; the resolve scores ONE matured day (≥9 calendar days old, ≤60 candle
  // fetches). Both idempotent; a re-dispatch is a no-op.
  dilution: ['op=dilutiontick', 'op=dilutionresolve'],

  // INSIDER CLUSTER BUYS (shadow, weight-0). One step: EDGAR daily index (tick day + the
  // day before) → universe Form 4 ownership XMLs (≤10 req/s, 120s budget, truncation
  // counted) → FROZEN clusters → write-once tx shard + ledger day. No FMP, no Yahoo burst
  // beyond ≤40 6mo histories at concurrency 3. Idempotent: an existing day doc is a no-op.
  insidercluster: ['op=insiderclustertick'],

  // STOCKTWITS BULL-RATIO FLAG (shadow, weight-0). Same two-step shape as dilution:
  // the tick reads StockTwits trending + ~30 symbol streams (no FMP, no Yahoo) and
  // appends the write-once ledger day of fresh ≥90%-bull crossings; the resolve DOES
  // spend a Yahoo candle fan-out (SPY + ≤60 names, bounded to concurrency 4) inside
  // the 22:00 window — count stbull in any Yahoo burst audit. Both steps idempotent;
  // the tick is a no-op on non-trading days.
  stbull: ['op=stbulltick', 'op=stbullresolve'],

  // CFR PROSPECTIVE SHADOW (weight-0). The tick fetches ~530 Yahoo daily series (3y)
  // and runs a ridge inference pass (~35s measured end-to-end); the resolve re-fetches
  // one matured day's cross-section (6mo). The heaviest Yahoo spender in the window —
  // count it FIRST in any burst audit. Both steps idempotent; the tick is a no-op on
  // non-trading days and when the benchmark has not settled the session.
  forecastshadow: ['op=forecastshadowtick', 'op=forecastshadowresolve'],

  // STRUCTURED BEAR CASE (display-only, weight-0). One step: self-fetches op=today and
  // makes ONE bounded Haiku call over the top board rows, persisting bearcase/<date>.json
  // (idempotent per day — a re-dispatch is a no-op). Runs post-close so it argues against
  // the board the day actually served.
  bearcase: ['op=bearcasetick'],

  // TECH OPERATIONAL EVIDENCE (research layer, weight-0). Two sequential steps, no
  // shared cursor: the tick collects official public sources (npm/GitHub/SEC/job
  // boards/status pages — none touch FMP, so it cannot contribute to the 22:00 FMP
  // burst-429s), folds series, derives signals and appends forward events; the resolve
  // scores matured ledger days (≥16 calendar days old, bounded candle fetches).
  // Both idempotent; a re-dispatch is a no-op. Backfill (op=techevbackfill) is
  // deliberately manual-only and NOT in any chain.
  techev: ['op=techevtick', 'op=techevresolve'],
  ...
  // PROMOTION_CEILING exit path for the Early:large cohort). OWN ROOT rather than a
  // tail on ticks1: it fans out bounded FMP 5-min fetches (≤25) plus bounded daily
  // candle fetches (≤25), and ticks1 already drains op=gapgoverify's own 25-call FMP
  // sweep — stacking a second sequential FMP sweep behind eight ticks risks the
  // documented tail-starvation, and racing two FMP sweeps inside ONE chain invocation
  // is fine but sharing a chain buys no ordering: swingverify only grades sessions
  // AFTER a pick's publication day, so tonight's op=track output is never tonight's
  // input (there is no track→swingverify dependency to honour). Idempotent per
  // (date,ticker) — a re-dispatch cannot double-grade; a budget-skip self-heals next
  // run inside the 8-day retry window. SHADOW-SAFE: it writes only episodes/screener/*;
  // grades change only when the Scoreboard reduction later derives real verification.
  swingverify: ['op=swingverify'],
};
...
const ROOT_CHAINS = ['ledger', 'capture', 'ticks1', 'ticks2', 'ticks3', 'aligned', 'universe', 'pulse', 'router', 'maturity', 'researchgrade', 'evolve', 'swing', 'atlasx', 'rlt', 'altprobes', 'govdemand', 'gridlock', 'swingsearch', 'pattern', 'patternresearch', 'cfl', 'psrl', 'peerprop', 'expgap', 'pitdata', 'alphacal', 'gexa', 'vrp', 'ephemeral', 'optionsv2', 'pulse2', 'omegaab', 'swingverify', 'dilution', 'techev', 'stbull', 'bearcase', 'forecastshadow', 'insidercluster', 'challenger'];

// budgetMs (nested dispatch only): the parent's REMAINING deadline budget, passed down so
// an awaited child cannot claim a fresh full deadline of its own. Without it a parent that
// spent 200s could await a child entitled to 240s more — blowing the parent's 300s wall
// mid-await, which kills the invocation with NO chain report (the silent-loss mode the
// 2026-08-06 postmortem above exists to prevent).
const pathFor = (step, budgetMs) => (step.startsWith('@')
  ? `/api/tracker?op=warmchain&name=${step.slice(1)}${Number.isFinite(budgetMs) ? `&budgetMs=${Math.max(0, Math.round(budgetMs))}` : ''}`
  : `/api/tracker?${step}`);

// ── Dispatch staggering ─────────────────────────────────────────────────────
// Firing every root chain simultaneously packed 30+ concurrent /api/tracker invocations
// (plus their self-fetch fan-outs) onto shared Fluid instances, and the instance was
// OOM-killed at 22:0x UTC on EVERY cron run 2026-08-07 → 2026-08-11 (Vercel runtime
// error "instance was killed because it ran out of available memory"). Each kill took
// down every in-flight invocation sharing the instance — which is why the failing ops
// varied night to night (ledger/patternlog/downdaytick/orbitresolve on 08-10) and why
// every one of them succeeded when triggered alone.
//
// Warm therefore dispatches the roots in WAVES: chain i starts after
// dispatchDelayMs(i). Peak concurrency drops ~4x while total dispatch still completes
// by ~60s — well inside warm's 280s drain. The long decision spine (`ledger`) is first
// in ROOT_CHAINS, so the one chain that needs its full deadline starts in wave 0; a
// last-wave chain that used its FULL 240s budget would report just past warm's drain
// and be recorded "running-past-warm" (honest, and the work itself is unaffected).
// HALVED 8/20s -> 4/10s on 2026-08-19. The waves were already staggering dispatch, but
// api/tracker was still OOM-killed 24 times in 11 days, with five kills landing in the
// SAME second — one Fluid instance dying and taking its co-tenants with it (atlasx,
// ticks3 and swing all returned 500 with complete:null on 2026-08-18). Eight heavy
// invocations arriving together is still too tall a peak. Halving BOTH numbers keeps the
// average dispatch rate identical (4 per 10s == 8 per 20s), so the last wave still lands
// well inside warm's drain, while halving how many land on one instance at once.
// GAP 10s -> 9s on 2026-09-19. The 40th root (index 39) landed exactly on the 90s ceiling
// that test/warm-chains.test.js pins, so the 41st root (forecastshadow) had nowhere to go.
// Trimming the gap by one second keeps the wave WIDTH at 4 — the OOM lesson above is about
// how many land on one instance at once, not about the second they land in — and moves
// the ceiling to 44 roots (index 43 -> wave 10 -> 90s). Beyond that, nest under a parent.
const DISPATCH_WAVE_SIZE = 4;
const DISPATCH_WAVE_GAP_MS = 9000;
const dispatchDelayMs = (index, { waveSize = DISPATCH_WAVE_SIZE, waveGapMs = DISPATCH_WAVE_GAP_MS } = {}) =>
  Math.floor(Math.max(0, index) / Math.max(1, waveSize)) * Math.max(0, waveGapMs);

// Run one named chain. `call(path) -> {ok, status}` and `now()` are injected so this is
// testable without a network or a wall clock.
// Pull a human-readable reason out of a failed step's response: the app's own JSON
// error shape first, then a snippet of whatever the body actually was.
function stepError(r) {
  const b = r && r.body;
  if (b && typeof b === 'object') {
    const msg = b.error || b.reason || b.message;
    if (msg) return String(msg).slice(0, 200);
  }
  const raw = r && r.raw;
  return raw ? String(raw).replace(/\s+/g, ' ').trim().slice(0, 200) : null;
}
...
async function runChain(name, { call, now = Date.now, deadlineMs = CHAIN_DEADLINE_MS } = {}) {
  ...
  for (const step of steps) {
    ...
    const t0 = now();
    try {
      const r = await call(pathFor(step, step.startsWith('@') ? Math.max(0, deadlineMs - (t0 - started)) : undefined));
      const ms = now() - t0;
      if (step.startsWith('@')) {
        ...
        // The failing op's OWN reason is in the body `call` already parsed. Dropping it is
        // why every 500 on the 2026-08-17 cron reported `error: null` in op=health and
        // could only be diagnosed by hand-reading Vercel logs.
        done.push({ op: step, status: `http:${r.status || '?'}`, ms, error: stepError(r) });
      } else {
        done.push({ op: step, status: 'ok', ms });
      }
    } catch (e) {
...
module.exports = { CHAINS, ROOT_CHAINS, CHAIN_DEADLINE_MS, pathFor, runChain, dispatchDelayMs, DISPATCH_WAVE_SIZE, DISPATCH_WAVE_GAP_MS };
