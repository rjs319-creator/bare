...
const CHAINS = {
  ...
  // self-cursoring — each step advances one ~25-name slice of the 2y study, the final
  // slice triggers the evaluate that writes pattern/evidence.json (the artifact every
  // family gate + calibrated probability depends on, which had never been built), and
  // once built the steps idle until the quarterly rebuild window. Three steps per night
  // ≈ 75 names, so a cold build completes in ~10 nights. OWN ROOT (unknown-cost candle
  // sweeps must not budget-starve patternlog/patterngrade in the `pattern` chain).
  patternresearch: ['op=patternresearch&mode=auto', 'op=patternresearch&mode=auto', 'op=patternresearch&mode=auto'],
...
const ROOT_CHAINS = ['ledger', 'capture', 'ticks1', 'ticks2', 'ticks3', 'aligned', 'universe', 'pulse', 'router', 'maturity', 'researchgrade', 'evolve', 'swing', 'atlasx', 'rlt', 'altprobes', 'govdemand', 'gridlock', 'swingsearch', 'pattern', 'patternresearch', 'peerprop', 'expgap', 'pitdata', 'alphacal', 'gexa', 'vrp', 'ephemeral', 'optionsv2', 'pulse2'];
