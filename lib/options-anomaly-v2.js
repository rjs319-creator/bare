...
function buildBaselines(historyDocs, { beforeSession = null } = {}) {
  ...
  for (const [t, acc] of perTicker) {
    ...
    out.set(t, {
      ...
      // RAW prior-session ATM IV series (swing bucket), strictly point-in-time by the
      // same `beforeSession` exclusion as everything else here. `robustStats` above
      // reduces to median/MAD, which cannot express a RANK or a PERCENTILE — those need
      // the distribution. Bounded by the baseline window, so this is a few dozen numbers.
      atmIVSeries: ts.atmIV.filter(v => v != null && Number.isFinite(v)),
    });
  }
