...
const CONTRACTS = {
  ...
  // INSIDER CLUSTER BUYS (2026-09, shadow). The retrospective (research/98) passed its frozen
  // gates on a matched-peer residual but was FLAT vs SPY and carried by 1% of events; the
  // prospective contract therefore grades the absolute cost-net record vs SPY at 21 sessions
  // ('1m') on next-open fills, and promotion additionally requires the robust gates named in
  // research/INSIDER-CLUSTER-RESIDUAL-2026-09.md.
  insidercluster: { scoringVersion: 'insidercluster-v1', horizon: 'position', metric: '1m', side: 'long',
    trigger: 'Form 4 open-market cluster buy (≥2 insiders / 14 days / ≥$50k; event = latest filing date; joint filings count once)',
    fillPolicy: 'next-session-open', fillVerified: false,
    stopPolicy: 'none', targetPolicy: 'none', timeExitSessions: 21, benchmark: ['SPY', 'sector'],
    costPolicy: 'tier-by-dollar-volume', borrowPolicy: null, episodeCooldownSessions: 21,
    promotionMetric: '1m', researchHorizons: ['5d', '1m', '3m'],
    primaryLabel: 'cost-net excess vs SPY @1m (absolute co-primary; the matched-peer residual is research-only)' },
  ...
  // Ticker-lookup horizon reads (alpha-research pass 3). fillVerified:false throughout —
  // these publish an ATR-derived risk reference, not an executed plan, and no episode
  // ledger has ever been reduced for them. Each is graded on its OWN clock; the dual-read
  // ledger (lib/dualread-backfill.js) is the venue that must accrue the record.
  'lookup-intraday': { scoringVersion: 'signal-v1', horizon: 'intraday', metric: '1d', side: 'long',
    trigger: 'stacked-EMA / VWAP / MACD / RSI composite crossing its action threshold',
    fillPolicy: 'next-session-open', fillVerified: false, stopPolicy: 'published stop (1.5x ATR)',
    targetPolicy: 'published target (2.5x ATR)', timeExitSessions: 1, benchmark: ['SPY'],
    costPolicy: 'tier-by-dollar-volume', borrowPolicy: null,
    episodeCooldownSessions: 1, primaryLabel: 'cost-net excess vs SPY @1d' },
  'lookup-swing': { scoringVersion: 'swing-v1', horizon: 'swing', metric: '5d', side: 'long',
    trigger: 'trend/RS/participation composite crossing its action threshold',
    fillPolicy: 'next-session-open', fillVerified: false, stopPolicy: 'published stop (daily ATR)',
    targetPolicy: 'published target (daily ATR)', timeExitSessions: 5, benchmark: ['SPY', 'sector'],
    costPolicy: 'tier-by-dollar-volume', borrowPolicy: null,
    episodeCooldownSessions: 5, primaryLabel: 'cost-net excess vs SPY and sector @5d' },
  'lookup-longterm': { scoringVersion: 'longterm-v1', horizon: 'position', metric: '1m', side: 'long',
    trigger: '7-factor ~1y trend composite crossing its action threshold',
    fillPolicy: 'next-session-open', fillVerified: false, stopPolicy: 'none published',
    targetPolicy: 'none published', timeExitSessions: 21, benchmark: ['SPY'],
    costPolicy: 'tier-by-dollar-volume', borrowPolicy: null,
    episodeCooldownSessions: 21, primaryLabel: 'cost-net excess vs SPY @1m' },
...
function fillVerifiedFor(id, evidence = null) {
  ...
  return {
    ...
    resolved: null,
    // The registry-level reason this is expected rather than anomalous. Carried on every
    // fail-closed verdict so a reader of ONE strategy's grade sees the systemic blocker
    // instead of concluding that this particular strategy underperformed.
    // See lib/strategy-registry.PROMOTION_CEILING.
    ceiling: require('./strategy-registry').PROMOTION_CEILING,
  };
}
