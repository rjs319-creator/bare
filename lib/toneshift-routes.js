...
// DATA_BASIS is surfaced so the proxy is never mistaken for the real thing: raw
// earnings-call transcripts are NOT available on this data tier, so the delta is
// read from PUBLIC COVERAGE of the two calls via web search — coverage tone, not
// transcript tone. It also means the search can surface post-call analyst reaction,
// so treat any read as a coverage-derived lead, not a point-in-time transcript delta.
...
async function runToneShift(req, res) {
  ...
  if (cached) {
    ...
    return res.json({ ok: true, cached: true, stale, disclaimer: DISCLAIMER, dataBasis: DATA_BASIS, proxyNote: PROXY_NOTE, ...cached, ageMins });
  }
  res.setHeader('Cache-Control', 'no-store');
