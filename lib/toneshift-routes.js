...
// DATA_BASIS is SURFACED, not just commented, so the proxy is never mistaken for the real
// thing. lib/toneshift.js already records why (FMP transcript endpoints do not work on this
// tier, so the delta is assessed by web_search over coverage of both calls) — but a code
// comment does not reach the consumer of the payload. Reading coverage rather than the
// transcript also means post-call analyst reaction can leak into the read, so this is a
// coverage-tone lead, NOT a point-in-time transcript delta.
const DATA_BASIS = 'coverage-proxy';
const PROXY_NOTE = 'Assessed from public COVERAGE of the calls (web search), NOT raw transcripts — transcripts are unavailable on this data tier. This is a coverage-tone proxy and may reflect post-call commentary; it is not a point-in-time transcript delta.';
const DISCLAIMER = 'The DELTA in an earnings call\'s tone vs the prior quarter — management shifting from hedged to confident (or the reverse) before the numbers catch up. BRIGHTENING = more upbeat/specific; DARKENING = more cautious. A slower SWING-horizon LEAD, forward-tracked — not a buy signal. NOTE: read from public coverage of the calls (web search), not raw transcripts — a coverage-tone proxy.';
...
async function runToneShift(req, res) {
  ...
  if (cached) {
    ...
    return res.json({ ok: true, cached: true, stale, disclaimer: DISCLAIMER, ...cached, ageMins, dataBasis: DATA_BASIS, proxyNote: PROXY_NOTE });
  }
  res.setHeader('Cache-Control', 'no-store');
  return res.json({ ok: false, error: 'warming up — building the tone-shift scan (try Refresh in a moment)', items: [], disclaimer: DISCLAIMER, dataBasis: DATA_BASIS, proxyNote: PROXY_NOTE });
}

module.exports = { runToneShift, runToneShiftTick, detect, tierFor, logSurfaced, CACHE_KEY, DISCLAIMER, DATA_BASIS, PROXY_NOTE };
