...
const { researchVerdict } = require('./omega-research-verdict');
...
function shortlistFromToday(today) {
  ...
  for (const [h, arr] of Object.entries(horizons)) for (const s of arr || []) {
    // Day-Trade names are excluded (2026-08-09): the daytrade section's own resolved
    // ledger shows them collapsing over exactly this funnel's hold window — cost-net
    // vs SPY −6.3% @5d / −7.2% @10d / −14.8% @20d with date-level CIs entirely
    // negative. An intraday burst is not a swing-continuation candidate.
    if (s.source === 'daytrade') continue;
...
// Cap on how many carried-forward (funnel-displaced) episodes we re-fetch + re-score per call.
const CARRY_FORWARD_MAX = 40;

// Join-back helper (Defect B): from the flat list of previously-published live-ledger picks,
// pick the MOST RECENT published episode per ticker — deduped by ticker+signalDate, NOT by
// observationId (observationId embeds the tier and fragments across a tier change). Keep only
// tickers ABSENT from today's current shortlist (those still in the funnel are scored normally),
// most-recent first, bounded by `cap`. Pure and store-agnostic.
function carryForwardEpisodes(livePicks, shortlistTickers, cap = CARRY_FORWARD_MAX) {
  const present = shortlistTickers instanceof Set ? shortlistTickers : new Set(shortlistTickers || []);
  const byTicker = new Map();
  for (const p of livePicks || []) {
    if (!p || !p.ticker || !p.signalDate) continue;
    const cur = byTicker.get(p.ticker);
    // Most recent signalDate wins; same ticker+signalDate collapses (dedup) regardless of tier.
    if (!cur || String(p.signalDate) > String(cur.signalDate)) byTicker.set(p.ticker, p);
  }
  const out = [];
  for (const [ticker, ep] of byTicker) { if (!present.has(ticker)) out.push(ep); }
  out.sort((a, b) => String(b.signalDate).localeCompare(String(a.signalDate)));
  return out.slice(0, cap);
}

// Map a persisted live-ledger episode back into the op=today signal shape buildOmega consumes,
// so a carried-forward name is re-scored through the SAME evaluateCandidate/buildOmega path.
function episodeToSignal(ep) {
  return {
    ticker: ep.ticker, sector: ep.sector || null, company: ep.company || null,
    score: ep.score != null ? +ep.score : 0,
    strategyFamily: ep.candidateSource || null,
    sources: Array.isArray(ep.sources) ? ep.sources : (ep.candidateSource ? [ep.candidateSource] : []),
    catalyst: ep.catalyst || null,
  };
}

// Build the LIVE OMEGA payload: today's shortlist PLUS carried-forward open episodes joined back
// from the omega/live ledger (Defect B). Best-effort — a ledger read or candle fetch failure must
// never break the screen. Shared by op=omega and op=omegalog so both monitor the same set.
async function buildLiveOmegaPayload(todayData) {
  const shortlist = shortlistFromToday(todayData);
  const shortlistTickers = new Set(shortlist.map(s => s.ticker));
  let carriedSignals = [];
  try {
    const livePicks = await S.readAllOmegaLive();               // omega/live ONLY — never research
    carriedSignals = carryForwardEpisodes(livePicks, shortlistTickers).map(episodeToSignal);
  } catch { carriedSignals = []; }
  const allSignals = [...shortlist, ...carriedSignals];
  const { map, bench } = await fetchCandleSet(allSignals.map(s => s.ticker), allSignals.map(s => s.sector));
  const carriedTickers = new Set(carriedSignals.map(s => s.ticker));
  const payload = buildOmega(allSignals, map, bench, todayData.regime || {}, { carriedTickers });
  return { payload, shortlist, carriedCount: carriedSignals.length };
}
...
function buildOmega(signals, candlesByTicker, benchCandles, regime = {}, opts = {}) {
  ...
  const carried = opts.carriedTickers instanceof Set ? opts.carriedTickers : new Set(opts.carriedTickers || []);
  ...
  signals.forEach((sig, i) => {
    ...
    // Carry-forward tagging (Defect B): a name re-injected from the persisted omega/live ledger
    // because it dropped out of today's top-60 shortlist is still MONITORED, not silently dropped.
    if (carried.has(sig.ticker)) {
      card.funnelDisplaced = true;
      card.carryReason = 'funnel displacement (dropped below the top-60 shortlist)';
    }
  ...
  return {
    ...
    // The survivorship-free research verdict, surfaced in the banner (one fetch). Recorded snapshot.
    researchEvidence: researchVerdict(),
...
async function runOmega(req, res) {
  ...
  const { payload, carriedCount } = await buildLiveOmegaPayload(today.data);
  payload.freshness = { today: today.ok, generatedAt: new Date().toISOString(), carriedForward: carriedCount };
  payload.configured = S.hasStore();
  // ⚡ GRIDLOCK shadow evidence family (physical_constraint): ANNOTATION ONLY. Weight-0,
  // never re-ranks, never boosts — it lists which OMEGA candidates also carry independent
  // physical-constraint evidence so the overlap can be studied prospectively. Fail-soft:
  // any error leaves the payload exactly as before. Momentum is NOT double-counted — the
  // gridlock score contains no momentum/RS/volume features (its timing is OMEGA's own).
  try {
    const gl = require('./gridlock-routes');
    if (gl.resolveGridlockMode() !== 'off') {
      const latest = await S.readJSON(gl.KEYS.latest, null);
      const cands = (latest && Array.isArray(latest.candidates)) ? latest.candidates : [];
      const inOmega = new Set((payload.cards || []).map(c => c.ticker));
      payload.evidenceFamilies = {
        ...(payload.evidenceFamilies || {}),
        physical_constraint: {
          family: 'physical_constraint', weight: 0, status: 'SHADOW',
          probability: null, portfolioEligible: false,
          source: 'gridlock-v1 (op=gridlock)',
          candidates: cands.slice(0, 20).map(c => ({
            ticker: c.ticker, classification: c.classification, scoreTotal: c.score && c.score.total,
            gateStatus: c.gate && c.gate.status, overlapsOmegaShortlist: inOmega.has(c.ticker),
          })),
          note: 'Independent causal physical evidence only — timing/momentum stays OMEGA\'s; annotation never affects ranks.',
        },
      };
    }
  } catch { /* annotation is optional; OMEGA payload unchanged on any failure */ }
  // 🕸 PEER PROPAGATION shadow evidence family: ANNOTATION ONLY, same contract as
  // GRIDLOCK above — weight-0, fail-soft, never re-ranks. The peer engine's features
  // are built on PEER/LEADER residuals, not the candidate's own momentum, so the
  // family stays independent of OMEGA's own timing evidence; the one own-name input
  // (its normalized reaction) is used to DEMOTE late names, never to boost.
  try {
    const pp = await S.readJSON(require('./peerprop/config').STORE.latest, null);
    const cands = (pp && Array.isArray(pp.candidates)) ? pp.candidates : [];
    const inOmega = new Set((payload.cards || []).map(c => c.ticker));
    if (cands.length) {
      payload.evidenceFamilies = {
        ...(payload.evidenceFamilies || {}),
        peer_propagation: {
          family: 'peer_propagation', weight: 0, status: 'SHADOW',
          probability: null, portfolioEligible: false,
          source: 'peerprop-v1 (op=peerprop)',
          candidates: cands.filter(c => c.stage === 'EARLY' || c.stage === 'CONFIRMING').slice(0, 20).map(c => ({
            ticker: c.ticker, stage: c.stage, score: c.score,
            unreflectedPeerStrength: c.features && c.features.unreflectedPeerStrength,
            leadersConfirming: c.features && c.features.leadersConfirming,
            overlapsOmegaShortlist: inOmega.has(c.ticker),
          })),
          note: 'Peer/leader residual evidence only — timing stays OMEGA\'s; annotation never affects ranks.',
        },
      };
    }
  } catch { /* annotation is optional; OMEGA payload unchanged on any failure */ }
  // PSRL (weight-0 SHADOW annotation): longer-horizon continuity + residual
  // leadership over the SAME price paths OMEGA reads — one correlated evidence
  // domain, NOT an independent confirmation. Overlap with the OMEGA shortlist
  // is labeled so nobody can double-count agreement as evidence.
  try {
    const psrl = await S.readJSON(require('./psrl/config').STORE.latest, null);
    const cards = (psrl && Array.isArray(psrl.cards)) ? psrl.cards : [];
    const inOmega = new Set((payload.cards || []).map(c => c.ticker));
    if (cards.length) {
      payload.evidenceFamilies = {
        ...(payload.evidenceFamilies || {}),
        psrl: {
          family: 'psrl', weight: 0, status: 'SHADOW',
          probability: null, portfolioEligible: false,
          source: 'persistent-staircase-relative-leadership-v1 (op=psrl)',
          candidates: cards.filter(c => c.quadrant === 'TRUE_LEADER' && !c.disqualified).slice(0, 20).map(c => ({
            ticker: c.ticker, quadrant: c.quadrant, stage: c.stage,
            combined: c.scores && c.scores.combinedAfterPenalties,
            entry: c.entry && c.entry.state,
            overlapsOmegaShortlist: inOmega.has(c.ticker),
          })),
          note: 'Same price/trend evidence domain as OMEGA (correlated by construction); annotation never affects ranks.',
        },
      };
    }
  } catch { /* annotation is optional; OMEGA payload unchanged on any failure */ }
...
// Map OMEGA cards → immutable live-ledger pick records. Persists EVERY tier — including AVOID
// and carried-forward names — so a pick's TRANSITION to Avoid (or its continued life below the
// funnel) is captured for grading, not dropped (Defect A/B). Provenance/version unchanged.
function omegaLivePicks(cards, date) {
  return (cards || []).map(c => ({
    ...
    // sector/company let a future carry-forward re-fetch the right sector benchmark.
    sector: c.sector || null, company: c.company || null,
    funnelDisplaced: !!c.funnelDisplaced,
...
async function runOmegaLog(req, res) {
  ...
  const { payload } = await buildLiveOmegaPayload(today.data);
  const picks = omegaLivePicks(payload.cards, date);
  let logged = 0;
  ...
  // reproduce it and earn live-funnel parity. Independent of pick logging. Carried-forward names
  // are NOT part of TODAY'S op=today funnel, so they are excluded from the funnel snapshot.
  const funnelPayload = { ...payload, cards: payload.cards.filter(c => !c.funnelDisplaced) };
  const funnel = await captureFunnel(today.data, funnelPayload, date, { skip: isMarketClosed }).catch(e => ({ error: String((e && e.message) || e) }));
...
async function runOmegaModel(req, res) {
  ...
  return res.json({
    ...
    // Survivorship-free research verdict (research/53) — the survivorshipSafe flag discharged
    // research-side. A RECORDED snapshot, not recomputed at request time. Distinct from the
    // app-side live replay, which stays survivor-biased on the free feed.
    researchEvidence: researchVerdict(),
    trainedModel: model ? { activeId: model.activeId, promoted: !!model.promoted, resolved: model.resolved || 0 } : null,
    note: 'Interpretable formula is the shipped ranker; a trained challenger overrides weights ONLY after it clears every gate in op=omegawf AND is promotable (live-funnel parity + survivorship-safe). Research (survivorship-free) verdict = no-edge, so on current evidence there is nothing to promote.',
  });
}
...
module.exports = {
  ...
  carryForwardEpisodes, episodeToSignal, buildLiveOmegaPayload, CARRY_FORWARD_MAX, omegaLivePicks,
};
