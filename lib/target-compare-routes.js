...
// Horizon rung under comparison. 21 sessions is the declared primary, but until
// enough 21-bar labels mature the 5- and 10-bar rungs are the only graded data —
// `?bars=` lets the comparison run early on them (the report stamps which ran).
const ALLOWED_BARS = [5, 10, 21];
...
async function runTargetCompare(req, res) {
  try {
    ...
    const bars = ALLOWED_BARS.includes(Number(req.query.bars)) ? Number(req.query.bars) : DEFAULT_BARS;
    ...
    // `diagnosis` accounts for every vector that DIDN'T become a row — a zero-row
    // report must explain itself (unfilled? rung pending? label missing?).
    const rows = [];
    let missingFeatures = 0;
    const diagnosis = { days: [], totals: { vectors: 0, filled: 0, resolvedAtBars: 0, labeled: 0 } };
    for (const { date, batch } of days) {
      const all = batch.vectors.filter(Boolean);
      const vectors = all.filter(v => v.fillStatus === 'filled');
      const resolvedAtBars = vectors.filter(v => (v.horizons || []).some(h => h && h.bars === bars && h.status === 'resolved'));
      const rungStatuses = {};
      for (const v of all) {
        const h = (v.horizons || []).find(x => x && x.bars === bars);
        const key = v.fillStatus !== 'filled' ? `unfilled:${(v.horizons && v.horizons[0] && v.horizons[0].reason) || 'unknown'}` : (h ? h.status + (h.status === 'pending' ? `:${h.reason}` : '') : 'no-rung');
        rungStatuses[key] = (rungStatuses[key] || 0) + 1;
      }
      diagnosis.days.push({ date, vectors: all.length, filled: vectors.length, resolvedAtBars: resolvedAtBars.length, rungStatuses });
      diagnosis.totals.vectors += all.length;
      diagnosis.totals.filled += vectors.length;
      diagnosis.totals.resolvedAtBars += resolvedAtBars.length;
      const byVariant = {};
      for (const v of VARIANTS) {
        byVariant[v] = new Map(buildTargetRows(vectors, { bars, variant: v, decisionTs: date }).map(r => [r.ticker, r]));
      }
    ...
    const payload = {
      ok: true, ...result, bars,
      primaryHorizon: bars === DEFAULT_BARS,        // early bars=5/10 reads are supplementary
      coverage: {
...
module.exports = { runTargetCompare, REPORT_PATH, VARIANTS, ALLOWED_BARS, DEFAULT_BARS };
