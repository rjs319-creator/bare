...
async function readAllByPrefix(prefix, re) {
  const blobs = await listBlobPaths(prefix, re);
  const docs = [];
  let unreadable = 0;
  await Promise.all(blobs.map(async b => {
    try {
      const res = await fetch(b.url + '?_=' + Date.now(), { cache: 'no-store' });
      if (!res.ok) { unreadable++; return; }   // a non-ok shard is a LOSS, not a skip
      docs.push(await res.json());
    } catch { unreadable++; }
  }));
  return withReadStats(docs.sort((a, b) => (String(a.date) < String(b.date) ? -1 : 1)), blobs.length, unreadable);
}
