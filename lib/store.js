...
// One day's estarchive doc (same-day second-run MERGE — mirrors readRevArchiveDay).
async function readEstArchiveDay(date) {
  const blobs = await listBlobPaths(ESTARCH_PREFIX, ESTARCH_DAILY_RE);
  const b = blobs.find((x) => x.pathname === `${ESTARCH_PREFIX}${date}.json`);
  if (!b) return null;
  try {
    const res = await fetch(b.url + '?_=' + Date.now(), { cache: 'no-store' });
    return res.ok ? await res.json() : null;
...
async function archiveShardPaths() {
  const [snaps, revs, revarch, gex, est] = await Promise.all([
  ...
  return { snaps, revs, revarch, gex, est };
}
...
module.exports = {
  ...
  CALARCH_PREFIX, REVARCH_PREFIX, GEXARCH_PREFIX, ESTARCH_PREFIX,
};
