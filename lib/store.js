...
// Rows to request when locating ONE doc by exact pathname via list({ prefix: path }).
// list() row order is not a contract this code may lean on: with limit:1, a blob whose
// path merely EXTENDS the target (e.g. `<path>.bak`) could be the single returned row and
// the exact match would be silently missed despite the doc existing. A small batch plus an
// exact pathname select makes the lookup order-independent.
const EXACT_MATCH_LIST_ROWS = 10;
...
async function readAllPicks() {
  ...
  return withReadStats(all, blobs.length, unreadable);
}
...
// ── Ghost FULL-candidate observation record (pre-move redesign Phase 1) ──────
// One IMMUTABLE snapshot per trading day under ghostobs/<date>.json holding the
// complete standalone-accumulation cross-section: selected, rejected, near-
// threshold and excluded candidates with point-in-time provenance (built by
// lib/premove-capture.js). Separate from the legacy ghost/ ledger, which is
// preserved for Scoreboard compatibility. SHADOW data — read by the pre-move
// inventory + research harness only, never by live ranks.
const GHOSTOBS_PREFIX = 'ghostobs/';
const GHOSTOBS_DAILY_RE = /^ghostobs\/\d{4}-\d{2}-\d{2}\.json$/;

async function writeGhostObsDay(date, record) {
  if (!hasStore()) throw new Error('Blob storage not configured (BLOB_READ_WRITE_TOKEN missing).');
  const { put } = require('@vercel/blob');
  const body = JSON.stringify({ date, ...record, savedAt: new Date().toISOString() });
  return put(`${GHOSTOBS_PREFIX}${date}.json`, body, {
    access: 'public', contentType: 'application/json',
    allowOverwrite: true, addRandomSuffix: false, cacheControlMaxAge: 300,
  });
}

async function readAllGhostObsDays() {
  if (!hasStore()) return [];
  const { list } = require('@vercel/blob');
  const blobs = [];
  let cursor;
  do {
    const r = await list({ prefix: GHOSTOBS_PREFIX, cursor, limit: 1000 });
    blobs.push(...r.blobs);
    cursor = r.cursor;
  } while (cursor);
  const days = [];
  let unreadable = 0;
  await Promise.all(blobs.filter(b => GHOSTOBS_DAILY_RE.test(b.pathname)).map(async b => {
    try {
      const res = await fetch(b.url + '?_=' + Date.now(), { cache: 'no-store' });
      if (!res.ok) { unreadable++; return; }
  ...
  return withReadStats(days.sort((a, b) => (a.date < b.date ? -1 : 1)), blobs.length, unreadable);
}

// ── Evidence Consensus & Thesis Change engine ledger (mirrors the Ghost pattern) ──
// One snapshot per trading day: the full evidence bundle for the day's active-attention
// names (events, clusters, consensus, thesis-change) is stored under `evidence/<date>.json`.
// `signals[]` inside each day file is the Scoreboard-compatible pick list (one row per name
// with a thesis-change tier) so a new signal folds into runScoreboard exactly like Ghost.
const EVIDENCE_PREFIX = 'evidence/';
const EVIDENCE_DAILY_RE = /^evidence\/\d{4}-\d{2}-\d{2}\.json$/;

async function writeEvidenceDay(date, snapshot) {
  if (!hasStore()) throw new Error('Blob storage not configured (BLOB_READ_WRITE_TOKEN missing).');
  const { put } = require('@vercel/blob');
  const body = JSON.stringify({ date, ...snapshot, savedAt: new Date().toISOString() });
  return put(`${EVIDENCE_PREFIX}${date}.json`, body, {
    access: 'public', contentType: 'application/json',
    allowOverwrite: true, addRandomSuffix: false, cacheControlMaxAge: 300,
  });
}

// Latest evidence snapshot (the full day file), for the live op=evidence read.
async function readLatestEvidence() {
  ...
  let cursor;
  do {
    const r = await list({ prefix: EVIDENCE_PREFIX, cursor, limit: 1000 });
    blobs.push(...r.blobs);
    cursor = r.cursor;
  } while (cursor);
  const days = blobs.filter(b => EVIDENCE_DAILY_RE.test(b.pathname)).sort((a, b) => (a.pathname < b.pathname ? 1 : -1));
  if (!days.length) return null;
  try {
    const res = await fetch(days[0].url + '?_=' + Date.now(), { cache: 'no-store' });
    if (!res.ok) return null;
    return await res.json();
  } catch { return null; }
}

// All evidence-day pick rows flattened, for the Scoreboard fold (mirrors readAllGhost).
async function readAllEvidence() {
  ...
  const all = [];
  let unreadable = 0;
  await Promise.all(blobs.filter(b => EVIDENCE_DAILY_RE.test(b.pathname)).map(async b => {
    try {
      const res = await fetch(b.url + '?_=' + Date.now(), { cache: 'no-store' });
      if (!res.ok) { unreadable++; return; }
      const j = await res.json();
...
// Three unrecoverable-data ledgers, all date-keyed write-once shards — NO
// read-modify-write (the apex/insider.json lost-update race is the failure
// mode this layout forbids by construction):
//   calarchive/<date>.json       forward earnings-calendar snapshot
//   calarchive/rev/<date>.json   scheduled-date revision events (the signal ledger)
//   revarchive/<date>.json       analyst grade + price-target event pulls
//   gexarchive/<date>-s<n>.json  per-name dealer-gamma snapshot, one shard/slice
const CALARCH_PREFIX = 'calarchive/';
const CALARCH_SNAP_RE = /^calarchive\/\d{4}-\d{2}-\d{2}\.json$/;      // excludes calarchive/rev/*
const CALARCH_REV_RE = /^calarchive\/rev\/\d{4}-\d{2}-\d{2}\.json$/;
const REVARCH_PREFIX = 'revarchive/';
const REVARCH_DAILY_RE = /^revarchive\/\d{4}-\d{2}-\d{2}\.json$/;
const GEXARCH_PREFIX = 'gexarchive/';
const GEXARCH_SHARD_RE = /^gexarchive\/\d{4}-\d{2}-\d{2}-s\d+\.json$/;
//   estarchive/<date>.json       analyst-estimate vintage snapshot (lib/est-archive.js)
const ESTARCH_PREFIX = 'estarchive/';
const ESTARCH_DAILY_RE = /^estarchive\/\d{4}-\d{2}-\d{2}\.json$/;
//   sec8karchive/<date>.json     market-wide 8-K filing feed (lib/filing-archives.js)
//   insarchive/<date>.json       market-wide insider-transaction feed (lib/filing-archives.js)
const SEC8K_PREFIX = 'sec8karchive/';
const SEC8K_DAILY_RE = /^sec8karchive\/\d{4}-\d{2}-\d{2}\.json$/;
const INSARCH_PREFIX = 'insarchive/';
const INSARCH_DAILY_RE = /^insarchive\/\d{4}-\d{2}-\d{2}\.json$/;

const stampSaved = (doc) => ({ ...doc, savedAt: new Date().toISOString() });
const writeCalArchiveDay = (date, doc) => writeJSON(`${CALARCH_PREFIX}${date}.json`, stampSaved(doc), 0);
const writeCalRevDay = (date, doc) => writeJSON(`${CALARCH_PREFIX}rev/${date}.json`, stampSaved(doc), 0);
const writeRevArchiveDay = (date, doc) => writeJSON(`${REVARCH_PREFIX}${date}.json`, stampSaved(doc), 0);
// One day's revarchive doc (for the same-day two-pull MERGE — the plan caps
// each pull at 100 events/feed, so the second pull must union, not replace).
async function readRevArchiveDay(date) {
  const blobs = await listBlobPaths(REVARCH_PREFIX, REVARCH_DAILY_RE);
  const b = blobs.find((x) => x.pathname === `${REVARCH_PREFIX}${date}.json`);
  if (!b) return null;
  try {
    const res = await fetch(b.url + '?_=' + Date.now(), { cache: 'no-store' });
...
const writeGexArchiveShard = (date, slice, doc) => writeJSON(`${GEXARCH_PREFIX}${date}-s${slice}.json`, stampSaved(doc), 0);
const writeEstArchiveDay = (date, doc) => writeJSON(`${ESTARCH_PREFIX}${date}.json`, stampSaved(doc), 0);
// One day's doc for the same-day multi-pull MERGE streams (mirrors readRevArchiveDay).
async function readArchiveDayByPrefix(prefix, re, date) {
  const blobs = await listBlobPaths(prefix, re);
  const b = blobs.find((x) => x.pathname === `${prefix}${date}.json`);
  if (!b) return null;
  try {
    const res = await fetch(b.url + '?_=' + Date.now(), { cache: 'no-store' });
    return res.ok ? await res.json() : null;
...
const readEstArchiveDay = (date) => readArchiveDayByPrefix(ESTARCH_PREFIX, ESTARCH_DAILY_RE, date);
const writeSec8kArchiveDay = (date, doc) => writeJSON(`${SEC8K_PREFIX}${date}.json`, stampSaved(doc), 0);
const readSec8kArchiveDay = (date) => readArchiveDayByPrefix(SEC8K_PREFIX, SEC8K_DAILY_RE, date);
const writeInsArchiveDay = (date, doc) => writeJSON(`${INSARCH_PREFIX}${date}.json`, stampSaved(doc), 0);
const readInsArchiveDay = (date) => readArchiveDayByPrefix(INSARCH_PREFIX, INSARCH_DAILY_RE, date);
// FMP capability audit (lib/fmp-audit.js): latest doc + a dated vintage copy so
// "what did the plan permit WHEN" is itself point-in-time answerable.
const FMP_CAPABILITY_PATH = 'fmp/capability-audit.json';
async function writeFmpCapabilityDoc(report) {
  const day = String(report.auditedAt || new Date().toISOString()).slice(0, 10);
  await writeJSON(`fmp/audit/${day}.json`, report, 0);
  return writeJSON(FMP_CAPABILITY_PATH, report, 0);
}
const readFmpCapabilityDoc = () => readJSON(FMP_CAPABILITY_PATH, null);

async function listBlobPaths(prefix, re) {
  if (!hasStore()) return [];
  const { list } = require('@vercel/blob');
  const blobs = [];
  let cursor;
  ...
  return blobs.filter(b => re.test(b.pathname));
}

// Snapshot DATES only (cheap list, no body fetches) — the diff step needs just
// "the most recent prior snapshot's date".
async function listCalArchiveDates() {
  const blobs = await listBlobPaths(CALARCH_PREFIX, CALARCH_SNAP_RE);
  return blobs.map(b => b.pathname.slice(CALARCH_PREFIX.length, -'.json'.length)).sort();
}

async function readCalArchiveDay(date) {
  const blobs = await listBlobPaths(CALARCH_PREFIX, CALARCH_SNAP_RE);
  const b = blobs.find(x => x.pathname === `${CALARCH_PREFIX}${date}.json`);
  if (!b) return null;
  try {
    const res = await fetch(b.url + '?_=' + Date.now(), { cache: 'no-store' });
    return res.ok ? await res.json() : null;
...
// Full-ledger readers for the research side (revision events / rev-archive days /
// gex shards). Bodies fetched cache-busted; unreadable shards skipped, counted.
// READ COMPLETENESS (alpha-research pass 3). Every per-day reader used to drop an
// unreadable shard silently — `catch { /* skip */ }` plus an unchecked `!res.ok` — so a
// partial CDN failure returned a SHORTER BUT SYNTACTICALLY VALID history. Downstream,
// excessN shrinks, Wilson bounds widen, and a Validated strategy can fall back under
// MIN_VALIDATED_EPISODES, with nothing anywhere recording that N days went missing.
//
// The loss is now counted and attached to the returned array as NON-ENUMERABLE metadata,
// so every existing caller keeps treating it as a plain array (JSON, spreads, deep-equal
// and .length are all unchanged) while a caller that cares can ask. lib/apex-routes'
// ledger loader reads `unreadable` and refuses to rewrite the evidence record on a
// materially incomplete read.
function withReadStats(arr, requested, unreadable) {
  Object.defineProperty(arr, 'requested', { value: requested, enumerable: false });
  Object.defineProperty(arr, 'unreadable', { value: unreadable, enumerable: false });
  Object.defineProperty(arr, 'complete', { value: unreadable === 0, enumerable: false });
  return arr;
}

// Pure: the newest `limit` blobs by pathname, returned OLDEST-FIRST so downstream
// date ordering is unchanged. Shard paths here are date-named, so pathname order is
// date order. No limit (null/0) means "everything" — the full-ledger callers.
function pickNewestBlobs(blobs, limit) {
  const all = [...(blobs || [])];
  if (!limit || limit >= all.length) return all;
  const path = (b) => (b && (b.pathname || b.url)) || '';
  return all
    .sort((a, b) => { const x = path(a), y = path(b); return x < y ? -1 : x > y ? 1 : 0; })
    .slice(-limit);
}

// Read every shard under a prefix.
//
// TWO BOUNDS, both added 2026-08-19 while tracing the /api/tracker OOM:
//
//   • `limit` — select the newest N by PATH before fetching any body. The options-flow
//     baseline caller did readAllByPrefix(...).slice(-26): it downloaded every
//     observation doc ever written, each carrying per-ticker observations for the whole
//     options universe, to use 26 of them. That grows without bound as the archive does.
//   • `concurrency` — the old `Promise.all(blobs.map(...))` opened one fetch per shard
//     at once and held every body simultaneously.
//
// Read stats stay honest: `requested` is what was fetched, `total` is what exists, and
// `truncated` says whether the limit bit — a bound must never read as "read everything".
async function readAllByPrefix(prefix, re, { limit = null, concurrency = 8 } = {}) {
  const blobs = await listBlobPaths(prefix, re);
  const selected = pickNewestBlobs(blobs, limit);
  const docs = [];
  let unreadable = 0;
  let i = 0;
  const worker = async () => {
    while (i < selected.length) {
      ...
      try {
        const res = await fetch(b.url + '?_=' + Date.now(), { cache: 'no-store' });
        if (!res.ok) { unreadable++; continue; }   // a non-ok shard is a LOSS, not a skip
        docs.push(await res.json());
      } catch { unreadable++; }
    }
  };
  await Promise.all(Array.from({ length: Math.max(1, Math.min(concurrency, selected.length)) }, worker));
  const out = withReadStats(docs.sort((a, b) => (String(a.date) < String(b.date) ? -1 : 1)), selected.length, unreadable);
  Object.defineProperty(out, 'total', { value: blobs.length, enumerable: false });
  Object.defineProperty(out, 'truncated', { value: selected.length < blobs.length, enumerable: false });
  return out;
}
const readAllCalRevDays = () => readAllByPrefix(CALARCH_PREFIX, CALARCH_REV_RE);
const readAllRevArchiveDays = () => readAllByPrefix(REVARCH_PREFIX, REVARCH_DAILY_RE);
const readAllGexArchiveShards = () => readAllByPrefix(GEXARCH_PREFIX, GEXARCH_SHARD_RE);
const readAllEstArchiveDays = () => readAllByPrefix(ESTARCH_PREFIX, ESTARCH_DAILY_RE);
const readAllSec8kArchiveDays = () => readAllByPrefix(SEC8K_PREFIX, SEC8K_DAILY_RE);
const readAllInsArchiveDays = () => readAllByPrefix(INSARCH_PREFIX, INSARCH_DAILY_RE);

// Telemetry support (op=archivehealth): shard PATHS (cheap, no body fetches)
// and the single LATEST body per prefix — never the full ledger.
async function archiveShardPaths() {
  const [snaps, revs, revarch, gex, est, sec8k, ins] = await Promise.all([
    listBlobPaths(CALARCH_PREFIX, CALARCH_SNAP_RE),
    listBlobPaths(CALARCH_PREFIX, CALARCH_REV_RE),
    listBlobPaths(REVARCH_PREFIX, REVARCH_DAILY_RE),
    listBlobPaths(GEXARCH_PREFIX, GEXARCH_SHARD_RE),
    listBlobPaths(ESTARCH_PREFIX, ESTARCH_DAILY_RE),
    listBlobPaths(SEC8K_PREFIX, SEC8K_DAILY_RE),
    listBlobPaths(INSARCH_PREFIX, INSARCH_DAILY_RE),
  ]);
  return { snaps, revs, revarch, gex, est, sec8k, ins };
}

async function latestBlobDoc(blobs) {
  if (!blobs.length) return null;
  const latest = [...blobs].sort((a, b) => (a.pathname < b.pathname ? -1 : 1)).pop();
  try {
...
// Existence probe for WRITE-ONCE guards. Returns whether a blob with this EXACT pathname
// exists, and THROWS when that cannot be determined (list failure) — callers must fail
// closed rather than treat "unknown" as "absent". Deliberately NOT readJSON: readJSON
// folds every failure into the fallback, which would let a transient read error overwrite
// an "immutable" doc.
...
// ── VRP live paper put-write ledger (vrp/<date>.json — lib/vrp-routes.js) ──
// One entry doc per entry date; the daily tick (single writer) writes
// resolutions back into the entry's own day doc.
const VRP_LEDGER = 'vrp/';
const VRP_DAILY_RE = /^vrp\/\d{4}-\d{2}-\d{2}\.json$/;
const writeVrpDay = (date, doc) => writeJSON(`${VRP_LEDGER}${date}.json`, { ...doc, date, savedAt: new Date().toISOString() }, 0);
async function readAllVrpDays() {
  if (!hasStore()) return [];
  const { list } = require('@vercel/blob');
  const blobs = []; let cursor;
  do { const r = await list({ prefix: VRP_LEDGER, cursor, limit: 1000 }); blobs.push(...r.blobs); cursor = r.cursor; } while (cursor);
  const days = [];
  await Promise.all(blobs.filter(b => VRP_DAILY_RE.test(b.pathname)).map(async b => {
    try {
      const res = await fetch(b.url + '?_=' + Date.now(), { cache: 'no-store' }); if (!res.ok) return;
      const j = await res.json();
      if (j && j.date) days.push(j);
    } catch { /* skip */ }
  }));
...
// ⚡ GRIDLOCK: daily ledger of physical-constraint candidates (Actionable / Tracked —
// gate passes AND gate failures both logged, prospective-only). Shares the gridlock/
// prefix with state/events/latest/resolved singletons, so the dated regex under
// gridlock/day/ is what keeps the Scoreboard read clean. SHADOW / weight-0.
// ── Peer Propagation + News Underreaction ledgers (shadow engines) ──
// Day files hold `picks[]` in the Scoreboard row shape (date/ticker/section/tier/
// entry) so both engines grade through the exact same fold as every other section.
// `peerprop/latest.json` / `underreaction/latest.json` singletons are excluded by
// the daily regexes on purpose (same trick as evidence/cache/*).
const PEERPROP_LEDGER = 'peerprop/';
const PEERPROP_DAILY_RE = /^peerprop\/\d{4}-\d{2}-\d{2}\.json$/;
const writePeerPropDay = (date, obj) => writeJSON(`${PEERPROP_LEDGER}${date}.json`, { date, ...obj, savedAt: new Date().toISOString() }, 0);
async function readAllPeerPropDays() {
  if (!hasStore()) return [];
  const { list } = require('@vercel/blob');
  const blobs = []; let cursor;
  do { const r = await list({ prefix: PEERPROP_LEDGER, cursor, limit: 1000 }); blobs.push(...r.blobs); cursor = r.cursor; } while (cursor);
  const days = [];
  await Promise.all(blobs.filter(b => PEERPROP_DAILY_RE.test(b.pathname)).map(async b => {
    try { const res = await fetch(b.url + '?_=' + Date.now(), { cache: 'no-store' }); if (!res.ok) return; const j = await res.json(); days.push(j); } catch { /* skip */ }
  }));
  days.sort((a, b) => (a.date < b.date ? -1 : 1));
  return days;
...
const UNDERREACTION_LEDGER = 'underreaction/';
const UNDERREACTION_DAILY_RE = /^underreaction\/\d{4}-\d{2}-\d{2}\.json$/;
const writeUnderreactionDay = (date, obj) => writeJSON(`${UNDERREACTION_LEDGER}${date}.json`, { date, ...obj, savedAt: new Date().toISOString() }, 0);
async function readAllUnderreactionDays() {
  if (!hasStore()) return [];
  const { list } = require('@vercel/blob');
  const blobs = []; let cursor;
  do { const r = await list({ prefix: UNDERREACTION_LEDGER, cursor, limit: 1000 }); blobs.push(...r.blobs); cursor = r.cursor; } while (cursor);
  const days = [];
  await Promise.all(blobs.filter(b => UNDERREACTION_DAILY_RE.test(b.pathname)).map(async b => {
    try { const res = await fetch(b.url + '?_=' + Date.now(), { cache: 'no-store' }); if (!res.ok) return; const j = await res.json(); days.push(j); } catch { /* skip */ }
  }));
  days.sort((a, b) => (a.date < b.date ? -1 : 1));
  return days;
...
// ── Expectation-Gap regime challenger ledger (reduce-only shadow) ──
// Day files exist ONLY for RISK_REDUCE days (a NEUTRAL day is not a signal) and
// hold one SPY short-framed pick so the Scoreboard grades the call falsifiably.
const EXPGAP_LEDGER = 'expgap/';
const EXPGAP_DAILY_RE = /^expgap\/\d{4}-\d{2}-\d{2}\.json$/;
const writeExpGapDay = (date, obj) => writeJSON(`${EXPGAP_LEDGER}${date}.json`, { date, ...obj, savedAt: new Date().toISOString() }, 0);
async function readAllExpGapDays() {
  if (!hasStore()) return [];
  const { list } = require('@vercel/blob');
  const blobs = []; let cursor;
  do { const r = await list({ prefix: EXPGAP_LEDGER, cursor, limit: 1000 }); blobs.push(...r.blobs); cursor = r.cursor; } while (cursor);
  const days = [];
  await Promise.all(blobs.filter(b => EXPGAP_DAILY_RE.test(b.pathname)).map(async b => {
    try { const res = await fetch(b.url + '?_=' + Date.now(), { cache: 'no-store' }); if (!res.ok) return; const j = await res.json(); days.push(j); } catch { /* skip */ }
  }));
  days.sort((a, b) => (a.date < b.date ? -1 : 1));
  return days;
...
const GRIDLOCK_LEDGER = 'gridlock/day/';
const GRIDLOCK_DAILY_RE = /^gridlock\/day\/\d{4}-\d{2}-\d{2}\.json$/;
const writeGridlockDay = (date, obj) => writeJSON(`${GRIDLOCK_LEDGER}${date}.json`, { date, ...obj, savedAt: new Date().toISOString() }, 0);
async function readAllGridlockDays() {
  if (!hasStore()) return [];
  const { list } = require('@vercel/blob');
  const blobs = []; let cursor;
  do { const r = await list({ prefix: GRIDLOCK_LEDGER, cursor, limit: 1000 }); blobs.push(...r.blobs); cursor = r.cursor; } while (cursor);
  const days = [];
  await Promise.all(blobs.filter(b => GRIDLOCK_DAILY_RE.test(b.pathname)).map(async b => {
    try { const res = await fetch(b.url + '?_=' + Date.now(), { cache: 'no-store' }); if (!res.ok) return; const j = await res.json(); days.push(j); } catch { /* skip */ }
  }));
  days.sort((a, b) => (a.date < b.date ? -1 : 1));
  return days;
...
// Insider cluster buys (shadow, weight-0): write-once tx shards (`insidercluster/tx/<date>.json`,
// the day's parsed universe Form 4 buys) and write-once ledger days (`insidercluster/<date>.json`).
// No read-modify-write anywhere: each tick writes its own shard and day doc; readers merge.
const IC_PREFIX = 'insidercluster/';
const IC_TX_PREFIX = 'insidercluster/tx/';
const IC_DAILY_RE = /^insidercluster\/\d{4}-\d{2}-\d{2}\.json$/;
const IC_TX_RE = /^insidercluster\/tx\/\d{4}-\d{2}-\d{2}\.json$/;
const insiderClusterDayKey = (date) => `${IC_PREFIX}${date}.json`;
const writeInsiderClusterDay = (date, obj) => writeJSON(insiderClusterDayKey(date), { date, ...obj, savedAt: new Date().toISOString() }, 0);
const writeInsiderClusterTxDay = (date, obj) => writeJSON(`${IC_TX_PREFIX}${date}.json`, { date, ...obj, savedAt: new Date().toISOString() }, 0);
async function readAllInsiderClusterDays({ since = null, limit = null } = {}) {
  const days = await readAllByPrefix(IC_PREFIX, IC_DAILY_RE, { limit });
  return since ? days.filter((d) => d && d.date >= since) : days;
}
async function readInsiderClusterTxDays({ since = null } = {}) {
  const days = await readAllByPrefix(IC_TX_PREFIX, IC_TX_RE);
  return since ? days.filter((d) => d && d.date >= since) : days;
}
...
async function readFundamentals() {
  ...
  await Promise.all(blobs.filter(b => FUND_SHARD_RE.test(b.pathname)).map(async b => {
    ...
    } catch { unreadable++; }   // a thrown fetch is a LOSS — count it like the !res.ok path
  }));
  return withReadStats(out, blobs.length, unreadable);
}
...
async function readAllEvolveDays() {
  ...
  await Promise.all(blobs.filter(b => EVOLVE_DAILY_RE.test(b.pathname)).map(async b => {
    try { const res = await fetch(b.url + '?_=' + Date.now(), { cache: 'no-store' }); if (res.ok) days.push(await res.json()); else unreadable++; }
    catch { unreadable++; }
  }));
  return withReadStats(days.sort((a, b) => (a.date < b.date ? -1 : 1)), blobs.length, unreadable);
}
...
// ── OMEGA-SWING ledgers (Phase 2: research and prospective evidence are PHYSICALLY separate) ──
//   omega/<date>.json        legacy MIXED days (pre-v2 — provenance unknown → migrated_legacy)
//   omega/live/<date>.json    PROSPECTIVE live picks (provenance prospective_live)
//   omega/research/<date>.json reconstructed history (provenance historical_reconstruction)
// Only prospective_live / paper_trade records may feed the DISPLAYED live track. Legacy days
// are read for compatibility but are NEVER silently relabeled as prospective.
const OMEGA_PREFIX = 'omega/';
const OMEGA_LIVE_PREFIX = 'omega/live/';
const OMEGA_RESEARCH_PREFIX = 'omega/research/';
const OMEGA_LEGACY_RE = /^omega\/\d{4}-\d{2}-\d{2}\.json$/;              // top-level (pre-v2)
const OMEGA_LIVE_RE = /^omega\/live\/\d{4}-\d{2}-\d{2}\.json$/;
const OMEGA_RESEARCH_RE = /^omega\/research\/\d{4}-\d{2}-\d{2}\.json$/;
const LIVE_TRACK_PROV = new Set(['prospective_live', 'paper_trade']);   // mirrors omega-contract

// Stamp a provenance onto every pick of a day (immutably) without overwriting an explicit one.
const stampProv = (picks, prov) => (picks || []).map(p => ({ ...p, provenance: p.provenance || prov }));

async function writeOmegaLiveDay(date, picks) {
  if (!hasStore()) throw new Error('Blob storage not configured (BLOB_READ_WRITE_TOKEN missing).');
  return writeJSON(`${OMEGA_LIVE_PREFIX}${date}.json`, { date, provenance: 'prospective_live', picks: stampProv(picks, 'prospective_live'), savedAt: new Date().toISOString() }, 0);
}
async function writeOmegaResearchDay(date, picks) {
  if (!hasStore()) throw new Error('Blob storage not configured (BLOB_READ_WRITE_TOKEN missing).');
  return writeJSON(`${OMEGA_RESEARCH_PREFIX}${date}.json`, { date, provenance: 'historical_reconstruction', picks: stampProv(picks, 'historical_reconstruction'), savedAt: new Date().toISOString() }, 0);
}
// Back-compat alias: pre-v2 callers of writeOmegaDay now write to the PROSPECTIVE live ledger.
const writeOmegaDay = writeOmegaLiveDay;

// Read OMEGA picks. `track:'live'` returns ONLY live-track-eligible records (prospective_live/
// paper_trade); default returns everything with provenance stamped (legacy → migrated_legacy).
async function readAllOmega({ track } = {}) {
  if (!hasStore()) return [];
  const { list } = require('@vercel/blob');
  const blobs = []; let cursor;
  do { const r = await list({ prefix: OMEGA_PREFIX, cursor, limit: 1000 }); blobs.push(...r.blobs); cursor = r.cursor; } while (cursor);
  const provFor = (path, dayProv) => OMEGA_LIVE_RE.test(path) ? (dayProv || 'prospective_live')
    : OMEGA_RESEARCH_RE.test(path) ? (dayProv || 'historical_reconstruction')
    : OMEGA_LEGACY_RE.test(path) ? 'migrated_legacy' : null;
  const all = [];
  let unreadable = 0;
  await Promise.all(blobs.map(async b => {
    const baseProv = provFor(b.pathname);
    if (baseProv == null) return;                          // not an OMEGA day file
    try {
      const res = await fetch(b.url + '?_=' + Date.now(), { cache: 'no-store' });
      if (!res.ok) { unreadable++; return; } const j = await res.json();
      if (!Array.isArray(j.picks)) return;
      for (const p of j.picks) {
        const prov = p.provenance || provFor(b.pathname, j.provenance) || baseProv;
        all.push({ ...p, provenance: prov });
      }
    } catch { /* skip */ }
  }));
  return track === 'live' ? all.filter(p => LIVE_TRACK_PROV.has(p.provenance)) : all;
}
// Prospective-only view for the displayed live track record.
const readAllOmegaLive = () => readAllOmega({ track: 'live' });

// ── OMEGA candidate-funnel snapshots (Phase 4 — prospective live-funnel capture) ──
// omega/funnel/<date>.json — the exact op=today candidate funnel OMEGA re-ranked, captured
// point-in-time. WRITE-ONCE: a funnel that can be rewritten after the fact is not evidence, and
// op=omega is called many times a day — without this the last call would replace the real one.
const OMEGA_FUNNEL_PREFIX = 'omega/funnel/';
const OMEGA_FUNNEL_RE = /^omega\/funnel\/\d{4}-\d{2}-\d{2}\.json$/;
async function writeOmegaFunnelDay(date, snapshot, { force = false } = {}) {
  if (!hasStore()) throw new Error('Blob storage not configured (BLOB_READ_WRITE_TOKEN missing).');
  if (!force) {
    // Write-once guard, fail-closed: "definitely absent" (list succeeded, no exact
    // pathname) is the ONLY state that permits a write. If existence cannot be verified
    // (thrown list), refuse — treating unknown as absent would overwrite the immutable
    // snapshot on any transient read failure.
    let exists;
    try { exists = await blobExists(`${OMEGA_FUNNEL_PREFIX}${date}.json`); }
    catch { return { written: false, reason: 'existence-unverifiable' }; }
    if (exists) return { written: false, reason: 'already-captured' };
  }
  await writeJSON(`${OMEGA_FUNNEL_PREFIX}${date}.json`, { date, savedAt: new Date().toISOString(), snapshot }, 0);
  return { written: true };
}
const readOmegaFunnelDay = (date) => readJSON(`${OMEGA_FUNNEL_PREFIX}${date}.json`, null);
async function readOmegaFunnelDates() {
  if (!hasStore()) return [];
  const { list } = require('@vercel/blob');
  const dates = []; let cursor;
  do { const r = await list({ prefix: OMEGA_FUNNEL_PREFIX, cursor, limit: 1000 }); for (const b of r.blobs) { const m = b.pathname.match(/(\d{4}-\d{2}-\d{2})\.json$/); if (m && OMEGA_FUNNEL_RE.test(b.pathname)) dates.push(m[1]); } cursor = r.cursor; } while (cursor);
  return dates.sort();
}
...
// GOV-DEMAND — SHADOW government-demand vertical (AI-Alpha-OS slice 1). Fully
// isolated govdemand/* prefix: collector state (cursor + seen txns + revision
// vintages), monthly event archives (append-only; every observed event with
// its status, selected AND rejected), daily PIT prediction docs and a
// resolved-outcome map. Never read by production ranking.
const GOVDEMAND_PREFIX = 'govdemand/';
const GOVDEMAND_DAILY_RE = /^govdemand\/pred\/\d{4}-\d{2}-\d{2}\.json$/;
const readGovDemandState = () => readJSON(`${GOVDEMAND_PREFIX}state.json`, null);
const writeGovDemandState = (s) => writeJSON(`${GOVDEMAND_PREFIX}state.json`, s, 0);
const readGovDemandEventsMonth = (ym) => readJSON(`${GOVDEMAND_PREFIX}events/${ym}.json`, null);
const writeGovDemandEventsMonth = (ym, doc) => writeJSON(`${GOVDEMAND_PREFIX}events/${ym}.json`, doc, 0);
const writeGovDemandPredDay = (date, obj) => writeJSON(`${GOVDEMAND_PREFIX}pred/${date}.json`, { date, ...obj, savedAt: new Date().toISOString() }, 0);
async function readAllGovDemandPredDays() {
  if (!hasStore()) return [];
  const { list } = require('@vercel/blob');
  const blobs = []; let cursor;
  do { const r = await list({ prefix: `${GOVDEMAND_PREFIX}pred/`, cursor, limit: 1000 }); blobs.push(...r.blobs); cursor = r.cursor; } while (cursor);
  const days = [];
  await Promise.all(blobs.filter(b => GOVDEMAND_DAILY_RE.test(b.pathname)).map(async b => {
    try { const res = await fetch(b.url + '?_=' + Date.now(), { cache: 'no-store' }); if (!res.ok) return; const j = await res.json(); days.push(j); } catch { /* skip */ }
  }));
  days.sort((a, b) => (a.date < b.date ? -1 : 1));
  return days;
}
const readGovDemandResolved = () => readJSON(`${GOVDEMAND_PREFIX}resolved.json`, {});
const writeGovDemandResolved = (m) => writeJSON(`${GOVDEMAND_PREFIX}resolved.json`, m, 0);
...
module.exports = {
  // generic prefix readers (shared by feature stores, e.g. lib/tech-evidence/store.js)
  listBlobPaths, readAllByPrefix, pickNewestBlobs,
  ...
  GOVDEMAND_PREFIX, readGovDemandState, writeGovDemandState,
  readGovDemandEventsMonth, writeGovDemandEventsMonth,
  writeGovDemandPredDay, readAllGovDemandPredDays,
  readGovDemandResolved, writeGovDemandResolved,
  writeIgnitionDay, readAllIgnition, IGNITION_PREFIX,
  writeOmegaDay, writeOmegaLiveDay, writeOmegaResearchDay, readAllOmega, readAllOmegaLive,
  writeOmegaFunnelDay, readOmegaFunnelDay, readOmegaFunnelDates,
  OMEGA_PREFIX, OMEGA_LIVE_PREFIX, OMEGA_RESEARCH_PREFIX, OMEGA_FUNNEL_PREFIX,
  ...
  writeGhostObsDay, readAllGhostObsDays, GHOSTOBS_PREFIX,
  writeEvidenceDay, readLatestEvidence, readAllEvidence, EVIDENCE_PREFIX,
  ...
  writeVrpDay, readAllVrpDays, VRP_LEDGER,
  ...
  writeGridlockDay, readAllGridlockDays, GRIDLOCK_LEDGER,
  writePeerPropDay, readAllPeerPropDays, PEERPROP_LEDGER,
  writeUnderreactionDay, readAllUnderreactionDays, UNDERREACTION_LEDGER,
  writeExpGapDay, readAllExpGapDays, EXPGAP_LEDGER,
  writeAnomalyDay, readAllAnomalyDays,
  insiderClusterDayKey, writeInsiderClusterDay, writeInsiderClusterTxDay, readAllInsiderClusterDays, readInsiderClusterTxDays,
  ...
  blobExists,            // fail-closed existence probe for WRITE-ONCE guards (throws → refuse)
  withReadStats,         // shared loss-accounting metadata for shard readers (lifecycle-store)
  writeCalArchiveDay, writeCalRevDay, readCalArchiveDay, listCalArchiveDates, readAllCalRevDays,
  writeRevArchiveDay, readRevArchiveDay, readAllRevArchiveDays,
  writeGexArchiveShard, readAllGexArchiveShards,
  writeEstArchiveDay, readEstArchiveDay, readAllEstArchiveDays,
  writeSec8kArchiveDay, readSec8kArchiveDay, readAllSec8kArchiveDays,
  writeInsArchiveDay, readInsArchiveDay, readAllInsArchiveDays,
  writeFmpCapabilityDoc, readFmpCapabilityDoc,
  archiveShardPaths, latestBlobDoc,
  CALARCH_PREFIX, REVARCH_PREFIX, GEXARCH_PREFIX, ESTARCH_PREFIX, SEC8K_PREFIX, INSARCH_PREFIX,
};
