...
function compositeScore({ confidence, regimeFit: rf, execution, tilt, evidenceMult, costPenalty }) {
...
function rankSignals(signals, { regime, scoreboard, includeInactive = false, redundancy = null } = {}) {
  const enriched = (signals || []).map((sig) => {
    ...
    const score = compositeScore({
      confidence, regimeFit: rf, execution: exq.quality, tilt, evidenceMult, costPenalty: cost.penalty,
    });
