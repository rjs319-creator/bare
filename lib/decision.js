...
function expectancyFor(section, tier, horizon, summary) {
  ...
  return {
    avgExcess: h.avgExcess ?? null, winRate: h.winRate ?? null,
...
function evidenceFor(sig, redundancy) {
  ...
  const sources = (sig.sources && sig.sources.length ? sig.sources : [sig.source]).filter(Boolean);
  if (sources.length < 2) return base;

  const RD = require('./redundancy');
  const eff = RD.effectiveEvidence(sources, {
    model: redundancy,
