...
function rankSignals(signals, { regime, scoreboard, includeInactive = false, redundancy = null, origins = null } = {}) {
  ...
  const enriched = (signals || []).map((sig) => {
    ...
    const confidence = confidenceScore({ rawConfidence: comparable, evidence, exp });
    ...
    return {
      ...
      scoreDecomposition: {
        ...
        formula: 'score = confidence x regimeFit x execution x expectancyTilt x evidenceMult x costPenalty x remainingMult',
