...
const EVIDENCE_FAMILIES = [
  ...
  // physicalConstraint (GRIDLOCK): physical-infrastructure/contract events mapped through a
  // verified exposure graph. Registered here as VOCABULARY so ensemble breadth can treat it
  // as independent of priceTrend; deliberately NOT mapped in SOURCE_FAMILY — no live screener
  // routes to it while the gridlock strategy is shadow/weight-0 (see lib/strategy-registry.js).
  'physicalConstraint',
];
const FAMILY_LABEL = {
  ...
  crossAsset: 'Cross-asset', physicalConstraint: 'Physical constraint',
};
...
const SOURCE_FAMILY = {
  ...
  // CANONICAL-ID COVERAGE (graduation-league PR-02). CERN's canonical id is `events`
  // (SECTION_TO_ID maps section CERN → 'events'); only the unreachable legacy key
  // `cern` was mapped, so familyOf('events') returned null and every below-gate CERN
  // pair got the FULLY-INDEPENDENT prior credit (1.0) instead of its forced-flow
  // family's 0.3 — the same silent-default hazard as the fixed `downday` bug. Every
  // id reachable via SECTION_TO_ID now resolves here (test: decision-family-coverage).
  events: 'catalystForcedFlow', gridlock: 'catalystForcedFlow', govdemand: 'catalystForcedFlow',
  emergingleader: 'priceTrend', omega: 'priceTrend', chartpattern: 'priceTrend',
  atlasx: 'priceTrend', premove: 'priceTrend', rlt: 'priceTrend', psrl: 'priceTrend',
  orbit: 'priceTrend', 'orbit-ml': 'priceTrend',
  thesis: 'sentimentAttention', underreaction: 'sentimentAttention',
  peerlab: 'crossAsset', expgap: 'crossAsset',
  'challenger-decision': 'priceTrend',
  // SECTION_TO_ID InsiderCluster → 'insidercluster' (2026-09 shadow ledger); insider evidence family.
  insidercluster: 'insider',
};
...
const FAMILY_DOMAIN = {
  ...
  // Physical events reach the market as news/catalyst information; sharing the 'news' domain
  // (like catalystForcedFlow) keeps breadth honest — a physical event already counted as a
  // catalyst must not light a second domain.
  physicalConstraint: 'news',
};
...
// NOTE (alpha-research pass 3): `hardToBorrow` was accepted here and applied a 0.8x
// execution penalty, but NO caller ever set it — executionQuality reads sig.liquidity,
// and no normalizer populates that field. It was removed rather than wired up.
//
// Wiring it was the tempting fix and would have been wrong: lib/costs.borrowCost derives
// hardToBorrow from a HARDCODED tier-prior APR table and stamps `borrowKnown: false` —
// a guess, not an observation. Feeding a prior-based guess into a live execution-quality
// penalty manufactures a signal from an assumption. lib/eligibility already fail-closes
// every short without an observed borrow feed, and no such feed exists in this repo, so
// the honest state is: no borrow input reaches ranking at all. Restore this parameter
// only alongside a real borrow/locate feed.
function executionQuality({ dollarVol, price, spreadPct, haltRisk, eventGapRisk, poorOptionLiq } = {}) {
  let q = 1;
  const penalties = [];
...
function regimeFit(side, regime) {
  const r = regime || {};
  // Risk-off is an AFFIRMATIVE signal (bearish tape or macro kill-switch), not merely
  // "not risk-on": computeRegime always emits a hard riskOn boolean, so treating
  // riskOn:false as risk-off made the neutral branch below unreachable and scored
  // every neutral tape at the 0.45 risk-off multiplier. A payload that carries ONLY
  // riskOn (no bearish field) still fails closed to risk-off.
  const riskOff = r.bearish === true || r.killSwitch === true
    || (r.riskOn === false && r.bearish === undefined);
...
//
// SCOPED EVIDENCE IDENTITY (defect #5). When the summary declares the scoped
// evidence-key contract (`evidenceKeyVersion`), the join additionally requires the
// signal's `universeScope` to match the group's `scope` — a large-cap Breakout
// record can no longer tilt a small/micro/expanded candidate (or vice versa), and
// a scoped signal with no scoped record gets an honest neutral buildingEvidence
// (never a fallback to the pooled/legacy record). Legacy summaries (no version
// field) keep the historical section:tier join so old stored docs still resolve —
// they are context, and the next scoreboard write migrates the contract forward.
function expectancyFor(section, tier, horizon, summary, universeScope = null) {
  const groups = (summary && summary.groups) || [];
  const scoped = !!(summary && summary.evidenceKeyVersion);
  const g = scoped
    ? groups.find(x => x.section === section && x.tier === tier && (x.scope || null) === (universeScope || null))
    : groups.find(x => x.section === section && x.tier === tier);
  const key = HORIZON_METRIC[horizon] || '1m';
  if (!g) return { avgExcess: null, winRate: null, n: 0, known: false, horizonKey: key, buildingEvidence: true, ...(scoped ? { scoped: true, scope: universeScope || null } : {}) };
  // EXACT-HORIZON CONTRACT — no fallback. The old chain (requested → '1m' → '5d') silently
  // graded an intraday signal on a 21-session record (objective mismatch) and then reported
  // the REQUESTED key as provenance. Missing exact-horizon evidence is now an honest
  // "building evidence" (neutral tilt), never a substituted horizon.
  const h = (g.horizons && g.horizons[key]) || null;
  if (!h) return { avgExcess: null, winRate: null, n: 0, known: false, horizonKey: key, buildingEvidence: true };
  ...
  // COST-NET HISTORY IS THE PRIOR (redesign Phase 9 item 4). The rank tilt reads the
  // strategy's realized NET-of-cost excess when the record carries that channel — gross
  // excess flatters exactly the sleeves whose edge friction eats. This is NOT
  // double-counting the setup's own cost geometry: that enters once through
  // `cost.penalty` (this trade's round trip), while this is the SOURCE's historical
  // cost-net record. Gross remains a labeled fallback for pre-cost summaries.
  const netKnown = Number.isFinite(h.avgNetExcess) && Number.isFinite(h.netExcessN) && h.netExcessN > 0;
  return {
    avgExcess: netKnown ? h.avgNetExcess : (h.avgExcess ?? null),
    grossAvgExcess: h.avgExcess ?? null,
    netAvgExcess: netKnown ? h.avgNetExcess : null,
    basis: netKnown ? 'cost-net' : 'gross (no cost-net channel on this record)',
    ...
    // DATE-LEVEL record (the statistic the tilt is allowed to act on). `n` above counts
    // PICKS; same-day picks share the market factor and overlapping horizons share days,
    // so a pick count cannot establish how much independent evidence exists. dateNet is
    // one equal-weight portfolio observation per decision date, with a HAC/bootstrap CI
    // and an autocorrelation-adjusted effective sample size. Absent on pre-dateNet
    // summaries ⇒ the tilt neutralises rather than guessing.
    dateN: (h.dateNet && Number.isFinite(h.dateNet.n)) ? h.dateNet.n : null,
    effectiveDates: (h.dateNet && Number.isFinite(h.dateNet.effectiveN)) ? h.dateNet.effectiveN : null,
    dateCiLo: (h.dateNet && h.dateNet.ci95 && Number.isFinite(h.dateNet.ci95.lo)) ? h.dateNet.ci95.lo : null,
    dateCiHi: (h.dateNet && h.dateNet.ci95 && Number.isFinite(h.dateNet.ci95.hi)) ? h.dateNet.ci95.hi : null,
    dateAvg: (h.dateNet && Number.isFinite(h.dateNet.avg)) ? h.dateNet.avg : null,
    ...(scoped ? { scoped: true, scope: universeScope || null } : {}),
  };
}
...
// EVIDENCE-GATED (alpha-research pass 3). Three defects were fixed here:
//
//   1. IT SHRANK ON PICKS, NOT INDEPENDENT DATES. `n/(n+8)` over PICKS meant 8 picks
//      bought half weight — and 40 picks made on 3 dates bought ~83%, despite being
//      roughly three observations. Same-day picks share the market factor. Shrinkage now
//      runs on the autocorrelation-adjusted effective DATE count.
//   2. NOISE COULD TILT THE RANK UP. Any positive point estimate boosted, with no
//      requirement that it be distinguishable from zero. A boost now requires the
//      date-level CI to clear zero; a record whose interval spans zero is evidence of
//      nothing and tilts neutral.
//   3. A PROVEN-NEGATIVE STRATEGY COULD NOT BE RANKED OUT. The symmetric ±0.3 clamp
//      floored the multiplier at 0.70, so a sleeve with a significantly negative record
//      was demoted by at most 30% relative to a positive one. When the date-level CI
//      lies entirely below zero the tilt now collapses to NEGATIVE_TILT.
//
// The tilt reads the DATE-LEVEL record only. A summary with no dateNet block cannot
// establish independence, so it neutralises (fail closed) rather than falling back to
// pick counts — that fallback is precisely what produced the overstated confidence.
const MIN_TILT_DATES = 8;     // independent decision dates before ANY tilt applies
const NEGATIVE_TILT = 0.1;    // significantly-negative record: sinks below everything
                              // that is merely unproven, while preserving relative order
                              // among such rows (0 would flatten them into a tie).
function expectancyTilt(exp) {
  const neutral = (reason) => ({ tilt: 1, shrink: 0, reason, basis: 'date-level' });
  if (!exp || !exp.known || !exp.n) return neutral('no resolved record at this horizon');

  // Independent evidence = effective dates when measured, else raw distinct dates.
  const independentN = Number.isFinite(exp.effectiveDates) ? exp.effectiveDates
    : (Number.isFinite(exp.dateN) ? exp.dateN : null);
  if (independentN == null) {
    return neutral('no date-level record — pick counts cannot establish independence (fails closed to neutral)');
  }
  if (independentN < MIN_TILT_DATES) {
    return neutral(`only ${independentN} independent decision dates (need ≥${MIN_TILT_DATES}) — too few to move the rank`);
  }

  const lo = exp.dateCiLo, hi = exp.dateCiHi;
  const haveCI = Number.isFinite(lo) && Number.isFinite(hi);
  if (!haveCI) return neutral('date-level record carries no confidence interval — cannot establish significance');

  const shrink = independentN / (independentN + SHRINK_K);

  // Significantly NEGATIVE: the whole interval sits below zero. This is the case the old
  // 0.70 floor could not express — a sleeve that has demonstrably lost money should rank
  // below one with no record at all, not 30% below it.
  if (hi < 0) {
    return { tilt: NEGATIVE_TILT, shrink: +shrink.toFixed(2), basis: 'date-level',
      reason: `date-level cost-net CI [${lo}, ${hi}] lies entirely below zero over ${independentN} independent dates — ranked out` };
  }

  // Interval spans zero ⇒ no evidence either way. Do not reward a positive point estimate
  // that a clustered interval cannot distinguish from noise.
  if (!(lo > 0)) {
    return neutral(`date-level CI [${lo}, ${hi}] spans zero over ${independentN} independent dates — no evidence to tilt on`);
  }

  // Significantly POSITIVE: tilt up, still shrunk and still capped.
  ...
  const tilt = 1 + Math.max(0, Math.min(0.3, raw)) * shrink;
  return { tilt: +tilt.toFixed(3), shrink: +shrink.toFixed(2), basis: 'date-level',
    reason: `date-level cost-net CI [${lo}, ${hi}] clear of zero over ${independentN} independent dates` };
}

// ── Score informativeness (rank-quality gate, 2026-09-09) ───────────────────
// The Scoreboard measures, per section, whether that section's OWN score ranks its own
// winners at 5d (lib/rankquality analyzeRankQuality → persisted `sectionDecile`). On the
// 2026-09-09 ledger every native section graded `noise` (overall date-clustered IC
// −0.019; the 90-100 bucket did WORSE than its base rate), yet the rank still multiplied
// a 95th-percentile name to ~2× the confidence of a 45th-percentile one. This factor
// scales the SPREAD of the comparable score around neutral by the persisted verdict —
// the level is never touched and a sign is never flipped (an `inverted` verdict is a
// lane-level AVOID question for expectancyTilt, not a rank inversion).
//
// Weights: predictive 1 (full spread), weak-positive 0.5, noise/inverted 0 (the score
// carries no ranking information — the composite then orders by realized lane record,
// execution and cost, which is the part of the formula the ledger supports),
// insufficient/unmeasured 0.5 (unknown ranks ABOVE measured-noise, mirroring the tilt's
// "unproven beats proven-losing" ordering). A summary with NO `sectionDecile` at all is
// feature-off (weight 1) so pre-verdict callers score byte-identically to before.
const INFORMATIVENESS_W = Object.freeze({ predictive: 1, 'weak-positive': 0.5, noise: 0, inverted: 0, insufficient: 0.5 });
const UNMEASURED_W = 0.5;
// A MERGED row's comparable value is the dependence-discounted conviction across several
// sources (mergeSignals), not the base section's own score — the verdict measured that
// base score alone. Flattening a corroborated row to neutral on its base section's
// `noise` would rank it below a lone unmeasured source. Floor the spread at the
// unmeasured weight instead: corroboration keeps half its spread, never more than a
// predictive verdict would grant.
const MERGED_FLOOR_W = UNMEASURED_W;
const NEUTRAL_CONFIDENCE = 50;
function scoreInformativeness(sectionDecile, section, { mergedFrom = 1 } = {}) {
  const single = singleSectionInformativeness(sectionDecile, section);
  if (!(mergedFrom > 1) || single.verdict === 'feature-off' || single.w >= MERGED_FLOOR_W) return single;
  return { ...single, w: MERGED_FLOOR_W, basis: `${single.basis}; corroborated by ${mergedFrom} sources — spread floored at ${MERGED_FLOOR_W} (merged conviction is not the base section's own score)` };
}
function singleSectionInformativeness(sectionDecile, section) {
  const hasVerdicts = !!(sectionDecile && typeof sectionDecile === 'object' && Object.keys(sectionDecile).length);
  if (!hasVerdicts) {
    return { w: 1, verdict: 'feature-off', ic: null, t: null, n: null, horizon: null,
      basis: 'no persisted rank-quality verdicts on this summary — score spread passed through unchanged' };
  }
  const entry = section ? sectionDecile[section] : null;
  const known = entry && Object.prototype.hasOwnProperty.call(INFORMATIVENESS_W, entry.verdict);
  if (!known) {
    return { w: UNMEASURED_W, verdict: 'unmeasured', ic: null, t: null, n: entry && entry.n != null ? entry.n : null, horizon: null,
      basis: `no rank-quality verdict for section "${section || '?'}" (fewer than the resolved minimum) — spread halved, ranks above measured noise` };
  }
  const w = INFORMATIVENESS_W[entry.verdict];
  const icTxt = Number.isFinite(entry.ic) ? `IC ${entry.ic}` : 'IC n/a';
  return { w, verdict: entry.verdict, ic: Number.isFinite(entry.ic) ? entry.ic : null, t: Number.isFinite(entry.t) ? entry.t : null,
    n: entry.n != null ? entry.n : null, horizon: entry.horizon || null,
    basis: `section "${section}" own-score rank quality at ${entry.horizon || '5d'}: ${entry.verdict} (${icTxt}, n ${entry.n != null ? entry.n : '?'}) — spread × ${w}` };
}
// The comparable score with its spread around neutral scaled by `w`. Level preserved.
function informedConfidence(comparable, w) {
  const c = Number.isFinite(comparable) ? comparable : NEUTRAL_CONFIDENCE;
  const k = Number.isFinite(w) ? Math.max(0, Math.min(1, w)) : 1;
  return +(NEUTRAL_CONFIDENCE + (c - NEUTRAL_CONFIDENCE) * k).toFixed(1);
}
...
function confidenceScore({ rawConfidence = 50, evidence, exp }) {
  ...
  // NOTE: this still shrinks on PICK count, which has the same independence problem the
  // tilt was just fixed for — a hundred picks over four dates is four observations' worth
  // of trust, not a hundred. Deliberately left alone in this change: it is a 0.9..1.0
  // band (a ±5% effect) rather than the tilt's ±30%, and moving it shifts unrelated
  // ranking baselines. Recorded as a follow-up rather than bundled in.
...
function makeSignal(input = {}) {
  ...
  return {
    signal: {
      ...
      // Score/probability semantics (defect #7): board signals carry RANKING numbers
      // (rawConfidence, score, percentile). No swing source ships an out-of-fold
      // calibrated model, so probability is structurally null here — it can only be
      // set by a calibrated artifact through lib/rank-semantics.describeRank, never
      // by transforming a score.
      probability: null,
      ...
      // Scoped evidence identity (defect #5): which universe scope produced this
      // signal (large/small/micro/expanded/biotech, null for scope-less sources).
      // Part of the track-record join — never a display label.
      universeScope: input.universeScope || null,
      // Upstream lifecycle reads count: Day Trade publishes `lifecycleState` and it was
      // silently dropped here (only `stateHint` was read), freezing every board row at
      // "newly detected". Either key now reaches the lifecycle derivation.
      stateHint: input.stateHint || input.lifecycleState || null,
...
function mergeSignals(signals) {
  ...
  for (const members of groups.values()) {
    ...
    // EXPLICIT MERGE MODEL (redesign Phase 9) — replaces `Math.max(rawConfidence)`, which
    // let the single loudest source (often an LLM read on a different scale entirely) set
    // the conviction for the merged row. Now: a frozen per-source prior on the strongest
    // NORMALIZED read, plus dependence-discounted, decaying increments from the others,
    // capped so correlated agreement cannot compound into false conviction. `rawConfidence`
    // keeps its old meaning for anything that still reads it; `mergedConviction` carries
    // the modelled value and its full decomposition.
    const SN = require('./score-normalize');
    const SCon = require('./strategy-contracts');
    const merge = SN.mergeConviction(members, {
      familyOf: (m) => m.family || SOURCE_FAMILY[m.source] || 'priceTrend',
      leadOnly: (m) => { const c = SCon.contractFor(m.source); return !!(c && c.fillPolicy === 'lead-only'); },
    });
    out.push({
      ...base,
      evidenceFamilies,
      ...
      sources,
      rawConfidence: merge.conviction,
      mergedConviction: merge,
      // The merged row's comparable value IS the modelled conviction — not the base
      // member's own normalized read, which would silently discard the merge.
      normalized: { value: merge.conviction, basis: 'merged conviction (frozen priors + dependence-discounted increments, capped)', sourceN: merge.sources, prior: null, missing: merge.missing },
...
// ── Trend-core consolidation of asserted trend-pair credits ──────────────────
// lib/trend-core.js exists because the trend-continuation engines all re-read ONE price
// series (measured on prod at ~0.96 return correlation): however many pile on, they are
// ONE price-trend evidence unit. effectiveEvidence() credits every extra engine at its
// pair credit — measured where the ledgers earned one, else the asserted same-family
// PRIOR (CORR_DISCOUNT). For a pair of trend-core engines that prior is exactly the
// double-count trend-core was built to remove, so an extra PRICE_TREND_ENGINE whose only
// backing is the asserted prior against another PRICE_TREND_ENGINE contributes ZERO.
//
// A trend pair the redundancy model has MEASURED keeps its earned credit: the model
// already discounted that pair from observed overlap/correlation, so consolidating it
// too would charge the same redundancy twice — and a measurement beats the blanket
// assertion (this repo's standing rule). The unmeasured fallback path (static family
// count) is untouched: distinct-family counting already collapses the trend sleeve.
// Conservative by construction: credits are only ever removed, so the evidence count —
// and evidenceMultiplier — can only fall, never rise, vs the unwired computation.
function consolidateTrendPriorCredits(eff, redundancy) {
  const isTrendEngine = (e) => TC.PRICE_TREND_ENGINES.includes(e);
  // Same pair-key layout redundancy.js uses for model.credits ("a|b", both directions
  // stored on measure) — guarded by test: a drift here would zero measured trend pairs.
  const isMeasuredPair = (a, b) => !!(redundancy && redundancy.credits
    && Number.isFinite(redundancy.credits[`${a}|${b}`]));
  const consolidatedEngines = [];
  let creditRemoved = 0;
  const credits = (eff.credits || []).map((c) => {
    const assertedTrendPair = c.against != null && c.credit > 0
      && isTrendEngine(c.source) && isTrendEngine(c.against)
      && !isMeasuredPair(c.source, c.against);
    if (!assertedTrendPair) return c;
    consolidatedEngines.push(c.source);
    creditRemoved += c.credit;
    return { ...c, credit: 0, consolidatedBy: 'trend-core' };
  });
  return {
    score: +(eff.score - creditRemoved).toFixed(2),
    credits,
    consolidatedEngines,
    creditRemoved: +creditRemoved.toFixed(2),
  };
}
...
function evidenceFor(sig, redundancy) {
  ...
  // Trend-core consolidation (see consolidateTrendPriorCredits above): asserted-prior
  // credit between two trend-core engines is removed; measured credits pass through.
  const cons = consolidateTrendPriorCredits(eff, redundancy);
  return {
    ...base,
    score: cons.score,
    effectiveCount: cons.score,
    measured: true,
    credits: cons.credits,
    redundantAgreement: engines.length >= 2 && cons.score < 1.5,
    priorScore: base.score,
    // The pre-consolidation crediting — what the rank used before trend-core was wired —
    // stays visible for audit, exactly like priorScore.
    unconsolidatedCount: eff.score,
    ...(cons.consolidatedEngines.length ? {
      trendCore: { consolidatedEngines: cons.consolidatedEngines, creditRemoved: cons.creditRemoved },
    } : {}),
  };
}
...
function rankSignals(signals, { regime, scoreboard, includeInactive = false, redundancy = null, origins = null } = {}) {
  ...
  const enriched = (signals || []).map((sig) => {
    // LIFECYCLE AGE FROM IMMUTABLE ORIGINS. Normalizers rarely carry ageBars, so every row
    // aged 0 forever — 'expired' was unreachable and re-emitted picks looked newly detected.
    // The origin snapshot (captured at FIRST detection, bars advanced once per session,
    // never rewritten) is the authoritative age; the signal's own ageBars can only add to
    // it, never reset it. detectedAt likewise falls back to the origin's first date.
    const origin = origins ? (origins[sig.id] || null) : null;
    const ageBars = Math.max(
      Number.isFinite(sig.ageBars) ? sig.ageBars : 0,
      origin && Number.isFinite(origin.bars) ? origin.bars : 0);
    const detectedAt = sig.detectedAt || (origin && origin.firstDate) || null;
    const state = lifecycleState({
      price: sig.price, entry: sig.entry, stop: sig.stop, target: sig.target,
      ageBars, horizon: sig.horizon, hint: sig.stateHint,
    });
    const evidence = evidenceFor(sig, redundancy);
    const breadth = domainBreadth(sig.evidenceFamilies || [sig.family]);
    const exp = expectancyFor(sig.section || sig.source, sig.tier || sig.setup, sig.horizon, scoreboard, sig.universeScope || null);
    const tiltOut = expectancyTilt(exp);
    const { tilt } = tiltOut;
    const exq = executionQuality(sig.liquidity || {});
    const rf = regimeFit(sig.side, regime);
    // COMPARABLE INPUT (redesign Phase 9): the confidence input is the WITHIN-SOURCE
    // normalized value (a percentile inside that screener's own same-day cross-section, or
    // a neutral-shrunk score when the cross-section is too thin to support one) — never the
    // raw 0–100 numbers, which mean different things per source. Absent ⇒ the raw value, so
    // a caller that has not normalized behaves exactly as before.
    const comparable = sig.normalized && Number.isFinite(sig.normalized.value) ? sig.normalized.value : sig.rawConfidence;
    // RANK-QUALITY GATE: the comparable score's spread is scaled by the section's persisted
    // own-score verdict (see scoreInformativeness). A section the ledger graded `noise`
    // enters at neutral, so its rows are ordered by realized record, execution and cost.
    const informativeness = scoreInformativeness(scoreboard && scoreboard.sectionDecile, sig.section || sig.source, { mergedFrom: sig.mergedFrom || 1 });
    const informed = informedConfidence(comparable, informativeness.w);
    const confidence = confidenceScore({ rawConfidence: informed, evidence, exp });
    ...
    return {
      // `expectancyTiltWhy` states WHY the rank was (or was not) tilted — neutral for
      // want of independent dates, ranked out on a significantly negative record, or
      // boosted on an interval clear of zero. Previously the multiplier appeared with
      // no way to tell a genuine edge from a small-sample artifact.
      ...sig, ageBars, detectedAt, state, evidence, breadth, expectancy: exp, expectancyTilt: tilt,
      expectancyTiltWhy: tiltOut.reason || null, expectancyTiltBasis: tiltOut.basis || null,
      // Explicit "ranked out" flag so a renderer never re-derives NEGATIVE_TILT as a literal.
      expectancyTiltNegative: tilt === NEGATIVE_TILT,
      execution: exq, regimeFit: rf, confidence, evidenceMult, cost, score,
      // Score decomposition (Phase 9 item 7) — every multiplicative factor, auditable.
      scoreDecomposition: {
        comparableConfidence: comparable,
        normalizationBasis: (sig.normalized && sig.normalized.basis) || 'raw (not normalized by this caller)',
        // Rank-quality gate (2026-09-09): the spread weight the section's own-score verdict
        // earned, and the confidence input actually used after applying it.
        scoreInformativeness: informativeness,
        informedConfidence: informed,
        confidence, regimeFit: rf, execution: exq.quality, expectancyTilt: tilt,
        evidenceMult, costPenalty: cost.penalty, remainingMult: origins ? remaining.mult : 1,
        formula: 'score = confidence(informedConfidence = 50 + (comparable - 50) x scoreInformativeness.w) x regimeFit x execution x expectancyTilt x evidenceMult x costPenalty x remainingMult',
        expectancyBasis: exp && exp.basis ? exp.basis : null,
      },
...
module.exports = {
  ...
  scoreInformativeness, informedConfidence, INFORMATIVENESS_W, MERGED_FLOOR_W, NEGATIVE_TILT,
