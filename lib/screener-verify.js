...
// read-modify-write) plus a rollup index (episodes/screener/rollup.json, splice-updated
// per run) and the compact persisted reduction (episodes/screener/fill-verification.json)
// that the Scoreboard reads. Verification accrues PROSPECTIVELY: nothing here can
...
const RESOLVE_CAP = 25;            // bounded UNIQUE (ticker, range) daily-candle fetches per run
const FETCH_CONCURRENCY = 6;       // bounded fan-out via mapLimit — the capped fetches used to run
                                   // SERIALLY (a 25-50s latency wall inside the warm-chain budget)
...
// Compact persisted reduction (a few hundred bytes) — the ONLY doc op=scoreboard
// reads. The rollup grows with history; the summary must never download it live.
const FILL_VERIFICATION_DOC = 'episodes/screener/fill-verification.json';
...
// ── Rollup maintenance ───────────────────────────────────────────────────────
// PURE splice-update: replace the prior rollup's entries for every date loaded in
// `shards` — this run's FULL mutation scope (the 21-day window + reopened dates;
// episodes are append-only per (date,ticker) and resolution mutations are confined to
// loaded dates) — with fresh projections, passing every other date's entries through
// untouched. Emits date-ascending groups in shard insertion order, exactly what a
// full rebuild produces, so splice and rebuild are interchangeable byte-for-byte.
function spliceRollupEpisodes(priorEpisodes, shards) {
  const groups = new Map(); // date → projected episodes[], insertion-ordered per date
  for (const e of Array.isArray(priorEpisodes) ? priorEpisodes : []) {
    if (!e) continue;
    const d = typeof e.date === 'string' && e.date ? e.date : '';
    if (Object.prototype.hasOwnProperty.call(shards, d)) continue; // superseded by the loaded shard
    if (!groups.has(d)) groups.set(d, []);
    groups.get(d).push(e);
  }
  for (const d of Object.keys(shards)) {
    const eps = Object.values((shards[d] && shards[d].episodes) || {}).filter(Boolean).map(projectEpisode);
    if (eps.length) groups.set(d, eps); // an empty loaded shard contributes nothing — its prior entries were already dropped above
  }
  return [...groups.keys()].sort().flatMap(d => groups.get(d));
}

// FULL rebuild from every shard — O(history) Blob reads BY DESIGN, so it is never the
// steady-state path: only ?rebuild=1 (explicit repair) or a missing/unreadable prior
// rollup (fail-safe: splice has no base) reach it.
async function rebuildRollupEpisodes(shards, listBlobs) {
  const blobs = []; let cursor;
  do { const r = await listBlobs({ prefix: EPISODE_PREFIX, cursor, limit: 1000 }); blobs.push(...(r.blobs || [])); cursor = r.cursor; } while (cursor);
  ...
  for (const d of allDates) {
    ...
    for (const e of Object.values((doc && doc.episodes) || {})) if (e) out.push(projectEpisode(e));
  }
  return out;
}

// Scoreboard-facing read: the compact persisted reduction, never the full rollup.
// FAIL CLOSED: an absent, null, or malformed doc returns null, the summary key stays
// absent, and the summary is byte-identical to the pre-pipeline one.
async function readFillVerificationForSummary() {
  const doc = await S.readJSON(FILL_VERIFICATION_DOC, null).catch(() => null);
  const fv = doc && doc.fillVerification;
  return fv && typeof fv === 'object' && fv.screener && typeof fv.screener === 'object' ? fv : null;
}
...
async function runScreenerVerify(req, res, overrides) {
  ...
  const listBlobs = deps.listBlobs || (opts => require('@vercel/blob').list(opts));
  ...
  // Prior rollup, read ONCE per run: the index of episodes older than the scan window
  // (used below to reopen stranded OPEN fills) AND the splice base for this run's
  // rollup update — the one full download this op performs.
  ...
  //
  // The capped fetches used to run SERIALLY inside the grading loop — up to FETCH_CAP
  // latency-bound calls one at a time. The jobs are independent, so candidates are now
  // pre-sliced to the cap (deduped by identical (ticker, window) so a duplicate ledger
  // row can never buy two identical calls) and fetched through mapLimit. Grading stays
  // ONE deterministic pass in date order AFTER the fetches land: each iteration writes
  // a distinct in-memory (date,ticker) key and persistence is a single pass below, so
  // fetch completion order cannot affect episode content.
  let graded = 0, unavailable = 0, deferred = 0, invalidPicks = 0, capped = 0;
  ...
  for (const d of Object.keys(pickDays).sort()) {
    for (const pick of pickDays[d]) {
      ...
      entryJobs.push({ d, pick, trigger, from, to, key: `${pick.ticker}|${from}|${to}` });
    }
  }
  const barsByKey = new Map();       // fetch key → barsBySession (shared read-only result)
  const entryFetchJobs = [];
  for (const job of entryJobs) {
    if (barsByKey.has(job.key)) continue;
    if (entryFetchJobs.length >= FETCH_CAP) { job.capped = true; capped++; continue; } // bounded — the rest retry next run
    barsByKey.set(job.key, null);
    entryFetchJobs.push(job);
  }
  await mapLimit(entryFetchJobs, FETCH_CONCURRENCY, async (job) => {
    barsByKey.set(job.key, await fetch5min(job.pick.ticker, job.from, job.to));
  });
  for (const job of entryJobs) {
    if (job.capped) continue;
    const { d, pick, trigger } = job;
    const shard = shards[d];
    {
      if (shard.episodes[pick.ticker]) continue;                 // duplicate row — first one already graded this pass
      const barsBySession = barsByKey.get(job.key);
  ...
  let resolvedNow = 0, reopenedBeyondWindow = 0, deferredResolves = 0;
  ...
  // Candidates in deterministic date order, then the daily-history fan-out DEDUPED by
  // (ticker, range): the same ticker open across several signal dates used to re-fetch
  // its identical history once PER DATE, serially. The cap now bounds UNIQUE fetches
  // (budget assigned oldest-date-first, as before); a candidate whose history is
  // already scheduled rides along free, and only cap-starved candidates defer.
  const resolveJobs = [];
  for (const d of Object.keys(shards).sort()) {
    ...
    for (const [ticker, e] of Object.entries(shards[d].episodes || {})) {
      ...
      resolveJobs.push({ d, ticker, e, range, key: `${ticker}|${range}` });
    }
  }
  const histByKey = new Map();       // fetch key → daily history (shared read-only result)
  const resolveFetchJobs = [];
  for (const job of resolveJobs) {
    if (histByKey.has(job.key)) continue;
    if (resolveFetchJobs.length >= RESOLVE_CAP) { job.deferred = true; deferredResolves++; continue; }
    histByKey.set(job.key, null);
    resolveFetchJobs.push(job);
  }
  await mapLimit(resolveFetchJobs, FETCH_CONCURRENCY, async (job) => {
    histByKey.set(job.key, await fetchDaily(job.ticker, job.range).catch(() => null));
  });
  for (const job of resolveJobs) {
    if (job.deferred) continue;
    const { d, ticker, e } = job;
    const hist = histByKey.get(job.key);
  ...
  // Rollup maintenance — SPLICE-UPDATE (audit 2026-08-15). The rollup used to be
  // rebuilt from EVERY historical shard on EVERY run: O(history) sequential Blob reads
  // outside the 21-day window, growing forever. Every date this run can mutate is
  // already loaded in `shards`, so the prior rollup only needs THOSE dates' entries
  // replaced (in-memory docs also override stale Blob read-backs, as before). The full
  // rebuild survives behind ?rebuild=1 for explicit repair, and runs automatically when
  // the prior rollup is missing/unreadable — splice has no base, so rebuilding is the
  // fail-safe, never a silent skip.
  const forceRebuild = !!(req && req.query && String(req.query.rebuild) === '1');
  let rollupEpisodes = [];
  try {
    const hasPrior = !!(priorRollup && Array.isArray(priorRollup.episodes));
    const rollupMode = (forceRebuild || !hasPrior) ? 'rebuild' : 'splice';
    rollupEpisodes = rollupMode === 'rebuild'
      ? await rebuildRollupEpisodes(shards, listBlobs)
      : spliceRollupEpisodes(priorRollup.episodes, shards);
    let shouldWrite = touched.size > 0 || forceRebuild;
    if (!shouldWrite && !hasPrior) {
      // Quiet-day bootstrap: write only when the rollup doc is PROVABLY absent.
      // S.blobExists THROWS when existence cannot be determined — unknown is NOT
      // absent: never clobber a doc that may exist behind a flaky read; the safe
      // path is to skip and retry next run (shards stay authoritative throughout).
      let exists = null;
      try { exists = await S.blobExists(ROLLUP_DOC); } catch { exists = null; }
      shouldWrite = exists === false;
    }
    if (shouldWrite) {
    ...
    // Compact fill-verification doc — the few-hundred-byte reduction op=scoreboard
    // reads (lib/apex-routes) instead of downloading the whole rollup. Written when
    // the in-memory rollup is trustworthy: a splice base IS the real prior rollup,
    // and a rebuild is trusted exactly when it was persisted above.
    if (shouldWrite || rollupMode === 'splice') {
      const updatedAt = new Date().toISOString();
      await S.writeJSON(FILL_VERIFICATION_DOC, {
        version: SCREENER_VERIFY_VERSION, updatedAt,
        fillVerification: fillVerificationForSummary({ episodes: rollupEpisodes, updatedAt }),
      }, 0);
    }
  } catch { /* rollup maintenance is retried next run; shards remain authoritative */ }
...
module.exports = {
  ...
  FETCH_CONCURRENCY, FILL_VERIFICATION_DOC,
  resolveEntryFromBars, isEtSessionComplete, entrySessionCalendarHoles,
  buildEpisode, timeExitFromDaily, projectEpisode, spliceRollupEpisodes,
  readEpisodeRollup, fillVerificationForSummary, readFillVerificationForSummary, runScreenerVerify,
};
