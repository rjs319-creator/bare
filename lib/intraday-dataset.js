...
// v2 (2026-08-03): SAMPLING-CONTRACT FIX. v1's hard 400-row slice could drop rows already
// stamped sampleProb:1 (at-risk) and silently shrink the effective inclusion probability of
// ordinary negatives — inverse-probability weights then over-weighted every survivor on
// exactly the wild-tape days that matter. v2 guarantees: a recorded probability IS the
// inclusion probability. When the cap binds, each class is thinned by a further
// DETERMINISTIC-HASH stage whose rate is RECORDED into the row's sampleProb (at-risk may
// carry <1 only under overflow, explicitly), and the bucket doc records preCap/postCap
// counts + the cap threshold. Old v1 rows are versioned out by the fail-closed join.
const DATASET_VERSION = 'intraday-dataset-v2';
...
function buildScanRows(universeRows, { date, at, sessionBucket = null } = {}) {
  ...
  for (const r of universeRows || []) {
    ...
    out.push({
      ...
      // Baseline-vintage + universe-inclusion provenance (auditable per row) and the data
      // provenance class: rows are captured LIVE and labeled strictly-after — prospective
      // by construction, never a historical reconstruction.
      baselineDate: r.baselineDate ?? null, baselineSource: r.baselineSource ?? null,
      inclusionReason: r.inclusionReason ?? null, inclusionProb: r.inclusionProb ?? null,
      provenance: 'prospective-live',
      sampleProb: d.sampleProb, atRisk: d.sampleProb === 1,
      datasetVersion: DATASET_VERSION,
    });
  }
  // CAP CONTRACT (v2). The recorded sampleProb must equal the TRUE inclusion probability:
  //   • At-risk rows keep sampleProb 1 whenever they all fit. If at-risk alone overflows the
  //     cap, the at-risk class is thinned by a deterministic hash at rate cap/atRiskN and
  //     every kept row's sampleProb becomes that rate (recorded, not silently violated).
  //   • Ordinary negatives get the remaining capacity; when their sampled set overflows it,
  //     a second hash stage thins them and sampleProb becomes NEG_RATE × secondStageRate.
  // Hash thinning (uniform over date|bucket|ticker) preserves unbiased 1/p weighting — a
  // severity-ordered slice would not (it selects the strongest tail and calls it random).
  const preCap = out.length;
  const atRiskRows = out.filter(r => r.atRisk);
  const ordinaryRows = out.filter(r => !r.atRisk);
  const hashFrac = r => (fnv1a(`cap|${date}|${bucket}|${r.ticker}`) % 100000) / 100000;
  const thin = (rows, rate) => rows.filter(r => hashFrac(r) < rate);

  let keptAtRisk = atRiskRows, atRiskStageRate = 1;
  if (atRiskRows.length > MAX_ROWS_PER_SCAN) {
    atRiskStageRate = MAX_ROWS_PER_SCAN / atRiskRows.length;
    keptAtRisk = thin(atRiskRows, atRiskStageRate).map(r => ({ ...r, sampleProb: +(r.sampleProb * atRiskStageRate).toFixed(5), capThinned: true }));
  }
  const remainingCapacity = Math.max(0, MAX_ROWS_PER_SCAN - keptAtRisk.length);
  let keptOrdinary = ordinaryRows, ordinaryStageRate = 1;
  if (ordinaryRows.length > remainingCapacity) {
    ordinaryStageRate = remainingCapacity > 0 ? remainingCapacity / ordinaryRows.length : 0;
    keptOrdinary = thin(ordinaryRows, ordinaryStageRate).map(r => ({ ...r, sampleProb: +(r.sampleProb * ordinaryStageRate).toFixed(5), capThinned: true }));
  }
  // Deterministic output order (severity first) for stable docs — ORDER no longer decides
  // survival, only the recorded-rate hash stages above do.
  const kept = [...keptAtRisk, ...keptOrdinary].sort((a, b) => ((b.atRisk ? 1 : 0) - (a.atRisk ? 1 : 0))
    || (severityOf(b) - severityOf(a))
    || (a.ticker < b.ticker ? -1 : a.ticker > b.ticker ? 1 : 0));
  const dropped = preCap - kept.length;
  const droppedByReason = dropped ? {
    capExceeded: dropped,
    atRisk: atRiskRows.length - keptAtRisk.length,
    ordinary: ordinaryRows.length - keptOrdinary.length,
  } : null;
  return {
    bucket, rows: kept, dropped, droppedByReason,
    capContract: {
      preCapRows: preCap, postCapRows: kept.length, capThreshold: MAX_ROWS_PER_SCAN,
      atRiskTotal: atRiskRows.length, atRiskStageRate: +atRiskStageRate.toFixed(5),
      ordinaryStageRate: +ordinaryStageRate.toFixed(5),
    },
  };
}
...
async function appendScanRows(date, built, { store = null } = {}) {
  ...
  await S.writeJSON(key, {
    ...
    capContract: built.capContract || null,
  ...
  for (const t of tickers) {
    ...
    for (const r of byTicker.get(t)) {
      const label = computeLabels({
        decisionPrice: r.last, decisionAt: r.at, atr: r.atr, sessionBars, prevClose: r.prevClose,
        // Tiered cost context: this row's own measured liquidity + session bucket.
        costContext: { avgDollarVol: r.avgDollarVol ?? null, sessionBucket: r.sessionBucket ?? null },
      });
