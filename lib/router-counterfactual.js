...
const STATIC_BURN_IN_ROWS = 40;    // rows of strictly-prior evidence before staticBest may lock
...
function evaluateRouterCounterfactual({ historyDays = [], rows = [], minDates = MIN_HISTORY_DATES } = {}) {
  ...
  const perDate = [];
  const arms = { router: [], equal: [], bestRecent: [], staticBest: [], cash: [] };
  let routerAbstained = 0, bestRecentSkipped = 0, staticBestSkipped = 0;
  // staticBest arm — the "static best validated algorithm" comparator: pick ONCE, from the
  // first STATIC_BURN_IN_ROWS of strictly-prior evidence, then hold forever. Never re-picks
  // (a re-picking 'static' arm would just be a slower chaser). Deterministic and leak-free:
  // the lock uses only rows dated before the lock date.
  let staticPick = null;
  ...
  for (const h of history) {
    ...
    // staticBest arm — lock once enough strictly-prior rows exist, then hold.
    let staticRet = null;
    {
      if (staticPick == null) {
        const priorDates = allRowDates.filter(d => d < h.date);
        const prior = new Map();
        let priorRows = 0;
        for (const d of priorDates) {
          const pm = algoMeans.get(d);
          for (const [a, v] of Object.entries(pm)) {
            if (!Number.isFinite(v)) continue;
            if (!prior.has(a)) prior.set(a, []);
            prior.get(a).push(v); priorRows++;
          }
        }
        if (priorRows >= STATIC_BURN_IN_ROWS && prior.size) {
          let bestAlgo = null, bestMean = -Infinity;
          for (const [a, xs] of prior) { const m = mean(xs); if (m > bestMean) { bestMean = m; bestAlgo = a; } }
          staticPick = bestAlgo;
        }
      }
      if (staticPick != null) {
        // Holds through inactive days at 0 — switching to cash would flatter it.
        staticRet = Number.isFinite(perAlgo[staticPick]) ? perAlgo[staticPick] : 0;
      } else {
        staticBestSkipped++;
      }
    }
    ...
    arms.staticBest.push(staticRet);
    arms.cash.push(0);
    perDate.push({
      date: h.date, activeAlgos: active.length,
      router: round3(routerRet), equal: round3(equalRet),
      bestRecent: round3(bestRet), staticBest: round3(staticRet), abstained: wSum <= 0,
    });
  }
  ...
  return Object.freeze({
    ...
    coverage: {
      ...
      staticBestSkippedDates: staticBestSkipped,
      staticBestPick: staticPick,
    ...
    arms: {
      ...
      staticBest: summarizeArm(arms.staticBest),
...
module.exports = {
  ROUTER_CF_VERSION, MIN_HISTORY_DATES, BEST_RECENT_WINDOW, BEST_RECENT_MIN_ROWS, STATIC_BURN_IN_ROWS,
