...
function advance({ prior = {}, horizon, rows = [], now = new Date(), sessionDay = null } = {}) {
  ...
  for (const row of rows) {
    ...
    const isNew = !p;
    const catalystAdded = !!(row.catalyst && (!p || p.catalyst !== row.catalyst));
    const priorContradictions = (p && p.contradicting) || [];
    const contradictionAdded = (row.contradicting || []).some(c => !priorContradictions.includes(c));
    const optionsChanged = !!(p && row.optionsState && p.optionsState !== row.optionsState);
    const attentionSaturated = row.attentionState === 'SATURATED' && (!p || p.attentionState !== 'SATURATED');
    ...
    const record = {
      ...
      optionsState: row.optionsState || null,
      attentionState: row.attentionState || null,
