...
const V3H = require('./pitdata/v3/health');

const DATA_HEALTH_VERSION = 'data-health-v2';   // v2: pit-data-v3 sections + three-tier readiness
...
async function buildDataHealthReport({ now = Date.now() } = {}) {
  ...
  // pit-data-v3: collector state, capability probe, latest run manifest,
  // reconciliation, raw-integrity sample, identity confidence, universe
  // coverage, label states, prospective-vs-reconstructed mix + the three-tier
  // readiness verdict. Failures are captured, never thrown.
  const pitdataV3 = await section('pitdataV3', async () => {
    const h = await V3H.buildV3Health({ readJSON, writeJSON, now: () => now });
    return {
      readiness: h.readiness,
      missingMandatorySections: h.missingMandatorySections,
      problems: h.problems,
      sections: h.sections,
      survivorshipSafe: h.survivorshipSafe,   // structurally false until gates + human promotion
    };
  });
  sections.push(pitdataV3);
  ...
  if (pitdataV3.ok && pitdataV3.readiness && !pitdataV3.readiness.operationallyHealthy) {
    problems.push('pitdataV3: not operationally healthy');
  }

  // Three-tier readiness (fail closed): a missing required key or
  // survivorshipSafe=false caps research/promotion readiness regardless of
  // anything else in this report.
  const keysSection = sections.find((s) => s.name === 'keys') || {};
  const requiredKeyMissing = !keysSection.fmp || !keysSection.blobStore;
  const v3r = (pitdataV3.ok && pitdataV3.readiness) || {};
  const readiness = {
    operationallyHealthy: problems.length === 0,
    researchReady: !requiredKeyMissing && v3r.researchReady === true,
    // promotion-ready = gates engineering-complete + a human promotion proposal
    // exists. survivorshipSafe itself flips only when the reviewed consumer
    // switch is APPLIED in code — a report can never grant it.
    promotionReady: !requiredKeyMissing && v3r.promotionReady === true,
    survivorshipSafe: pitdataV3.ok ? pitdataV3.survivorshipSafe === true : false,
  };
