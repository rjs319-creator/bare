...
const clean = (items) => (items || []).filter(x => x && Number.isFinite(+x.score) && Number.isFinite(+x.outcome))
  .map(x => ({ score: +x.score, outcome: +x.outcome, won: x.won == null ? +x.outcome > 0 : !!x.won, date: x.date || null }));
...
// POOLED-DESCRIPTIVE ONLY (graduation-league QM-1): pooling picks across decision
// dates mixes market-timing variation into a cross-sectional claim, and the t
// treats correlated same-day, overlapping-horizon picks as independent. The
// significance that a verdict may consume is `dateClusteredIC` below.
function informationCoefficient(items) {
  ...
  return { ic: +ic.toFixed(3), n, t: Number.isFinite(t) ? +t.toFixed(2) : t, significant: Math.abs(t) >= 2, basis: 'pooled-iid-descriptive' };
}

// Date-clustered rank-IC (QM-1, the harness-v3 pattern): one Spearman IC per
// decision date over that date's own cross-section, then a HAC/Newey-West mean over
// the per-date IC series. Requires items to carry `date`; dates with fewer than
// `minPerDate` items are skipped (a 2-pick "cross-section" is not a ranking).
function dateClusteredIC(items, { minPerDate = 5, minDates = 8, horizonBars = 5 } = {}) {
  const byDate = new Map();
  for (const x of items) { if (!x.date) continue; (byDate.get(x.date) || byDate.set(x.date, []).get(x.date)).push(x); }
  const rows = [];
  for (const [date, arr] of byDate) {
    if (arr.length < minPerDate) continue;
    const r = informationCoefficient(arr);
    if (r.ic != null) rows.push({ date, v: r.ic, n: arr.length });
  }
  if (rows.length < minDates) return { ic: null, dates: rows.length, significant: false, basis: 'date-clustered', note: `need ≥${minDates} dates with ≥${minPerDate} picks (have ${rows.length})` };
  try {
    ...
    // 2dp for display, but per-date IC means live at ~0.004-0.05 — quantizing before the
    // t turned a real mean into ic 0.00 with t 0.00, and `significant` (the verdict
    // authority) was decided from that garbage. Prefer avgExact/seExact; fall back to the
    // rounded fields only for summaries persisted before the exact fields existed.
    const avg = s && Number.isFinite(s.avgExact) ? s.avgExact : (s ? s.avg : null);
    const se = s && Number.isFinite(s.seExact) ? s.seExact : (s ? s.se : null);
    if (s && Number.isFinite(avg) && Number.isFinite(se) && se > 0) {
      const t = avg / se;
      return { ic: +avg.toFixed(3), dates: s.n, effectiveN: s.effectiveN ?? null, t: +t.toFixed(2), significant: Math.abs(t) >= 2, ci95: s.ci95 || null, basis: 'date-clustered (per-date IC, HAC mean)' };
    }
  } catch { /* fall through */ }
  const m = mean(rows.map(r => r.v));
  return { ic: +m.toFixed(3), dates: rows.length, t: null, significant: false, basis: 'date-clustered (HAC unavailable — unsignificant by default)' };
}
...
function analyzeRankQuality(rawItems, opts = {}) {
  ...
  // Significance authority (QM-1): the DATE-CLUSTERED statistic when the feed carries
  // dates; the pooled t is descriptive and may not mint 'predictive'/'inverted' on its
  // own when a clustered lane exists.
  const icClu = dateClusteredIC(items, { horizonBars: opts.horizonBars || 5 });
  const hasClu = icClu.ic != null;
  const ic = hasClu ? icClu.ic : icRes.ic;
  const sig = hasClu ? icClu.significant : icRes.significant;
  const verdictBasis = hasClu ? icClu.basis : (icRes.basis || 'pooled-iid-descriptive') + ' (no per-date lane — interpret significance cautiously)';
  let verdict;
  if (ic == null) verdict = 'insufficient';
  else if (ic >= 0.05 && sig && mono.monotone) verdict = 'predictive';
  else if (ic >= 0.03 && mono.rho != null && mono.rho > 0) verdict = 'weak-positive';
  else if (ic <= -0.05 && sig) verdict = 'inverted';
  else verdict = 'noise';
...
module.exports = { RQ_VERSION, analyzeRankQuality, informationCoefficient, dateClusteredIC, quantileStats, monotonicity, calibration, averageRanks, pearson };
