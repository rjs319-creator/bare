...
function dateClusteredIC(items, { minPerDate = 5, minDates = 8, horizonBars = 5 } = {}) {
  ...
  try {
    ...
    if (s && Number.isFinite(s.avg) && Number.isFinite(s.se) && s.se > 0) {
      const t = s.avg / s.se;
      return { ic: +s.avg.toFixed(3), dates: s.n, effectiveN: s.effectiveN ?? null, t: +t.toFixed(2), significant: Math.abs(t) >= 2, ci95: s.ci95 || null, basis: 'date-clustered (per-date IC, HAC mean)' };
    }
  } catch { /* fall through */ }
