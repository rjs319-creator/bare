...
// How many scan slots the nightly cron dispatches in total (4 nested chains × 2 steps).
// The stride advances the whole window by SLOTS_PER_NIGHT slices per day so consecutive
// nights cover consecutive regions of the list and wrap.
const SLOTS_PER_NIGHT = 8;

// The epoch day is anchored to the CRON NIGHT, not raw UTC midnight (review 2026-08-15).
// The nightly chains fire at 22:00 UTC and GitHub's documented throttle can push later
// steps past 00:00 UTC — with a raw floor(now/86400000) computed PER REQUEST, those
// post-midnight slots jumped a full SLOTS_PER_NIGHT stride forward and ~8 slices were
// silently skipped that night. Subtracting an 18-hour anchor maps the whole
// 18:00Z → (next-day) 17:59Z window — comfortably containing the cron night plus its
// throttle drift — to ONE epoch day, so every slot of a straddling chain shares a stride.
const SLOT_DAY_ANCHOR_MS = 18 * 60 * 60 * 1000;   // day boundary at 18:00 UTC
function slotEpochDay(nowMs = Date.now()) {
  return Math.floor((nowMs - SLOT_DAY_ANCHOR_MS) / 86400000);
}

// Deterministic slice start for ?slot=N — a pure function of (epoch day, slot) with NO
// stored cursor, immune to the Blob overwrite read-back lag that made back-to-back
// cursor scans re-scan the same slice. Idempotent shards key by this start.
function slotStart({ slot, listLength, limit, epochDay = slotEpochDay() }) {
  const lim = Math.max(1, limit | 0);
  const slices = Math.max(1, Math.ceil(Math.max(0, listLength | 0) / lim));
  const idx = ((epochDay * SLOTS_PER_NIGHT + (slot | 0)) % slices + slices) % slices;
  return idx * lim;
}
...
async function runUniverseScan(req, res) {
  ...
  // ?cursor=1 (legacy) auto-advances through a Blob-persisted cursor and wraps. Known
  // hazard: two cursor scans dispatched back-to-back in one chain read the SAME cursor
  // (Blob overwrites propagate with a 10-30s+ read-back lag) and silently re-scan the
  // same slice — halving nightly coverage. The cron should use ?slot=N instead.
  const useCursor = req.query.cursor === '1';
  // ?slot=N (cron): a DETERMINISTIC slice with no Blob dependency at all — start is a
  // pure function of (epoch day, slot), advancing one stride of SLOTS_PER_NIGHT slices
  // each day and wrapping, so the scan self-completes over a few days and then
  // refreshes continuously, and concurrent same-night slots can never collide.
  const slot = req.query.slot != null ? Math.max(0, parseInt(req.query.slot, 10) || 0) : null;
  let start = Math.max(0, parseInt(req.query.start, 10) || 0);
  if (slot != null) {
    start = slotStart({ slot, listLength: list.length, limit });
  } else if (useCursor) { const cur = await readJSON('universe/scan-cursor.json', null); start = (cur && cur.next) || 0; if (start >= list.length) start = 0; }
...
module.exports = {
  ...
  slotStart, slotEpochDay, SLOTS_PER_NIGHT,
