 ...
 */
function detectTransition({ candles, residual = null, asOf, params = null } = {}) {
  // Versioned research parameters (atlasx-config TRANSITION_PARAMS). The default
  // set reproduces the historical inline constants exactly; `params` exists so
  // the research harness can compare alternative scalings — no eyeball retunes.
  const P = params || require('./atlasx-config').TRANSITION_PARAMS;
  ...
  const s = {};
  {
    const p = P.compressionToExpansion;
    s.compressionToExpansion = shrink(conf,
      clamp01(wasCompressed) * sig(p.expGain * (expansionNow - p.expOffset)) * sig(p.volGain * (volAccel - p.volOffset)));
  }
  {
    const p = P.accumulationToDemand;
    s.accumulationToDemand = shrink(conf,
      sig(p.adGain * ad) * sig(p.volGain * (volAccel - p.volOffset)) * sig(p.retGain * ((ret5 || 0))));
  }
  s.neutralToPositiveResidual = shrink(conf, residCross(resid5, resid10, P.neutralToPositiveResidual));
  {
    const p = P.momentumAcceleration;
    s.momentumAcceleration = shrink(conf,
      sig(p.retGain * (ret5 || 0)) * (residAccel != null ? sig(p.accelGain * residAccel) : sig(p.fallbackGain * ((ret5 || 0) - (ret20 || 0) / 4))));
  }
  {
    const p = P.firstPullback;
    s.firstPullback = shrink(conf,
      sig(p.retGain * (ret20 || 0)) * clamp01(higherLowFrac) *
      bell(pullbackDepth, p.depthCenter, p.depthWidth) * sig(p.sellVolGain * (p.sellVolOffset - (sellVol || 1))) * clamp01(clocRecent + 0.1));
  }
  {
    const p = P.breakoutAcceptance;
    s.breakoutAcceptance = shrink(conf,
      (brokeOut ? 1 : 0) * sig(p.volGain * (volAccel - p.volOffset)) * clamp01(closeLoc(last)) * sig(p.wickGain * (1 - upperWick * 2)));
  }
  {
    const p = P.breakoutRejection;
    s.breakoutRejection = shrink(conf,
      (tagged ? 1 : 0) * sig(p.wickGain * (upperWick - p.wickOffset)) * sig(p.closeGain * (p.closeOffset - expansionNowClose(last))) *
      (last.c < priorHigh ? 1 : 0.5));
  }
  {
    const p = P.distributionOnset;
    s.distributionOnset = shrink(conf,
      sig(p.volGain * (volAccel - p.volOffset)) * sig(p.adGain * (-(ad))) * sig(p.clocGain * (p.clocOffset - clocRecent)));
  }
  {
    const p = P.exhaustion;
    s.exhaustion = shrink(conf,
      sig(p.retGain * ((ret20 || 0) - p.retOffset)) * sig(p.volGain * (volAccel - p.volOffset)) * sig(p.wickGain * (upperWick - p.wickOffset)));
  }
  {
    const p = P.capitulationReversal;
    s.capitulationReversal = shrink(conf,
      sig(p.downGain * (downMove - p.downOffset)) * sig(p.volGain * (volAccel - p.volOffset)) * clamp01(closeLoc(last)) *
      (last.c > prev.c ? 1 : 0.3));
  }
  {
    // sector rotation needs residual sector context; without it, low + flagged
    const p = P.sectorRotation;
    s.sectorRotation = shrink(conf, resid10 != null ? sig(p.residGain * resid10) * p.scale : p.unknown);
  }
...
function residCross(short, long, p = { crossGain: 120, unknown: 0.15, confirmedScale: 0.4, floor: 0.1 }) {
  if (short == null || long == null) return p.unknown; // unknown residual → low, not zero
  // neutral-or-negative longer residual turning positive on the short window
  if (long <= 0.005 && short > 0.005) return clamp01(sig(p.crossGain * short));
  if (short > 0 && short > long) return clamp01(p.confirmedScale * sig(p.crossGain * short));
  return p.floor;
}
