...
function assessIvRvSample({ archiveDays = [], coilDays = [], asOf, minMaturable = MIN_MATURABLE_JOINED } = {}) {
  ...
  const perDay = [];
  const totals = { coilPicks: 0, joined: 0, joinedWithIv: 0, joinedWithRv: 0, usable: 0, maturable: 0, selectedUsable: 0, controlUsable: 0 };
  for (const cd of coilDays) {
    ...
    let joined = 0, joinedWithIv = 0, usable = 0, maturableN = 0, sel = 0, ctl = 0;
    for (const p of cd.picks) {
      ...
      if (iv != null && rv != null && rv > 0) {
        ...
        if (maturable) {
          maturableN++;
          if (p.selected === false) ctl++; else sel++;
        }
      }
    }
