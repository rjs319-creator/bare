...
function assessSource(source, { registry, gov, registryVersionOf } = {}) {
  ...
  // The FROZEN policy cohort the promoted claim was earned on (registry Phase 5).
  // Carried on the assessment so assessSignal can exclude out-of-cohort rows — the
  // registry declares Breakout/Setup and small/micro lanes as proven-negative
  // controls, but until this gate they could originate live picks under Early:large's
  // clearance. Pinned sources (Day-Trade) keep their own contract and are not gated here.
  const regList = registry || require('./strategy-registry').STRATEGY_REGISTRY;
  const regEntry = regList.find(x => x && x.id === source) || null;
  const policyTiers = (!pinned && regEntry && Array.isArray(regEntry.policyTiers) && regEntry.policyTiers.length)
    ? regEntry.policyTiers : null;
  const policyScopes = (!pinned && regEntry && Array.isArray(regEntry.policyScopes) && regEntry.policyScopes.length)
    ? regEntry.policyScopes : null;
  ...
  return { source, staticStatus, pinned: false, governance, displayEligible: true, tradeEligible, sizingWeight, reasonCode, reasons, policyTiers, policyScopes };
}
...
function assessSignal(sig, srcAssessment, { borrowFeed = null, dataGate = null } = {}) {
  ...
  // POLICY-COHORT GATE: the registry freezes the tier/scope cohort every displayed
  // statistic and earned clearance describes (e.g. screener = Early:large). A row
  // outside that cohort is an excluded CONTROL — visible as research, but it must not
  // originate a live position under evidence earned by a different population. A
  // missing tier/scope on the signal fails CLOSED: an unlabeled row cannot prove it
  // belongs to the validated cohort.
  if (tradeEligible && a.policyTiers) {
    if (!sig.tier || !a.policyTiers.includes(sig.tier)) {
      tradeEligible = false;
      codes.push(REASON_CODE.POLICY_COHORT);
      reasons.push(`tier '${sig.tier || 'unlabeled'}' is outside the promoted policy cohort [${a.policyTiers.join(', ')}] — excluded control, research only (fail closed)`);
    }
  }
  if (tradeEligible && a.policyScopes) {
    if (!sig.universeScope || !a.policyScopes.includes(sig.universeScope)) {
      tradeEligible = false;
      codes.push(REASON_CODE.POLICY_COHORT);
      reasons.push(`scope '${sig.universeScope || 'unlabeled'}' is outside the promoted policy cohort [${a.policyScopes.join(', ')}] — excluded control, research only (fail closed)`);
    }
  }
