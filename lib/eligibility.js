...
function indexGovernance(governanceDoc, nowMs) {
  ...
  const fresh = Number.isFinite(savedAt) && Number.isFinite(nowMs)
  ...
  return { byId, fresh, savedAt: governanceDoc.savedAt || null };
}
...
function assessSource(source, { registry, gov, registryVersionOf } = {}) {
  ...
  if (staticStatus !== 'production') {
    ...
    reasons.push(`governance state stale (savedAt ${gov.savedAt || 'unknown'}) — fail closed`);
  } else {
    ...
    if (!rec) {
      ...
      if (expectVersion && rec.version && rec.version !== expectVersion) {
        reasons.push(`governance evidence is for scoring version ${rec.version}, current is ${expectVersion} — reset required (fail closed)`);
