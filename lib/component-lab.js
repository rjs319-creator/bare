...
const CONFIG = {
  ...
  MIN_REGIME_N: 10,  // min matched pairs in a regime for it to count toward robustness
};
...
function runComponentLab(records, { components, config = {} } = {}) {
  ...
  const results = (components || []).map((comp) => {
    ...
    const byRegime = byRegimeStability(pairs);
    const { verdict, recommendation } = verdictFor(matched, cfg);
    // Regime robustness: is the effect the SAME sign in ≥2 regimes (each with enough pairs)?
    // If not, an actionable verdict is softened to "observe" — this app's edges routinely die
    // out-of-regime, so a single-window effect is an observation, not a decision.
    const overall = matched.mean ?? 0;
    const robustRegimes = Object.values(byRegime).filter(r => r.n >= cfg.MIN_REGIME_N && Math.sign(r.incremental) === Math.sign(overall) && overall !== 0).length;
    const regimeRobust = robustRegimes >= 2;
    let finalRec = recommendation, caveat = null;
    if (['additive', 'harmful', 'redundant'].includes(verdict) && !regimeRobust) {
      finalRec = 'observe';
      caveat = 'Single-regime result — the effect is not confirmed across ≥2 regimes, so this is an observation, not yet an actionable recommendation (this app’s edges have repeatedly died out-of-regime).';
    }
    ...
    return {
      ...
      byRegime, regimeRobust,
      verdict, recommendation: finalRec, verdictRecommendation: recommendation, caveat, examples,
    };
  });
  return {
