...
//
// This module adds ONE bounded, PARAMETRIC claude-fable-5-1 call (via lib/fable-call) that reads both
...
async function analyzeDualRead(ctx, timeoutMs = CALL_TIMEOUT_MS) {
  ...
  const r = await callFableTool({
    ...
    label: 'dualread-fable',
  });
  if (!r || !r.input) return null;   // graceful — caller keeps the mechanical verdict
  return parseDualRead(r.input);
}
