...
function targetScore(a, opt) {
  ...
  // FAIL CLOSED on unmeasured inputs: an absent quality signal used to default to a
  // PERFECT 1.0 — the single most flattering substitution a router can make. Unmeasured
  // now takes the uninformative midpoint 0.5 (never a boost), and unknown uncertainty
  // is treated as ELEVATED (0.5), not zero.
  const factors = clamp01(num(a.scenarioCompat, 0.5)) * clamp01(num(a.calibrationQuality, 0.5))
    * clamp01(num(a.independentValue, 0.5)) * clamp01(num(a.executionQuality, 0.5))
    * (1 - clamp01(num(a.uncertainty, 0.5)));
