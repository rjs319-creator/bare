...
async function loadCikMap() {
  ...
  const r = await fetch('https://www.sec.gov/files/company_tickers.json', { headers: H });
...
async function fetchForm4List(cik, fromDate, maxFilings = 60) {
  const r = await fetch(`https://data.sec.gov/submissions/CIK${cik}.json`, { headers: H });
  if (!r.ok) return [];
  const j = await r.json();
...
async function fetchForm4Xml(cik, accession, primaryDoc) {
  ...
  if (!xmlName) {
    try {
      const ir = await fetch(`${base}/index.json`, { headers: H });
      if (ir.ok) {
  ...
  const dr = await fetch(`${base}/${xmlName}`, { headers: { ...H, 'Accept': 'application/xml,text/xml' } });
