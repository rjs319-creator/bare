...
function parseForm4(xml) {
  ...
  // Additive (2026-09): 10b5-1 plan flag + number of reporting owners on the form (a joint
  // filing lists several) — the insider-cluster feed needs both; nothing else reads them.
  const tenB51 = /<aff10b5One>\s*(1|true)\s*<\/aff10b5One>/i.test(xml);
  const ownerCount = (xml.match(/<rptOwnerName>/gi) || []).length || (owner ? 1 : 0);
  const txs = [];
  ...
  return { owner, isDirector, isOfficer, isTenPct, tenB51, ownerCount, txs };
}
...
function aggregateInsider(txs, { windowStart = null, asOf = null } = {}) {
  ...
  for (const t of txs) {
    ...
    // PUBLICATION gate (graduation-league data-lineage F-1): the transaction date is
    // when the insider traded, not when the market could KNOW. Form 4s are due T+2
    // business days and amendments can be weeks late — a tx dated ≤ asOf but filed
    // after asOf was not public at asOf, and including it makes every historical
    // as-of reconstruction (ghost IN pillar walk-forward) forward-looking.
    const publicAt = t.filingDate || t.date;
    if (asOf && publicAt && publicAt > asOf) continue;
