...
const { buildTriggers, parseGraph, alreadyMovedFlag, rankItems, callFable, batchTriggers, mergeGraphs, MAX_TRIGGERS_RAW, TICK_TIMEOUT_MS } = require('./readthrough');
...
const PRICE_DEADLINE_MS = 45000;            // stop tape checks well before the wall (Stage 2 is fast anyway)
...
async function runReadThroughTick(req, res) {
  ...
  try {
    ...
    // PARALLEL Fable calls — a single call caps at ~3 triggers in 60s, so we fan out one
    // call per batch (concurrent → same wall as one call) and merge. A batch that errors
    // or times out yields nothing; the others still return (graceful partial).
    const batches = batchTriggers(triggers);
    const parsed = await Promise.all(batches.map(b =>
      callFable(b, TICK_TIMEOUT_MS).then(inp => parseGraph(inp, triggers)).catch(() => ({ items: [], notes: '' }))
    ));
    const { items, notes } = mergeGraphs(parsed);
    const okBatches = parsed.filter(p => p.items.length).length;
    const raw = { triggerDate: latest.date, items, notes, triggers, regime: latest.regime || null, generatedAt: new Date().toISOString() };
    if (hasStore()) await writeJSON(RAW_KEY, raw, 0).catch(() => {});
    return res.json({ ok: true, triggerDate: latest.date, rawCount: items.length, triggers: triggers.length, batches: batches.length, okBatches, elapsedMs: Date.now() - t0 });
  } catch (e) {
