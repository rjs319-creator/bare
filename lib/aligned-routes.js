...
// Episode dedup (book v2, pure): a ticker's reappearance within `cooldown` TRADING
// SESSIONS of its LAST sighting extends the same episode (not re-graded); a
// reappearance after the cooldown opens a NEW graded episode. Replaces "first
// appearance per ticker forever", which could never grade a name twice — even a year
// later under a different setup. The contract declares the cooldown in SESSIONS
// (episodeCooldownSessions); the original book v2 compared it against calendar days,
// splitting episodes ~40% too early — the gap is now counted in sessions (weekday
// roll minus NYSE holidays). `days` = ledger [{date, picks:[…]}] in any order.
function episodeEntries(days, cooldown) {
  ...
  for (const d of sorted) for (const p of (d.picks || [])) {
    ...
    if (prev != null) {
      const gapSessions = calendarSessionsBetween(prev, d.date);
      if (!Number.isFinite(gapSessions) || gapSessions < cooldown) continue;   // same episode — not re-graded
    }
