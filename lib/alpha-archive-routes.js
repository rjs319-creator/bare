...
const { hasStore, writeCalArchiveDay, writeCalRevDay, readCalArchiveDay, listCalArchiveDates,
        writeRevArchiveDay, readRevArchiveDay, writeGexArchiveShard } = require('./store');
...
// WHY A FAILURE HERE HAS TO SAY WHAT IT WAS.
//
// This discarded every scrap of evidence: the HTTP status, the response body, the
// exception, and — most misleadingly — a 200 whose body simply was not an array. It
// returned a bare `null`, so op=calarchive could only ever report "calendar fetch failed
// (after retries)". That failed identically on 2026-08-17 (15101 ms) and 2026-08-20
// (15508 ms): a persistent, reproducible failure that nothing in the app could name.
//
// The non-array case matters most, because it is how FMP reports a plan problem — a 200
// carrying {"Error Message": "...Exclusive Endpoint..."} — and this app has already hit
// exactly that on batch-quote (402 Restricted Endpoint). "Fetch failed" and "your
// subscription does not include this endpoint" call for completely different responses,
// and the old code made them indistinguishable.
//
// `lastReason` carries the most recent attempt's reason back to the caller. The URL is
// NEVER included: it embeds the API key.
async function fetchJsonRetry(url, { tries = 4, pauseMs = 2500, onReason = null } = {}) {
  ...
  for (let i = 0; i < tries; i++) {
    try {
      ...
      if (r.ok) {
        ...
        // A 200 that is not an array is a provider-level error, not an empty day.
        reason = `HTTP 200 but body was not an array: ${String(body).replace(/\s+/g, ' ').slice(0, 160)}`;
      } else {
        const body = await r.text().catch(() => '');
        reason = `HTTP ${r.status}${body ? ` — ${String(body).replace(/\s+/g, ' ').slice(0, 160)}` : ''}`;
      }
    } catch (e) {
      reason = `network: ${String((e && e.message) || e).slice(0, 160)}`;
    }
    if (i < tries - 1) await new Promise(res => setTimeout(res, pauseMs * (i + 1)));
  }
  if (typeof onReason === 'function') onReason(reason);
...
async function fetchForwardCalendar(fromDate, onReason = null) {
  ...
  for (let off = 0; off < CAL_FORWARD_DAYS; off += CAL_CHUNK_DAYS) {
    ...
    const j = await fetchJsonRetry(`https://financialmodelingprep.com/stable/earnings-calendar?from=${a}&to=${b}&apikey=${FMP()}`, { onReason });
...
async function runCalArchive(req, res) {
  ...
  let failReason = null;
  const rows = await fetchForwardCalendar(date, (r) => { failReason = r; });
  if (rows === null) {
    ...
    return res.status(503).json({
      ok: false, op: 'calarchive', date,
      error: `calendar fetch failed (after retries) — no snapshot written${failReason ? `: ${failReason}` : ''}`,
      reason: failReason,
    });
  }
...
// Fixed page sweep (no cursor state — overlap dedups at read time). 8×100 per
// feed covers well over a typical day's event volume; the telemetry's oldest
// timestamps make any under-coverage visible rather than silent.
// GOTCHA (found on the first quiet-window run, 2026-08-06): this plan CAPS the
// limit param at 100 AND the page param at 0 — so 100 events per feed per pull
// is the hard ceiling (probing limit=250 or page=1 returns an error body every
// time; the eager first-night "burst 429" diagnosis was wrong — the failure
// was deterministic, and the 503-on-total-failure change is what exposed it).
// Coverage therefore comes from PULL FREQUENCY: the evidence-tick workflow
// runs this twice per weekday (13:30 + 22:30 UTC); read-time dedup absorbs
// the overlap. The op writes one shard per calendar day keyed by ET date, so
// the second pull's write REPLACES the first with a fresher pull — each pull
// re-fetches the latest 100, and the day's union survives via dedup keys only
// if both pulls' events overlap... they may not on heavy days, so the second
// write must MERGE, not replace (see runRevArchive).
const REV_PAGES = 1, REV_PAGE_LIMIT = 100;
...
async function runRevArchive(req, res) {
  ...
  // MERGE with any earlier same-day pull (two pulls/day; the plan caps each
  // pull at 100 events/feed, so replacing would drop the morning's capture).
  const prior = await readRevArchiveDay(date);
  const grades = dedupeEvents([...((prior && prior.grades) || []), ...(gradesRaw || []).map(compactGradeEvent)], gradeEventKey);
  const targets = dedupeEvents([...((prior && prior.targets) || []), ...(targetsRaw || []).map(compactTargetEvent)], targetEventKey);
...
async function runArchiveHealth(req, res) {
  ...
  const [snap, rev, ra, gexDocsLatest, estLatest, sec8kLatest, insLatest] = await Promise.all([
    latestBlobDoc(p.snaps), latestBlobDoc(p.revs), latestBlobDoc(p.revarch), latestBlobDoc(p.gex), latestBlobDoc(p.est),
    latestBlobDoc(p.sec8k), latestBlobDoc(p.ins),
  ]);
  ...
  return res.status(200).json({
    ...
    estarchive: {
      days: p.est.length, latestDate: latestDateOf(p.est, 'estarchive/'.length),
      latestSymbols: estLatest && estLatest.symbols ? Object.keys(estLatest.symbols).length : null,
      latestPlanned: estLatest ? estLatest.planned ?? null : null,
      latestTruncatedAt: estLatest ? estLatest.truncatedAt ?? null : null,
      latestGatedPeriods: estLatest ? estLatest.gatedPeriods ?? null : null,
    },
    sec8karchive: {
      days: p.sec8k.length, latestDate: latestDateOf(p.sec8k, 'sec8karchive/'.length),
      latestEvents: sec8kLatest && Array.isArray(sec8kLatest.events) ? sec8kLatest.events.length : null,
      latestPagesFetched: sec8kLatest ? sec8kLatest.pagesFetched ?? null : null,
      latestPageCapped: sec8kLatest ? sec8kLatest.pageCapped ?? null : null,
    },
    insarchive: {
      days: p.ins.length, latestDate: latestDateOf(p.ins, 'insarchive/'.length),
      latestEvents: insLatest && Array.isArray(insLatest.events) ? insLatest.events.length : null,
      latestPagesFetched: insLatest ? insLatest.pagesFetched ?? null : null,
      latestPageCapped: insLatest ? insLatest.pageCapped ?? null : null,
    },
    elapsedMs: Date.now() - t0,
  });
}
