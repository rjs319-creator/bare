...
// One retry with a pause: these ops run inside the 22:00 UTC cron burst where
// altprobes alone drains ~450 sequential FMP calls, so a transient 429 here is
// expected — and a lost calendar day is unrecoverable, unlike a lost probe run.
async function fetchJsonRetry(url, { tries = 2, pauseMs = 1500 } = {}) {
  for (let i = 0; i < tries; i++) {
    ...
    if (i < tries - 1) await new Promise(res => setTimeout(res, pauseMs));
  }
...
async function runCalArchive(req, res) {
  ...
  if (rows === null) {
    return res.status(200).json({ ok: false, op: 'calarchive', date, error: 'calendar fetch failed (after retry) — no snapshot written' });
  }
...
async function runRevArchive(req, res) {
  ...
  if (gradesRaw === null && targetsRaw === null) {
    return res.status(200).json({ ok: false, op: 'revarchive', date, error: 'both event feeds failed (after retry) — no shard written' });
  }
...
module.exports = { runCalArchive, runRevArchive, runGexArchive, fetchForwardCalendar, gexUniverse, sliceOf };
