...
const STATES = Object.freeze({
  ...
  FAILED: 'FAILED',
  EXPIRED: 'EXPIRED',
  TARGET_REACHED: 'TARGET_REACHED',              // the live plan's target printed — setup played out
  MANAGING: 'MANAGING',
  CLOSED: 'CLOSED',
});

// Retired = non-actionable but STILL OBSERVED (shown in "Retired Today", graded forward).
const RETIRED_STATES = Object.freeze(new Set([STATES.TOO_EXTENDED, STATES.STALLING, STATES.FAILED, STATES.EXPIRED, STATES.TARGET_REACHED]));
...
const REASON = Object.freeze({
  ...
  STOP_REACHED: 'STOP_REACHED',                 // the ACTIVE live plan's frozen stop printed
  TARGET_REACHED: 'TARGET_REACHED',             // the ACTIVE live plan's frozen target printed
  DAY_COLLAPSE: 'DAY_COLLAPSE',                 // current-session collapse vs prior close
  PLAN_EXPIRED: 'PLAN_EXPIRED',                 // the ACTIVE live plan's expiry passed
...
// Internal consistency of a live plan the actionable gate may rely on.
function planUsable(plan) {
  return !!(plan && Number.isFinite(plan.entry) && Number.isFinite(plan.stop) && Number.isFinite(plan.target)
    && plan.entry > plan.stop && plan.target > plan.entry);
}
...
// With cfg.requireLivePlan (the Day Trade production config) an actionable state ALSO
// requires a usable current live plan — a name without executable levels can never be
// presented as a live buy, no matter how strong the signal evidence.
function isActionableEligible(ev, cfg) {
  ...
  return isFresh(ev)
    ...
    && !ev.breakoutFailed
    && (cfg.requireLivePlan !== true || planUsable(ev.livePlan))
...
function decide(prev, record, ev, cfg, now) {
  ...
  // strategy-specific, NORMALIZED evidence (not a bare "down X%"): stop breach (against the
  // ACTIVE frozen live-plan stop when one is in force — ev.stopBasis distinguishes it from
  // the daily-detection fallback), breakout failure, ≥2 VWAP-loss closes, a current-session
  // day collapse vs the prior close, or an ATR-normalized loss from the detection price past
  // the configured threshold (a HIMS-style collapse inconsistent with the long thesis).
  if (ev.breakoutFailed === true) return [STATES.FAILED, REASON.FAIL_BREAKOUT];
  if (ev.stopBreached === true) return [STATES.FAILED, ev.stopBasis === 'live-plan' ? REASON.STOP_REACHED : REASON.FAIL_BREAKOUT];
  if (ev.dayCollapsed === true) return [STATES.FAILED, REASON.DAY_COLLAPSE];
  ...
  // 1b. Live-plan target printed — the setup PLAYED OUT (a positive retirement, not a
  // failure). This is a setup outcome, NOT a position exit: no fill is implied.
  if (ev.targetReached === true) return [STATES.TARGET_REACHED, REASON.TARGET_REACHED];

  // 2. Expiration — the ACTIVE plan's expiry passed, or no valid trigger by the deadline.
  if (ev.planExpired === true) return [STATES.EXPIRED, REASON.PLAN_EXPIRED];
...
function explain(to, reasonCode, ev, clock) {
  ...
  switch (reasonCode) {
    ...
    case REASON.STOP_REACHED: return `Retired ${clock} — the live plan's frozen stop printed (setup invalidated; no position implied).`;
    case REASON.TARGET_REACHED: return `Played out ${clock} — the live plan's frozen target printed (setup complete; no position implied).`;
    case REASON.DAY_COLLAPSE: return `Retired ${clock} — current session collapsed vs the prior close, inconsistent with a long momentum thesis.`;
    case REASON.PLAN_EXPIRED: return `Expired ${clock} — the live plan's entry window closed without follow-through.`;
...
function transitionRecord(from, to, at, reasonCode, ev, cfg) {
  return {
    ...
    // Completed-bar / forming-bar / quote-only provenance for the evidence that produced
    // this transition (null for daily-evidence or legacy evaluations).
    evidenceBasis: ev.evidenceBasis ? { ...ev.evidenceBasis } : null,
...
function advanceLifecycle(record, ev, opts = {}) {
  ...
  if (changed) {
    ...
    if (to === STATES.STALLING || to === STATES.TOO_EXTENDED || to === STATES.EXPIRED || to === STATES.TARGET_REACHED) {
...
function summarizeBoard(records = []) {
  ...
  for (const r of records) {
    ...
    switch (r.state) {
      ...
      case STATES.FAILED:
      case STATES.EXPIRED:
      case STATES.TARGET_REACHED: board.retiredToday.push(r); break;
...
module.exports = {
  ...
  isRetired, isPostEntry, isTerminal, isActionable, isActionableEligible, isReclaimEligible, planUsable,
