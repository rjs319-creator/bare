 ...
 */
async function buildSnapshot({ mode = 'lite', now = new Date(), t0 = Date.now() } = {}) {
  ...
  const universe = await UNI.loadTechUniverse({ now }).catch(e => {
...
async function buildLongTermSection({ universe, context, allCandles, benchmarks, regime, focusTickers, limit, now, t0, deadline }) {
  ...
  // Bounded shortlist: the largest technology names present on a board, plus the
  // largest mega-caps, so the long-term board is not a function of intraday noise.
  const boardNames = focusTickers.slice(0, limit);
  const megaNames = universe.securities.filter(s => (s.marketCap || 0) >= UNI.MEGA_CAP_USD).slice(0, limit).map(s => s.ticker);
  const shortlist = [...new Set([...boardNames, ...megaNames])].slice(0, limit);
