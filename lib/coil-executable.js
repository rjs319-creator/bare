...
function coilExecutable({ current, entry, stop, target, dailyVol, horizon = 10, roundTripCostPct = DEFAULT_ROUND_TRIP_COST_PCT }) {
  ...
  return Object.freeze({
    ...
    // In-horizon split (target-first / stop-first / timeout sum to 1) — exposed
    // additively for the Stage-B utility waterfall (premove-stage-b).
    pTargetFirst: +pTargetFirst.toFixed(3),
    pStopFirst: +pStopFirst.toFixed(3),
    pTimeout: +pTimeout.toFixed(3),
