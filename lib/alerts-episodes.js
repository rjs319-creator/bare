  ...
  for (const lead of ordered) {
    ...
    if (!ep) {
      ep = {
        ...
        // IMMUTABLE price-at-alert record (price + provider quote timestamp + market
        // session + source, or an explicit UNAVAILABLE with a reason). Written ONCE here
        // and never rewritten below — a later post must not restamp an older decision.
        priceAtAlert: lead.priceAtAlert ?? null,
