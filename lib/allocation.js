...
function computeAllocation(sleevesRaw, opts = {}) {
  ...
  // can never size UP. A sleeve with no governance record defaults to full clearance
  // (backward-compatible — nothing is penalized until it's actually governed).
  ...
  const govDeployed = stats.map((s, k) => {
    const w = Number.isFinite(govWeights[s.name]) ? Math.max(0, Math.min(1, govWeights[s.name])) : 1;
    const deployed = +(funded[k] * w).toFixed(0);
    return {
      name: s.name,
      status: govStatus[s.name] || null,
      clearance: w,                 // 0..1 sizing weight from the model's status
  ...
  const governance = {
    ...
    note: !govApplied
      ? 'No governance statuses supplied — every sleeve runs at its cash-aware weight (full clearance).'
      : govCash >= 99
        ? 'No sleeve’s model currently holds governance clearance to size — hold cash until a model earns Production/Reduced status.'
        : `Each sleeve is capped by its model-governance status: Production 100% · Reduced 50% · Probation 25% · Paper/Disabled/Retired 0%. Freed capital (${govCash}% total incl. cash-when-thin) is held as cash, never reallocated — governance can only reduce size.`,
  };
