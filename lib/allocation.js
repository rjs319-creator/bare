...
function computeAllocation(sleevesRaw, opts = {}) {
  ...
  // GOVERNANCE HAIRCUT — the model-governance status (Production / Reduced / Probation /
  // Paper-only / Disabled / Retired, from lib/governance) CONTROLS how much of each
  // sleeve's cash-aware weight is actually deployed. Applied AFTER cash-when-thin and
  // strictly one-directional: governance can only CUT a sleeve's size (freed capital
  // goes to CASH, never reallocated into another bet), so an unproven or weakening model
  // can never size UP.
  //
  // FAIL CLOSED (quant-redesign-3 H8): a sleeve with NO governance record gets ZERO
  // clearance — missing governance means no clearance, never full clearance. The old
  // "defaults to full clearance (backward-compatible)" behavior silently deployed
  // ungoverned sleeves at 100%; it survives only as an explicit legacy escape hatch
  // (opts.govDefault = 1) for comparison views, never as the default.
  const govWeights = opts.govWeights || {};
  const govStatus = opts.govStatus || {};
  const govDefault = Number.isFinite(opts.govDefault) ? Math.max(0, Math.min(1, opts.govDefault)) : 0;
  const govApplied = Object.keys(govWeights).length > 0;
  // Optional binding ROUTER CAP (quant-redesign-3 Phase 4D): a second, one-directional
  // multiplier from the market-regime router. Only ever REDUCES (min with governance),
  // and the caller must only supply it when the router's inputs are valid.
  const routerCaps = opts.routerCaps || null;
  const govDeployed = stats.map((s, k) => {
    const gw = Number.isFinite(govWeights[s.name]) ? Math.max(0, Math.min(1, govWeights[s.name])) : govDefault;
    const rc = routerCaps && Number.isFinite(routerCaps[s.name]) ? Math.max(0, Math.min(1, routerCaps[s.name])) : null;
    const w = rc == null ? gw : Math.min(gw, rc);
    const deployed = +(funded[k] * w).toFixed(0);
    return {
      name: s.name,
      status: govStatus[s.name] || null,
      clearance: w,                 // 0..1 sizing weight = min(governance, router cap)
      governanceClearance: gw,
      routerCap: rc,
      cashAwareWeight: funded[k],   // what cash-when-thin alone would fund
      deployedWeight: deployed,     // after the governance haircut
      haircut: +(funded[k] - deployed).toFixed(0),
      governed: Number.isFinite(govWeights[s.name]),
    };
  });
  const govCash = +(100 - govDeployed.reduce((a, d) => a + d.deployedWeight, 0)).toFixed(0);
  const governance = {
    applied: govApplied,
    failClosed: govDefault === 0,
    routerCapApplied: !!routerCaps,
    cashWeight: govCash,
    sitOut: govCash >= 99,
    deployed: govDeployed,
    note: !govApplied
      ? (govDefault === 0
        ? 'No governance state available — FAIL CLOSED: no sleeve holds clearance to size; capital stays in cash until governance runs.'
        : 'LEGACY comparison mode (govDefault=1): ungoverned sleeves run at full clearance. Not a live setting.')
      : govCash >= 99
        ? 'No sleeve’s model currently holds governance clearance to size — hold cash until a model earns Production/Reduced status.'
        : `Each sleeve is capped by its model-governance status: Production 100% · Reduced 50% · Probation 25% · Paper/Disabled/Retired 0% · ungoverned 0% (fail closed)${routerCaps ? ', then by the router risk cap (min, never up)' : ''}. Freed capital (${govCash}% total incl. cash-when-thin) is held as cash, never reallocated — governance can only reduce size.`,
  };
