...
// Sonnet 5 (not Fable 5) for the relational graph: Fable's extended thinking made a 3-trigger
// call variable and often >54s, so the self-contained tick logged nothing. Sonnet 5 is a
// strong relational reasoner AND much faster parametrically (~20-30s), which makes the daily
// tick reliable and lets us keep more triggers. (Parametric — no web search.) maxRetries:0.
...
const BATCH_SIZE = 3;
// The tick is now SELF-CONTAINED (one 60s invocation): single Fable call (~50s) + inline
// tape enrich (~4s) + forward-log. 3 triggers is the reliable fit — parallel batching to 6
// left no room for the enrich and forced cross-invocation orchestration the daily cron
// couldn't do reliably (warm 504'd). Reliable daily forward-log > trigger count.
const MAX_TRIGGERS_RAW = 3;         // 3 relational triggers completes comfortably; 5 overran even Sonnet
const ALREADY_MOVED_PCT = 4;        // a beneficiary that itself moved >= this % today is "already priced"
const CALL_TIMEOUT_MS = 50000;      // maxRetries:0 (see below)
const TICK_TIMEOUT_MS = 50000;      // Sonnet parametric on 3 triggers ~25-40s; leaves room for the tape enrich under the 60s wall
...
// Common provider aliases (FMP/Yahoo spellings) → the same ETFs. Kept separate so
// SECTOR_NAMES (the canonical GICS list fed to the LLM tool) stays canonical.
const SECTOR_ETF_ALIASES = {
  'Information Technology': 'XLK', 'Healthcare': 'XLV', 'Financial Services': 'XLF',
  'Consumer Cyclical': 'XLY', 'Consumer Defensive': 'XLP', 'Basic Materials': 'XLB',
};
const SECTOR_ETF_CI = Object.fromEntries(
  [...Object.entries(SECTOR_ETF), ...Object.entries(SECTOR_ETF_ALIASES)]
    .map(([k, v]) => [k.toLowerCase(), v])
);
/** Beneficiary GICS sector (canonical or provider alias, any case) → sector ETF, or null. */
function benchFor(sector) {
  if (!sector) return null;
  return SECTOR_ETF[sector] || SECTOR_ETF_CI[String(sector).trim().toLowerCase()] || null;
}
...
async function callFable(triggers, timeoutMs = CALL_TIMEOUT_MS) {
  ...
  const digest = await require('./feedback-digest').getFeedbackDigest('ReadThrough');  // Layer 4: in-context self-calibration
  const msg = await client.messages.create({
    model: MODEL,
    max_tokens: 5000,   // fits <=BATCH_SIZE triggers' thinking + the tool call (3 triggers used ~3.1k)
    tools: [READTHROUGH_TOOL],
    messages: [{ role: 'user', content: buildPrompt(triggers) + digest }],
  }, { timeout: timeoutMs });
