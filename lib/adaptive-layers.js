...
const ADAPTIVE_LAYERS_VERSION = 'adaptive-layers-v2';

// ── Central policy (the CONTROL half — v2) ─────────────────────────────────
// A human-editable per-layer switch, stored in its own doc so the daily
// governance rewrite can never clobber it:
//   allow   — the layer behaves exactly as it does today (DEFAULT)
//   freeze  — the layer keeps its CURRENT adapted state but may not adopt new
//             fits/promotions (fit-sites no-op). Only meaningful for layers
//             with persisted adoption (apex, timing, dualread); derived-flag
//             layers (evolve tilt, alerts-fable) treat freeze as allow.
//   disable — apply-sites revert to shipped behavior (adapted state ignored)
//             and fit-sites no-op.
// FAIL-OPEN by design: an absent/broken policy doc means 'allow' — a Blob
// outage must not change live behavior. This is the one deliberate fail-open
// gate in the governance stack, and it is disclosed as such.
...
// Pure resolution — bad values and unknown layers resolve to 'allow' (fail-open).
function policyFor(doc, layer) {
  const v = doc && doc.layers && doc.layers[layer];
  return POLICIES.includes(v) ? v : 'allow';
}
...
async function readAdaptiveLayers() {
  ...
  return Object.freeze({
    ...
    note: 'These layers self-adapt inside already-production strategies. v2: each layer now honors the central adaptive policy (allow/freeze/disable, governance/adaptive-policy.json) at its apply- and fit-sites — auto-promotions are centrally REVOCABLE, not just visible. Default allow = behavior unchanged; absent doc fails OPEN by design.',
...
module.exports = {
  ADAPTIVE_LAYERS_VERSION, POLICY_DOC, POLICIES, LAYERS,
