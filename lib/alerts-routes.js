...
async function runAlerts(req, res) {
  ...
  // Central adaptive policy: 'disable' pins the Fable A/B report to
  // not-promoted so no reader can act on a Fable-reordered ranking.
  const fablePolicy = await require('./adaptive-layers').layerPolicy('alerts-fable').catch(() => 'allow');
  const fableEdgeRaw = alertsFable.fableEdgeReport(v1log || []);
  const fableEdge = fablePolicy === 'disable'
    ? { ...fableEdgeRaw, promoted: false, adaptivePolicy: fablePolicy, verdict: `DISABLED by adaptive policy — ${fableEdgeRaw.verdict}` }
