...
async function runAlertsIngest(req, res) {
  ...
  // P0: capture the alert-time price BEFORE folding, so every new episode carries an
  // immutable, timestamped price-at-alert. A quote-provider failure degrades to explicit
  // UNAVAILABLE records — it never stamps server time and never backfills later.
  const quoteCtx = await captureAlertQuotes(freshRecords, now);
  const { leads } = pipeline.buildLeads(freshRecords, { skillModel, ...quoteCtx, now });
...
// Alert-time quote capture. Extracts the candidate tickers from the incoming batch, pulls
// one bulk quote, and returns the context `buildLeads` needs to stamp an immutable
// price-at-alert. Every failure path returns an EMPTY map — `capturePriceAtAlert` then
// records UNAVAILABLE with a reason rather than inventing a price.
async function captureAlertQuotes(records, nowMs) {
  const empty = { quoteByTicker: new Map(), quoteSource: null, sessionInfo: null };
  try {
    const { parsePost } = require('./alerts-lifecycle');
    const tickers = new Set();
    for (const r of records || []) {
      if (!r || !r.text) continue;
      for (const pt of (parsePost(r.text).perTicker || [])) if (pt.ticker) tickers.add(pt.ticker);
    }
    if (!tickers.size) return empty;
    const { sessionInfoAt } = require('./market-session');
    const sessionInfo = sessionInfoAt(new Date(nowMs));
    const { fetchBulkQuotes } = require('./quote-provider');
    const { rows, coverage } = await fetchBulkQuotes([...tickers].slice(0, 100));
    const quoteByTicker = new Map((rows || []).map(q => [q.ticker, q]));
    return { quoteByTicker, quoteSource: (coverage && coverage.provider) || null, sessionInfo };
  } catch {
    return empty;   // provider failure ⇒ UNAVAILABLE capture records, never a fabricated price
  }
}
...
async function runLegacyIngest(incoming, now) {
  ...
  for (const r of ranked) {
    ...
    // Keep symbology the grader can never resolve out of the log entirely. Conservative by
    // design — see isGradeableSymbol; anything arguable is admitted and left to the cap.
    if (!alerts.isGradeableSymbol(r.ticker)) continue;
...
async function runAlerts(req, res) {
  ...
  // Central adaptive policy. Derived-flag layer: the Fable promotion is re-derived
  // from the A/B log every run, so applying it IS an adoption — only an explicit
  // 'allow' lets a promoted report through. 'freeze' (the fail-closed default) and
  // 'disable' both pin not-promoted so no reader can act on a Fable-reordered ranking.
  const fablePolicy = await require('./adaptive-layers').layerPolicy('alerts-fable').catch(() => 'freeze');
  const fableEdgeRaw = alertsFable.fableEdgeReport(v1log || []);
  const fableEdge = fablePolicy !== 'allow'
    ? { ...fableEdgeRaw, promoted: false, adaptivePolicy: fablePolicy, verdict: `${fablePolicy === 'disable' ? 'DISABLED' : 'FROZEN (no explicit allow)'} by adaptive policy — ${fableEdgeRaw.verdict}` }
    : { ...fableEdgeRaw, adaptivePolicy: fablePolicy };
...
async function runAlertsGrade(req, res) {
  ...
  // Side-aware regime needs a sector leg: each ticker's SPDR proxy return vs SPY today.
  const sectorByTicker = await buildSectorContext(activeEpisodes, spy);
  const { decisions, views } = pipeline.buildDecisions(activeEpisodes, {
    candlesByTicker, spy, skillModel, regime, earningsByTicker, semanticById, sectorByTicker, now,
  });
...
// Ticker → { sector, vsSpyPct }: the sector leg of the side-aware regime read. A ticker with
// no resolvable sector, or a sector ETF whose bars failed, is simply ABSENT from the map —
// `assessRegimeFit` then reports the sector alignment UNKNOWN instead of assuming neutral.
async function buildSectorContext(episodes, spy) {
  const out = new Map();
  try {
    if (!Array.isArray(spy) || spy.length < 2) return out;
    const spyRet = ((spy[spy.length - 1].close - spy[spy.length - 2].close) / spy[spy.length - 2].close) * 100;
    const { SECTOR_OF } = require('./universe');
    const { SECTOR_ETF } = require('./readthrough');
    const wanted = new Map();   // etf -> sector name
    for (const e of episodes || []) {
      const sector = SECTOR_OF ? SECTOR_OF[e.ticker] : null;
      const etf = sector && SECTOR_ETF ? SECTOR_ETF[sector] : null;
      if (etf) wanted.set(etf, sector);
    }
    if (!wanted.size) return out;
    const etfRet = new Map();
    await Promise.all([...wanted.keys()].map(async etf => {
      try {
        const d = await fetchDailyHistory(etf);
        const cs = d && d.candles;
        if (cs && cs.length >= 2) etfRet.set(etf, ((cs[cs.length - 1].close - cs[cs.length - 2].close) / cs[cs.length - 2].close) * 100);
      } catch { /* absent ⇒ UNKNOWN sector alignment, not a neutral pass */ }
    }));
    for (const e of episodes || []) {
      const sector = SECTOR_OF ? SECTOR_OF[e.ticker] : null;
      const etf = sector && SECTOR_ETF ? SECTOR_ETF[sector] : null;
      if (!etf || !etfRet.has(etf)) continue;
      out.set(e.ticker, { sector, etf, vsSpyPct: +(etfRet.get(etf) - spyRet).toFixed(2) });
    }
  } catch { /* sector context is optional evidence — never fatal */ }
  return out;
}
...
async function runLegacyGrade(now) {
  ...
  // `ungradeable` entries are retired: they stay in the log (and in loggedTotal) but never
  // re-enter the fetch set. Without this a symbol the provider cannot serve was retried on
  // EVERY run forever, because a missed fetch skipped the entry without marking it.
  const pending = log.filter(e => !e.graded && !e.ungradeable && e.direction !== 'neutral');
  ...
  await Promise.all(Array.from({ length: Math.min(8, tickers.length) }, worker));
  let graded = 0, retired = 0;
  // A missed fetch is only counted against the entry when the run could otherwise have
  // graded it — if SPY itself failed, nothing was gradeable and the miss says nothing
  // about the symbol.
  if (spy) for (const e of pending) {
    const c = hist.get(e.ticker);
    if (!c) { alerts.markUngradeable(e); if (e.ungradeable) retired++; continue; }
  ...
  // Persist when anything changed — retirements must survive the run, or the entry comes
  // back pending next time and the loop never terminates.
  if (graded || retired) { await writeJSON(V1.LOG, log); }
  if (graded) { await writeJSON(V1.RECORD, record); }
