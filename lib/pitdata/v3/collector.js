...
//
// opts.forceNewRun: privileged operational recovery — start a fresh SAME-DAY
// run after a completed one (e.g. when concurrent hand-triggering raced the
// Blob store). Appends a new runId; never rewrites the previous run.
// CONCURRENCY CONTRACT: this collector assumes ONE serial writer (the warm
// chain). Hand-triggered calls must be spaced ≥30s apart — Vercel Blob has no
// compare-and-swap, so overlapping invocations can lose shard updates.
async function collectStepV3(deps, opts = {}) {
  ...
  // tier) — recording a redacted snippet turns an opaque 402/403 into a
  // diagnosable capability limitation.
  const bodyNoteOf = (r) => {
    ...
    if (run.step === 'stock-list') {
      ...
      while (pages < DELISTED_PAGES_PER_STEP && deps.now() - t0 < STEP_DEADLINE_MS) {
        ...
        if (!r.ok && r.status === 402 && params.limit > 100) {
          // The plan may gate the LIMIT as well as the page — record it and
          // retry the same page at the vendor's default before truncating.
        ...
        if (!r.ok && r.status === 402) {
          // 402 = the vendor PLAN refuses this page ("page can only be 0 on
          // your current subscription"). That is a capability boundary, not a
          // transient failure: retrying forever would wedge the run and block
          // all prospective accumulation. Record the truncation and move on —
          // the page stays in requestedPages and NOT in completedPages, so the
          // reconciliation allPagesCollected gate honestly fails.
          (manifest.limitations ||= []).push({
            page: pageId, httpStatus: 402, bodyNote: bodyNoteOf(r),
      ...
      did = { step: 'delisted', pages, rows, nextPage: run.delistedPage, ...(truncatedByPlanGate ? { truncatedByPlanGate } : {}) };
      ...
      // Run complete. A plan-gated truncation is carried in the status so no
      // reader can mistake a limited run for full coverage.
      run.step = 'complete';
