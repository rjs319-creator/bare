...
async function fetchTrending(fetchImpl) {
  ...
  // Gradeability filter is the canonical one from lib/alerts.js — the SAME StockTwits
  // feed symbology, and it already rejects everything fetchDailyHistory can't serve
  // (crypto .X, foreign .TSX/.NSE, futures _F, forex pairs, dotted classes). A ticker
  // that can't be graded must never enter the write-once ledger.
  const { isGradeableSymbol } = require('./alerts');
  const syms = (d && Array.isArray(d.symbols) ? d.symbols : [])
    .map((s) => String(s && s.symbol || '').toUpperCase())
    .filter(isGradeableSymbol);
