...
const VERSIONS = Object.freeze({
  ...
  path: 'atlasx-path-v1',
  experts: 'atlasx-experts-v2',   // v2: + relativeLeadershipTransition (RLT)
  router: 'atlasx-router-v2',     // v2: RLT correlation pairs added
...
// ── VERSIONED RESEARCH PARAMETERS (pre-move redesign Phase 5.7) ─────────────
// The transition-score sigmoid gains/offsets were previously inline literals —
// impossible to audit or compare. They are now a NAMED, VERSIONED parameter set
// with values IDENTICAL to the historical inline constants (no retuning by eye;
// any alternative set must be compared in the research harness, never swapped
// silently). detectTransition accepts an override for exactly that comparison.
const TRANSITION_PARAMS = Object.freeze({
  version: 'atlasx-transition-params-v1',
  compressionToExpansion: Object.freeze({ expGain: 2.2, expOffset: 1.4, volGain: 2, volOffset: 1.2 }),
  accumulationToDemand: Object.freeze({ adGain: 1.5, volGain: 2, volOffset: 1.1, retGain: 3 }),
  neutralToPositiveResidual: Object.freeze({ crossGain: 120, unknown: 0.15, confirmedScale: 0.4, floor: 0.1 }),
  momentumAcceleration: Object.freeze({ retGain: 3, accelGain: 60, fallbackGain: 2 }),
  firstPullback: Object.freeze({ retGain: 4, depthCenter: 0.05, depthWidth: 0.05, sellVolGain: 2, sellVolOffset: 1.1 }),
  breakoutAcceptance: Object.freeze({ volGain: 2, volOffset: 1.1, wickGain: 2 }),
  breakoutRejection: Object.freeze({ wickGain: 3, wickOffset: 0.4, closeGain: 2, closeOffset: 0.9 }),
  distributionOnset: Object.freeze({ volGain: 2, volOffset: 1.2, adGain: 3, clocGain: 2, clocOffset: 0.45 }),
  exhaustion: Object.freeze({ retGain: 3, retOffset: 0.15, volGain: 2, volOffset: 1.4, wickGain: 3, wickOffset: 0.35 }),
  capitulationReversal: Object.freeze({ downGain: 4, downOffset: 0.08, volGain: 2, volOffset: 1.3 }),
  sectorRotation: Object.freeze({ residGain: 50, scale: 0.6, unknown: 0.1 }),
});

// Explicit thresholds for the ONLY places a binary read of the continuous
// compression values is genuinely required (stage labels). The continuous
// values themselves are never coerced to booleans (Phase 5.1 fix).
const EXPERT_PARAMS = Object.freeze({
  version: 'atlasx-expert-params-v2',
  expansionNowThreshold: 1.2,   // recent/baseline range ratio read as "expanding now"
  wasCompressedThreshold: 0.6,  // ATR-compression percentile read as "was compressed"
  // relativeLeadershipTransition (v2): binary reads of the injected leadership
  // cross-section. The continuous values themselves stay continuous.
  rltMinPctile: 0.5,            // below the peer median → not a leadership candidate
  rltVelocityFloor: 0.02,       // 5-session percentile velocity read as "improving"
  rltStaleDiscount: 0.6,        // applicability multiplier when the cross-section is a session old
});
