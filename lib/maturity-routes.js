...
async function runMaturity(req, res) {
  ...
  const adaptiveLayers = hasStore()
    ...
    : null;
  if (hasStore()) {
    writeJSON(GOV_STATE, { ...governance, adaptiveLayers, savedAt: new Date().toISOString() }).catch(() => {});
  }
...
async function runAdaptivePolicy(req, res) {
  ...
  return res.status(error ? 400 : 200).json({
    ...
    policyDoc: policyDoc || { layers: {}, note: 'no policy set — every layer defaults to allow (fail-open by design)' },
