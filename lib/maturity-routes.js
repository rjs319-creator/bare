...
const { STRATEGY_REGISTRY, PROMOTION_CEILING } = require('./strategy-registry');
...
async function runMaturity(req, res) {
  ...
  //
  // WRITE AUTHORITY (alpha-research pass 3). op=maturity is a PUBLIC read — the evidence
  // banner on ~35 tabs fetches it — so it cannot join PRIVILEGED_OPS without blanking
  // the UI. But the grading read and the capital-control WRITE are different acts, and
  // only the write needs authority. Untrusted callers now get the full computed board
  // (identical response shape) with persistence skipped.
  //
  // Two concrete failures this closes: (1) an anonymous GET rewrote governance/latest.json,
  // which assigns `weight`/`newPositions` to every strategy; (2) the write is a
  // read-modify-write of `prevGov`, and inside Blob's 10-30s read-back lag two overlapping
  // anonymous calls both read the PRE-write doc, so `grandfatheredSince` gets re-stamped
  // and the grandfather window slides forward indefinitely — a strategy could hold
  // grandfathered clearance forever by being polled.
  const mayPersist = isTrusted(req);
  const persisted = { attempted: hasStore() && mayPersist, ok: false, error: null, skipped: mayPersist ? null : 'untrusted-caller' };
  ...
  const transitions = (hasStore() && mayPersist)
    ...
    : { count: 0, logged: false, error: hasStore() ? 'untrusted-caller' : 'no store' };
  if (hasStore() && mayPersist) {
    try {
  ...
  return res.json({
    ...
    governance,
    // Declared, reviewable reason no non-Day-Trade strategy can currently reach
    // `validated`. Surfaced at the top level so the ceiling is visible on the governance
    // board itself, not only inside each strategy's fillVerification detail.
    promotionCeiling: PROMOTION_CEILING,
    adaptiveLayers,
