...
const { benchFor: etfForSector } = require('./readthrough');   // GICS sector → SPDR sector ETF
...
function buildCandidate({ ticker, candles, spy, sector, meta = {}, ctx = {} }) {
  ...
  // Expert context (Phase 5 fixes): the STRUCTURED catalyst record + as-of date
  // reach the catalyst-drift expert (which otherwise abstains — never inferring
  // from a text label), and riskOff reaches redTapeReversal.
  const eCtx = {
    ...ctx,
    ticker,   // v2: relativeLeadershipTransition looks itself up in ctx.rltLeadership
    sector: meta.sector, sectorEtf: meta.sectorEtf, liqTier: meta.liqTier,
    asOf: ctx.asOf || ctx.date || null,
    riskOff: ctx.riskOff === true || ctx.regimeRiskOff === true,
    catalyst: meta.catalystRecord || null,
  };
...
function runEngine({ universe, priceLookup, benchLookup, ctx = {} }) {
  ...
  for (const ticker of universe.evalTickers) {
    ...
    const poolEntry = (universe.pool && universe.pool[ticker]) || {};
    // Phase 5 fixes: candidates that did NOT originate in op=today (near-miss /
    // episode / specialist scan) get their sector from the app's sector map and
    // their price/liquidity from the pool stats — previously all undefined, which
    // silently degraded every such name to a market-only residual with no
    // liquidity read. Unknown sector still yields a LABELED market-only residual
    // (atlasx-residual partial flag), never a pretend sector control.
    const sector0 = cur.sector || poolEntry.sector || null;
    const sectorEtf = cur.sectorEtf || (sector0 ? etfForSector(sector0) : null);
    const sector = sectorEtf && benchLookup ? benchLookup(sectorEtf) : [];
    // Structured, timestamped catalyst ONLY (from op=today's event record). A
    // missing/dateless event stays null → the catalyst expert abstains.
    const evWhen = cur.event && (cur.event.when || cur.event.date);
    const evTs = evWhen ? Math.floor(Date.parse(evWhen) / 1000) : null;
    const catalystRecord = Number.isFinite(evTs)
      ? { type: (cur.event && cur.event.type) || 'earnings', tsUnix: evTs, surprise: null, source: 'op=today event' }
      : null;
    const meta = {
      company: cur.company || null, sector: sector0, sectorEtf,
      price: cur.price ?? poolEntry.price ?? null,
      liqTier: poolEntry.capGroup && poolEntry.capGroup !== 'unknown' ? poolEntry.capGroup : null,
      catalystRecord,
      selectionReasons: (universe.selectionReasons && universe.selectionReasons[ticker]) || null,
...
// Map a cap/liquidity group to the cost model's scope buckets.
// Phase 5 fix: UNKNOWN liquidity maps to the MOST EXPENSIVE tier (micro), never
// the cheapest — unknown data must not earn the cheapest cost assumption.
function capToScope(cap) {
  if (cap === 'micro' || cap === 'unknown' || cap == null) return 'micro';
