...
async function runPsrlTick(req, res) {
  ...
  try {
    ...
    const snapshot = runPsrl({ rows: scanRows, spyCandles: spy, asOf, prev });
...
async function runPsrlResearch(req, res) {
  try {
    cached(res, 300);
    const { experimentStatus } = require('./psrl/research');
    const ledger = await PS.readLedger();
    return res.status(200).json({ ok: true, shadow: SHADOW_BANNER, ...experimentStatus({ ledger }) });
  } catch (e) {
    noStore(res);
    return res.status(200).json({ ok: false, error: String(e && e.message || e) });
  }
