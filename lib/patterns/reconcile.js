...
function higherTimeframeTrend(detections, context = {}) {
  // A dead structure cannot define the higher-timeframe trend: FAILED/EXPIRED
  // patterns are excluded, so they can't silently "strengthen" live setups.
  const longest = detections
    .filter(d => !d.structurallyInvalid && d.phase !== 'FAILED' && d.phase !== 'EXPIRED')
...
function reconcileDetections(detections = [], { context = {}, eligibilityFor = null } = {}) {
  const usable = detections.filter(d => !d.structurallyInvalid && d.plan);
  // Phase semantics (correlation with reality, not hope):
  //   EXPIRED → ignored entirely.
  //   FAILED  → can NEVER be primary and can never add to its former direction's
  //             alignment — it contributes AGAINST that direction instead.
  const valid = usable.filter(d => d.phase !== 'FAILED' && d.phase !== 'EXPIRED');
  const failed = usable.filter(d => d.phase === 'FAILED');
  if (!valid.length) return { primary: null, secondary: [], conflicts: [], alignment: null, htf: { direction: 'UNKNOWN' }, ranked: [], failed };
  ...
  // Direction alignment weighted by trade relevance (not similarity). A FAILED
  // pattern counts AGAINST its former direction with its structural weight — a
  // dead bull flag is evidence against the bull case, never for it.
  let wAgree = 0;
  let wTotal = 0;
  ...
  for (const d of failed) {
    const w = round(0.35 * (isNum(d.structuralValidity) ? d.structuralValidity : 0.5));
    if (w > 0) {
      wTotal += w;
      if (d.direction !== primary.detection.direction) wAgree += w;
      failedNotes.push(`${d.label || d.family} (${d.direction}, ${d.timeframe}) FAILED — counts against the ${d.direction} case.`);
    }
  }
