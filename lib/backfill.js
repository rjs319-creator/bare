  ...
  for (const date of dates) {
    ...
    for (const row of rows) {
      ...
      signals.push({
        ...
        // Sessions to resolution — REQUIRED by the drift panel's age-matched (censored)
        // baseline: a live ledger only ~N sessions old can only be compared against seed
        // outcomes that had ALSO resolved within N sessions (wins resolve far slower than
        // stop-outs, so an uncensored baseline makes every young ledger read BROKEN).
        hold: Number.isFinite(out.hold) ? out.hold : null,
      });
