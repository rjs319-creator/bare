...
function analyzePutSetup(candles) {
  ...
  // The setup describes WHY this is a put-selling candidate (price action). The actual
  // tradeable strike is a REAL listed contract selected from the chain by the route
  // (lib/putsell-contract) — never this modeled `strike`, which is only an internal
  // support reference. So the reasons describe the SETUP, not a synthetic strike.
  const reasons = [
    ...
    `${supportBasis} support ~${bufferPct.toFixed(0)}% below spot — a strike below it carries an OTM cushion`,
  ].filter(Boolean);
...
// Sanity floor for ATM IV from the free option-chain feed. Real US-equity option
// IV sits ~10%–300%; a reading outside that is almost always broken/stale/empty
// data — treat it as missing rather than trusting it.
const IV_MIN = 0.10, IV_MAX = 3.0;
const ivUsable = iv => iv != null && iv >= IV_MIN && iv <= IV_MAX;
...
function finalizePutSell(setup, { atmIV, earningsInDays, contracts } = {}) {
  const iv = ivUsable(atmIV) ? atmIV : null;   // drop implausible readings
  const out = { ...setup, atmIV: iv, earningsInDays: earningsInDays != null ? earningsInDays : null };
  if (atmIV != null && iv == null) out.ivUnreliable = true;   // had a reading, but it was junk
  if (iv != null) {
    out.ivLevel = iv >= 0.5 ? 'high' : iv >= 0.3 ? 'moderate' : 'low';
...
// Rank + letter grade for a put-selling opportunity. Blends the price-action setup
// quality (0-1 score) with premium richness (ATM IV — the seller's yield): a great
// chart at a fat premium grades highest. Returns { rankScore 0-100, grade }.
function gradePutSell(setup) {
  const clamp01 = v => Math.max(0, Math.min(1, v));
  const setupPart = clamp01(setup.score || 0);
  const iv = setup.atmIV;
  // 60% IV ≈ full premium credit. Missing OR implausible IV (sanity floor) grades
  // NEUTRAL — not a penalty (bad data shouldn't sink a good setup) but not a free
  // top-rank either, so a name with confirmed good premium still outranks an
  // unknown-premium one, and a confirmed LOW-premium name ranks below neutral.
  const ivPart = ivUsable(iv) ? clamp01(iv / 0.6) : 0.5;
  const rankScore = Math.round((setupPart * 0.7 + ivPart * 0.3) * 100);
  const grade = rankScore >= 80 ? 'A+' : rankScore >= 72 ? 'A' : rankScore >= 63 ? 'B' : rankScore >= 54 ? 'C' : 'D';
  return { rankScore, grade };
}

module.exports = { analyzePutSetup, finalizePutSell, gradePutSell, niceStrike };
