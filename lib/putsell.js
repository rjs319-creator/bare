...
function gradePutSell(setup) {
  ...
  // 60% IV ≈ full premium credit. No data OR an implausible reading (sanity floor)
  // → lean on the setup alone, so bad free-feed IV can't unfairly sink the grade.
  const ivPart = ivUsable(iv) ? clamp01(iv / 0.6) : setupPart;
