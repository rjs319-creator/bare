...
const HARNESS_V3_VERSION = 'research-harness-v3';
...
function runExperimentV3(events, rankers, opts = {}, meta = {}) {
  ...
  for (let f = 1; f < folds; f++) {
    ...
    const trainRows = comparable.filter((r) => r.decisionTs < dates[lo]);
    foldDefs.push({ fold: f, from: dates[lo], to: dates[hi - 1], trainN: trainRows.length, testN: testRows.length });
    for (const r of all) {
  ...
  // Verdicts — decided AFTER correction, with the control binding.
  const controlMean = summaries[controlName] ? summaries[controlName].meanIC : null;
  const verdicts = {};
  for (const t of withQ) {
    const s = summaries[t.id];
    const costNet = opts.costNetByRanker && Number.isFinite(opts.costNetByRanker[t.id]) ? opts.costNetByRanker[t.id] : null;
    const checks = {
      ...
      costNetPositive: costNet == null ? null : costNet > 0,
      notOutlierDriven: !s.outlierDependence || s.outlierDependence.dominantFrac == null || s.outlierDependence.dominantFrac < 0.5,
    };
    const required = ['beatsBenchmarkNominal', 'qPassesFdr', 'sufficientEffectiveN', 'multiplePositivePeriods', 'beatsControl', 'notOutlierDriven'];
    const pass = required.every((k) => checks[k] === true) && checks.costNetPositive !== false;
    verdicts[t.id] = {
      ...t, checks,
      verdict: pass
        ? 'PASS-PROVISIONAL — survives FDR + dependence-aware inference; promotion still requires survivorship safety and prospective agreement'
        : 'NOT-CONFIRMED',
