...
const { POLICIES } = require('../execution-policy');
...
// v2: rows carry the canonical decision envelope (selectionState, strategy/calibration
// identity, code commit/deploy id, provenance, cohortSize) — additive only.
const BRIDGE_VERSION = 'live-bridge-v2';
...
function predictionFromSignal(sig, ctx = {}) {
  const {
    ...
    universeSnapshotId = null, regime = null, envelope = null,
  } = ctx;
  ...
  return S.makePrediction({
    ...
    // ── decision-envelope fields (schema v2) ───────────────────────────────────
    // selectionState is the canonical five-value view of `state`; strategy identity
    // resolves through the registry from the signal's source; code identity and
    // provenance come from the per-snapshot envelope context (resolved once).
    selectionState: ENV.normalizeSelectionState(rejected ? 'rejected' : 'selected'),
    symbolAtDecision: ticker,
    ...ENV.strategyIdentity(sig.source || null),
    calibrationVersion: (envelope && envelope.calibrationVersion) || null,
    codeCommit: (envelope && envelope.codeCommit) || null,
    deployId: (envelope && envelope.deployId) || null,
    provenance: (envelope && envelope.provenance) || 'prospective_live',
    cohortSize: ctx.cohortSize != null ? ctx.cohortSize : null,
    // Governance verdict captured verbatim at decision time — a compact projection of
    // lib/eligibility assessSignal (attached to ranked rows by decision-routes classify).
    // Absent when the row predates the gate; null is honest, never backfilled.
    eligibility: sig.eligibility ? {
      signalClass: sig.eligibility.signalClass ?? null,
      sizingWeight: sig.eligibility.sizingWeight ?? null,
      reasonCodes: Array.isArray(sig.eligibility.reasonCodes) ? sig.eligibility.reasonCodes : [],
      governanceStatus: (sig.eligibility.governance && sig.eligibility.governance.status) || null,
      tradeEligible: sig.eligibility.tradeEligible === true,
      sizingEligible: sig.eligibility.sizingEligible === true,
    } : null,
    // The fill rule is DECLARED here and merely honored by the grader — same constant
    // lib/research/grade.js grades with, so a policy drift is a code-review diff.
    intendedFillRule: POLICIES.NEXT_OPEN,
    // Features come from the decision session's close: cutoff = decision close.
    dataCutoffTs: decisionTs,
    // Decision-time feature persistence: the numeric inputs on the signal plus the
    // audited score decomposition the rank was built from. A learner trains from
    // THIS, not from a later re-derivation that may see different data.
    features: {
      raw: {
        entry: Number.isFinite(sig.entry) ? sig.entry : null,
        stop: Number.isFinite(sig.stop) ? sig.stop : null,
        target: Number.isFinite(sig.target) ? sig.target : null,
        rawConfidence: Number.isFinite(sig.rawConfidence) ? sig.rawConfidence : null,
        dollarVol: (sig.liquidity && Number.isFinite(sig.liquidity.dollarVol)) ? sig.liquidity.dollarVol : null,
        ageBars: Number.isFinite(sig.ageBars) ? sig.ageBars : null,
        lifecycleState: sig.state || null,
      },
      normalized: sig.scoreDecomposition || null,
    },
  });
}
...
function buildDecisionSnapshot(ranked, ctx = {}) {
  ...
  // Envelope context resolved ONCE per snapshot (code identity, provenance), then
  // stamped on every row along with the cohort size — a row is only interpretable
  // relative to the population it was chosen from.
  const envelope = ctx.envelope || ENV.envelopeContext();
  const rowCtx = { ...ctx, envelope, cohortSize: rows.length };
  const predictions = rows.map(sig => predictionFromSignal(sig, rowCtx));
