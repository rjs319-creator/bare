...
// MICRO-CAPITAL scenario (capacity asymmetry, 2026-08-05): the program's
// surviving edges live in thin event corners institutions cannot deploy in —
// but the default $10k notional no-fills exactly those names via the
// participation cap. Evaluations that quote a micro tier run a SECOND
// executableOutcome pass with { notionalUsd: MICRO_NOTIONAL_USD } and publish
// it as its own cost-evidence artifact (strategyId suffixed '-micro'); the
// bps cost model is size-independent here, so the micro read differs ONLY in
// which fills the participation guard admits — that is the honest point.
const MICRO_NOTIONAL_USD = 2500;
...
module.exports = {
  EXEC_ENGINE_VERSION, ENTRY_BASIS, LIQUIDITY_TIERS, MAX_PARTICIPATION, MICRO_NOTIONAL_USD,
