...
const HARNESS_VERSION = 'research-harness-v2';           // v2: required benchmarks auto-appended
// Rankers every runExperiment reports against on identical folds (see runExperiment).
const REQUIRED_BENCHMARKS = Object.freeze(['control-random', 'momentum-12-1']);
...
function runExperiment(events, rankers, opts = {}, meta = {}) {
  // REQUIRED BENCHMARKS: every experiment reports against the no-information control and the
  // 12-1 momentum swing benchmark on identical folds. Auto-appended when absent so no experiment
  // can quietly skip the comparison (opts.requireBenchmarks === false opts out, tests only).
  // Benchmarks are yardsticks, not trials: relatedExperimentsAttempted still counts the CALLER's
  // ranker list, and being the benchmark never promotes momentum itself to a live winner.
  let bench = [];
  if (opts.requireBenchmarks !== false) {
    const B = require('./baseline-ranker');
    const have = new Set((rankers || []).map((r) => r && r.name));
    bench = REQUIRED_BENCHMARKS.map((n) => (n === 'control-random' ? B.randomRanker : B.momentum121Ranker))
      .filter((r) => !have.has(r.name));
  }
  const allRankers = [...(rankers || []), ...bench];
  const result = compareRankers(events, allRankers, opts);
  ...
  const manifest = makeExperimentManifest({
    ...
    // Trial denominator: never LESS than the hypothesis registry knows about this
    // family — a single compareRankers call must not shrink the multiple-testing
    // correction below the program-level trial count.
    relatedExperimentsAttempted: (() => {
      const caller = meta.relatedExperimentsAttempted == null ? rankers.length : meta.relatedExperimentsAttempted;
      const registryN = meta.experimentFamilyId
        ? require('./hypothesis-registry').familyTrials(meta.experimentFamilyId) : 0;
      return Math.max(caller, registryN);
    })(),
  ...
  const momBench = result.perRanker['momentum-12-1'] || null;
  return {
    version: HARNESS_VERSION,
    champion: champion ? { ranker: champion[0], meanIC: champion[1].meanIC, ci90: champion[1].ci90 } : null,
    // The required swing benchmark comparison: a candidate only claims incremental value when it
    // beats 12-1 momentum on the same folds. null benchmark IC = benchmark not measurable on this
    // universe (e.g. <1yr of history) — that is reported, never silently substituted.
    vsMomentumBenchmark: momBench
      ? {
          benchmarkMeanIC: momBench.meanIC,
          championBeatsBenchmark: champion && champion[1].meanIC != null && momBench.meanIC != null
            ? champion[1].meanIC > momBench.meanIC && champion[0] !== 'momentum-12-1'
            : null,
        }
      : null,
    result, manifest,
...
module.exports = {
  HARNESS_VERSION, MIN_NAMES_PER_DATE, REQUIRED_BENCHMARKS,
