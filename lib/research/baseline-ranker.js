...
// THE REQUIRED SWING BENCHMARK (12-1 momentum, Jegadeesh-Titman skip-month). Every swing-horizon
// experiment must report against it — the harness auto-appends it (harness.REQUIRED_BENCHMARKS).
// Deliberately NO fallback proxy: a row without a year of history scores 0 (uninformative), so a
// thin-history universe shows up as a null/weak benchmark IC instead of a silently different one.
// Benchmark status does NOT make momentum a live winner; the survivorship-free measurement of its
// own edge on this universe is ≈0 (research/MOMENTUM-SURVIVORSHIP-FREE-2026-07.md).
const momentum121Ranker = {
  name: 'momentum-12-1',
  fit() { return null; },
  score(_m, row) {
    const m = feat(row, 'mom121');
    return m != null ? m : 0;
  },
};
...
const ALL_RANKERS = [randomRanker, residualMomentumRanker, momentum121Ranker, productionCompositeRanker, ridgeRanker];

module.exports = {
  RANKER_VERSION,
  randomRanker, residualMomentumRanker, momentum121Ranker, productionCompositeRanker, ridgeRanker,
