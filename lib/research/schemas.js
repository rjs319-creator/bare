...
// v2: Prediction gains the canonical decision-envelope fields (selectionState, strategy/
// calibration identity, code identity, provenance, cohortSize) — all additive, all null
// when unknown, so v1 records remain readable and nothing is silently re-labeled.
const RESEARCH_SCHEMA_VERSION = 'research-schema-v2';
...
function makePrediction(input = {}) {
  return Object.freeze({
    ...
    // ── decision-envelope fields (v2, all optional/null) ────────────────────────
    // Canonical five-value selection state (lib/decision-envelope.js owns the enum and
    // the mapping from the older per-ledger vocabularies). `state` above is retained
    // for backward compatibility; selectionState is the normalized view.
    selectionState: orNull(input.selectionState),
    symbolAtDecision: orNull(input.symbolAtDecision),
    strategyId: orNull(input.strategyId),
    ...
    deployId: orNull(input.deployId),
    provenance: orNull(input.provenance),
    cohortSize: orNull(input.cohortSize),
    // ── decision-time audit fields (2026-08-11) ─────────────────────────────────
    // The governance verdict, the DECLARED fill rule, and the data cutoff belong on
    // the record at decision time: a grader may not choose an execution policy after
    // the fact, and an auditor must see why a row was or wasn't sizable without
    // re-running governance against long-gone state.
    eligibility: input.eligibility ? Object.freeze({ ...input.eligibility }) : null,
    intendedFillRule: orNull(input.intendedFillRule),
    dataCutoffTs: orNull(input.dataCutoffTs),
    // Feature persistence (2026-08-11): the inputs AS SEEN at decision time — without
    // them the ledger can be graded but never learned from (a re-derivation from later
    // data is not the same evidence). `raw` = the signal's own numeric fields;
    // `normalized` = the audited score decomposition the rank was actually built from.
    features: input.features ? Object.freeze({
      raw: Object.freeze({ ...(input.features.raw || {}) }),
      normalized: input.features.normalized ? Object.freeze({ ...input.features.normalized }) : null,
    }) : null,
  });
}
