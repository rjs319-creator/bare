...
const HYPOTHESES = Object.freeze([
  ...
  {
    id: 'insider-cluster-drawdown', familyId: 'alt-signals', mode: 'exploratory', status: 'open',
    ...
    evidence: 'no run yet at registration — the EDGAR pull for this universe/window has not been made',
    note: 'PIT discipline: event timing uses FILING dates (the congress-trades lesson: disclosure lag would leak the future if transaction dates anchored entry). Known risks: 184-name universe is present-day lists (survivorship-reduced — cohort comparisons share the bias); EDGAR coverage of micro caps is complete but price caches are not (58 names drop, counted). The 2026-06 pilot measured buys as only ~3.6% of small-cap Form-4 flow, so event counts may be thin — a fail-closed minimum of 40 cluster+drawdown events is frozen in the runner.',
  },
]);
