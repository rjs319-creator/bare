...
//
// Statuses: open | confirmed | provisional | inconclusive | no-edge | invalid-data | retired
//
// `inconclusive` vs `no-edge` (F-10): no-edge is the GRAVEYARD — an idea filed
// there is deliberately not re-run. It is an affirmative claim that the thing was
// tested and failed, and it requires an affirmative not-confirmed record.
// `inconclusive` means evidence exists but decided nothing; the question is still
// open to a better-powered test. deriveStatus used to collapse the second into the
// first, burying undecided hypotheses in the graveyard.
// Pure module: no network, no clock, no store.

const REGISTRY_VERSION = 'hypothesis-registry-v1';

const STATUSES = Object.freeze(['open', 'confirmed', 'provisional', 'inconclusive', 'no-edge', 'invalid-data', 'retired']);
...
const HYPOTHESES = Object.freeze([
  ...
  {
    // DOWNGRADED provisional → open (graduation-league F-08, 2026-08-12): the
    // "independent re-test 2026-08-02 (ALPHA-STRATEGY-TEST-REPORT)" cited below does
    // not exist anywhere in the repository, so its load-bearing numbers (450 fills,
    // PF 1.47, DSR 0.992, PBO 0.457) are unreproducible. The in-repo rig evidence
    // (research/intraday/*) stands, but it alone never supported 'provisional'.
    // Restore provisional only by regenerating the re-test artifact into the repo.
    id: 'unscheduled-gap-orb', familyId: 'intraday-gap', mode: 'confirmatory', status: 'open',
  ...
  // ── Insider cluster buys, matched-residual (registered 2026-09-09) — EXPLORATORY ──
  // A DIFFERENT hypothesis from insider-cluster-drawdown (no drawdown condition,
  // matched-control residual instead of SPY-excess, cost-net, next-open entry) on a
  // DIFFERENT dataset (SEC bulk Form 345 over the ~10k-name research cache with the
  // v3 security master's delistings, ≈40× the prior universe). The prior no-edge
  // result and its comparison cohorts are declared as KNOWN in the preregistration.
  {
    id: 'insider-cluster-residual', familyId: 'alt-signals', mode: 'exploratory', status: 'provisional',
    hypothesis: 'Open-market cluster purchases by ≥2 distinct insiders (14-day window, ≥$50k combined; event date = latest filing date) in small/micro-cap names carry positive 21-session COST-NET forward return in excess of matched non-event controls (same decision date, same as-of ADV tier, same 6-1 momentum quintile).',
    mechanism: 'independent insiders committing capital is an information-bearing, costly signal; matching on date, liquidity tier and momentum quintile removes the co-movement that made the earlier IN-pillar signal real-but-redundant inside a momentum composite',
    primaryMetric: 'RES_21: per-decision-date equal-weight mean of (event cost-net next-open→+21 close return − mean matched-control cost-net return), lib/evidence-stats HAC t / CI / effective N / 4 blocks; Benjamini-Hochberg across RES_5/RES_21/RES_63 at q ≤ 0.10; research-promising iff RES_21 mean > 0 AND q ≤ 0.10 AND ≥3/4 blocks positive AND placebo (−126-session shift, matched identically) shows |mean| < ½ the event mean with a CI spanning zero; insufficient-data below 200 primary events or 60 distinct decision dates',
    baseline: 'matched controls (≤20 per event, seeded) with SPY-excess fallback below 5 matches (counted, flagged, primary also reported matched-only); placebo −126 sessions; descriptive cohorts: single-buyer, 10b5-1, liquid tier, officer/director-only, ≥3 owners, ≥$250k, drawdown split, by tier, by year, with/without the 2026-08 universe',
    universe: 'SEC Form 345 bulk quarterly datasets 2021q2→2026q2 (research/97-form345-build.js) joined to the research price cache (~10k symbols, 2021-06→2026-06, delistings retained); primary = small+micro as-of ADV tiers ($0.5M ≤ ADV60 < $20M), close ≥ $1, ≥126 bars of history, non-10b5-1, 21-session per-name cooldown; decision dates 2022-01-03→2026-03-20',
    costs: 'one tiered round trip (lib/costs via experiment-kit.costFractions on as-of ADV) subtracted from BOTH the event leg and every control leg',
    expectedDirection: 'positive RES_21; RES_63 ≥ RES_21 if the effect is drift rather than a filing-day pop',
    stoppingRule: 'ONE exploratory pass (research/98-insider-cluster-residual.js) with every parameter frozen in research/PREREGISTRATION-INSIDER-CLUSTER-RESIDUAL-2026-09.md BEFORE the bulk build finished; no subgroup becomes a claim; the 2022-2026 window is spent on completion; a promising verdict earns only a weight-0 shadow prospective ledger, never weight',
    evidence: 'EVALUATED 2026-09-09 (the one registered pass, window now SPENT; sealed at commit 053afa6, run 13s): research/experiments/registry.json id insider-cluster-residual-2026-09 + research/data/insider-cluster-residual/. 2,369 primary events / 832 decision dates (99.9% matched, 20 controls each). FROZEN GATES PASSED → research-promising: RES_21 +1.36% [0.25, 2.58] HAC t 2.41 q 0.016 effN 721, 3/4 blocks; RES_5 +0.64% [0.20, 1.09] 4/4; RES_63 +1.85% [0.35, 3.59] 4/4; placebo −0.21% [−1.09, 0.68] (null). HONEST READ (pre-declared descriptives, not gates): the date-level MEDIAN is +0.19% and the hit rate 51.1%; excluding the top 1% of events (≈24 clusters) the date-level mean is +0.01% — the mean is carried by a handful of multi-baggers. Versus SPY the event names are FLAT cost-net (+0.54% n.s. at 21s, 0.00% at 63s); the residual exists because the matched small/micro control basket LAGGED SPY by −0.82%/21s. Descriptives: officer/director-only +1.66% t 3.0; drawdown +2.19% t 2.6; single buyers +0.99% t 1.97; liquid tier +0.06% (nothing); by year 2022 +0.17 / 2023 +2.71 / 2024 −0.38 / 2025 +3.37 / 2026 −1.25.',
    note: 'Known at registration: insider-cluster-drawdown (no-edge, 2026-08-05) and its comparison cohorts on a ~184-name present-day universe; the ~130 prior events are a subset of this sample (legacy-universe cohort +2.43% t 0.95; ex-legacy +1.28% t 2.23). PROVISIONAL means: the frozen gates passed, so the hypothesis earns a weight-0 SHADOW prospective ledger — and nothing else. It is a relative-to-peers, lottery-skewed effect, not an absolute long-only edge. The prospective preregistration MUST add a robust-location gate (date-level median > 0 or 10%-trimmed mean > 0) and a dominant-event cap, which this pass did not have and would not have met.',
  },
  ...
  // risk. Synthetic study only (no historical option prices on hand).
  {
    ...
    note: 'SECONDARY OBSERVATION, not a result: the UNGATED put-write beat SPY risk-adjusted on this window (Sharpe 1.03 vs 0.80, maxDD −13.1% vs −20.3%, worst month −7.2%, 74% positive) with LOWER total return (49.6% vs 66.3%) — consistent with the CBOE PUT/BXM literature class. Caveats that keep this a lead only: one 5y window with no 2020-class crash (the left tail IS the premium’s price), synthetic no-smile pricing, no gap-risk realism. Any follow-up needs real option marks — the gexarchive IV stream as it matures — and would be a NEW hypothesis about the UNGATED harvest, not a gate. FOLLOW-UP REGISTERED same day: vrp-put-write-live (prospective, real quotes).',
  },

  // ── VRP live paper put-write (registered 2026-08-05) — CONFIRMATORY, prospective ──
  // The preregistered follow-up to vrp-overlay-synthetic's UNGATED secondary
  // lead, at REAL quoted prices, on entries that had not occurred at
  // registration. Deliberately UNGATED: the synthetic pass refuted the gates.
  {
    id: 'vrp-put-write-live', familyId: 'volatility-structure', mode: 'confirmatory', status: 'open',
    hypothesis: 'A systematic paper SPY put-write — sell one 30-day-nearest ATM put at the real end-of-day bid every 5th trading session, cash-secured, held to expiry — earns the variance risk premium prospectively: positive mean period return net of costs, with a risk-adjusted profile at least matching SPY buy-and-hold over the accrual span.',
    mechanism: 'the variance risk premium: option buyers systematically overpay for crash insurance; the seller earns the premium as compensation for bearing the left tail (CBOE PUT/BXM evidence class), measured here at real bids so the smile and spread are IN the marks',
    primaryMetric: 'on the prospective ledger only: mean per-entry period return (P&L ÷ strike×100 collateral, net of $1/contract) with OVERLAP-AWARE Newey-West t ≥ 2 (entries share 30d windows on a 5-session grid — lag ≈ 6), AND ledger Sharpe ≥ SPY buy-and-hold Sharpe over the identical span, AND max drawdown of the cumulative curve > −25% of single-contract collateral, BH-corrected within volatility-structure at familyTrials-at-evaluation',
    baseline: 'SPY buy-and-hold over the identical span; cash (zero) as the period-return null',
    universe: 'SPY only; entry/tenor/strike/pricing rules FROZEN in lib/vrp-routes.js (5-session grid, DTE 21-45 nearest 30, strike nearest spot with bid > 0, sell at bid, settle to intrinsic at the first session ≥ expiry); skipped entries are RECORDED with reasons and count as no-trade periods',
    costs: 'sell-at-bid pays the real spread by construction, plus $1/contract; no other haircut',
    expectedDirection: 'positive net mean period return; Sharpe ≥ SPY; drawdown bounded',
    stoppingRule: 'PREREGISTERED 2026-08-05 (research/PREREGISTRATION-VRP-LIVE-2026-08.md, sealed BEFORE the first entry existed): ONE evaluation, no earlier than 2028-02-01 AND only once ≥60 resolved entries spanning ≥18 months INCLUDING at least one month where SPY fell ≥5% (the premium’s price must be observed, not assumed); NO gating, NO strike/tenor changes, NO interim performance reads (op=vrpbook is a public paper display — its existence is not an evaluation; the sealed act is quoting it as evidence); any deviation is a NEW hypothesis',
    evidence: 'no entry existed at registration — the ledger begins with the next cron tick',
    note: 'This is compensated RISK TRANSFER, not mispricing alpha, and the app surface says so: the Options-tab panel is labeled paper/preregistered with no performance claim. The stopping rule’s ≥5%-down-month condition exists because a put-write record without a drawdown month is unfalsifiable marketing, not evidence. Live capital is a separate human decision outside this registry.',
  },

  // ── Ephemeral edge factory (registered 2026-08-06) — CONFIRMATORY, prospective ──
  // The disciplined version of "search endless scenarios for temporary
  // mismatches": a mechanical grammar (nothing hand-picked), Deflated-Sharpe
  // screening at the FULL grammar trial count, and a live paper queue as the
  // ONLY evidence lane. Temporary edges are RENTED: survivor sets recompute
  // every tick, so decayed cells silently stop firing.
  {
    id: 'ephemeral-edge-factory', familyId: 'novel-signal-lab', mode: 'confirmatory', status: 'open',
    hypothesis: 'The factory’s survivor stream — grammar cells clearing DSR ≥ 0.95 at the full 200-cell trial count on the trailing ~230-session window, recomputed daily — produces a pooled LIVE paper record with positive net excess at its fast (2/5-session) horizons.',
    mechanism: 'temporary dislocations exist and decay (post-publication factor decay, crowding cycles); a deflation-gated rescanner can hold whatever is currently exploitable without believing any single cell is durable — the claim is about the STREAM, never a cell',
    primaryMetric: 'on live-logged paper picks only: ≥200 resolved picks spanning ≥3 calendar months, pooled mean net excess > 0 with DATE-CLUSTERED t ≥ 2 (per-pick t is overlap-naive and never citable), net of the frozen 62bps round trip, BH-corrected within novel-signal-lab at familyTrials-at-evaluation',
    baseline: 'zero (cost-charged); the DSR screen’s sr0 (expected max Sharpe under the null at 200 trials) is the in-sample bar',
    universe: 'large+small candle-cache universe (~690 names); grammar, features, z-threshold, horizons, cost, and caps FROZEN in lib/ephemeral.js (200 cells: 5 cross-sectional features × hi/lo, singles + cross-feature pairs, × long/short × 2/5 sessions; MAX 15 picks/tick with overflow recorded)',
    costs: '62bps round trip charged per event in BOTH the screen and the live record (a dislocation that cannot pay the small-tier spread is noise)',
    expectedDirection: 'positive pooled live net excess',
    stoppingRule: 'PREREGISTERED 2026-08-06 (research/PREREGISTRATION-EPHEMERAL-2026-08.md, sealed BEFORE the first scan ran): ONE evaluation, no earlier than 2026-12-01 AND only once the sample condition holds; NO grammar/threshold/cost edits while accruing (any edit = a NEW factory version whose prior stream is closed and recorded); the in-sample survivor stats (op=ephemeral scan block) are selection artifacts BY CONSTRUCTION and may never be cited as evidence; the live lane is the only lane',
    evidence: 'no scan had run at registration',
    note: 'This factory is the program’s canonical answer to "grind scenarios until something works": grinding is allowed, BELIEVING is gated. In-sample survivors are expected to appear every tick (that is what selection does); the sealed question is whether the stream of them carries anything live. Fastest-maturing sealed hypothesis in the program (~200 resolved picks plausibly by late 2026).',
  },

  // ── Index-timing overlays (registered 2026-08-06) — EXPLORATORY ──
  // The ORTHOGONAL AXIS: every prior hypothesis asks "which name?" or "after
  // which event?" — this asks "WHEN does the index itself pay exposure?" The
  // program's one validated lever (regime avoidance) lives on this axis but was
  // only ever a brake, never mined for timing alpha. THREE frozen sub-signals,
  // one pass, one trial against the regime family.
  {
    id: 'index-timing-overlays', familyId: 'regime', mode: 'exploratory', status: 'no-edge',
    hypothesis: 'Calendar-time and cross-market state variables carry SPY timing information over 2021-2026: (a) TURN-OF-MONTH — returns concentrate in the sessions around month turn (mechanical contribution/reinvestment flows); (b) CREDIT DIVERGENCE — credit-down/equity-up states precede weak forward returns (credit leads); (c) VIX TERM SLOPE — spot>3M inversion states differ sharply from contango in forward returns.',
    mechanism: 'forced calendar flows (pension/401k sweeps, coupon reinvestment) for TOM; credit investors pricing distress before equity holders for divergence; the vol term structure encoding stress-regime persistence for slope',
    primaryMetric: 'per sub-signal, FROZEN: state-vs-complement forward-return difference with Newey-West t (lag 4, overlapping 5-session forwards; TOM uses daily in-window vs out-of-window) ≥ 2 AND the naive long-only overlay Sharpe > SPY buy-and-hold Sharpe on the identical window — both required for a LEAD; anything else is null for that sub-signal',
    baseline: 'SPY buy-and-hold; the complement state as the internal control',
    universe: 'index level only: SPY, HYG, ^VIX, ^VIX3M daily bars, 5y (~1,254 sessions). Definitions FROZEN in research/72-index-timing.js: TOM window = last session of month through first 3 of next; divergence state = trailing-21-session HYG return < 0 AND SPY trailing-21-session return > 0; inversion state = ^VIX close > ^VIX3M close.',
    costs: 'overlays reported gross AND net of 2bps per state-change switch (index ETF spread class)',
    expectedDirection: 'TOM window > rest; divergence state < complement; inversion state < contango',
    stoppingRule: 'ONE exploratory pass (research/72-index-timing.js) with all three definitions frozen here before the run; NO window/threshold/lookback tuning; the 2021-2026 window is spent for all three on completion; any surviving lead requires a NEW prospective registration before any live-timing use (the regime router stays untouched)',
    evidence: 'EVALUATED 2026-08-06 (the one registered pass, window now SPENT): research/data/evidence/index-timing-overlays/ record 95a0a0b68e90f84e (1,254 sessions). VERDICT NOT-CONFIRMED — all three sub-signals null as registered: TOM in-window forward returns LOWER than rest (overlay Sharpe 0.30 vs 0.73 buy-hold); CREDDIV directionally right (state fwd −0.05% vs +0.31%) but NW-t −1.77 underpowered, overlay 0.61; VIXSLOPE registered direction REVERSED (inversion days precede REBOUNDS, +1.27% vs +0.18% fwd5, t 1.85).',
    note: 'SECONDARY OBSERVATION, not a result: the VIX-slope BRAKE (out of market only on the 64 spot>3M inversion days) posted overlay Sharpe 1.35 vs 0.73, +159% vs +74%, maxDD −15.7% vs −25.4% — inversion days own returns are catastrophic even though the 5-session rebound begins inside the state. This is the program’s validated REGIME-AVOIDANCE lever in sharper form (slope beats level as the brake), NOT new alpha; the registered forward-return direction failed. Any use = a prospective shadow test as an alternative regime-router input, a NEW registration. Original priors note: TOM is decades-documented but 60 months is power-limited; credit divergence and slope are regime-adjacent so the risk is rediscovering the known brake rather than new alpha — the overlay-must-beat-buy-hold requirement is the guard against relabeling risk-avoidance as alpha. Counted as ONE trial with three named sub-signals; sub-signal cherry-picking at evaluation is prohibited (all three verdicts recorded together).',
  },

  // ── PSRL — Persistent Staircase Relative Leadership (registered 2026-08-11;
  //    see research/PREREGISTRATION-PSRL-2026-08.md; engine lib/psrl/*, shadow
  //    weight-0). Three related but SEPARATE trials, each widening the
  //    swing-ranking denominator. RLT context is binding prior evidence: on
  //    this universe rank-level, rank-acceleration and residual-momentum
  //    rankers measured indistinguishable from the random control, and that
  //    window may not be re-sliced. The 2022-2026 panel window is spent
  //    exploratory motivation for all three.
  {
    id: 'psrl-continuity-conditional',
    familyId: 'swing-ranking',
    mode: 'confirmatory',
    status: 'open',
    hypothesis: 'Conditional on total past momentum (within-decile), stocks whose advance was CONTINUOUS (low information discreteness, low top-1/top-3 return concentration, no jump-plateau) outperform jump-driven advances over 21 and 63 forward sessions.',
    mechanism: 'Frog-in-the-pan underreaction (Da/Gurun/Warachka): information arriving in small continuous doses is underpriced relative to discrete repricings, which are fully absorbed on the event day.',
    primaryMetric: 'per-date Spearman rank IC of the continuity contrast WITHIN momentum deciles (decile-demeaned), Newey-West HAC t, BH q ≤ 0.10 within swing-ranking familyTrials at evaluation; ESS ≥ 30; beats shuffled control; not outlier-driven (dominant-date fraction < 0.5)',
    baseline: 'plain m121 momentum decile rank (continuity ignored); control-random',
    universe: 'panel-v3 eligible cohorts (cap band, ADV floor, identity-resolved, cohort-eligibility ledger) for historical; live candle-cache scopes (survivorship-reduced, disclosed) for prospective shadow accrual via the psrl trend-episode ledger',
    costs: 'exec-engine-v1 next-open fills; net > 0 under base, doubled and stressed-liquidity costs required for any economic claim',
    expectedDirection: 'high-continuity > low-continuity within decile; STEADY_ADVANCE > JUMP_PLATEAU',
    stoppingRule: 'confirmatory evaluation ONLY on data unseen at registration (prospective episodes after 2026-08-11, or a proven-safe historical era if one is ever built); ONE verdict attempt when ESS ≥ 30 first holds; interim ICs never computed on accruing data; no threshold tuning, no subgroup selection; the 2022-2026 exploratory window is permanently excluded',
    evidence: 'none — registered 2026-08-11, no confirmatory data existed or was inspected at registration',
  },
  {
    ...
    mode: 'confirmatory',
    status: 'open',
    hypothesis: 'Beta-adjusted residual leadership (market + sector-orthogonalized, atlasx-residual engine) ranks forward returns better than raw return, stock-minus-SPY, and stock-minus-sector excess returns.',
    mechanism: 'Raw excess return conflates beta carried by the tape with stock-specific strength; residualization removes the passenger component (Blitz/Huij/Martens residual momentum).',
    primaryMetric: 'incremental per-date rank IC of residual rankers over the raw relative-strength rankers on IDENTICAL folds, Newey-West HAC t, BH q ≤ 0.10 within swing-ranking; ESS ≥ 30',
    baseline: 'raw 63-session return; stock-minus-SPY; stock-minus-sector; control-random (the RLT baseline ladder, same folds)',
    universe: 'same as psrl-continuity-conditional; sector classification is current-vendor NOT point-in-time — disclosed on every sector-conditioned cut and the market-only fallback is reported separately',
    costs: 'exec-engine-v1, base/doubled/stressed',
    expectedDirection: 'residual > raw-relative > raw absolute (prior evidence is AGAINST: RLT measured residual-momentum ≈ 0 here; a null confirms the RLT result)',
    stoppingRule: 'same sealed conditions as psrl-continuity-conditional; one verdict attempt at ESS ≥ 30; no re-slicing of the RLT window',
    evidence: 'none — registered 2026-08-11, no confirmatory data existed or was inspected at registration',
  },
  {
    ...
    mode: 'confirmatory',
    status: 'open',
    hypothesis: 'Adding PSRL continuity and residual-leadership axes to OMEGA improves out-of-sample ranking and economics versus OMEGA alone.',
    mechanism: 'OMEGA is short-horizon momentum/setup evidence; the claim is that LONGER-horizon path quality (continuity, persistence, jump-vs-plateau) carries incremental information about 21-63-session continuation.',
    primaryMetric: 'incremental rank IC, Precision@K and opportunity recall of OMEGA+continuity / OMEGA+residual / OMEGA+both / full challenger over OMEGA alone, purged walk-forward, HAC t, BH q ≤ 0.10 within swing-ranking; target-before-stop rate, stagnation and severe-loss reductions; net expected utility after exec-engine-v1 costs',
    baseline: 'production OMEGA ranking; existing swing ranker; 12-1 / 6-1 / 3-1 momentum; raw market and sector relative strength; persistent-trend-without-leadership; control-random',
    universe: 'same as psrl-continuity-conditional',
    costs: 'exec-engine-v1, base/doubled/stressed',
    expectedDirection: 'challenger ladder > OMEGA alone; if continuity adds nothing beyond momentum, this is a NULL and is recorded as such',
    stoppingRule: 'same sealed conditions; one verdict attempt at ESS ≥ 30; promotion additionally requires the full strategy-gate ladder and explicit registry approval — a pass here alone changes nothing in production',
    evidence: 'none — registered 2026-08-11, no confirmatory data existed or was inspected at registration',
  },
  {
    id: 'psrl-predictive-layer',
    familyId: 'swing-ranking',
    mode: 'confirmatory',
    status: 'open',
    hypothesis: 'The PSRL predictive addendum components — trend-life survival, causal change-point evidence, sector breadth structure, catalyst confirmation, and the stagnation/breakdown failure vetoes — each add incremental out-of-sample value beyond the base PSRL evidence score, measured by feature-family ablation.',
    mechanism: 'Trend age and path regime carry duration information momentum levels do not; independent failure vetoes remove the left tail rather than adding to the right; breadth/catalyst context distinguishes stock-specific persistence from sector carry.',
    primaryMetric: 'per-family ablation deltas (base / +trend-life / +change-point / +breadth / +catalyst / +vetoes / full) on identical purged folds: incremental rank IC (HAC t, BH q ≤ 0.10 within swing-ranking), survival-model concordance vs a trend-age-only baseline, stagnation precision/recall, breakdown recall, warning time, and cost-net opportunity value; ESS ≥ 30; placebo features and shuffled controls required alongside',
    baseline: 'base PSRL combined evidence score; trend-age-only hazard; deterministic plateau logic (the change-point detector must beat it to matter); control-random',
    universe: 'psrl trend-episode ledger (prospective accrual, startDate > 2026-08-11) with right-censored episodes contributing exposure only; historical panels inherit survivorship-reduced and cannot promote',
    costs: 'exec-engine-v1, base/doubled/stressed',
    expectedDirection: 'each family ≥ 0 incremental; any family measuring ≤ 0 is dropped from the challenger and recorded as a null',
    stoppingRule: 'ONE ablation evaluation when the episode ledger first reaches ESS ≥ 30 resolved episodes; no interim peeking (op=psrlresearch reports accrual only); no per-family threshold tuning after registration; the isolated-leadership sign is a measured output, never an assumption',
    evidence: 'none — registered 2026-08-11, no episode had resolved at registration (the ledger did not exist)',
  },
  {
    id: 'transparent-challenger-v1',
    familyId: 'swing-ranking',
    mode: 'confirmatory',
    status: 'open',
    hypothesis: 'A fixed-sign, equal-weight cross-sectional RANK composite — +sector-residual momentum 126→21, +tightness 20d, −extension in ATR, −idiosyncratic vol 63d, −expected round-trip cost — carries positive cost-net cross-sectional information, and each term contributes incrementally over the residual-momentum-only arm on identical dates.',
    mechanism: 'Residual momentum is the one factor family this repo has repeatedly measured positive (~0.10 IC); tightness proxies supply absorption; extension and idio vol are crash/reversal exposure priors; charging expected cost INSIDE the score prefers implementable names over gross look-alikes.',
    primaryMetric: 'per-arm identical-date rank IC (date-clustered t) and top-K cost-net excess at base/doubled costs (research/75-transparent-challenger.js); harness validity requires the frozen-inverse arm to mirror the full arm and a within-date shuffled placebo |IC| < 0.02; BH q ≤ 0.10 within swing-ranking',
    baseline: 'residualMomentumOnly (the composite must beat its own strongest term incrementally); frozenExisting (the shipped quant composite on the same rows); control-random via placebo',
    universe: 'research cache (survivorship-reduced, labeled) for the ablation study; prospective decisions for any promotion-relevant claim',
    costs: 'lib/costs cost-v2 inside the score; experiment-kit base/doubled on outcomes',
    expectedDirection: 'full ≥ residualMomentumOnly ≥ 0; frozenInverse ≈ −full; placebo ≈ 0',
    stoppingRule: 'ONE historical ablation read (research/75), then prospective accrual only; signs and equal weights are FROZEN — any reweighting is a new hypothesis under a new version',
    evidence: 'first identical-date ablation 2026-08-11 (research/75; 97 dates, 600-name survivorship-reduced cache, market-basis residuals): harness VALID — frozen inverse mirrors the full arm to −0.00005 and the shuffled placebo IC is +0.006. full arm rank-IC +0.037 (t 2.26) BUT top-10 cost-net −1.12%/21d (−1.38% doubled) — it FAILS its own expectedDirection at the top of book; residualMomentumOnly is the only arm with positive top-K net (+0.97% base / +0.53% doubled, IC +0.023 t 1.76). Read: the composite adds cross-sectional ORDERING information (mostly avoid-side) without a harvestable top-book edge beyond plain residual momentum. Controls added same day (re-runs recorded append-only as .r2/.r3): CSCV PBO across the four selectable arms = 0.63 — in-sample arm selection is mostly noise, corroborating the top-book read; one-bar-delayed full arm IC 0.039 vs 0.037 same-bar (Δ −0.002 ≈ 0) — the expected shape for a slow 126→21 momentum family, and proof the harness does not leak the decision bar (the freshest bar buys nothing). Stays shadow/weight-0; prospective accrual only; no reweighting permitted under this version.',
  },
  // ── External-event family (registered 2026-09-19, sealed b43f45a) — EXPLORATORY ──
  // Four documented event anomalies the program had NEVER touched, each reachable with
  // multi-year point-in-time data already in hand (EDGAR full-text search, the cached
  // FMP dividend/grade histories). One pass each, FDR across all 21 development cells,
  // sealed holdout 2025-01-02..2026-03-31 read once. Design and verdict rule:
  // research/PREREGISTRATION-EXTERNAL-EVENTS-2026-09.md; runner research/100-external-events-study.js.
  {
    id: 'activist-13d-initial', familyId: 'event-drift', mode: 'exploratory', status: 'no-edge',
    hypothesis: 'An initial Schedule 13D filing (activist ≥5% stake; SC 13D/A amendments excluded) is followed by positive cost-net SPY-excess return over the next 21 sessions from the next open.',
    mechanism: 'activist entry is a costly, public commitment that precedes engagement; the literature (Brav-Jiang-Partnoy-Thomas 2008) documents abnormal returns around the filing and drift after it, and the small-cap segment where activists concentrate is where limits to arbitrage let it persist',
    primaryMetric: 'variant A @ 21 sessions: per-decision-date equal-weight mean of (event next-open→+21 close return − tiered round trip − SPY same-window return), lib/evidence-stats HAC t / effective N / 4 blocks; BH q ≤ 0.10 across the 21 family cells on the development window, then the sealed-holdout gates of the preregistration §3',
    baseline: 'SPY-excess after one tiered round trip; placebo = the same names 126 sessions earlier; variant B (small/micro as-of tier) and 5/63-session horizons are observations only',
    universe: 'EDGAR full-text search hits (form exactly SC 13D, deduped by accession, subject = first display name with a ticker) 2021-07→2026-03, joined to the research price cache (~10k symbols incl. delisted; kit eligibility: ≥60 prior bars, close ≥ $2, as-of ADV60 ≥ $2M; one event per name per 63 sessions)',
    costs: 'one tiered round trip (experiment-kit.costFractions on as-of ADV) on the event leg; doubled cost reported and gated on the holdout',
    expectedDirection: 'positive at 21 sessions; 63 ≥ 21 if the effect is engagement drift rather than a filing-day pop',
    stoppingRule: 'ONE exploratory pass (research/100-external-events-study.js) with every parameter frozen in the preregistration BEFORE the pull; the 2021-2026 window is spent on completion; any confirmatory claim requires a new preregistration on future filings',
    evidence: 'EVALUATED 2026-09-19 (the one registered pass; r1 missed every holdout filing because the SEC renamed the form to "SCHEDULE 13D" in Dec 2024 — re-pulled under both labels and recorded append-only as research/experiments/registry.json id activist-13d-initial-2026-09.r2; artifact research/data/evidence/external-events/). 26,587 hits → 5,787 initial filings → 3,560 with a ticker → 509 scored (2,554 subjects below the $2M ADV floor or not cached). PRIMARY A@21 development: n 324 on 252 dates, cost-net SPY-excess −1.21% t −1.12 q 0.39, 1/4 blocks positive → FAILED. Holdout (read once, descriptive): −3.41% t −2.14. Observation: A@63 is robustly NEGATIVE (dev −5.22% t −3.16 q 0.038; holdout −6.63% t −3.12) but the same-name placebo 126 sessions earlier is also negative (t −2.3 / −3.0): activist targets are chronic SPY-underperformers, so most of the 63-session loss is a name effect, not a filing effect. NOT a buy signal; not a clean avoid signal either.',
    note: 'Known at registration: none of the prior passes touched 13D filings. Small/micro variant B is not better (dev B@21 −1.42% t −0.93).',
  },
  {
    id: 'buyback-authorization-8k', familyId: 'event-drift', mode: 'exploratory', status: 'no-edge',
    hypothesis: 'A stand-alone 8-K announcing a share/stock repurchase program (item 8.01 without an item 2.02 earnings release) is followed by positive cost-net SPY-excess return over the next 21 sessions from the next open.',
    mechanism: 'a repurchase authorization signals management’s view that shares are undervalued and adds a price-insensitive buyer; Ikenberry-Lakonishok-Vermaelen 1995 document long-run drift after open-market repurchase announcements; the stand-alone filter separates the announcement from the earnings-day information it is usually bundled with',
    primaryMetric: 'variant B @ 21 sessions: same per-date cost-net SPY-excess statistic, HAC t, BH q ≤ 0.10 across the 21 family cells on development, then the sealed-holdout gates',
    baseline: 'SPY-excess after one tiered round trip; placebo = the same names 126 sessions earlier; variant A (every matching 8-K, including earnings releases) and 5/63-session horizons are observations only',
    universe: 'EDGAR full-text search hits for "share repurchase program" OR "stock repurchase program" restricted to form 8-K, 2021-07→2026-03, deduped by accession, filer ticker parsed from the display name, joined to the research price cache with the kit eligibility and a 63-session per-name cooldown',
    costs: 'one tiered round trip on the event leg; doubled cost gated on the holdout',
    expectedDirection: 'positive at 21 sessions',
    stoppingRule: 'ONE exploratory pass (research/100-external-events-study.js); phrase, items filter, horizons and split frozen in the preregistration before the pull; the window is spent on completion',
    evidence: 'EVALUATED 2026-09-19 (registry id buyback-authorization-8k-2026-09 / .r2, identical cells in both runs; artifact research/data/evidence/external-events/). 16,376 text hits → 12,776 plain 8-Ks with a ticker → 4,994 scored after the kit floors and the 63-session cooldown (variant B stand-alone item 8.01 without 2.02: 564 dev / 225 holdout events). PRIMARY B@21 development: n 564 on 386 dates, cost-net SPY-excess −1.61% t −2.88 q 0.047 — it SURVIVES the family FDR but in the WRONG direction, 0/4 blocks positive → FAILED as a long. Sealed holdout (read once): −2.31% t −2.08, doubled-cost −2.75%, same-name placebo −0.41% t −0.32 in the holdout and −0.21% t −0.25 in development. OBSERVATION (not a claim under this preregistration): stand-alone repurchase announcements were followed by consistent SPY-underperformance in BOTH windows while the placebo was flat — an event-specific negative, largest in the liquid tier (dev −1.27% t −2.12, holdout −2.84% t −2.41) and NOT explained by name drift. A follow-up as an AVOID overlay requires a NEW preregistration on future filings (negative-lane semantics, like extended-gap-avoid / dilution-events).',
    note: 'Known at registration: none of the prior passes touched repurchase announcements. The literature\'s positive drift (1980s-1990s open-market programs) did not appear in 2021-2026 cost-net vs SPY.',
  },
  {
    id: 'dividend-initiation', familyId: 'event-drift', mode: 'exploratory', status: 'no-edge',
    hypothesis: 'The declaration of a first cash dividend after ≥3 years without one (or the first ever) is followed by positive cost-net SPY-excess drift over the next 63 sessions from the next open.',
    mechanism: 'an initiation is a costly, hard-to-reverse signal of sustainable cash flow and widens the holder base to income mandates; Michaely-Thaler-Womack 1995 document a year of positive drift after initiations',
    primaryMetric: 'variant A @ 63 sessions: per-date cost-net SPY-excess statistic, HAC t, BH q ≤ 0.10 across the 21 family cells on development, then the sealed-holdout gates',
    baseline: 'SPY-excess after one tiered round trip; placebo = the same names 126 sessions earlier; 5/21-session horizons are observations only',
    universe: 'FMP dividend histories cached in research/data/corpactions (4,217 names, declaration dates present on ~86% of rows): a dividend > 0 with no prior dividend in the previous 1,095 calendar days, event date = declarationDate (must exist and be ≤ ex-date), joined to the research price cache with the kit eligibility and a 63-session cooldown',
    costs: 'one tiered round trip on the event leg; doubled cost gated on the holdout',
    expectedDirection: 'positive and growing from 21 to 63 sessions',
    stoppingRule: 'ONE exploratory pass (research/100-external-events-study.js); gap, date rule, horizons and split frozen before the pull; the window is spent on completion',
    evidence: 'EVALUATED 2026-09-19 (registry id dividend-initiation-2026-09 / .r2; artifact research/data/evidence/external-events/). 1,540 initiations in the 4,217-name corpactions cache, 1,013 declared before the price cache begins (2021-07) and 982 rows without a usable declaration date → 108 scored (86 development / 22 holdout). PRIMARY A@63 development: n 83 on 74 dates, cost-net SPY-excess −2.15% t −1.00 q 0.43, 2/4 blocks → FAILED. Holdout (read once): n 20, −0.32% t −0.06 — too thin to say anything. The declaration-date coverage of the cached dividend history is the binding data wall, not the mechanism.',
    note: 'Known at registration: no prior pass tested dividend events.',
  },
  {
    id: 'analyst-upgrade-cluster', familyId: 'event-drift', mode: 'exploratory', status: 'no-edge',
    hypothesis: 'Two or more distinct brokers upgrading the same name within 3 sessions is followed by positive cost-net SPY-excess return over the next 5 sessions from the next open.',
    mechanism: 'a single upgrade is largely priced at the open; a cluster of independent upgrades is a stronger consensus shift that institutional flows follow over days (Womack 1996; Jegadeesh-Kim-Krische-Lee 2004); the event is NOT the monthly rating-count revision breadth already tested (analyst-revisions, D-revision-breadth)',
    primaryMetric: 'variant A @ 5 sessions: per-date cost-net SPY-excess statistic, HAC t, BH q ≤ 0.10 across the 21 family cells on development, then the sealed-holdout gates',
    baseline: 'SPY-excess after one tiered round trip; placebo = the same names 126 sessions earlier; variant B (≥3 brokers) and 21/63-session horizons are observations only',
    universe: 'FMP grades history per symbol (action = upgrade, distinct gradingCompany) for every research-cache name with ADV60 ≥ $2M (~3.4k), 2021-08→2026-03; event date = the date of the 2nd (or 3rd) upgrade inside a 3-session window; kit eligibility; 63-session cooldown',
    costs: 'one tiered round trip on the event leg; doubled cost gated on the holdout',
    expectedDirection: 'positive at 5 sessions, fading by 21',
    stoppingRule: 'ONE exploratory pass (research/100-external-events-study.js); window, broker counts, horizons and split frozen before the pull; the window is spent on completion',
    evidence: 'EVALUATED 2026-09-19 (registry id analyst-upgrade-cluster-2026-09.r2 — r1 had a collapse defect that left variant B structurally empty, fixed and re-run append-only; artifact research/data/evidence/external-events/). 3,378 grade histories, 9,656 upgrade rows → 561 cluster sessions → 388 scored (260 development / 128 holdout for A). PRIMARY A@5 development: n 260 on 215 dates, cost-net SPY-excess −0.27% t −0.58 q 0.61 → FAILED; holdout (read once) −0.57% t −0.70. No horizon is positive in development (A@21 −0.92%, A@63 −1.99%). Variant B (≥3 brokers) has only 15 development events. Caveat: FMP broker names are not canonical ("B. Riley" / "B. Riley FBR"), which can only INFLATE cluster counts; case-insensitive matching only.',
    note: 'Known at registration: analyst-revisions (monthly rating-count Δconsensus) and D-revision-breadth were no-edge; this is a different event (same-window multi-broker upgrades).',
  },
]);
...
const HOLDOUTS = Object.freeze([
  ...
  {
    id: 'ephemeral-live-prospective',
    description: 'ephemeral-edge-factory: the ephemeral/day/* live paper-pick stream accruing from the first scan tick; ONE evaluation, no earlier than 2026-12-01 and ≥200 resolved picks across ≥3 months; the in-sample survivor scan is a selection artifact and citable never — only the live stream is evidence',
    sealedAt: '2026-08-06', openedAt: null, openedBy: null,
  },
  {
    id: 'vrp-live-prospective',
    description: 'vrp-put-write-live: the vrp/* paper ledger accruing from the first cron entry; ONE evaluation, no earlier than 2028-02-01 and ≥60 resolved entries across ≥18 months including ≥1 month with SPY down ≥5%; the op=vrpbook display is a paper surface, not an evaluation — QUOTING it as performance evidence opens the holdout',
    sealedAt: '2026-08-05', openedAt: null, openedBy: null,
  },
]);
