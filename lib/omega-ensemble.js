...
function buildSummary(today, health) {
  ...
  const validation = h && h.dsr ? {
    known: true,
    trials: num(h.dsr.trials),
    passing: num(h.dsr.passing),
    // 0 survivors is a RESULT, not a missing value — it is the multiple-testing gate
    // doing its job, and the page must say so plainly rather than look broken.
    verdict: h.dsr.passing === 0
      ? 'no specialist×regime×horizon cell survives the deflated-Sharpe multiple-testing gate'
      : `${h.dsr.passing} of ${h.dsr.trials} cells survive multiple testing`,
