...
function buildSummary(today, health) {
  ...
  return {
    ...
    mode: redundancy && redundancy.method === 'measured' ? 'measured-redundancy' : 'asserted-family-prior (fallback)',
...
function buildEnsembleView({ today, health } = {}) {
  ...
  // THE BOOK IS SIZING-ELIGIBLE ONLY (redesign Phase 2). op=today already builds
  // `portfolio.selected` from the ACTIONABLE + sizing-eligible set, so the primary path is
  // simply to trust it. The legacy fallbacks (`top` / flattened `topByHorizon`) exist for
  // pre-portfolio payloads and are NOT class-filtered upstream — so they are filtered here
  // rather than allowed to smuggle research rows into a page called "the Book".
  const rawSelected = (pf && Array.isArray(pf.selected)) ? pf.selected
    : (Array.isArray(t.top) ? t.top : Object.values(t.topByHorizon || {}).flat());
  const classOf = (s) => s.signalClass || s.evidenceClass || (s.eligibility && s.eligibility.signalClass) || null;
  const sizable = (s) => {
    if (s.sizingEligible === true) return true;
    if (s.eligibility) return s.eligibility.sizingEligible === true;
    // No verdict attached at all ⇒ the row predates the taxonomy. Admit it only when the
    // whole payload predates it (nothing else would be admissible either) — never mix.
    return classOf(s) == null;
  };
  const selected = rawSelected.filter(sizable);
  const annotations = rawSelected.filter(s => !sizable(s)).map(s => ({
    ticker: s.ticker, horizon: s.horizon, signalClass: classOf(s),
    reason: (s.eligibility && s.eligibility.reasons && s.eligibility.reasons[s.eligibility.reasons.length - 1]) || null,
    note: 'Research/lead overlay — visible for context; contributes no weight, no rank and no slot to the Book.',
  }));
  ...
  return {
    ...
    // Research/qualified-lead overlays on names the Book does NOT hold. Rendered in an
    // annotation column; numerically excluded from every executable figure on the page.
    annotations,
    bookBasis: {
      basis: 'ACTIONABLE + sizing-eligible only',
      considered: rawSelected.length,
      held: selected.length,
      annotationsOnly: annotations.length,
      universe: (pf && pf.universe) || null,
    },
    excluded,
    portfolio: pf ? {
    ...
    disclosures: [
      'The Book contains ONLY sizing-eligible ACTIONABLE ideas (cleared strategy + complete executable plan + known liquidity + current data). Qualified leads and research signals appear as annotations and contribute nothing to weight, rank or slots.',
