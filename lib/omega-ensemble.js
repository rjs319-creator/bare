...
function buildEnsembleView({ today, health } = {}) {
  ...
  const selected = (pf && Array.isArray(pf.selected)) ? pf.selected
    // Legacy `top` (pre horizon-separation) or the per-horizon shortlists flattened —
    // fallback display only, never a cross-horizon "best overall" claim.
