...
const OX = require('./omega-execution');  // executable entry (next-open/conditional fills, gap invalidation)
const OS = require('./omega-sizing');     // portfolio-aware, HARD-capped position sizing
...
function expectedUtility(pred, f, ctx = {}) {
  if (!pred) return 0;
  // Residual expectancy + a fraction of the prob-weighted favorable excursion. The residual is
  // already market-relative (small by construction); the excursion is an absolute path stat, so
  // it enters at a discount to stay on the same scale.
  const upside = (pred.expResidual10 || 0) + 0.5 * (pred.pPositive - 0.5) * (pred.expMFE || 0);
  // Downside as an EXPECTED LOSS (probability × magnitude), tail-emphasized — NOT the full
  // theoretical adverse excursion (every long has one; subtracting it whole would reject
  // everything regardless of edge). tailLossProb weights how likely a real drawdown is.
  const adverse = Math.abs(pred.expMAE || 0) * (0.15 + 0.6 * (pred.tailLossProb ?? 0.3));
  ...
  const uncertainty = (1 - (pred.core ?? 0.5)) * 0.005;
...
function riskPlan(candles, entry, f, { maxRiskPct = 0.01 } = {}) {
  ...
  // Risk-normalized sizing, HARD-capped so a tight stop can never imply ~100% of equity. The
  // full portfolio-aware size (with evidence haircuts) is computed in evaluateCandidate via
  // lib/omega-sizing; this is the conservative standalone fallback.
  const rawSizePct = (maxRiskPct / (riskPerShare / entry)) * 100;
  const sizePctOfEquity = +clamp(rawSizePct, 0, OS.MAX_POSITION_PCT * 100).toFixed(1);
  return {
    ...
    sizePctOfEquity, stopBasis: lv ? lv.stopBasis : 'atr', levels: lv || null,
  };
}
...
function evaluateCandidate({ ticker, candles, bench = {}, ctx = {} }) {
  ...
  // Executable entry PLAN (§3): map the entry intent to a real next-open/conditional fill model.
  // In live scoring the candle series ends at the signal bar, so this returns the PLAN (pending
  // fill); the backfill/replay resolves the actual T+1 fill separately with the full series.
  const signalDate = (candles && candles.length) ? candles[candles.length - 1].date : (ctx.signalDate || null);
  const tierName = OX.tierForDollarVol(f.dollarVol);
  const execution = risk ? OX.planOmegaEntry({
    candles, signalDate, entryClass: entry.classification, f, levels: risk.levels,
    stop: risk.invalidation, target1: risk.target1, target2: risk.target2,
    tier: tierName, maxGapPct: ctx.maxGapPct || OX.DEFAULT_MAX_GAP_PCT,
  }) : null;

  // Portfolio-aware, HARD-capped sizing (§11) — never 100% of equity. Anchored at the assumed
  // fill when known, else the signal close. Shadow strategy + uncalibrated probs shrink it.
  const sizeEntry = (execution && Number.isFinite(execution.assumedFillPrice)) ? execution.assumedFillPrice : f.price;
  const sizing = risk ? OS.positionSizing({
    entry: sizeEntry, stop: risk.invalidation, dollarVol: f.dollarVol,
    atrPct: f.atrPct != null ? f.atrPct / 100 : null,
    ctx: {
      maturity: ctx.maturity || 'shadow', calibrated: !!ctx.calibrated,
      binaryEvent: !!ctx.binaryEventInWindow, tailLossProb: pred ? pred.tailLossProb : null,
      sectorExposurePct: ctx.sectorExposurePct, clusterExposurePct: ctx.clusterExposurePct,
    },
    accountSize: ctx.accountSize || 100000,
  }) : null;
  if (risk && sizing && sizing.ok) {
    risk.sizePctOfEquity = sizing.sizePctOfEquity;
    risk.maxAcceptableEntryPrice = execution ? execution.maxAcceptableEntryPrice : null;
    risk.maxAcceptableGapPct = execution ? execution.maxAcceptableGapPct : null;
  }
  ...
  return {
    ...
    pred, entry, execution, sizing, risk, features: f,
  };
}
