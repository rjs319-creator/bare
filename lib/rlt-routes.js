...
const { assertShadow, modelHealth, promotionView, REGISTRATION } = require('./rlt-governance');
...
// SEQUENTIAL BY SCOPE, ON PURPOSE (changed 2026-08-19).
//
// This used to load all four scope caches with one Promise.all, so 'large', 'small',
// 'micro' AND 'expanded' — the full-market cache — were resident simultaneously before
// a single row was derived. Four chains share this loader (rlt, peerprop, psrl,
// target-compare), so several concurrent invocations could each hold all four on the
// same Fluid instance that was being OOM-killed at 22:0x.
//
// Deriving rows scope-by-scope and dropping each document as we go makes the peak one
// scope doc + the accumulated (much smaller) rows, instead of every doc + the rows.
// `loadScope` is injectable so the one-at-a-time property is unit-testable.
async function loadUniverseRows({ bands = null, loadScope = loadCandleCache } = {}) {
  const rows = [];
  const seen = new Set();
  const scopesLoaded = [];
  for (const scope of SCOPES) {
    const doc = await loadScope(scope).catch(() => null);
    if (!doc || !doc.data) continue;
    scopesLoaded.push(scope);
    ...
    // `doc` goes out of scope on the next iteration — nothing else retains it.
  }
  // Optional cap-band restriction BEFORE the liquidity cap — otherwise the
  // top-N-by-dollar-volume slice is inevitably large-cap and a "small-cap run"
  // would silently test large caps again.
  const banded = Array.isArray(bands) && bands.length
    ? rows.filter(r => bands.includes(r.capGroup)) : rows;
  // Deterministic cap: keep the most liquid names (ties broken by ticker).
  banded.sort((a, b) => (b.dollarVol || 0) - (a.dollarVol || 0) || (a.ticker < b.ticker ? -1 : 1));
  const capped = banded.length > SCAN_CAP;
  return { rows: banded.slice(0, SCAN_CAP), totalPool: banded.length, capped, scopes: scopesLoaded, bands: bands || null };
}
...
async function buildRltBoard({ persist = false } = {}) {
  // Load the calibration artifact FIRST so the shadow invariant can distinguish
  // an earned production flip (valid artifact) from an un-earned registry edit.
  const artifact = await STORE.loadCalibration().catch(() => null);
  assertShadow({ artifact });
  ...
  // The 12-criterion gate, evaluated on what is actually recorded. Independent
  // dates come from the resolved ledger; every qualitative criterion is absent
  // (fail-closed) — the recorded real-data walk-forwards found NO EDGE in either
  // cap band, so nothing here may claim otherwise.
  const promotion = promotionView({
    minIndependentDates: new Set(resolved.map(r => r.date || r.firstDecisionDate).filter(Boolean)).size,
  });
...
const WF_BANDS = new Set(['large', 'mid', 'small', 'micro', 'unknown']);
...
async function runRltWalkForward(req, res) {
  try {
    ...
    // ?band=small,mid restricts the universe to those liquidity/cap bands
    // BEFORE the liquidity cap (large-cap results do not transfer down-cap).
    const bands = String(req.query.band || '').split(',').map(s => s.trim()).filter(s => WF_BANDS.has(s));
    const [{ rows }, bench] = await Promise.all([loadUniverseRows({ bands: bands.length ? bands : null }), loadBenchmarks()]);
    const spyBars = bench.SPY || [];
    ...
    const bandTag = bands.length ? `-${bands.join('+')}` : '';
    const comparison = runRltComparison(events, { experimentId: `rlt-wf${bandTag}-${decisionDates[0]}-${decisionDates[decisionDates.length - 1]}` });
    return json(res, 200, { ok: true, universe: wfRows.length, bands: bands.length ? bands : 'all', decisionDates: decisionDates.length, comparison }, 300);
  } catch (e) {
    return json(res, 200, { ok: false, error: String((e && e.message) || e) }, 30);
  }
}

// loadUniverseRows/loadBenchmarks are shared with lib/peerprop-routes.js — one
// candle-pool loader, not two (the peer engine reads the same caches + ETFs).
module.exports = { runRlt, runRltLog, runRltResolve, runRltWalkForward, buildRltBoard, sectorLeadershipSummary, NOVICE_ACTION, loadUniverseRows, loadBenchmarks };
