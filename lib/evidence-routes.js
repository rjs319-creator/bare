...
// Why did a name's news fetch yield nothing? fetchCompanyNewsRich throws
// "news providers unavailable (fmp:… finnhub:…)" when EVERY provider failed
// (rate-limit/outage/timeout) and returns [] only when one answered "no articles" —
// so the tick diag can tell a throttled run apart from a genuinely quiet name.
function classifyNewsSkip(newsErr) {
  if (!newsErr) return 'noNews';
  return /providers unavailable/.test(newsErr) ? 'rateLimited' : 'newsErr';
}
...
async function buildTickerEvidence(cand, ctx) {
  ...
  const to = date;
  let news = [], newsErr = null;
  try { news = await fetchCompanyNewsRich(cand.ticker, from, to, 24); } catch (e) { news = []; newsErr = String(e && e.message || e); }
  if (!news.length) return { skip: classifyNewsSkip(newsErr), ticker: cand.ticker, err: newsErr };
  ...
  let events, cached = false, extractErr = null;
  ...
  if (prior && prior.fingerprint === fp && Array.isArray(prior.events)) {
    ...
    events = ex.events; extractErr = ex.error || null;
    try { await writeJSON(cachePath, { fingerprint: ex.fingerprint, events, at: new Date().toISOString() }, 0); } catch { /* cache is best-effort */ }
  }
  if (!events.length) return { skip: extractErr ? 'extractErr' : 'noEvents', ticker: cand.ticker, newsCount: news.length };
...
async function runEvidenceTick(req, res) {
  ...
  let universe = [], regime = 'neutral', universeErr = null;
  try { const u = await attentionUniverse(); universe = u.universe; regime = u.regime; } catch (e) { universeErr = String(e && e.message || e); }
  ...
  const raw = await mapLimit(rotated, CONCURRENCY, async (cand) => {
    if (Date.now() - started > DEADLINE_MS) return { skip: 'pastDeadline', ticker: cand.ticker };
    return buildTickerEvidence(cand, ctx);
  });
  const results = raw.filter(r => r && !r.skip);
  // Why did names yield nothing? (noNews / newsErr / noEvents / extractErr / pastDeadline)
  const diag = { universeErr, attempted: rotated.length, ok: results.length };
  for (const r of raw) { if (r && r.skip) diag[r.skip] = (diag[r.skip] || 0) + 1; }
  ...
  const snapshot = {
    ...
    cachedHits: results.filter(r => r.cached).length, diag,
  ...
  return res.status(err ? 502 : 200).json({
    ...
    thesisChanges: signals.length, diag, url, error: err, tookMs: snapshot.tookMs,
  });
}

// ── op=evidencediag ──────────────────────────────────────────────────────────
// Bounded, side-effect-free diagnostic (EXPENSIVE, rate-limited): runs the SAME
// universe → news → extraction pipeline over the first ?n (default 2) rotated names and
// reports exactly where the pipeline yields nothing — WITHOUT writing the snapshot. This is
// how we tell an empty snapshot's cause (no universe / no news / no events / extract error)
// apart without the cron bearer.
async function runEvidenceDiag(req, res) {
  const n = Math.max(1, Math.min(4, parseInt(req.query.n, 10) || 2));
  const { date } = nowET();
  const out = { date, hasKeys: { fmp: !!process.env.FMP_API_KEY, finnhub: !!process.env.FINNHUB_API_KEY, anthropic: !!process.env.ANTHROPIC_API_KEY } };
  let universe = [], regime = 'neutral';
  try { const u = await attentionUniverse(); universe = u.universe; regime = u.regime; } catch (e) { out.universeErr = String(e && e.message || e); }
  out.universeSize = universe.length; out.regime = regime;
  const from = new Date(Date.now() - NEWS_LOOKBACK_DAYS * 864e5).toISOString().slice(0, 10);
  const picks = rotateByDay(universe.filter(c => c.ticker), date).slice(0, n);
  out.samples = [];
  for (const cand of picks) {
    const s = { ticker: cand.ticker };
    try {
      const news = await fetchCompanyNewsRich(cand.ticker, from, date, 24);
      s.newsCount = news.length;
      s.sampleTitles = news.slice(0, 2).map(x => (x.title || '').slice(0, 90));
      if (news.length) {
        const ex = await extractEvents(cand.ticker, { company: cand.company, news, detectedAt: date, timeoutMs: 30000 });
        s.eventCount = ex.events.length; s.extractError = ex.error || null; s.called = ex.called;
        if (ex.events.length) { const cl = clusterEvents(ex.events); s.clusterCount = cl.length; s.topEvent = cl[0] && cl[0].primary ? { type: cl[0].primary.eventType, claim: cl[0].primary.claim } : null; }
      }
...
async function runEvidence(req, res) {
  ...
  return res.status(200).json({
    ...
    universeSize: snap.universeSize, diag: snap.diag || null,
...
module.exports = {
  runEvidenceTick, runEvidence, runEvidenceStock, runEvidenceDiag, classifyNewsSkip,
