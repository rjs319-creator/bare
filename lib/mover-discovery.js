...
//                              BEFORE 09:30 it fires on the premarket print itself (gap vs
//                              prior close with real premarket participation), since there
//                              is no open to hold yet
//
...
function evaluateLanes(input, cfg = DISCOVERY) {
  const {
    ...
    session = null, preGapPct = null, preRelVol = null,
  } = input;
  ...
  if (Number.isFinite(gapPct) && gapPct >= cfg.GAP_MIN_PCT) {
    ...
    if (holdingOpen !== false) {
      ...
      evidence.GAP_PREMARKET = { gapPct, open, holdingOpen, basis: 'regular' };
    }
  } else if (session === 'premarket' && Number.isFinite(preGapPct) && preGapPct >= cfg.GAP_MIN_PCT
      && Number.isFinite(preRelVol) && preRelVol >= cfg.PREMARKET_GAP_MIN_REL_VOL) {
    // PREMARKET: no open exists yet, so "holding the open" is undefined (null, not false).
    // The print must be real — a gap on a handful of shares is not a discovery event, hence
    // the participation floor, measured against the full-day average and labelled so.
    lanes.push(LANES.GAP_PREMARKET);
    evidence.GAP_PREMARKET = { gapPct: preGapPct, open: null, holdingOpen: null, basis: 'premarket', preRelVol };
  }
...
function buildCandidates({
  ...
  now = new Date(), minutesSinceOpen = null, cfg = DISCOVERY, session = null,
} = {}) {
  ...
  // PREMARKET VIEW of a quote: the regular fields are YESTERDAY's close/volume until 09:30,
  // so before the open the observation is the premarket print or nothing. A name with no
  // print is rejected (counted), never scored on stale fields — pacing yesterday's full
  // volume against a 2% expected fraction is how every dormant name used to read 45× rvol.
  const premarketView = q => {
    if (!premarket) return q;
    if (!(Number.isFinite(q.preMarketPrice) && q.preMarketPrice > 0)) return null;
    return {
      ...q, price: q.preMarketPrice, dayVolume: q.preMarketVolume ?? null, asOf: q.preMarketTime || q.asOf,
      open: null, dayHigh: null, dayLow: null,
    };
  };
  if (premarket) rejected.noPremarketPrint = 0;

  for (const q0 of quotes) {
    if (!q0 || !q0.ticker) continue;
    const q = premarketView(q0);
    if (!q) { rejected.noPremarketPrint++; continue; }
    const t = q.ticker;
    ...
    // PREMARKET basis: the venue's premarket print vs the prior close, and premarket volume
    // over the FULL-DAY average (unpaced, by rule). Both null outside premarket or when the
    // venue reports no print — never derived from the regular fields, which before the open
    // are yesterday's close and yesterday's volume.
    const preMarketPrice = premarket ? q.price : null;
    const preGapPct = preMarketPrice != null && Number.isFinite(prevClose) && prevClose > 0
      ? +(((preMarketPrice / prevClose) - 1) * 100).toFixed(2) : null;
    const preRelVol = preMarketPrice != null && Number.isFinite(q.preMarketVolume) && Number.isFinite(avgDailyVolume) && avgDailyVolume > 0
      ? +(q.preMarketVolume / avgDailyVolume).toFixed(4) : null;
    ...
    // Premarket relVol is the UNPACED share of a normal day's volume (thin tape is never
    // normalized against the regular-hours curve); regular hours use the time-of-day curve.
    const relativeVolume = premarket ? preRelVol : sessionRelativeVolume(sessionVolume, avgDailyVolume, minutesSinceOpen);
    ...
    candidates.push({
      ...
      dayChangePct,
      gapPct,
      // Premarket-basis fields (null outside premarket / without a print).
      preMarketPrice,
      preGapPct,
      preRelVol,
      sessionBasis: preMarketPrice != null ? 'premarket' : 'regular',
      fiveMinChange,
      fifteenMinChange,
      thirtyMinChange,
...
async function runMoverDiscovery({
  ...
  now = new Date(), minutesSinceOpen = null, cfg = DISCOVERY, t0 = Date.now(), session = null,
} = {}) {
  ...
  const built = buildCandidates({ quotes: rows, baselines, state, floatByTicker, catalystByTicker, now, minutesSinceOpen, cfg, session });
