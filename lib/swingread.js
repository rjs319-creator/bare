...
const PLAN_TIME_STOP_SESSIONS = 15;  // a setup that hasn't worked in ~3 weeks is void
...
function buildPlan(action, candles, state, risk) {
  ...
  if (action === 'BUY') {
    ...
    return {
      ...
      timeStop: `${PLAN_TIME_STOP_SESSIONS} sessions`,
      expiry: `Setup void if the trigger ($${trigger}) is not reclaimed within ~${PLAN_TIME_STOP_SESSIONS} sessions`,
...
// A WAIT with real evidence must still carry a USEFUL watch plan — the engine
// has already computed pivots/ATR/structure; discarding them left "high
// evidence, empty plan" cards. The watch plan reuses the BUY/SELL geometry but
// is explicitly watch-only (the trigger has not been hit).
function buildWatchPlan(signedScore, candles, state, risk) {
  const side = signedScore >= 0 ? 'BUY' : 'SELL';
  const p = buildPlan(side, candles, state, risk);
  if (!p) return null;
  return { ...p, watchOnly: true, note: `${p.note} Watch-only: the trigger has not been hit.` };
}
...
function swingRead(dailyCandles, spyCandles, opts = {}) {
  ...
  // A FORMING daily candle is never a confirmed close: the confirmed read is
  // computed from completed bars only, and the payload is flagged provisional
  // so no consumer can label an intraday poke through a pivot as "breakout".
  if (opts.lastBarIncomplete === true && dailyCandles.length > 1) {
    const completed = dailyCandles.slice(0, -1);
    const confirmed = swingRead(completed, spyCandles, { ...opts, lastBarIncomplete: false, asOf: null });
    return { ...confirmed,
      provisionalBar: true,
      provisionalNote: 'Today\'s daily candle is still forming — this read uses the last CONFIRMED close. Intraday movement cannot confirm a swing trigger until the close.',
    };
  }
  ...
  let plan = buildPlan(action, dailyCandles, state, risk);
  ...
  // A constructive WAIT still gets a watch plan (levels to confirm/invalidate)
  // instead of an empty shrug — except when the veto is liquidity itself.
  if (action === 'WAIT' && !plan && setup !== 'illiquid-avoid' && Math.abs(signedScore) >= 0.1) {
    plan = buildWatchPlan(signedScore, dailyCandles, state, risk);
  }
