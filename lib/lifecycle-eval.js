...
function intradayEv(pick, f, { now, quote = null } = {}) {
  ...
  // Current-session invalidation evidence. THE ACTIVE FROZEN LIVE PLAN IS AUTHORITATIVE:
  // when a live plan is in force its frozen stop/target/expiry drive the breach, target-hit
  // and expiry checks. The stale daily-detection stop is only a LABELED fallback for names
  // that never confirmed a live plan (its breach reads as thesis invalidation, never as a
  // live-plan stop print). All checks use the last COMPLETED close (f.last).
  const lp = f.livePlan || null;
  const stopLevel = lp && Number.isFinite(lp.stop) ? lp.stop : (pick.stop ?? null);
  const stopBasis = lp && Number.isFinite(lp.stop) ? 'live-plan' : (pick.stop != null ? 'daily-plan' : null);
  const stopBreached = (stopLevel != null && f.last != null) ? f.last <= stopLevel : undefined;
  const targetReached = (lp && Number.isFinite(lp.target) && f.last != null) ? f.last >= lp.target : undefined;
  // Current-session day collapse vs the prior close — the configured THESIS.DAY_COLLAPSE_PCT
  // rule, now computed in PRODUCTION from completed-bar evidence (undefined when prevClose
  // is unavailable — missing evidence never asserts a collapse).
  const collapsePct = require('./daytrade-config').THESIS.DAY_COLLAPSE_PCT;
  const dayCollapsed = f.dayChangePct != null ? f.dayChangePct <= collapsePct : undefined;
  ...
  return {
    ...
    metrics: {
      ...
      gapPct: f.gapPct ?? null,
      distFromOrHigh: f.distFromOrHigh ?? null,
      timeOfDayDollarRelVol: f.timeOfDayDollarRelVol ?? null,
      closesAboveVwapStreak: f.closesAboveVwapStreak ?? null,
      schemaVersion: f.schemaVersion ?? null,
    },
    // Evidence-basis provenance — which class of evidence produced this evaluation
    // (completed bars vs forming bar vs quote-only). Captured on transitions/snapshots.
    evidenceBasis: {
      basis: (f.dataQuality && f.dataQuality.evidenceBasis) || 'completed-bars',
      completedBars: (f.dataQuality && f.dataQuality.completedBars) ?? f.bars ?? null,
      formingBarPresent: (f.dataQuality && f.dataQuality.formingBarPresent) ?? null,
      barIntervalMin: (f.dataQuality && f.dataQuality.barIntervalMin) ?? null,
      completedAsOf: f.completedAsOf ?? null,
      completedBarEndAt: f.completedBarEndAt ?? null,
    },
    provisionalBreak: f.provisionalBreak || null,
