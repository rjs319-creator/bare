...
// Pure: the REAL failures of a run, in the form the health banner should show.
//
// THE BUG THIS FIXES: public/js/app.js derived its warning from `failed` + `warmFails`
// alone. The chain refactor moved ordered work into warmchain roots, so a chain that
// 500s lands in `chainDispatchFails` and one that never reports back lands in
// `lateChainFails` — neither of which the client looked at. On 2026-08-18 the server
// reported failCount:5 (atlasx/ticks3/swing OOM-killed, swingsearch 504'd) while
// failed:[] and warmFails:[], so the client computed zero and hid the banner. Seven
// consecutive failed runs showed the user nothing.
//
// Deriving it here means the client renders a list instead of reconstructing a shape it
// does not fully know — and it becomes testable, which the app.js source-grep tests
// could never be.
// PROPORTIONALITY: a shadow ledger that declined to log is not a market-data outage.
//
// Every problem rendered identically — one loud red banner, plus a "N runs in a row"
// streak — whether the app's own market data was days behind or a weight-0 research
// stream skipped a night. On 2026-08-21 the user had been looking at a red alert for
// three days while `data` read stale:false / sessionsBehind:0 the whole time. An alarm
// that cannot distinguish those two teaches you to ignore it, which is how the genuinely
// silent week of failures went unnoticed in the first place.
//
// CONSERVATIVE BY CONSTRUCTION: only chains VERIFIED from their own source as
// shadow/research are listed. Anything not listed — including any chain added later —
// counts as user-facing and still alarms. Downgrading something the user actually reads
// is a far worse failure than over-alarming, so the default must stay loud.
const BACKGROUND_CHAINS = new Set([
  // lib/challenger-routes.js: "SHADOW ONLY: reads/writes its own shadow/* Blob prefix".
  'challenger',
  // lib/warm-chains.js: "ALPHA-ARCHIVE streams (archive-first collection)" — research
  // capture for the backtest roadmap, not anything a screen or pick reads today.
  'alphacal',
  // lib/insider-cluster-routes.js: "shadow route handlers (weight 0; prospective ledger only)".
  'insidercluster',
  // lib/forecast-shadow-routes.js: weight-0 prospective ledger for the forecast ranker
  // (forecastshadow/v1/*); nothing a screen or pick reads. Docs: docs/forecast-shadow.md.
  'forecastshadow',
]);

// Split the problem list into what affects the data the user reads vs what does not.
// `background` is still REPORTED — never hidden — just not dressed as an emergency.
function classifyProblems(problems = [], { dataStale = false } = {}) {
  const data = [], background = [];
  for (const p of problems) {
    const chain = typeof p === 'string' && p.startsWith('chain:') ? p.slice(6).split('/')[0] : null;
    if (chain && BACKGROUND_CHAINS.has(chain)) background.push(p);
    else data.push(p);
  }
  // A stale feed outranks any classification: if the market data itself is behind, the
  // banner is about the data, whatever the failing steps happen to be called.
  if (dataStale && !data.length && background.length) return { data: background.slice(), background: [] };
  return { data, background };
}

function runProblems(run) {
  if (!run) return [];
  // Budget-deferred stages stay suppressed: best-effort work that self-heals is not an
  // error, and alarming on every capacity-bound run is the fatigue that hid the real bug.
  const deferred = new Set(run.budgetSkipped || []);
  const out = [];
  for (const stage of run.failed || []) if (!deferred.has(stage)) out.push(stage);
  for (const w of run.warmFails || []) if (w && w.path) out.push(w.path);
  for (const c of run.chainDispatchFails || []) out.push(`chain:${c}`);
  for (const c of run.lateChainFails || []) out.push(`chain:${c}`);
  return [...new Set(out)];
}
...
function buildHealthResponse(runs, { spyDate = null, spyDates = null, ageDays = null, now = Date.now(), dataQuality = null, auth = require('./auth').validateAuthEnv(), persistedChains = null } = {}) {
  ...
  return {
    ...
    chronicSkips,
    // The banner's render list. `ok` above is the ENVELOPE (the request succeeded);
    // `healthy` + `problems` are the verdict. Never read `ok` to decide "is the app fine".
    problems: runProblems(last),
    // Split so the client can size the alarm to the impact instead of shouting equally
    // about a stale price feed and a shadow ledger that declined to log a night.
    problemsBySeverity: classifyProblems(runProblems(last), { dataStale: stale }),
...
module.exports = { summarizeRun, detectChronicSkips, runProblems, classifyProblems, BACKGROUND_CHAINS, buildHealthResponse, overlayPersistedChains, writeHealthRun, readHealth, runHealth, HEALTH_PATH };
