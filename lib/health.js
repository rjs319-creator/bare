...
const NON_STAGE = new Set(['ok', 'host', 'at', 'warmed', 'warmedExtra', 'stageStatus', 'elapsedMs', 'aiTicksKicked', 'calibKicked']);

// Compact a warm-cron result object into a health record: per-stage ok/error.
// A stage the budget guard deferred (skipped:'budget') is graded not-ok so a cron
// that's running long and dropping late jobs is VISIBLE — but a legitimate
// weekend/degraded skip (which carries ok:true) stays healthy.
function summarizeRun(result) {
  ...
  for (const [k, v] of Object.entries(result || {})) {
    ...
    if (v && typeof v === 'object') {
      ...
      else if (v.skipped === 'budget') stages[k] = { ok: false, skipped: 'budget' };
  ...
  const budgetSkipped = Object.entries(stages).filter(([, s]) => s.skipped === 'budget').map(([k]) => k);
  return {
