...
function buildHealthResponse(runs, { spyDate = null, ageDays = null, now = Date.now(), dataQuality = null, auth = require('./auth').validateAuthEnv(), persistedChains = null } = {}) {
  const rs = runs || [];
  // How many completed sessions the benchmark's newest EOD bar is behind the exchange
  // calendar. This — not elapsed wall time — is what "stale market data" means.
  ...
  const sessionsBehind = (spyDate && calendarSession)
    ? countSessionsBetween(spyDate, calendarSession, new Date(now)) : null;
  ...
  return {
    ...
    data: { spyDate, ageDays: ageDays != null ? +ageDays.toFixed(1) : null, stale, sessionsBehind, calendarSession },
    dataQuality,
...
async function runHealth(req, res) {
  ...
  let spyDate = null, ageDays = null, dataQuality = null;
  try {
  ...
  return res.json(buildHealthResponse(runs, { spyDate, ageDays, dataQuality, persistedChains }));
}
