...
function buildHealthResponse(runs, { spyDate = null, ageDays = null, now = Date.now(), auth = require('./auth').validateAuthEnv() } = {}) {
  const rs = runs || [];
...
async function runHealth(req, res) {
  ...
  return res.json(buildHealthResponse(runs, { spyDate, ageDays }));
}
