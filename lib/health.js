...
function buildHealthResponse(runs, { spyDate = null, ageDays = null, now = Date.now(), dataQuality = null, auth = require('./auth').validateAuthEnv() } = {}) {
  const rs = runs || [];
  ...
  return {
    ...
    healthy: !!last && last.ok && !(ageDays != null && ageDays > STALE_DAYS) && chronicSkips.length === 0 && auth.ok,
    warning: chronicSkips.length ? `${chronicSkips.map(c => c.name).join(', ')} skipped on ${CHRONIC_MIN}+ of the last ${chronicWindow} runs — not self-healing` : null,
    chronicSkips,
...
async function runHealth(req, res) {
  ...
  return res.json(buildHealthResponse(runs, { spyDate, ageDays, dataQuality }));
}

module.exports = { summarizeRun, detectChronicSkips, buildHealthResponse, writeHealthRun, readHealth, runHealth, HEALTH_PATH };
