...
// Per-action templated guidance (novice + trader read). LLM insight-prose generation does
// not fit the serverless budget for either the search-laden gather or the thinking-heavy
// refine, so the presentational insight is DERIVED from the computed states + the LLM's own
// idea/whyMoves/crowding/contrarian. Reliable, fast, honest — never an LLM latency risk.
const TRADER_READ = {
  'INVESTIGATE NOW': 'Fresh and credibly sourced, and price has not fully reacted — worth a close look now.',
  'EMERGING — WATCH': 'Early attention — track it and wait for confirmation rather than chasing.',
  'CONFIRMED MOMENTUM': 'Attention, price and volume agree — the move is real; manage risk on entries.',
  'WAIT FOR ENTRY': 'Interesting but stretched — wait for a pullback or a base rather than chasing.',
  'TOO EXTENDED': 'Already ran hard above its mean — chasing here is poor risk/reward.',
  'CROWDED — DO NOT CHASE': 'Consensus and crowded — high fade risk; do not chase the obvious trade.',
  'CONTRARIAN WATCH': 'There is an explicit contrarian case here — size and time it carefully.',
  'CONTEXT ONLY': 'Market-wide context, not a single-name trade.',
  'UNVERIFIED': 'No credible independent source yet — treat as a rumour until confirmed.',
  'STALE': 'This read is old — refresh before acting.',
};
const NOVICE_READ = {
  'INVESTIGATE NOW': 'This looks new and real — worth researching, but do not rush in.',
  'EMERGING — WATCH': 'It is getting early attention — watch it, do not buy on hype.',
  'CONFIRMED MOMENTUM': 'The move is real and underway — but buying late is riskier.',
  'WAIT FOR ENTRY': 'It has already jumped — buying now is riskier; wait for it to settle.',
  'TOO EXTENDED': 'It has already jumped a lot — buying now carries extra risk.',
  'CROWDED — DO NOT CHASE': 'Lots of people are already in — chasing this is risky.',
  'CONTRARIAN WATCH': 'Some think the crowd is wrong here — this is advanced and risky.',
  'CONTEXT ONLY': 'This is background about the whole market, not one stock.',
  'UNVERIFIED': 'This is just online chatter right now — it is not confirmed.',
  'STALE': 'This information is old — check for updates first.',
};

/** Round an ATR-extension figure for copy. */
const atrPhrase = e => (e && e.atrExt != null ? `${Math.abs(e.atrExt).toFixed(1)} ATR ${e.atrExt >= 0 ? 'above' : 'below'} its 20-day mean` : null);

/**
 * Complete, honest presentational insight for an item, from its states + the LLM's own
 * fields. Prefers any LLM-provided value (whatChanged/whyItMatters/…) and templates the
 * rest. Pure. `action`/`lifecycle` are the already-derived states.
 */
function deriveInsight(item, { enrichment = null, action, lifecycle } = {}) {
  const ext = atrPhrase(enrichment);
  const whatChanged = item.whatChanged || (item.idea ? clip(item.idea, LIMITS.whatChanged) : null);
  const whyItMatters = item.whyItMatters || item.whyMoves || null;
  let traderRead = item.traderRead || TRADER_READ[action] || null;
  if (!item.traderRead && ext && (action === 'TOO EXTENDED' || action === 'WAIT FOR ENTRY')) {
    traderRead = `Already ${ext} — ${action === 'TOO EXTENDED' ? 'chasing here is poor risk/reward.' : 'wait for a pullback or base.'}`;
  }
  const noviceTranslation = item.noviceTranslation || NOVICE_READ[action] || null;
  let primaryRisk = item.primaryRisk || item.caution || null;
  if (!primaryRisk) {
    if (action === 'UNVERIFIED') primaryRisk = 'The story may be false or exaggerated — no credible source yet.';
    else if (lifecycle === 'Crowded' || action === 'TOO EXTENDED') primaryRisk = 'The move may already be over — chasing risks buying the top.';
    else primaryRisk = 'The catalyst may already be priced in, or reverse.';
  }
  const invalidation = item.invalidation || 'Thesis weakens if the reported catalyst is denied or volume fades below normal.';
  const horizon = oneOf(item.horizon, HORIZONS, item.category === 'macro' ? 'context' : (item.velocity === 'exploding' ? 'intraday' : 'days'));
  return {
    whatChanged: whatChanged ? clip(whatChanged, LIMITS.whatChanged) : null,
    whyItMatters: whyItMatters ? clip(whyItMatters, LIMITS.whyItMatters) : null,
    traderRead: traderRead ? clip(traderRead, LIMITS.traderRead) : null,
    noviceTranslation: noviceTranslation ? clip(noviceTranslation, LIMITS.noviceTranslation) : null,
    primaryRisk: clip(primaryRisk, LIMITS.primaryRisk),
    invalidation: clip(invalidation, LIMITS.invalidation),
    horizon,
  };
}
...
function deriveStates(item, { ageDays = 0, enrichment = null, stale = false } = {}) {
  ...
  const enr = enrichment || item.enrichment || null;
  const insight = deriveInsight(item, { enrichment: enr, action, lifecycle });
  return {
    ...item,
    ...insight,
...
module.exports = {
  ...
  priceConfirms, extensionState, deriveActionState, deriveInsight, deriveStates,
