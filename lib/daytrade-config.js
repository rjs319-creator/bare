...
// ── LANE PURPOSES ────────────────────────────────────────────────────────────
// Shipped on the board payload so every legacy section states what it is FOR. These lanes
// answer different questions and were being read as if they answered the same one ("what
// should I buy today?"). Naming the question each one actually answers is the cheapest
// correctness fix available, and it keeps the historical ledgers meaningful — none of these
// lanes has been redefined, only labelled.
//
// The same-session explosion question these lanes do NOT answer is handled by the separate
// staged system in lib/lowfloat-* / lib/intraday-* (Low-Float Ignition Radar and Intraday
// Breakout Radar), which has its own versions, its own ledger and its own promotion gate.
const LANE_PURPOSES = Object.freeze({
  momentum_liquid: {
    label: 'Momentum & Liquid',
    purpose: 'Liquid momentum watchlist.',
    detail: 'Daily-bar momentum and relative volume over the liquid cap bands. An identification list of names already moving — not a same-session explosion model.',
  },
  explosive_small: {
    label: 'Explosive Small-Cap',
    purpose: 'Legacy small/micro-cap proxy lane.',
    detail: 'Uses SMALL+MICRO market-capitalization membership as a PROXY for low float. Market cap is not float: see the Low-Float Ignition Radar for the lane built on actual float data.',
  },
  momentum_run: {
    label: 'Momentum Run',
    purpose: 'Reactive multi-day continuation watchlist.',
    detail: 'Sustained multi-day strength with repeated unusual-volume days. Reactive by construction — it catches a move already underway.',
  },
  momentum_expanded: {
    label: 'Expanded universe',
    purpose: 'The same Momentum & Liquid rules over the free full-market universe.',
    detail: 'A broader net using the liquid-momentum gate, not an explosive or low-float model. Not part of the tracked ledger.',
  },
  pcarry: {
    label: 'pcarry',
    purpose: 'Multi-session carry/fade context — NOT same-session explosion probability.',
    detail: 'A calibrated 3-session fade-avoidance model. It shapes ranking and flags fade risk; it can neither suppress an intraday setup nor be read as the odds of a same-session move. The same-session discovery stack does not consult it at all.',
  },
});

module.exports = { FRESHNESS, THESIS, RECLAIM, GATE, EXECUTION, STAGE2, LIVE_PLAN, REGIME, DISCOVERY, EARLY, ANTIFLAP, ALERTS, FIVE_MIN_MS, LANE_PURPOSES };
