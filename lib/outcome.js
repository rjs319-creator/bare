...
function resolveTrade(candles, fromDate, entry, stop, target, maxHold = MAX_HOLD, short = false) {
  ...
  // GAP-THROUGH (graduation-league EA-10): a session that OPENS beyond a barrier
  // cannot fill at the barrier price — the first tradable price is the open. Both
  // barriers therefore realize the OPEN when gapped through: a stop that gaps
  // against you fills worse than the stop (the old barrier fill clipped every
  // overnight-gap loss), and a target that gaps in your favor fills at the open the
  // way a real resting limit order would.
  if (short) {
    ...
    for (let h = 1; h <= maxHold && idx + h < candles.length; h++) {
      ...
      if (c.high >= stp) { const x = c.open > stp ? c.open : stp; return { outcome: 'LOSS', r: (e - x) / e, hold: h, exitDate: c.date, gapThrough: c.open > stp || undefined }; } // stop first (conservative)
      if (c.low <= tgt) { const x = c.open < tgt ? c.open : tgt; return { outcome: 'WIN', r: (e - x) / e, hold: h, exitDate: c.date, gapThrough: c.open < tgt || undefined }; }
    }
  ...
  for (let h = 1; h <= maxHold && idx + h < candles.length; h++) {
    const c = candles[idx + h];
    if (c.low <= stp) { const x = c.open < stp ? c.open : stp; return { outcome: 'LOSS', r: (x - e) / e, hold: h, exitDate: c.date, gapThrough: c.open < stp || undefined }; }  // stop first (conservative)
    if (c.high >= tgt) { const x = c.open > tgt ? c.open : tgt; return { outcome: 'WIN', r: (x - e) / e, hold: h, exitDate: c.date, gapThrough: c.open > tgt || undefined }; }
  }
  if (candles.length - 1 - idx >= maxHold) {
    const j = idx + maxHold;
