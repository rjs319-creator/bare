...
const BROAD_SCOPES = ['large', 'small', 'micro'];
...
async function broadShadowCards(excludeTickers, regime) {
  ...
  for (const scope of BROAD_SCOPES) {
    ...
    for (const t of Object.keys(doc.data)) {
      if (excludeTickers.has(t)) continue;
      const entry = cacheGet(doc, t);
      if (!entry || !Array.isArray(entry.candles) || entry.candles.length < BROAD_MIN_BARS) continue;
  ...
  return {
    ...
    scopesMissing,
    note: 'SHADOW independent Stage-1 over the large/small/micro candle caches (price/volume only, no catalyst enrichment — the funnel lane and this lane are separate evidence versions). Weight-0: ledgered under BROAD_* tiers for a prospective record; cannot originate or boost anything until that record clears promotion.',
  };
}
...
async function runIgnition(req, res) {
  ...
  try {
    payload.broadShadow = await broadShadowCards(new Set(shortlist.map(s => s.ticker)), today.data.regime || {});
...
async function runIgnitionLog(req, res) {
  ...
  let broadPicks = [];
  try {
    const broad = await broadShadowCards(new Set(payload.cards.map(c => c.ticker)), today.data.regime || {});
  ...
  return res.json({ ok: true, date, logged, funnelPicks: picks.length, broadShadowPicks: broadPicks.length, candidates: payload.cards.length, closed: isMarketClosed });
}
