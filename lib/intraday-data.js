...
function sessionStatus(now = new Date()) {
  ...
  // market-session exposes `marketSession` (lowercase: premarket/regular/afterhours/closed).
  // This used to read a non-existent `info.session`, so with the calendar loaded `session`
  // was undefined and `isRegularHours` was false all day; only the minutes-since-open
  // fallthrough in offSessionSkip kept the regular-hours pipeline running.
  const calendarSession = info && info.marketSession ? String(info.marketSession).toUpperCase() : null;
  const isRegular = calendarSession ? calendarSession === 'REGULAR' : (minutesSinceOpen >= 0 && minutesUntilClose > 0);
  return {
    now: new Date(now).toISOString(),
    etDate,
    session: calendarSession || (isRegular ? 'REGULAR' : 'CLOSED'),
