...
// RSI mean-reversion, MACD, price-action structure) scored together. HONESTY: these 5 are
// mostly ONE trend family, so the rank is NOT a raw "how many agree" vote — it is the
// correlation-DISCOUNTED independent score (see STRATEGY_FAMILY / CORR_DISCOUNT below):
// the first strategy in a family counts full, each additional agreeing strategy in the
// SAME family counts far less. "4/5 agree" is reported as ~one independent trend family,
// not four confirmations. Pure: candles in, signals out (no network, no state) so it runs
// in the live op AND the backtest harness.
//
...
// INDEPENDENT EVIDENCE FAMILIES (family-v1). Four of the five strategies are trend-
// followers reading the same price series, so raw agreement double-counts ONE factor —
// "4 of 5 agree" can be 4 correlated trend votes, not 4 independent confirmations.
// Grouping into families lets confluence reward INDEPENDENT evidence (trend AND
// mean-reversion) over piling more votes onto a single factor.
const CONFLUENCE_FAMILY_VERSION = 'family-v1';
const STRATEGY_FAMILY = { ema: 'trend', supertrend: 'trend', macd: 'trend', priceAction: 'trend', rsi: 'meanReversion' };
// The first bullish strategy in a family counts full weight; each ADDITIONAL agreeing
// strategy in the SAME family is discounted (correlated, not independent evidence).
const CORR_DISCOUNT = 0.3;

// Correlation-discounted score over a set of (already weighted) votes grouped by family.
// weightsBull: { family: [w1, w2, …] }. Within a family, sort desc and discount extras.
function independentScore(byFamilyWeights) {
  let s = 0;
  for (const fam of Object.keys(byFamilyWeights)) {
    byFamilyWeights[fam].slice().sort((a, b) => b - a)
      .forEach((wv, i) => { s += wv * (i === 0 ? 1 : CORR_DISCOUNT); });
  }
  return +s.toFixed(2);
}
...
function confluence(candles, weights, condition) {
  ...
  // Independent-evidence view: group the bullish votes by family and score with a
  // within-family correlation discount. `indepScore` is the honest ranking key;
  // `familyBullCount` is how many INDEPENDENT families agree (not raw strategy count);
  // `singleFamily` flags the misleading case — multiple strategies agree but they're
  // all the same factor (e.g. all-trend), so it's really one confirmation dressed up.
  const byFamily = {};
  for (const st of bull) (byFamily[STRATEGY_FAMILY[st]] = byFamily[STRATEGY_FAMILY[st]] || []).push(st);
  const familyBull = Object.keys(byFamily);
  const bullWeightsByFamily = {};
  for (const fam of familyBull) bullWeightsByFamily[fam] = byFamily[fam].map(wOf);
  const allWeightsByFamily = {};
  for (const st of STRATEGIES) (allWeightsByFamily[STRATEGY_FAMILY[st]] = allWeightsByFamily[STRATEGY_FAMILY[st]] || []).push(wOf(st));
  const indepScore = independentScore(bullWeightsByFamily);
  const maxIndepScore = independentScore(allWeightsByFamily);
  const freshTriggers = [s.emaFresh && 'ema', s.stFlip && 'supertrend', s.macdFresh && 'macd'].filter(Boolean);
  return {
    score, maxScore, bull, bear, matched, bullishCount: bull.length, perStrategy: s, freshTriggers,
    condition: condition || null, atr: ind.atr[k],
    // family-v1 independent-evidence fields
    byFamily, familyBull, familyBullCount: familyBull.length, indepScore, maxIndepScore,
    singleFamily: familyBull.length < 2 && bull.length >= 2,
  };
}
...
module.exports = {
  ...
  STRATEGY_FAMILY, CORR_DISCOUNT, CONFLUENCE_FAMILY_VERSION, independentScore,
};
