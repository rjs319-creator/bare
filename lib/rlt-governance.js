...
const { VERSIONS, HURDLES, validateArtifact } = require('./rlt-config');
...
/**
 * Invariant: RLT may be registered production ONLY with EARNED clearance — a
 * valid, current, version-matched promotion artifact. Shadow always passes.
 *
 * The old form threw on production unconditionally, which made the documented
 * promotion path ("explicit reviewable registry maturity flip") a guaranteed
 * outage: the invariant and the promotion policy were the same statement
 * negated. What the invariant actually protects against is an UN-EARNED flip —
 * someone editing the registry without the evidence artifact — so that is what
 * it now asserts. Note this does not promote anything: the registry stays
 * shadow, and rltGate additionally requires enforce mode + this same artifact
 * before any live influence, capped at CANARY_CAP.
 */
function assertShadow({ artifact = null } = {}) {
  const status = statusOf(STRATEGY_ID);
  if (status === 'production') {
    const check = validateArtifact(artifact, { kind: 'rlt-model' });
    if (!check.valid) {
      throw new Error(`RLT invariant violated: registered production WITHOUT a valid promotion artifact (${check.reasons.join('; ')}) — flip the registry back to shadow or supply the earned artifact`);
    }
  }
  return status;
}
