...
async function runOptionsScanV2(req, res) {
  ...
  let history = [];
  try { history = (await readAllByPrefix(OBS_PREFIX, OBS_RE)).slice(-BASELINE_DOCS_MAX - 1); } catch { /* none yet */ }
  ...
  for (const item of items) {
    ...
    try { attachEvidenceLayers(item, { nowMs, capabilities }); } catch { /* additive only */ }
  }
  ...
  const radar = buildRadar({ session: sc.session, scanId, items, rawRows, events: folded.events, alerts: folded.alerts, transitions: allTransitions, scanHealth, capabilities, baselines, universe: uni });
...
function attachEvidenceLayers(item, { nowMs, capabilities }) {
  ...
  item.falsePositiveHypotheses = hyp.buildHypotheses({
    ...
    corpAction: null,     // no corporate-action calendar is wired — the ledger says so
  ...
  item.strikeConcentrationFullChain = oex.strikeConcentration({ chainRows: rows, chainComplete: false });

  const atmIV = selected && selected.iv != null ? selected.iv : null;
  item.volatilityContext = oex.volatilityContext({
    currentIV: atmIV, ivHistory: null,             // no per-name IV history series is persisted yet
...
function buildRadar({ session, scanId, items, rawRows, events, alerts, transitions, scanHealth, capabilities, baselines, universe }) {
