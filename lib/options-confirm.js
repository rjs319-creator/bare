...
// Evidence quality from the honest signals. A provisional directional AGGREGATE already
// means there is a defensible lean (aggregateDirection only emits it when directional
// notional clears its floor), so a provisional lean is at least 'mixed' evidence — enough
// to surface as a confirmation, labeled honestly. It is 'clear' only when OI is building or
// the lean is strongly directional (low unknown share). 'thin' when the aggregate is NOT
// provisional or is almost entirely unknown. NOTE: suspected multi-leg contracts are already
// forced to DIRECTION_UNKNOWN by the classifier, so they inflate unknownShare here — we do
// NOT re-penalize the raw multi-leg count (an absolute count made every liquid name thin).
function evidenceQuality(flow) {
  ...
  if (!provisional) return 'thin';                              // mixed/unknown aggregate = thin evidence
  if (unknownShare > 0.85) return 'thin';                       // a nominal lean but almost all unknown
  if ((oiConfirmed && unknownShare < 0.5) || unknownShare < 0.35) return 'clear';
  return 'mixed';                                               // a provisional lean, honestly mid-confidence
}
