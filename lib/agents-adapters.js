...
const { FABLE_MODEL, callFableText } = require('./fable-call');
 ...
 */
function anthropicAdapter({ model = FABLE_MODEL, variant = 'champion', callImpl = null, apiKey = process.env.ANTHROPIC_API_KEY } = {}) {
...
function defaultAnthropicCall(model, apiKey) {
  // claude-fable-5-1 plain-text call with refusal handling + server-side fallback (lib/fable-call).
  return async prompt => callFableText({ prompt, maxTokens: 1200, apiKey, model });
}
