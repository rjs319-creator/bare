...
const { foldEpisodes, ageDays } = require('./options-episodes');
const { gradeEpisode, summarizeEpisodes } = require('./options-grade');
const { governanceReport } = require('./options-governance');
const { evaluateSetup } = require('./stock-setup');
const { buildDecision, bucketViews } = require('./options-confirm');

// Candles for a specific set of tickers from the warmed candle cache (no network) —
// the confirmation engine's independent stock setup is computed from these.
...
async function runOptionsFlow(req, res) {
  ...
  // DECLARED INFORMATION CUTOFF (lib/data-gates). `date` above is the CLOCK — it is
  // `new Date()` at response time and says nothing about the chains underneath, so the
  // gate deliberately refuses to read it. The session a delayed-chain snapshot can
  // actually attest is the last REGULAR session completed at snapshot time (an
  // intraday snapshot cannot yet attest the session it is inside).
  const observedSession = require('./market-session').lastCompletedRegularSession(new Date());
  const out = {
    ok: true, date, generatedAt: new Date().toISOString(),
    freshness: {
      decisionSession: observedSession,
      basis: 'last completed regular session at snapshot time (delayed Yahoo chains cannot attest an in-progress session)',
    ...
    note: 'QUARANTINED RAW DIAGNOSTICS (v1). The investor decision surface is the Options Activity Radar (op=optionsradar); this endpoint serves the transparent underlying measurements from DELAYED, free Yahoo option chains (volume vs open interest + estimated premium) — not live tick tape and no trade-level OPRA data. CALLS ARE NOT BULLISH AND PUTS ARE NOT BEARISH: `callPremiumSharePct`/`premiumSkew`/`grade` describe how the unusual premium SPLIT between calls and puts and are composition statistics, not direction. The only directional field is `directionState`, which is PROVISIONAL (and commonly UNKNOWN/MIXED) because we cannot tell opening from closing, bought from sold, or a bet from a hedge, roll, spread leg, or covered-call write. High-turnover / large-notional / premium-activity are heuristic size classes, not confirmed sweeps or blocks. This overlay cannot by itself create or boost a Today\'s Pick.',
  };
  ...
  // CONFIRMATION ENGINE: judge each ticker's options evidence against an INDEPENDENT,
  // chart-math stock setup → a deterministic REVIEW/WAIT/AVOID decision. Powers the four
  // coordinated views (Swing Confirmations / Contradictions & Risk / Raw Activity).
  try {
    const candleMap = await candlesForTickers((out.byTicker || []).map(r => r.ticker));
    out.decisions = (out.byTicker || []).map(r => buildDecision(evaluateSetup(candleMap.get(String(r.ticker).toUpperCase()) || []), r));
    const v = bucketViews(out.decisions);
    out.viewCounts = { confirmations: v.confirmations.length, contradictions: v.contradictions.length, raw: v.raw.length };
  } catch { out.decisions = out.decisions || []; /* decisions best-effort — raw flow still stands */ }
...
// Stamp the Fable bias + evidence-clarity onto today's ungraded ledger entries so
...
async function stampLedgerBias(analyses, date) {
  ...
  for (const e of (led.entries || [])) {
    ...
    if (!a) continue;
    e.aiBias = a.bias;
    e.aiClarity = a.evidenceClarity;   // grounded evidence quality (replaces the old 0-100 conviction)
    touched++;
  }
...
// op=optionsepisodes : grade the IMMUTABLE decision episodes the honest way — next-open
// entry (never the decision close), multi-horizon, SPY-relative, cost-aware — and report
// whether the options-flow shadow layer has produced any economically useful, independent
// track record yet. Heavy (candle fetches) → cached to Blob; only episodes old enough to
// resolve the horizons are graded. This is measurement, NOT a live signal.
const GRADES_KEY = 'optionsflow/episode-grades.json';
const GRADE_HORIZONS = [1, 3, 5, 10, 21];
const GRADE_CAP = 80;

async function runOptionsEpisodes(req, res) {
  const force = req.query.refresh === '1' || req.query.force === '1';
  if (!force) {
    const cached = await readJSON(GRADES_KEY, null).catch(() => null);
    if (cached) { res.setHeader('Cache-Control', 's-maxage=600'); return res.json({ ...cached, cached: true }); }
  }

  const doc = await readJSON(EPISODES_KEY, { episodes: [] }).catch(() => ({ episodes: [] }));
  const episodes = doc.episodes || [];
  const today = new Date().toISOString().slice(0, 10);
  const maxH = Math.max(...GRADE_HORIZONS);
  const minAgeDays = Math.ceil(maxH * 1.5);   // ~sessions→calendar-days buffer for weekends
  const resolvable = episodes.filter(e =>
    (e.side === 'bullish' || e.side === 'bearish') && e.firstSeenDate && ageDays(e.firstSeenDate, today) >= minAgeDays);

  if (!resolvable.length) {
    const payload = { ok: true, asOf: today, totalEpisodes: episodes.length, graded: 0,
      note: 'No decision episodes are old enough to grade yet — the immutable episode ledger accrues as op=optionsflow runs daily. Grading is prospective and next-open; nothing to report until episodes mature.' };
    if (hasStore()) await writeJSON(GRADES_KEY, payload, 0).catch(() => {});
    res.setHeader('Cache-Control', 's-maxage=600');
    return res.json(payload);
  }

  const toGrade = resolvable.slice(-GRADE_CAP);
  const tickers = [...new Set(toGrade.map(e => e.ticker))];
  ...
  let ti = 0;
  const worker = async () => {
    while (ti < tickers.length) {
      const t = tickers[ti++];
      try { const d = await fetchDailyHistory(t); if (d) hist.set(t, d.candles); } catch { /* skip a name */ }
    }
  };
  await Promise.all(Array.from({ length: Math.min(6, tickers.length) }, worker));

  const graded = [];
  for (const e of toGrade) {
    const candles = hist.get(e.ticker);
    if (!candles) continue;
    // sector-relative is SUPPORTED by the grader but not wired (no ticker→sector-ETF map
    // yet); SPY-relative is the primary de-beta. excessVsSector stays null until a map lands.
    const g = gradeEpisode(e, { candles, spy }, { horizons: GRADE_HORIZONS });
    if (g.graded) graded.push(g);
  }

  const summaries = {};
  for (const h of GRADE_HORIZONS) summaries[h] = summarizeEpisodes(graded, { horizon: h, metric: 'excessVsSpy' });

  // Governance: promotion readiness vs the app-wide gate, the (hard-off) calibration gate,
  // a purged/embargoed walk-forward, and champion/challenger state. Honest by construction —
  // the layer stays shadow and shows no probability until the evidence clears every criterion.
  const governance = governanceReport(graded, { horizon: 21 });

  const payload = {
    ok: true, asOf: today, generatedAt: new Date().toISOString(),
    maturity: 'shadow', dataDelayed: true,
    totalEpisodes: episodes.length, resolvable: resolvable.length, graded: graded.length,
    horizons: GRADE_HORIZONS, summaries, governance,
    sample: graded.slice(-30),
    note: 'Prospective grading of immutable decision episodes: entry is the NEXT session OPEN after the decision date (never the decision close), returns are SPY-relative and round-trip-cost-adjusted, and repeated daily appearances were already collapsed into one episode so these are INDEPENDENT observations. Sector-relative is supported but not yet wired (no ticker→sector-ETF map). This measures whether the options-flow shadow layer is economically useful; it is not a live signal and cannot promote itself.',
  };
  if (hasStore()) await writeJSON(GRADES_KEY, payload, 0).catch(() => {});
  res.setHeader('Cache-Control', 's-maxage=600');
  return res.json(payload);
...
module.exports = { runOptionsFlow, runOptionsPerf, runOptionsAssess, runOptionsEpisodes, resolveAt, summarize };
