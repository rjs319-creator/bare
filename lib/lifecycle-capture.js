...
function buildSnapshot({ record, ev = {}, pick = {}, displayed = false, displayPosition = null, at = null } = {}) {
  ...
  // ATR fallback chain — run-scan and discovery picks carry no `orb`, and dropping them
  // from grading for a missing scale would silently bias the graded sample.
  const atr = (pick.orb && pick.orb.atr != null) ? pick.orb.atr
    : (pick.atr != null ? pick.atr : (metrics.dailyAtr != null ? metrics.dailyAtr : null));
  return Object.freeze({
    ...
    // Completed-bar / forming-bar / quote-only provenance for the evidence at decision time.
    ...
    // THE PLAN IN FORCE at decision time. For a confirmed setup this is the ACTIVE frozen
    // live plan (the levels the system actually held out) — the prior-session daily plan is
    // preserved separately as `dailyPlan` provenance and is never captured AS the plan.
    plan: (record.activePlan || ev.livePlan)
      ? (() => { const lp = record.activePlan || ev.livePlan; return {
          entry: lp.entry ?? null, stop: lp.stop ?? null, target: lp.target ?? null,
          rr: lp.rr ?? null, trigger: lp.trigger ?? null,
          basis: lp.basis || 'live-intraday', frozen: lp.frozen === true,
          computedAt: lp.computedAt || null, expiresAt: lp.expiresAt || null,
        }; })()
      : {
          entry: pick.entry ?? null, stop: pick.stop ?? null, target: pick.target ?? null,
          rr: pick.rr ?? null, trigger: pick.orb ? pick.orb.trigger : null,
          basis: 'daily-detection', frozen: false, computedAt: null, expiresAt: null,
        },
    dailyPlan: {
      entry: pick.entry ?? null, stop: pick.stop ?? null, target: pick.target ?? null,
      rr: pick.rr ?? null, trigger: pick.orb ? pick.orb.trigger : null,
    ...
    decisionPrice,
    atr,
    // Deterministic runner/dud research scores at decision time (uncalibrated — labeled).
    // A promoted learned model would write its outputs here under its own version.
    modelOutputs: (ev.runnerScore != null || ev.dudScore != null || ev.shadowModel != null)
      ? {
        runnerScore: ev.runnerScore ?? null, dudScore: ev.dudScore ?? null, basis: ev.scoreBasis || null,
        // A promoted-to-shadow learned model's outputs land HERE beside the deterministic
        // scores (version + uncalibrated basis attached by lib/model-registry) — never in
        // ranking or display. Null until a champion is ever promoted.
        shadow: ev.shadowModel || null,
      } : null,
    policyVersion: record.strategyVersion || null,
    modelVersion: ev.scoreVersion || null,
    displayed: !!displayed,
...
function firstEntryEpisodes(snapshots) {
  return firstSnapshotWhere(snapshots, s => s.state === STATES.ACTIONABLE_NOW)
    .map(s => ({
      ...
      plan: s.plan, ranking: s.ranking, features: s.features || null,   // feature vector for the survival model
    }))
