...
// close,volume} objects. Returns ascending [{date, o,h,l,c,v}] on ONE basis.
//
// THE BUG THIS FIXES (alpha-research pass 3). This used to set `c = adjClose ?? close`
// while leaving o/h/l RAW, producing a bar whose close and whose high/low/open came from
// different price series. Yahoo adjusts BACKWARD, so for any dividend payer adjClose is
// below close on every bar before the last distribution, by the cumulative dividend
// factor. Consequences, all on live ATLAS-X features:
//   • lib/atlasx-path.js gap = (b.o − prev.c)/prev.c fabricated a positive gap on EVERY
//     historical bar — raw open against an adjusted prior close;
//   • b.c could fall BELOW b.l, so closePosition = (b.c − b.l)/range clamped to 0;
//   • body = (b.c − b.o)/range was systematically negative, i.e. every historical bar of
//     a dividend payer read as a down-close within its own range;
//   • true range = b.h − prev.c and upper wick = b.h − b.c were both inflated.
// For a 3% yielder the distortion is ~3% against features scaled to ATR/range (~2%) —
// larger than the signal being measured.
//
// THE FIX: scale the WHOLE bar by that bar's own adjustment factor (adjClose/close), so
// open/high/low/close stay on one series and the dividend adjustment is preserved. That
// is strictly better than dropping adjClose, which would silently discard the adjustment.
//
// AND ONE BASIS PER SERIES. The old code also fell back to raw `close` per-bar whenever
// adjClose was missing (legacy 6-tuples, provider omissions), so a return computed across
// that boundary was a fabricated jump equal to the accumulated adjustment. Coverage is
// now all-or-nothing: partial coverage uses RAW for every bar rather than mixing. The
// chosen basis is reported as a non-enumerable `basis` property so callers can see it
// without changing the array's shape for JSON or deep-equality.
function toBars(candles) {
  ...
  for (const row of candles) {
    if (Array.isArray(row)) {
      ...
      raw.push({ date: String(date), o: num(o), h: num(h), l: num(l), c: Number(c), v: num(v),
        adj: (adj != null && isFinite(Number(adj))) ? Number(adj) : null });
    } else if (row && typeof row === 'object') {
      const date = row.date || row.d || row.t;
      // Prefer the RAW close as the scaling denominator. A row carrying only adjClose is
      // already adjusted, so it becomes both close and adj (factor 1) rather than being
      // dropped — preserving the old behaviour for that shape.
      const rawClose = row.close != null ? row.close : (row.c != null ? row.c : row.adjClose);
      if (date == null || rawClose == null) continue;
      raw.push({
        date: String(date), o: num(row.open ?? row.o), h: num(row.high ?? row.h),
        l: num(row.low ?? row.l), c: Number(rawClose), v: num(row.volume ?? row.v),
        adj: (row.adjClose != null && isFinite(Number(row.adjClose))) ? Number(row.adjClose) : null,
      });
    }
  }
  raw.sort((a, b) => (a.date < b.date ? -1 : a.date > b.date ? 1 : 0));

  const adjusted = raw.length > 0 && raw.every(b => b.adj != null && Number.isFinite(b.c) && b.c > 0);
  const out = raw.map(b => (adjusted
    // Per-bar factor: applied to the whole bar so geometry (o<=h, l<=c<=h) is preserved.
    ? (f => ({ date: b.date, o: b.o * f, h: b.h * f, l: b.l * f, c: b.adj, v: b.v }))(b.adj / b.c)
    : { date: b.date, o: b.o, h: b.h, l: b.l, c: b.c, v: b.v }));

  Object.defineProperty(out, 'basis', {
    value: adjusted ? 'total-return' : 'price-return', enumerable: false, writable: false,
  });
  Object.defineProperty(out, 'adjustmentCovered', {
    value: adjusted, enumerable: false, writable: false,
  });
  return out;
}
