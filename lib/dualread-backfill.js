...
function replayTicker(candles, spyCandles, opts = {}) {
  ...
  for (let i = minLB; i + H < candles.length; i += step) {
    ...
    // COST-NET label, same cost model as the live resolver — backfill and live rows
    // must be measured identically or the tuner learns the difference, not the signal.
    const { roundTripCostPct } = require('./costs');
    const fwd = +((sRet - mRet) * 100 - roundTripCostPct('liquid')).toFixed(3);
    rows.push({ signals: lt.signals, group, date: d, fwd, ticker: opts.ticker || null });
  }
  return rows;
}
