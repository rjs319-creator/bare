...
async function dailyHorizonsFor(ticker, stAction) {
  try {
    ...
    // Independent swing read (2–12 weeks) off the SAME bars.
    const swing = swingRead(daily.candles, spy, {});
...
function classifyIntradayFreshness(source, marketState, lastCandleT, nowSec) {
  ...
  if (marketState === 'PRE' || marketState === 'PREPRE') return 'premarket';
  if (marketState === 'POST') return 'afterhours';
  if (marketState === 'CLOSED') return 'closed';
  const ageMin = lastCandleT ? (nowSec - lastCandleT) / 60 : Infinity;
...
// live BUY/SELL off daily-fallback or stale data.
function shapeIntradayHorizon(live, source, marketState, freshness, dataAsOf) {
  ...
  if (fallback || stale) {
    ...
    live.action = 'HOLD';
    live.label = fallback ? 'Intraday unavailable (daily fallback)' : 'Intraday data stale';
    live.levels = null;
  }
  ...
  return {
    ...
    levels: live.levels,                       // intraday ATR levels (null when demoted)
...
async function analyze(ticker, opts = {}) {
  ...
  // the demoted `live` (fallback/stale) propagates to every consumer.
  const nowSec = Math.floor(Date.now() / 1000);
  const intradayFreshness = classifyIntradayFreshness(source, marketState, lastCandle.t, nowSec);
  const intradayAsOf = source === 'yahoo' ? lastCandle.date : lastCandle.date;
  const intraday = shapeIntradayHorizon(live, source, marketState, intradayFreshness, intradayAsOf);
  ...
  const out = {
    ...
    marketState,
    price,
    live,                       // legacy field (demoted in place when fallback/stale)
