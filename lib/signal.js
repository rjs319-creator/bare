...
// Active long-term weights come from the self-tuner (dualread/weights.json). Cache
// them in-process (TTL) so the hot chart path never blocks on Blob — and so the live
// read scores with the SAME weights op=dualreadlog logs (train/serve consistency).
let ltWeightsCache = { at: 0, weights: DEFAULT_LT_WEIGHTS };
const LTW_TTL_MS = 15 * 60 * 1000;
async function loadLtWeights() {
  const now = Date.now();
  if (now - ltWeightsCache.at < LTW_TTL_MS) return ltWeightsCache.weights;
  try {
    const { readJSON, hasStore } = require('./store');
    if (hasStore()) {
      const w = await readJSON('dualread/weights.json', null);
      ltWeightsCache = { at: now, weights: (w && w.weights) ? w.weights : DEFAULT_LT_WEIGHTS };
      return ltWeightsCache.weights;
    }
  } catch { /* fall through to default */ }
  ltWeightsCache = { at: now, weights: DEFAULT_LT_WEIGHTS };
  return ltWeightsCache.weights;
}
...
async function longTermFor(ticker, stAction) {
  try {
    ...
    const [daily, spy, weights] = await Promise.all([
