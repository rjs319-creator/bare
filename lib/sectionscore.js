...
//   OMEGA        — the ACTUAL logged point-in-time OMEGA score (`pick.score`). OMEGA already
//                  persists its real 0–100 continuation score with each pick, so reconstructing
//                  it with a generic momentum proxy would be strictly worse — we use the logged
//                  score directly (method 'omega-logged'). No candle recompute needed.
const RECON = { screener: 'apex', Ghost: 'ghost', momentum: 'momentum', daytrade: 'daytrade', coil: 'coil', OMEGA: 'logged' };
...
function reconstruct(picks, opts = {}) {
  ...
  // ── Logged-score sections (OMEGA): the pick already carries its REAL point-in-time score,
  // so validate conviction against THAT, not a generic recompute. Honest fidelity: the score
  // shown live is the score graded.
  (picks || []).forEach((p, i) => {
    if (RECON[p.section] !== 'logged') return;
    if (Number.isFinite(p.score)) out[i] = { score: +(+p.score).toFixed(2), method: 'omega-logged', section: p.section };
  });
