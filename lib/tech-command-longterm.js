...
const CHEAP_PEG = 1.2;
// Above this trailing EPS-growth rate the PEG stops being a valuation read at all:
// growth off a cyclical trough (a memory-cycle recovery prints >700% YoY) drives the
// ratio toward zero and makes ANY multiple look subdued. Observed live: MU P/E 20 at
// 700% growth -> PEG 0.03; AMD P/E 124 at 124% growth -> PEG 1.00. A ratio built from
// a base effect is not evidence of cheapness, so it is treated exactly like a missing
// valuation rather than a passing one.
const MAX_MEANINGFUL_EPS_GROWTH = 100;
...
function buildRow(c, u, regime, nowIso) {
  ...
  // ── Valuation vs growth ───────────────────────────────────────────────────
  const rawPeg = (f && f.pe != null && f.epsGrowth != null && f.epsGrowth > 0) ? +(f.pe / f.epsGrowth).toFixed(2) : null;
  // A PEG off a recovery base is reported but never RELIED ON (see the constant above).
  const pegUnreliable = rawPeg != null && f.epsGrowth > MAX_MEANINGFUL_EPS_GROWTH;
  const peg = pegUnreliable ? null : rawPeg;
  if (peg != null) {
    items.push(EV.makeEvidence({
  ...
  return Object.freeze({
    ...
    valuation: {
      pe: f ? num(f.pe) : null, peToGrowth: peg,
      tension: peg != null && peg >= RICH_PEG,
      available: peg != null,
      reportedPeToGrowth: rawPeg,
      unreliableBasis: pegUnreliable
        ? `trailing EPS growth of ${f.epsGrowth}% is a base effect, not a run rate — P/E ÷ growth of ${rawPeg} says nothing about what the price embeds`
        : null,
      note: peg == null
        ? (pegUnreliable
          ? `P/E ÷ growth computes to ${rawPeg}, but only because trailing EPS growth (${f.epsGrowth}%) is a recovery off a depressed base. It is shown for transparency and NOT used — this name is held at WATCH_VALUATION rather than accumulated on a ratio that cannot mean what it appears to mean.`
          : 'No P/E-to-growth pair is available, so what the price already embeds cannot be assessed — this name is held at WATCH_VALUATION rather than accumulated on the strength of the other families.')
        : null,
    },
...
function deriveAction({ composed, coverage, peg, trend, fundamentals }) {
  ...
  if (score >= 30) {
    // MISSING VALUATION MUST NOT BE A FREE PASS. With no P/E-to-growth pair there is
    // no way to say what the price already embeds — and ACCUMULATE is precisely the
    // claim that the expectations/risk balance is attractive. Reading a missing
    // valuation as "no valuation objection" would make absent data *easier* to act
    // on than present data, inverting the rule that missing data reduces confidence.
    if (peg == null) return 'WATCH_VALUATION';
    if (peg >= RICH_PEG) return 'WATCH_VALUATION';
    return 'ACCUMULATE';
  }
  return 'HOLD';
}
...
module.exports = {
  ...
  MIN_COVERAGE_FOR_ACTION, RICH_PEG, CHEAP_PEG, MAX_MEANINGFUL_EPS_GROWTH,
