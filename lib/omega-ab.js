...
const FROZEN = Object.freeze({
  ...
  // Waiting is not failing — but not forever. A row whose history is STILL null (or
  // still short of the horizon) this many SESSIONS past maturity is no longer waiting
  // on a slow provider: the ticker is gone (delisted/acquired). Past this grace the
  // waiting states start burning the attempts budget too, so such a row reaches
  // terminal UNRESOLVABLE instead of pending forever (and its date instead of
  // haunting the resolve loop every tick).
  missingHistoryGraceSessions: 10,
...
function resolveDecisionDoc(doc, { histories, spy, resolvedAt = null }) {
  ...
  // How far past the decision this run is, in SPY sessions. While the doc is within
  // maturity + grace, waiting states cost nothing; beyond it, "still waiting" means
  // the history is never coming (delisted ticker → fetchDailyHistory null forever)
  // and waiting converts into a counted failed attempt so the row can terminate.
  const sessionsSinceDecision = (spy.length - 1) - si;
  const waitingGraceExpired = sessionsSinceDecision > H + FROZEN.missingHistoryGraceSessions;
  let pending = 0;
  const rows = doc.rows.map((r) => {
    ...
    // until the grace above expires, after which they do.
    const failedAttempt = () => {
    ...
    const stillWaiting = () => {
      if (waitingGraceExpired) return failedAttempt();
      pending++;
      return r;
    };
    const candles = histories.get(r.ticker);
    if (!candles) return stillWaiting();
    ...
    if (i + H > candles.length - 1) return stillWaiting();               // not matured in this series yet
...
// time >= that session's close + a 5-minute settle. Before this, only cron timing
// (22:00 UTC) kept a manual/misfired tick from stamping a forming intraday bar as the
// immutable decision of record. Pure given `now` — the clock default is the caller
// convenience, tests always pass an explicit instant.
//
// The completion threshold is DERIVED from lib/market-session's calendar for the current
// ET date (review 2026-08-15) — regular days close 16:00, EARLY_CLOSES days 13:00 —
// instead of a hardcoded 16:05 that duplicated the calendar and silently drifted (an
// early-close day's completed 13:00 bar was refused until 16:05). If the session lookup
// fails, the guard falls CLOSED to the regular-close threshold (16:05 ET).
const SESSION_SETTLE_MIN = 5;                        // minutes past the close before a bar counts as final
const SESSION_COMPLETE_FALLBACK_MIN = 16 * 60 + 5;   // 16:05 ET — fail-closed default
function isCompletedSessionBar(barDate, now = new Date()) {
  ...
  let completeMin = SESSION_COMPLETE_FALLBACK_MIN;
  try {
    const closeMin = MS.sessionInfoAt(now).regularCloseMin;   // calendar close for `date`
    if (Number.isFinite(closeMin)) completeMin = closeMin + SESSION_SETTLE_MIN;
  } catch { /* fail closed: keep the regular-close threshold */ }
  return minutes >= completeMin;             // same date: only after the close settles
}
