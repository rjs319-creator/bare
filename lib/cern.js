...
// Round-trip trading cost (spread + slippage) as a FRACTION, scaled by how liquid
// the name is. The engine previously assumed a flat 60bps (2×30bps) for EVERY name,
// which made illiquid recent-IPO / micro-cap trades look profitable on paper — and
// the Scoreboard showed those were the biggest losers. A $50M/day name is cheap to
// trade; a $500k/day micro-cap costs multiples more. Falls back to the flat estimate
// when we have no dollar-volume for the name.
function roundTripCost(advUsd, fallbackBps = 30) {
  if (!(advUsd > 0)) return 2 * fallbackBps / 1e4;
  const m = advUsd / 1e6; // avg daily $ volume, in millions
  if (m >= 50) return 0.003;  //  30 bps — very liquid
  if (m >= 10) return 0.006;  //  60 bps — liquid
  if (m >= 3)  return 0.012;  // 120 bps — moderate
  if (m >= 1)  return 0.020;  // 200 bps — thin
  return 0.035;               // 350 bps — micro / recent IPO
}
...
// DETERMINISTIC SAMPLING (alpha-research pass 3). This used Math.random(), which made
// CERN's Thompson exploration ranking unreproducible from stored state: the same ledger
// replayed twice could probe different event types, so a signal could not be re-derived
// from the record that produced it. That breaks replay and audit for a strategy whose
// registry criteria demand per-event-type decay validation.
//
// Seeded from the arm's OWN posterior (type key + mu/sd/n) plus a caller-supplied salt,
// following the repo's existing FNV convention (cf. lib/orbit-controls.js). Exploration
// still varies ACROSS arms and as the posterior updates — what it no longer does is vary
// between two runs over identical state.
function fnv1a(str) {
  let h = 2166136261;
  for (let i = 0; i < str.length; i++) { h ^= str.charCodeAt(i); h = Math.imul(h, 16777619); }
  return h >>> 0;
}
// Two independent uniforms in (0,1] from one seed — Box-Muller needs a pair.
function seededPair(seed) {
  const a = fnv1a(seed + '|u1'), b = fnv1a(seed + '|u2');
  return [((a % 100000) + 1) / 100001, ((b % 100000) + 1) / 100001];
}
/** Sample N(mu, sd) — Box-Muller, deterministic in `seed`. */
function sampleNormal(mu, sd, seed = '') {
  const [u1, u2] = seededPair(String(seed));
...
const EVENT_TYPES = {
  ...
  // S&P 400/600 changes — the same mechanical passive flow at several times the
  // event frequency and a larger flow-vs-ADV impact (thin small/mid books).
  // SEPARATE types (not merged into the 500 cousins) so their κ-posteriors and
  // Scoreboard tiers accrue on their own flow regime rather than polluting the
  // large-cap evidence. Same priors as the 500 types: the deletion-reversal
  // literature is if anything strongest in the 600.
  INDEX_DELETE_SMID:   { horizon: 40, priorKappa: 0.70 },
  INDEX_ADD_FADE_SMID: { horizon: 30, priorKappa: 0.45 },
  ...
  // logOnly: proven loser in our own Scoreboard (n=18: −1.24% overall, −7.32% in
  // risk-on) and the "softest" forced-flow — an analyst downgrade is part sentiment,
  // not the mechanical supply shock the thesis needs. Keep logging for the
  // counterfactual archive (the moat) but never take a real position.
  FORCED_DOWNGRADE: { horizon: 45, priorKappa: 0.65, logOnly: true },
};
...
class CERN {
  ...
  addEvent(ev) {
    ...
    // The market session this event belongs to, not the clock that detected it.
    // op=cerntick runs daily including weekends; stamping nowMs produced decision
    // dates on Saturdays, Sundays and July 4th.
    if (!ev.sessionDate && Number.isFinite(ev.dateMs)) {
      const { lastCompletedRegularSession } = require('./market-session');
      ev = { ...ev, sessionDate: lastCompletedRegularSession(new Date(ev.dateMs)) || undefined };
    }
  ...
  dailyTick(dataFor, marketCtx = {}, nowMs = Date.now()) {
    ...
    for (const ev of this.s.ledger) {
      ...
      const advUsd = mean(d.bars.slice(-20).map(bb => bb.close * (bb.volume || 0)));
      const cost = roundTripCost(advUsd, costBps);
      ...
      ev.signal = {
        ...
        x, predMu, predSd, pProfit, cost, regime,
  ...
  _measure(ev, d, regime) {
    ...
    // (Removed the estimate-revisions haircut: we have no estimate-revisions feed, so
    //  d.estimateRevisions was always null and this check never fired — dead input.)
  ...
  _resolveMatured(dataFor, nowMs) {
    ...
    for (const ev of this.s.ledger) {
      ...
      if (ev.signal.action === 'TRADE') {
        const cost = ev.signal.cost ?? 0.006; // liquidity-aware cost stored at signal time
        const net = R - cost;
  ...
  _thompsonRanking(salt = '') {
    return Object.entries(this.s.types)
      // Seed from the arm's own posterior + an explicit salt so the ranking is
      // reproducible from stored state (see sampleNormal).
      .map(([k, t]) => ({ type: k, sample: sampleNormal(t.mu, t.sd, `${k}|${t.mu}|${t.sd}|${t.n}|${salt}`),
