...
const EVENT_TYPES = {
  ...
  LOCKUP_EXPIRY:   { horizon: 40, priorKappa: 0.55 },
...
class CERN {
  ...
  dailyTick(dataFor, marketCtx = {}, nowMs = Date.now()) {
    ...
    for (const ev of this.s.ledger) {
      ...
      const trade = pProfit >= 0.65 && !t.drifted &&
      ...
      ev.signal = {
        ...
        action: trade ? 'TRADE'
              : this._thompsonProbe(ev.type) ? 'PROBE' : 'LOG_ONLY',
        size: trade ? size : 0,
      };
  ...
  _measure(ev, d, regime) {
    ...
    const D = Math.max((sectorMove - stockMove) * -ev.direction * -1, 0);
