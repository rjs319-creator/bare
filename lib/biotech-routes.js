...
async function runBiotech(req, res) {
  ...
  if (cached) {
    ...
    return res.json({
      ok: true, cached: true, stale, disclaimer: DISCLAIMER,
      // Defect #7 — explicit semantics: nothing on this board is a probability.
      scoreSemantics: {
        score: 'Research Priority 0-100 — a ranking score, never a probability',
        confidence: 'LLM-asserted 1-5 conviction dots — a subjective descriptor, never a probability',
        probability: null, calibrationStatus: 'uncalibrated',
