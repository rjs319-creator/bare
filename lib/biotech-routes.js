...
// Biotech names to scan = curated BIOTECH list (dedicated candle cache) UNIONED with the
// biotech-named tail of the expanded universe (its candles live in the expanded cache). This
// is the durable coverage fix: new biotech runners are picked up automatically as the free
// universe expansion seeds them, instead of only names hand-added to BIOTECH. Returns
// [{ ticker, doc }] pairs (ticker → which cache holds its candles), curated first.
async function biotechScanSources() {
  const { loadCandleCache } = require('./candle-cache');
  const { BIOTECH } = require('./universe');
  const [bioDoc, expDoc] = await Promise.all([
    loadCandleCache('biotech').catch(() => null),
    loadCandleCache('expanded').catch(() => null),
  ]);
  const sources = []; const seen = new Set();
  const push = (ticker, doc) => {
    const u = String(ticker || '').toUpperCase();
    if (!u || seen.has(u) || !doc) return;
    seen.add(u); sources.push({ ticker: u, doc });
  };
  for (const t of BIOTECH) push(t, bioDoc);
  if (expDoc) {
    for (const t of await expandedBiotechTickers()) push(t, expDoc);
  }
  return sources;
}

// Symbols in the expanded-universe candidate list whose company name reads as biotech/pharma.
async function expandedBiotechTickers() {
  try {
    const { readJSON } = require('./store');
    const { isBiotechName } = require('./universe-expand');
    const doc = await readJSON('universe/candidates.json');
    if (!doc || !Array.isArray(doc.tickers)) return [];
    return doc.tickers.filter(t => isBiotechName(t.name)).map(t => t.symbol);
  } catch { return []; }
}
...
async function detect(limit, ctx) {
  ...
  const sources = await biotechScanSources();
  if (!sources.length) return { movers: [], asOf: null };
  ...
  for (const { ticker, doc } of sources) {
    ...
    out.push({
      ticker, pct5d: +(m.pct5d || 0).toFixed(1), relVol: +m.relVol.toFixed(1),
...
async function runBiotechTick(req, res) {
  ...
  try {
    ...
    return res.json({ ok: true, asOf, itemCount: scored.length, investigated: cands.length, logged, detected: movers.length, regime, etfPct5d: etf5, elapsedMs: Date.now() - t0 });
  } catch (e) {
...
module.exports = { runBiotech, runBiotechTick, detect, tagNewsless, logSurfaced, biotechScanSources, expandedBiotechTickers, CACHE_KEY, BIOTECH_ETF };
