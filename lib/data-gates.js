...
function cutoffOf(payload) {
  ...
  return (f && (f.decisionSession || f.dataCutoffSession || f.sessionDate || f.asOfDate))
    || payload.decisionSession || payload.spyLastDate || payload.asOf
    // Fields that genuinely name a DATA session under another historical name:
    //   triggerDate — the ledger day a read-through/second-wave lead is derived from
    //   baselineAsOf — the session an options baseline was computed through
    || payload.triggerDate || payload.baselineAsOf
    || null;
  // DELIBERATELY NOT READ: a bare top-level `date`. Several routes set it to
  // `new Date().toISOString().slice(0,10)` — i.e. "today", the moment the response was
  // built — which proves nothing about the data underneath and would let a stale chain or
  // a stale cache present as current. A cutoff must name the session actually observed.
}
