...
function classifyTransition(tr) {
  ...
  if ((to === STATES.FAILED || to === STATES.STALLING || to === STATES.EXPIRED || to === STATES.TARGET_REACHED) && ACTIVE_BEFORE.has(from)) {
...
function confirmationMissing(c) {
  ...
  return missing.length ? missing : ['full signal gate not yet evaluated'];
}
...
function buildAlert(tr, { now } = {}) {
  ...
  return {
    ...
    // "SIGNAL gate", deliberately — the gate checks trigger/VWAP/volume/residual/R:R/
    // freshness. It does NOT verify spread, halt state or fill plausibility (no execution
    // conditions are observed on this feed), so claiming an "execution gate" would overstate
    // what was checked.
    actionability: (cls.kind === 'entry' || cls.kind === 'revive')
      ? 'confirmed trigger — signal gate passed at emission time (execution conditions not verified: spread/halt unknown); conditions can invalidate at any moment'
...
function feedTitle(a) {
  switch (a.kind) {
    ...
    case 'retire': return a.transition && a.transition.to === 'TARGET_REACHED'
      ? `🎯 ${a.ticker} setup played out — live-plan target printed ${a.atET} (no position implied)`
      : `🛑 ${a.ticker} retired — ${String(a.transition && a.transition.reasonCode || '').replace(/_/g, ' ').toLowerCase()}`;
