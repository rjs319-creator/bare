...
// SECTOR_ETF keys are TitleCase ('Technology', 'Health Care', …). A lowercased
// probe returned null for every sector, which silently disabled sector
// residualization in the live path (every residual fell back to market-only
// with partial:true). Index the map case-insensitively once instead.
const SECTOR_ETF_CI = Object.fromEntries(
  Object.entries(SECTOR_ETF).map(([k, v]) => [k.toLowerCase(), v])
);
const etfForSector = name => (name ? SECTOR_ETF_CI[String(name).trim().toLowerCase()] || null : null);


// `isPublic: true` sends NO Authorization header. internalHeaders() attaches
// `Authorization: Bearer <CRON_SECRET>`, and a CDN will not serve a cached response to
// an authenticated request — so pulling the PUBLIC op=today that way missed the edge on
// every call and recomputed it at origin. Same defect fixed in lib/ignition-routes
// (e2f675c), where removing the header took the route from ~6.5s to ~0.4s. The
// ledger-writing path keeps the authenticated read: it must not persist a possibly-stale
// edge copy to save latency no user sees.
async function pull(path, { isPublic = false } = {}) {
  try {
    const headers = isPublic ? { 'x-warm': '1' } : internalHeaders();
    const r = await fetch('https://' + HOST + path, { headers, signal: AbortSignal.timeout(12000) });
...
async function buildAtlasBoard({ persist }) {
  ...
  const today = await pull('/api/tracker?op=today', { isPublic: !persist });
  ...
  // Pre-move redesign (Phase 4): PREMOVE_V2_MODE shadow/enforce activates the
  // two-pass specialist-balanced universe; 'off' keeps the legacy momentum-only
  // near-miss funnel. ATLAS-X is shadow either way — this changes what it
  // OBSERVES, never a production rank.
  const premoveMode = require('./premove-flags').resolvePremoveMode();
  // Benchmark window returns for the scan's market-residual ranks (cache first,
  // one bounded fetch as fallback; null → the funnel labels itself unresidualized).
  let benchRet = null;
  {
    let spyCandles = null;
    for (const d of Object.values(candleDocs)) { const g = cacheGet(d, 'SPY'); if (g && g.candles) { spyCandles = g.candles; break; } }
    ...
    if (spyCandles && spyCandles.length > 21) {
      ...
      benchRet = {
        ret5: c[c.length - 6] > 0 ? lastC / c[c.length - 6] - 1 : null,
        ret20: c[c.length - 21] > 0 ? lastC / c[c.length - 21] - 1 : null,
      };
    }
  }
  const universe = buildUniverse({ todayData: today.data, prevEpisodes, candleDocs, opts: {
    nowMs,
    mode: premoveMode === 'off' ? 'legacy' : 'twopass',
    benchRet,
  } });
  ...
  // Cross-sectional leadership snapshot from the RLT scan (rlt/latest.json).
  // Absent or stale ⇒ the relativeLeadershipTransition expert abstains — it
  // never re-derives peer rank from a single chart.
  const rltLeadership = await require('./rlt-store').loadLatest()
    .then(d => (d && d.leadershipSnapshot) || null)
    .catch(() => null);
  const ctx = {
    date, generatedAt, regime, regimeRiskOff, isHoliday: isMarketHoliday, cooldownSessions: 3,
    rltLeadership,
    // Phase 5 fixes: the experts read `riskOff` and `asOf` (previously never
    // supplied — redTapeReversal was permanently disabled and catalyst
    // freshness was always unknown).
    riskOff: regimeRiskOff,
    asOf: date,
    premoveMode,
  ...
  //
  // THREE cohorts, not two (alpha-research pass 3). `actionable` is the UTILITY gate and
  // does NOT imply a valid entry, so it cannot stand in for "what the system would act
  // on" — the meaning selection-lift needs. Splitting it here keeps `selected` honest
  // without pushing utility-passing names into `rejected`, which means "failed the gate".
  const { isEnterable } = require('./atlasx-portfolio');
  const utilityPassed = candidates.filter(c => c.actionable);
  const actionable = utilityPassed.filter(c => isEnterable({ entryAction: c.entry ? c.entry.action : null }));
  const qualifiedNoEntry = utilityPassed.filter(c => !isEnterable({ entryAction: c.entry ? c.entry.action : null }));
  const portfolio = buildAtlasPortfolio(utilityPassed.map(rankRow), { });
  const rejected = candidates.filter(c => !c.actionable);
  // Controls are matched for the SELECTED cohort — they exist to measure that cohort's
  // lift, so matching them against names the system could not enter would dilute it.
  ...
  const capture = buildCapture({
    ...
    qualifiedNoEntry: qualifiedNoEntry.map(controlKey),
...
function rankRow(c) {
  return { ticker: c.ticker, expert: c.expert, strategyFamily: c.expert, sector: c.sector, sectorEtf: c.sectorEtf,
    ...
    // Entry availability travels with the row. The portfolio is a book of HELD
    // positions, and `actionable` (the utility gate) does not imply a valid entry —
    // without this the book admitted names the board files under Avoid.
    entryAction: c.entry ? c.entry.action : null };
}
