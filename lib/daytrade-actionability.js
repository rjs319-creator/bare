...
function selectDeepPoolDetailed(picks, { priorRecords = {}, maxNames = STAGE2_MAX_NAMES, now = null } = {}) {
  ...
  const entries = pool.map(p => {
    ...
    const priority = z(p.relVol, rv) + z(p.pctChange, pc) + z(p.excessPct, ex) + discoveryPriority(disc, nowMs);
    return { p, lane, boost, disc, priority };
  });
  ...
  const diagnostics = {
    ...
    rejected: entries.filter(e => !chosen.has(e.p.ticker))
      .sort((a, b) => b.priority - a.priority)
      .map(e => ({ ticker: e.p.ticker, lane: e.lane, priority: +e.priority.toFixed(3), reason: 'stage2-budget' })),
  };
...
async function stage2Evaluations(picks, { now, sessionDate, t0, deadline = 38000, maxNames = STAGE2_MAX_NAMES, priorRecords = {} } = {}) {
  ...
  return { evByTicker, stage2: Object.keys(evByTicker).length, skipped: null, deepPool: pool.map(p => p.ticker), selection };
}
...
function classifyPool(picks, { now, evByTicker = {}, priorRecords = {} } = {}) {
  ...
  for (const pick of picks || []) {
    ...
    const record = stampActivePlan(advanceLifecycle(priorRecords[pick.ticker] || null, { strategy: 'daytrade', ...ev }, LIFECYCLE_CFG), ev);
...
module.exports = {
  ...
  computeLivePlan, stampActivePlan, collectTransitions,
