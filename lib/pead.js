...
// ── True standardized-earnings-surprise (SUE) PEAD ──────────────────────────
// The real test the earlier study COULDN'T run. FMP Premium now returns paired
// epsActual + epsEstimated going back decades, so we can compute a genuine
// analyst-based SUE = (actual − estimate) / σ(the firm's OWN trailing surprises)
// — the academic PEAD standard, which normalises away firms whose analysts are
// chronically off and puts every name on a comparable surprise scale. We then
// measure forward SPY-excess drift by SUE quintile, split by YEAR and by the
// VIX/credit MACRO regime, so a single risk-on window can't masquerade as a
// durable edge (the trap that killed the reaction proxy). All PIT: the SUE σ
// uses only PRIOR earnings, and we enter the close AFTER the announcement.
const { buildMacroLookup } = require('./macro');
const sampleStd = arr => { const n = arr.length; if (n < 2) return null; const m = mean(arr); return Math.sqrt(arr.reduce((s, x) => s + (x - m) ** 2, 0) / (n - 1)); };

async function runSurprisePEAD({ scope = 'large', months = 60, holds = [5, 21, 63], limit = 150, minHistory = 4, deadlineMs = 55000 } = {}) {
  if (!FMP) return { error: 'FMP_API_KEY required' };
  const t0 = Date.now();
  const list = scope === 'micro' ? MICRO_CAPS : scope === 'small' ? SMALL_CAPS : LARGE;
  let universe = [...new Set(list)]; if (limit > 0) universe = universe.slice(0, limit);
  ...
  // 1. Per-symbol earnings with paired actual+estimate (throttled for the plan).
  //    limit=40 quarters (~10y) gives ample trailing history for the SUE σ.
  const bySym = new Map(); let withEst = 0, fetched = 0;
  for (const sym of universe) {
    if (Date.now() - t0 > deadlineMs * 0.55) break;  // reserve time for prices + compute
    try {
      const r = await fetch(`https://financialmodelingprep.com/stable/earnings?symbol=${sym}&apikey=${FMP}&limit=40`);
      if (r.ok) {
        const rows = await r.json();
        if (Array.isArray(rows)) {
          const evs = rows
            .filter(e => e.date && (e.epsActual ?? e.eps) != null && (e.epsEstimated ?? e.epsEstimate) != null)
            .map(e => ({ date: String(e.date).slice(0, 10), act: +(e.epsActual ?? e.eps), est: +(e.epsEstimated ?? e.epsEstimate) }))
            .sort((a, b) => a.date < b.date ? -1 : 1);   // chronological → trailing-window SUE
          if (evs.length) { bySym.set(sym, evs); withEst++; }
        }
      }
      fetched++;
    } catch {}
  ...
  // 2. PIT SUE per event — trailing σ uses only PRIOR surprises (strictly < i).
  const events = [];  // { symbol, date, sue, surprisePct }
  for (const [sym, evs] of bySym) {
    for (let i = minHistory; i < evs.length; i++) {
      const prior = evs.slice(Math.max(0, i - 12), i).map(e => e.act - e.est);   // excludes i → no lookahead
      const sd = sampleStd(prior);
      if (sd == null || sd < 1e-6) continue;
      if (evs[i].date < cutoff) continue;
      const sue = Math.max(-8, Math.min(8, (evs[i].act - evs[i].est) / sd));      // winsorise fat tails
      events.push({ symbol: sym, date: evs[i].date, sue, surprisePct: surprise(evs[i].act, evs[i].est) });
    }
  }
  const byYearCount = {}; events.forEach(e => { const y = e.date.slice(0, 4); byYearCount[y] = (byYearCount[y] || 0) + 1; });
  const ds = events.map(e => e.date).sort();
  const diag = { scope, months, symbolsFetched: fetched, symbolsWithEstimates: withEst, eventsTotal: events.length, coverage: { earliest: ds[0] || null, latest: ds[ds.length - 1] || null, byYear: byYearCount } };
  if (events.length < 300) return { ...diag, note: 'Too few paired actual/estimate events for a SUE study (need ≥300). Widen scope/limit or check estimate coverage.', elapsedMs: Date.now() - t0 };

  // 3. SPY (beta removal) + macro-regime lookup (VIX/credit, 5y point-in-time).
  ...
  // 4. Price history for symbols that actually have events.
  const symbols = [...new Set(events.map(e => e.symbol))]; const hist = new Map(); let fi = 0;
  const fw = async () => { while (fi < symbols.length) { const s = symbols[fi++]; if (Date.now() - t0 > deadlineMs) return; try { const d = await fetchDailyHistory(s, '5y'); if (d) hist.set(s, d.candles); } catch {} } };
  await Promise.all(Array.from({ length: 16 }, fw));

  // 5. Forward SPY-excess drift, entering the CLOSE AFTER the announcement.
  const recs = [];
  for (const e of events) {
    const c = hist.get(e.symbol); if (!c) continue;
    let idx = -1; for (let k = 0; k < c.length; k++) { if (c[k].date >= e.date) { idx = k; break; } }
    ...
    if (!ok) continue;
    recs.push({ sue: e.sue, year: e.date.slice(0, 4), regime: regimeAt(c[entryI].date), ex });
  }
  diag.resolvedEvents = recs.length;
  if (recs.length < 200) return { ...diag, note: 'too few resolvable events (price history / horizon room)', elapsedMs: Date.now() - t0 };

  // 6. SUE-quintile drift + robustness splits (signed drift aligns with SUE sign).
  const horizons = {};
  for (const H of holds) {
    const withH = recs.filter(r => r.ex[H] != null).sort((a, b) => a.sue - b.sue);
    ...
    for (const r of withH) { (byYear[r.year] = byYear[r.year] || []).push(Math.sign(r.sue) * r.ex[H]); }
    for (const R of ['risk-on', 'neutral', 'risk-off']) byRegime[R] = stats(withH.filter(r => r.regime === R).map(r => Math.sign(r.sue) * r.ex[H]));
    horizons[H] = {
      topQuintile: stats(top.map(r => r.ex[H])),          // biggest beats — should drift UP
      bottomQuintile: stats(bottom.map(r => r.ex[H])),     // biggest misses — should drift DOWN
      longShort: stats(ls),                                // top − bottom (paired)
      signedOverall: stats(withH.map(r => Math.sign(r.sue) * r.ex[H])),  // does SUE direction predict drift?
      byYear: Object.fromEntries(Object.entries(byYear).filter(([, a]) => a.length >= 30).map(([y, a]) => [y, stats(a)])),
      byRegime,
    };
  }

  // 7. Honest verdict at 63d (the standard PEAD window): significant AND robust
  //    across years + regimes, or it's just another single-window artifact.
  ...
  if (lsT >= 2 && signedT >= 2 && yrs.length >= 3 && posYears / yrs.length >= 0.6 && posRegimes >= 2) {
    verdict = 'CONFIRMED';
    reason = `SUE→drift is significant (long-short t ${lsT}, signed t ${signedT} at 63d) AND holds across ${posYears}/${yrs.length} years and ${posRegimes} regimes — not a single-window artifact.`;
  } else if (lsT >= 2 || signedT >= 2) {
    verdict = 'REGIME-DEPENDENT';
    reason = `Significant overall (long-short t ${lsT}, signed t ${signedT}) but concentrated: positive in only ${posYears}/${yrs.length} years and ${posRegimes} regime(s). Usable as a regime-gated filter, not a standalone durable edge.`;
  } else {
    verdict = 'NOT-CONFIRMED';
    reason = `SUE does not predict durable drift (long-short t ${lsT}, signed t ${signedT} at 63d). No standalone PEAD edge in this universe/period.`;
  }

  return { ...diag, elapsedMs: Date.now() - t0, method: 'analyst-based SUE · PIT trailing-σ · SPY-excess drift · macro-regime & yearly robustness', horizons, verdict, reason };
}

module.exports = { runPEAD, runReactionPEAD, runSurprisePEAD };
