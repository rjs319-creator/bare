...
// ── MFE-capture rules (the Scoreboard's big leak: large run-ups given all the way
//    back). trailPct trails a stop G% below the running peak → banks the favorable
//    excursion once the name rolls over; its initial stop (peak=entry) also caps the
//    downside at ~G%. takeProfit banks a fixed +P% and cuts the fat loss tail with a
//    wide −D% disaster stop. ──
function exTrailPct(c, i, e, G) { // trail G% below the highest high seen so far
  let hh = c[i].high;
  for (let h = 1; h <= HOLD && i + h < c.length; h++) {
    const x = c[i + h]; const tstop = hh * (1 - G);
    if (x.low <= tstop) return { r: (tstop - e) / e, hold: h };
    hh = Math.max(hh, x.high);
  }
  const j = Math.min(i + HOLD, c.length - 1); return { r: (c[j].close - e) / e, hold: j - i };
}
function exTakeProfit(c, i, e, P, D) { // bank at +P%, cap disasters at −D%
  const tgt = e * (1 + P), stp = D ? e * (1 - D) : 0;
  for (let h = 1; h <= HOLD && i + h < c.length; h++) {
    const x = c[i + h];
    if (stp && x.low <= stp) return { r: (stp - e) / e, hold: h };
    if (x.high >= tgt) return { r: (tgt - e) / e, hold: h };
  }
  const j = Math.min(i + HOLD, c.length - 1); return { r: (c[j].close - e) / e, hold: j - i };
}
...
const STRATS = {
  ...
  trail8:  (c, i, e) => exTrailPct(c, i, e, 0.08),
  trail12: (c, i, e) => exTrailPct(c, i, e, 0.12),
  trail20: (c, i, e) => exTrailPct(c, i, e, 0.20),
  tp15:    (c, i, e) => exTakeProfit(c, i, e, 0.15, 0.20),
  tp25:    (c, i, e) => exTakeProfit(c, i, e, 0.25, 0.20),
};
...
async function runExitStudy({ scope = 'large', step = 10, months = 12, limit = 0, range = '2y', deadlineMs = 50000 } = {}) {
  ...
  for (const date of dates) {
    ...
    for (const c of cohort) {
      ...
      tagged.push({ date, regime, ...out });   // every strat's return per selection → any KEY can be broken out OOS
    }
  }
  ...
  const KEYS = ['time63', 'trail12', 'trail20', 'tp25', 'catastrophic'];
