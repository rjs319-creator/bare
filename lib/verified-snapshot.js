 ...
 * Pure.
 */
function snapshotRow(candles, asOfDate) {
  if (!Array.isArray(candles) || !candles.length) return { ok: false, reason: 'no-data' };
  const rows = candles.filter((c) => c && c.date && c.close > 0);
 ...
 */
function snapshotBlock(candlesBySymbol, asOfDate) {
  const symbols = Object.keys(candlesBySymbol || {});
  if (!symbols.length) return '';
  const lines = symbols.map((s) => rowLine(s, snapshotRow(candlesBySymbol[s], asOfDate)));
  return [
 ...
 * event markets by heat with market-implied probability and week-over-week drift.
 * Framed as crowd positioning, never an edge. Empty string when nothing usable. Pure.
 */
function predMarketsLine(scoredMarkets, { top = 5 } = {}) {
  const rows = (scoredMarkets || [])
    .filter((m) => m && m.title && m.prob != null && m.prob >= 0 && m.prob <= 1)
    .slice(0, top);
  if (!rows.length) return '';
  const one = (m) => {
    const p = Math.round(m.prob * 100);
    const drift = m.movePts != null && m.movePts >= 1
      ? ` (${m.prob >= (m.probPrev != null ? m.probPrev : m.prob) ? '↑' : '↓'}${Math.round(m.movePts)}pts)` : '';
