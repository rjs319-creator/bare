...
/**
 * Should a fresh snapshot REPLACE the persisted one? Pure.
 * A tick that measured nothing (weekend/holiday: no session bars; or a total provider
 * outage) must NOT overwrite a snapshot that has real intraday data — the last good
 * market read is preserved and the read-time clocks age it honestly. A no-data tick
 * only replaces a no-data (or absent) snapshot.
 */
function shouldReplaceSnapshot(prevDoc, newState) {
  const newCount = newState && newState.coverage ? newState.coverage.intradayCount : 0;
  if (newCount > 0) return { replace: true, reason: 'fresh intraday data' };
  const prevCount = prevDoc && prevDoc.state && prevDoc.state.coverage
    ? prevDoc.state.coverage.intradayCount : 0;
  if (prevCount > 0) return { replace: false, reason: 'no intraday bars this tick (closed day or provider outage) — keeping the last good snapshot' };
  return { replace: true, reason: 'no prior snapshot with data' };
}
