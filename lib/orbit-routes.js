...
function scoreTicker({ ticker, candles, factorCloses, tier, sector, artifact, scenarioVec }) {
  ...
  if (!snap) return null;
  // Kept on the decision so op=orbitlog can persist the PIT factor exposures with each
  // prediction — the label engine needs them to compute the RESIDUAL forward return the
  // monitor consumes. Without them every live-resolved label was positiveResidual:null
  // and the prospective monitor sat at INSUFFICIENT_DATA (n=0) forever.
  const factorExposures = (snap.factor && snap.factor.exposures) || null;
  ...
  const decided = Dec.decideCandidate({
  ...
  return decided ? { ...decided, factorExposures } : decided;
}
...
async function runOrbitLog(req, res) {
  ...
  try {
    ...
    const logged = built.board.map(p => ({ ticker: p.ticker, decisionTs: p.decisionTs, classification: p.classification, confidence: p.confidence, horizonProbabilities: p.horizonProbabilities, expectedNetReturn: p.expectedNetReturn, severeLossProbability: p.severeLossProbability, exposures: p.factorExposures || null }));
...
// The label engine only produces the RESIDUAL forward return (positiveResidual — the
// label the monitor consumes) when it is given the prediction's factor exposures; the
// old resolver never passed them, so every live-resolved record carried
// positiveResidual:null and orbit-monitor filtered ALL of them out (prospective health
// stuck at INSUFFICIENT_DATA/n=0 while 600+ outcomes accrued). Exposures now come from
// the logged prediction when present (PIT, logged by op=orbitlog), else are recomputed
// as-of the decision bar from PRE-decision candles — the same engine and asOfIdx
// convention the backfill uses, so live labels match walk-forward labels.
// `?repair=1` re-labels done records whose resolved horizons carry null residual labels
// (a derived-label recompute from historical candles; the underlying outcomes are
// unchanged and the candle source remains stamped survivorshipSafe:false).
function exposuresFor(prediction, hist, factorBundle, sectorCandles) {
  if (prediction.exposures) return prediction.exposures;
  const asOfIdx = hist.candles.findIndex(c => c.date >= prediction.decisionTs);
  if (asOfIdx <= 0) return null;
  const dates = hist.candles.map(c => c.date);
  const fc = {
    marketCloses: FEAT.alignByDate(dates, factorBundle.market),
    sectorCloses: FEAT.alignByDate(dates, sectorCandles),
    smallCloses: FEAT.alignByDate(dates, factorBundle.small),
    volCloses: FEAT.alignByDate(dates, factorBundle.vol),
  };
  const snap = FEAT.orbitFeatures(hist.candles, fc, { asOfIdx });
  return (snap && snap.factor && snap.factor.exposures) || null;
}

function needsResidualRepair(prior) {
  if (!prior || !prior.horizons) return false;
  // Null residual labels (the pre-exposures resolver) OR missing per-horizon scores
  // (records written before `probs` was carried) both starve the monitor — the first
  // empties its label lane, the second its IC/brier lanes.
  return Object.values(prior.horizons).some(h => h && h.resolved && h.positiveResidual == null)
    || prior.probs === undefined;
}

// ONE history fetch per ticker, misses included.
//
// The nightly resolver called fetchHistory(ticker, '2y') once PER OPEN PREDICTION. With
// ~60 names on the board and a 63-session horizon that is thousands of sequential 2y
// fetches a night: on 2026-09-17 AND 2026-09-18 AVB (a name the provider returns 27 bars
// for) was refetched every ~2s for the whole 60s+ run, and every miss fell through to the
// stooq fallback until its breaker opened. On 2026-09-18 the invocation died at 63s
// (FUNCTION_INVOCATION_FAILED) with resolved.json unwritten — the resolver wrote ONLY at
// the end, so a killed run lost the whole night and the open backlog only grew.
// A miss is cached too: a dataless name costs one call, not one per prediction.
function memoHistory(fetchHistory) {
  const cache = new Map();
  const stats = { fetches: 0, misses: 0 };
  const fetchOnce = async (ticker, range) => {
    const k = `${ticker}|${range}`;
    if (!cache.has(k)) {
      stats.fetches++;
      cache.set(k, fetchHistory(ticker, range).then((h) => {
        if (!h || !h.candles) stats.misses++;
        return h || null;
      }, () => { stats.misses++; return null; }));
    }
    return cache.get(k);
  };
  return { fetchOnce, stats };
}

// tracker maxDuration is 300s and the chain step deadline 240s; pre-fix runs measured
// 60-160s and were ALL fetch storm. Checkpoint before the budget runs out so a slow
// night still lands its progress instead of losing it.
const RESOLVE_BUDGET_MS = 200000;

// Pure core of op=orbitresolve — store, clock and history feed injected. Returns a NEW
// resolved map (the input is never mutated) plus the run's accounting.
async function resolveOpenPredictions({ days, resolved, fetchHistory, repair = false, now = Date.now, budgetMs = RESOLVE_BUDGET_MS, range = '2y', today = null }) {
  const started = now();
  ...
  for (const day of days) {
    for (const p of (day.predictions || [])) {
      ...
      const prior = resolved[key];
      if (prior && prior.done && !(repair && needsResidualRepair(prior))) continue;
      openPredictions++;
      if (!byTicker.has(p.ticker)) byTicker.set(p.ticker, []);
      byTicker.get(p.ticker).push({ p, key, prior });
    }
  }
  const { fetchOnce, stats } = memoHistory(fetchHistory);
  const sectorEtfs = [...new Set([...byTicker.keys()].map((t) => FM.sectorEtfFor(SECTOR_OF[t])).filter(Boolean))];
  const factorBundle = await loadFactorBundle(fetchOnce, range, sectorEtfs);
  const out = { ...resolved };
  let newlyResolved = 0, repaired = 0, tickersDone = 0, truncated = false;
  for (const [ticker, work] of byTicker) {
    if (now() - started > budgetMs) { truncated = true; break; }
    const hist = await fetchOnce(ticker, range);
    tickersDone++;
    if (!hist || !hist.candles) continue;
    const sectorEtf = FM.sectorEtfFor(SECTOR_OF[ticker] || null);
    const sectorCandles = (sectorEtf && factorBundle.sectors[sectorEtf]) || null;
    for (const { p, key, prior } of work) {
      const exposures = exposuresFor(p, hist, factorBundle, sectorCandles);
      const labels = Labels.orbitLabels(hist.candles, p.decisionTs, { tier: 'liquid', atrPct: null, marketCandles: factorBundle.market, sectorCandles, exposures });
      if (!labels.resolvable) continue;
      if (!HORIZONS.some((h) => labels.horizons[h] && labels.horizons[h].resolved)) continue;
      if (prior && prior.done) repaired++; else newlyResolved++;
      // `probs` carries the prediction's per-horizon scores/calibrated probabilities into
      // the resolved record — flattenResolved feeds them to the monitor as score/calUp,
      // without which groupedIC (and so the health classifier) can never compute.
      out[key] = { ticker: p.ticker, decisionTs: p.decisionTs, classification: p.classification, probs: p.horizonProbabilities || null, horizons: labels.horizons, done: HORIZONS.every((h) => labels.horizons[h] && labels.horizons[h].resolved), at: today };
    }
  }
  return {
    resolved: out, newlyResolved, repaired, truncated,
    stats: { openPredictions, tickers: byTicker.size, tickersDone, fetches: stats.fetches, misses: stats.misses, elapsedMs: now() - started },
  };
}
...
async function runOrbitResolve(req, res) {
  ...
  try {
    ...
    const today = (safeNowET() && safeNowET().date) || null;
    const r = await resolveOpenPredictions({ days, resolved, fetchHistory, repair, today });
    await store.writeOrbitResolved(r.resolved);
    try { await require('./immutable-ledger').append('orbit', { kind: 'resolution-batch', at: today, resolved: r.newlyResolved, repaired: r.repaired, truncated: r.truncated }); } catch { /* best-effort */ }
    return res.status(200).json({ ok: true, shadow: true, newlyResolved: r.newlyResolved, repaired: r.repaired, truncated: r.truncated, stats: r.stats, total: Object.keys(r.resolved).length });
  } catch (e) {
    return res.status(200).json({ ok: false, shadow: true, error: String(e && e.message || e) });
  }
}
...
function flattenResolved(map) {
  ...
  for (const key in map) {
    ...
    for (const h of HORIZONS) {
      ...
      // score/calUp come from the prediction's own per-horizon record (carried as
      // `probs` by the resolver): rankScore is what groupedIC correlates with net,
      // residualUp is the calibrated probability brier/logLoss grade. Hardcoding these
      // null starved the monitor's IC and calibration lanes on every record.
      const pb = (r.probs && r.probs[h]) || null;
      // decideCandidate publishes the rank score as `uncalibratedRankScore` (the raw
      // model score always present once trained); calibrated probabilities are null
      // until the horizon's calibrator exists — honest absences either way.
      const score = pb ? (pb.uncalibratedRankScore != null ? pb.uncalibratedRankScore : (pb.rankScore != null ? pb.rankScore : null)) : null;
      rows.push({
        date: r.decisionTs, ticker: r.ticker, horizon: h,
        score,
        calUp: pb && pb.residualUp != null ? pb.residualUp : null,
        label: hz.positiveResidual, net: hz.netReturn, severe: hz.severeLoss,
      });
    }
  }
  return rows;
...
module.exports = { runOrbit, runOrbitLog, runOrbitResolve, runOrbitWalkForward, runOrbitHealth, runAlgorithmRouter, buildOrbitBoard, scoreTicker, trainArtifact, exposuresFor, needsResidualRepair, resolveOpenPredictions, memoHistory, RESOLVE_BUDGET_MS };
