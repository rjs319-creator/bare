...
async function runOrbitResolve(req, res) {
  ...
  try {
    ...
    let newlyResolved = 0, repaired = 0;
    const range = '2y';
    const sectorEtfs = [...new Set(days.flatMap(d => (d.predictions || []).map(p => FM.sectorEtfFor(SECTOR_OF[p.ticker]))).filter(Boolean))];
    const factorBundle = await loadFactorBundle(fetchHistory, range, sectorEtfs);
    for (const day of days) {
      for (const p of (day.predictions || [])) {
        ...
        resolved[key] = { ticker: p.ticker, decisionTs: p.decisionTs, classification: p.classification, probs: p.horizonProbabilities || null, horizons: labels.horizons, done: HORIZONS.every(h => labels.horizons[h] && labels.horizons[h].resolved), at: (safeNowET() && safeNowET().date) || null };
      }
    }
    await store.writeOrbitResolved(resolved);
    try { await require('./immutable-ledger').append('orbit', { kind: 'resolution-batch', at: (safeNowET() && safeNowET().date) || null, resolved: newlyResolved, repaired }); } catch { /* best-effort */ }
    return res.status(200).json({ ok: true, shadow: true, newlyResolved, repaired, total: Object.keys(resolved).length });
  } catch (e) {
    return res.status(200).json({ ok: false, shadow: true, error: String(e && e.message || e) });
  }
}
...
module.exports = { runOrbit, runOrbitLog, runOrbitResolve, runOrbitWalkForward, runOrbitHealth, runAlgorithmRouter, buildOrbitBoard, scoreTicker, trainArtifact, exposuresFor, needsResidualRepair };
