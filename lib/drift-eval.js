...
function clusteredStats(rows, horizonBars) {
  ...
  try {
    ...
    // 95% interval: the exact Student-t interval WIDENED PER SIDE against the same seeded
    // moving-block bootstrap the summary uses (review 2026-08-15) — the widest-of(t,
    // bootstrap) rule of evidence-stats / challenger-eval, which the precision fix had
    // dropped: a heavy-tailed series whose bootstrap interval is wider could claim a
    // tighter CI than the summary's own s.ci95. The bootstrap is RECOMPUTED at full
    // precision (identical params → identical seeded interval) rather than read from
    // s.ci95, because s.ci95 is display-rounded to 2dp — whole-percent quantization for a
    // fractional-units drift series, exactly the damage the exact-field fix removed.
    let ciLo = avg - tCrit * se, ciHi = avg + tCrit * se;
    try {
      const boot = ES.movingBlockCI95(ded.series.filter(Number.isFinite),
        { blockLen: horizonBars ? Math.max(2, horizonBars) : null });
      if (boot && Number.isFinite(boot.lo)) ciLo = Math.min(ciLo, boot.lo);
      if (boot && Number.isFinite(boot.hi)) ciHi = Math.max(ciHi, boot.hi);
    } catch { /* bootstrap unavailable — keep the exact t-interval */ }
    return {
      ...
      ci95Pct: { lo: +(ciLo * 100).toFixed(2), hi: +(ciHi * 100).toFixed(2) },
