...
function clusteredStats(rows, horizonBars) {
  ...
  try {
    ...
    if (!s || !Number.isFinite(s.avg) || !Number.isFinite(s.se) || s.se <= 0) return { n, basis: 'date-clustered', note: 'insufficient date series' };
    return {
      n, dates: s.n, effectiveN: s.effectiveN ?? null,
      meanPct: +(s.avg * 100).toFixed(2),
      hitRate: Math.round((rows.filter(r => r.v > 0).length / n) * 100),
      tStat: +(s.avg / s.se).toFixed(2),
      ci95Pct: s.ci95 ? { lo: +(s.ci95.lo * 100).toFixed(2), hi: +(s.ci95.hi * 100).toFixed(2) } : null,
