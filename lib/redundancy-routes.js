...
const { SECTION_TO_ID } = require('./strategy-contracts');
...
// Ledger section → canonical strategy id, resolved through the ONE authoritative
// map (lib/strategy-contracts.SECTION_TO_ID) rather than a second local table.
// The local copy went stale the moment a section↔id pair diverged — CERN would
// have resolved to 'cern' while the registry id is 'events', silently emptying
// that algorithm's series in every downstream join. Unmapped sections still fall
// back to their lowercased name and simply won't match a live source — honest,
// not silently mislabelled.
const sourceForSection = (s) => SECTION_TO_ID[s] || String(s || '').toLowerCase() || null;
...
async function buildRows({ limitTickers = 400 } = {}) {
  ...
  const rows = deduped.map((r) => {
    ...
    // tier/short ride along so downstream consumers (the router's rank IC) can
    // rank conviction within a date without re-reading the ledgers.
    return { date: r.date, ticker: r.ticker, algorithm: r.algorithm, excess, tier: r.tier, short: r.short };
  });
...
module.exports = { runRedundancy, loadRedundancyModel, buildRows, MODEL_DOC, HORIZON_DAYS, sourceForSection };
