...
function isWeakening(stats, prev) {
  ...
  // Persisted governance records (governRegistry output → governance/latest.json) nest
  // the prior stats at prev.stats.avgExcess — reading only a top-level prev.avgExcess
  // (a shape the runtime never emits) left prevAvg null in production, so the
  // "slipped >1 point vs prior" decay branch was DEAD (fail-open: a decaying strategy
  // kept full weight while its average stayed above zero). Prefer the runtime shape;
  // keep the top-level read for backward compatibility with any legacy doc.
  const prevRaw = prev ? (Number.isFinite(prev.stats?.avgExcess) ? prev.stats.avgExcess : prev.avgExcess) : null;
  const prevAvg = Number.isFinite(prevRaw) ? prevRaw : null;
