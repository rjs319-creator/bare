...
const GOVERNANCE_VERSION = 'gov-v2';

// PROMOTION ARTIFACT (gov-v2) — an earned Validated grade is NECESSARY but no longer
...
// { version, approve: true, approvedAt, by?, evidence? } } }. The artifact must match
// the CURRENT scoring version — an approval earned under an old version is void.
// Missing/mismatched/unapproved ⇒ fail closed to paper/probation, never production.
function validPromotionArtifact(artifact, version) {
  return !!(artifact && artifact.approve === true && artifact.version && version && artifact.version === version);
}
...
function governStrategy(graded, prev, artifact = null) {
  const g = graded || {};
  ...
  if (grade === 'validated' && n >= MIN_VERDICT) {
    ...
    if (!validPromotionArtifact(artifact, version)) {
      const st = wasLive ? 'probation' : 'paper';
      return {
        ...base, status: st, weight: weightFor(st),
        reason: artifact
          ? `Validated grade earned, but the promotion artifact is ${artifact.approve !== true ? 'not approved' : `for scoring version ${artifact.version || '?'} (current ${version || '?'})`} — fail closed pending a current reviewable approval.`
          : 'Validated grade earned, but NO reviewable promotion artifact exists — fail closed. Promotion requires an explicit, version-matched approval record (governance/promotion-artifacts.json).',
        versionReset: false, awaitingPromotionArtifact: true,
      };
    }
...
function governRegistry(classified, prevMap, artifacts) {
  ...
  const arts = artifacts instanceof Map ? artifacts : new Map(Object.entries((artifacts && artifacts.strategies) || artifacts || {}));
  const governed = strategies.map(s => governStrategy(s, prev.get(s.id) || null, arts.get(s.id) || null));
...
module.exports = {
  ...
  validPromotionArtifact, governStrategy, governRegistry,
};
