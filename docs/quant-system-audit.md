...
| 9 | Ensemble discards candidate raw strength | **CONFIRMED (P1), SHADOW + GATED PROMOTION** | `lib/evolve.js` specialistProb = pooled base rate | equal-context candidates → identical P | **`candidateStrengthTilt`** differentiates by percentile; **`strengthOOSComparison`** (rank-IC + Brier vs base) **auto-promotes** it into the decision ONLY when it beats base OOS by a predeclared margin (Part XV champion/challenger). Strength is now **logged forward**; the gate reads `ready:false` until it accrues, so it is **still shadow today** | strength-tilt + promotion-gate tests |
...
survivorship-safe backtest). The candidate-strength tilt (#9) now has a **governed auto-promotion
gate** (`strengthOOSComparison` → `ctx.strengthPromoted`): it flips into the decision automatically
once the shadow OOS comparison clears a predeclared margin, and strength is now logged forward so it
accrues toward that decision. It is still shadow today only because no strength-carrying resolved
events exist yet — the code needs no further change, just accrued data. (`universeAt` is now partially wired via `?pit=1` — see §6. Addressed since the initial audit:
